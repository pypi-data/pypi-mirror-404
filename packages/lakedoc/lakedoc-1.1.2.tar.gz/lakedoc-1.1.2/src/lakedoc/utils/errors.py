...
class LakeContentTypeError(LakeBaseError, TypeError):
    def __init__(self, name: str, expected_type: str, actual_type: str):
...
class LakePickNotFoundError(LakeContextError):
    def __init__(self, converter: str):
        info = f"类型`{converter}`的转换器并未注册"
        super().__init__(info)


class LakeSourceEmptyError(LakeContextError):
    def __init__(self):
        info = "转换时传入 Source 不能为空"
        super().__init__(info)


class LakeConverterNotFoundError(LakeContextError):
    def __init__(self):
        info = "转换器类不存在"
        super().__init__(info)
