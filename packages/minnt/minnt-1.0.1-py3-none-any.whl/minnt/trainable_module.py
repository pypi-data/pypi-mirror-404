...
The [TrainableModule][minnt.TrainableModule] class is
...
- It provides a high-level API for training, evaluation, and prediction
  via [fit][minnt.TrainableModule.fit], [evaluate][minnt.TrainableModule.evaluate],
  and [predict][minnt.TrainableModule.predict] methods. Each can be customized
  by overriding the corresponding [train_step][minnt.TrainableModule.train_step],
...
- The module inputs and outputs can be either single tensors or _tensor structures_
  (the [TensorOrTensors][minnt.type_aliases.TensorOrTensors] type), where the latter
  ...
  values are automatically moved to the correct device during [fit][minnt.TrainableModule.fit],
  [evaluate][minnt.TrainableModule.evaluate], and [predict][minnt.TrainableModule.predict].

- The module provides API for serialization and deserialization of the model,
  both the weights ([save_weights][minnt.TrainableModule.save_weights],
  [load_weights][minnt.TrainableModule.load_weights]) and the model options
  ([save_options][minnt.TrainableModule.save_options],
  [load_options][minnt.TrainableModule.load_options]).
