...
/**
 * @brief Raw compact bunch memory layout.
 */
#pragma pack(push, 1)
struct RawCompactBunch
{
    int16_t x;
    int16_t y;
    int16_t cx;
...
{ 
    ...
    constexpr size_t DATA_OFFSET_START = 12;
    ...
    const std::byte* data_ptr = reinterpret_cast<const std::byte*>(data) + position + DATA_OFFSET_START;

    u32 end = index + n_bunches;

    RawCompactBunch bunch;
    
    for (u32 i=index; i<end; ++i) {
        // Get bunches from buffer
        std::memcpy(&bunch, data_ptr, sizeof(RawCompactBunch));

        // Position
        bunches.pos[i].x = static_cast<B>(bunch.x) * POS_SCALE;
        bunches.pos[i].y = static_cast<B>(bunch.y) * POS_SCALE;

        // Direction
        bunches.dir[i].cx = std::clamp(static_cast<B>(bunch.cx) * DIR_SCALE, DIR_CLAMP_MIN, DIR_CLAMP_MAX);
        bunches.dir[i].cy = std::clamp(static_cast<B>(bunch.cy) * DIR_SCALE, DIR_CLAMP_MIN, DIR_CLAMP_MAX);

        // Arrival time
        bunches.time[i] = static_cast<B>(bunch.time) * TIME_SCALE;

        // Emission altitude
        bunches.zem[i] = std::pow(ZEM_BASE, static_cast<B>(bunch.zem) * ZEM_EXP_SCALE);

        // Photons
        bunches.photons[i] = static_cast<B>(bunch.photons) * PHOTONS_SCALE;

        // Wavelength
        bunches.wavelength[i] = static_cast<B>(bunch.wavelength);

        // k += 1;
        data_ptr += BUNCH_RECORD_SIZE;
    }
}
...
{ 
    ...
    // Offset of each element in the record.
    // Bunches are strided by 12 bytes, so that
    // zem, photons and wavelength are written
    // in the first 12 bytes of the next 32-byte record.
    ...
    const std::byte* data_ptr = reinterpret_cast<const std::byte*>(data) + position;

    size_t end = index + n_bunches;
    for (size_t i=index; i<end; ++i) {
        // Get bunches from buffer
        std::memcpy(&bunches.pos[i].x,      data_ptr + POS_X_OFFSET,      ENTRY_SIZE);
        std::memcpy(&bunches.pos[i].y,      data_ptr + POS_Y_OFFSET,      ENTRY_SIZE);
        std::memcpy(&bunches.dir[i].cx,     data_ptr + DIR_X_OFFSET,      ENTRY_SIZE);
        std::memcpy(&bunches.dir[i].cy,     data_ptr + DIR_Y_OFFSET,      ENTRY_SIZE);
        std::memcpy(&bunches.time[i],       data_ptr + TIME_OFFSET,       ENTRY_SIZE);
        std::memcpy(&bunches.zem[i],        data_ptr + ZEM_OFFSET,        ENTRY_SIZE);
        std::memcpy(&bunches.photons[i],    data_ptr + PHOTONS_OFFSET,    ENTRY_SIZE);
        std::memcpy(&bunches.wavelength[i], data_ptr + WAVELENGTH_OFFSET, ENTRY_SIZE);
