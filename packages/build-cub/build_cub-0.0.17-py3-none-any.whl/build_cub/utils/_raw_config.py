...
def write_toml_file(name: str, write_path: Path) -> None:
    from datetime import datetime

    from jinja2 import Template

    if not name:
        name = global_config.paths.cwd.name.replace("-", "_").replace(" ", "_").lower()

    template = Template(RAW_CONFIG)
    date: datetime = datetime.now(tz=datetime.now().astimezone().tzinfo)
    iso_date: str = date.isoformat(timespec="seconds")
    write_path.write_text(
        template.render(project_name=name, build_date=iso_date),
        encoding="utf-8",
    )


RAW_CONFIG = """### BUILD SETTINGS FOR {{ project_name | upper }} ###############################################
#
# This file configures the build system for {{ project_name }}. It uses a defaults +
...
#
# Generated by build_cub on {{ build_date }}!
#################################################################################

[general]
name = "{{ project_name }}"               # Name of the package
enabled = true                            # Master switch for all compilation
debug_symbols = false                     # Whether to include debug symbols in compiled extensions
...
name = "python_version"
output = "src/{{ project_name }}/_internal/_version.py"
content = '''{{% raw %}}
...
# Example C++ version header template
# [[versioning.templates]]
# name = "cpp_version"
# output = "src/{{ project_name }}/_cpp/version.hpp"
# content = '''{{% raw %}}
# #pragma once
# #define VERSION_MAJOR {{ version.major }}
# #define VERSION_MINOR {{ version.minor }}
# #define VERSION_PATCH {{ version.patch }}
# #define VERSION_STRING "{{ version.version_str }}"
# {{% endraw %}}'''
...
#################################################################################
[defaults]
merge_mode = "replace"               # How backend settings merge with defaults ("replace" or "merge")
...
binary = "/usr/bin/gperf"            # Path to the gperf binary
...
targets = [                            # Explicit list of .pyx files to compile
    # { name="core", sources=["src/{{ project_name }}/_cython/core.pyx"] },
]
...
targets = [                            # Explicit list of .cpp files to compile
    # { name="core", sources=["src/{{ project_name }}/_cpp/core.cpp"] },
    # { name="bindings", sources=[ ## EXAMPLE FOR MULTI-FILE MODULE
    #     "src/{{ project_name }}/_internal/_cpp/bindings.cpp",
    #     "src/{{ project_name }}/_internal/_cpp/lexer.cpp",
    #     "src/{{ project_name }}/_internal/_cpp/parser.cpp"] },
]
...
targets = [                            # List of Cargo.toml paths to compile
    # {name = "bindings", sources = "Cargo.toml", module_path = "{{ project_name }}._rust.bindings"},
    # {name = "core", sources = "src/{{ project_name }}/_rust/Cargo.toml", module_path = "{{ project_name }}._rust.core"},
]
