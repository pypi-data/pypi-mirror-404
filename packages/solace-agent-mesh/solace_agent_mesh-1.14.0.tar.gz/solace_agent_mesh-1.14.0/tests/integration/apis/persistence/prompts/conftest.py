"""
Pytest fixtures for prompts feature flag testing.
...
def _create_custom_config_client(
    db_url: str,
    prompt_library_enabled: bool = True,
    feature_flag_enabled: bool = True
):
    """
    Helper to create a test client with custom prompt library configuration.

    Args:
        db_url: Database URL to use
        prompt_library_enabled: Value for prompt_library.enabled config
        feature_flag_enabled: Value for frontend_feature_enablement.promptLibrary
    ...
    def custom_get_config(key, default=None):
        ...
        if key == "prompt_library":
            return {"enabled": prompt_library_enabled}
        if key == "frontend_feature_enablement":
            return {"promptLibrary": feature_flag_enabled}
        if key == "name":
...
@pytest.fixture
def prompts_disabled_client(db_provider):
    """API client with prompts explicitly disabled via prompt_library.enabled=false"""
...
def feature_flag_disabled_client(db_provider):
    """API client with prompts disabled via frontend_feature_enablement.promptLibrary=false"""
    if hasattr(db_provider, "get_gateway_url_with_credentials"):
        db_url = db_provider.get_gateway_url_with_credentials()
    else:
        db_url = str(db_provider.get_sync_gateway_engine().url)
...
def both_disabled_client(db_provider):
    """API client with both prompt_library.enabled and feature flag disabled"""
    if hasattr(db_provider, "get_gateway_url_with_credentials"):
        db_url = db_provider.get_gateway_url_with_credentials()
    else:
        db_url = str(db_provider.get_sync_gateway_engine().url)
...
def both_enabled_client(db_provider):
    """API client with both prompt_library.enabled and feature flag enabled"""
    if hasattr(db_provider, "get_gateway_url_with_credentials"):
        db_url = db_provider.get_gateway_url_with_credentials()
    else:
        db_url = str(db_provider.get_sync_gateway_engine().url)
