...
def get_os_subdir() -> str:
    """Get OS-specific subdirectory name for work directory.
...
class OrchestratorRunner:
    ...
    def _apply_config(self, config: dict[str, Any]) -> None:
        ...
        if "source_dir" in config:
            CONFIG.paths.SOURCE_DIR = str(Path(config["source_dir"].strip()))

        if config.get("work_dir"):
            CONFIG.paths.WORK_DIR = str(Path(config["work_dir"].strip()))
        else:
