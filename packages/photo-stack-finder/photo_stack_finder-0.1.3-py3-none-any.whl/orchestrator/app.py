...
async def serve_review_identical() -> FileResponse:
    ...
    docs_path = (
        Path(CONFIG.orchestrator.DOCS_DIR)
        if CONFIG.orchestrator.DOCS_DIR
        else Path(__file__).parent.parent.parent / "docs"
    )
    return FileResponse(docs_path / "review_identical.html")
...
async def serve_review_sequences() -> FileResponse:
    ...
    return FileResponse(docs_path / "review_sequences.html")
...
# Mount docs directory for review HTML assets (review_common.js, etc.)
docs_dir = Path(CONFIG.orchestrator.DOCS_DIR) if CONFIG.orchestrator.DOCS_DIR else None
if docs_dir and docs_dir.exists():
    # Serve .js files from docs at root level for review pages
    # Capture docs_dir in closure to avoid mypy error
    docs_path_for_closure = docs_dir

    @app.get("/review_common.js")
    async def serve_review_common_js() -> FileResponse:
        """Serve review_common.js from docs directory."""
        return FileResponse(docs_path_for_closure / "review_common.js", media_type="application/javascript")
