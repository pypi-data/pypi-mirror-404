...
class KVCache:
    ...
    def set_time_extension(self, time_extension: int) -> None:
        ...
        :raise Exception:
            ValueError if time_extension is not valid
    ...
    def get(self, key, ex=None) -> typing.Any | None:
        """
    ...
    def mapupdate(self, key, valuedict, ex: int | None = None):
        """
        ...
        with self._lock_release(writelock=True):
            if key not in self._kv_data:
                ...
                oldex, valmap = self._kv_data.get(key, (None, None))
                if isinstance(valmap, dict):
                    valmap.update(valuedict)
                else:
                    raise ValueError('data stored with key {0} is not a dict, cannot update'.format(key))
                if ex is None:
                    self._kv_data[key] = (oldex, valmap)
                else:
                    ex = int(time.time() + int(ex))
                    self._kv_data[key] = (ex, valmap)
                    if ex < oldex:
