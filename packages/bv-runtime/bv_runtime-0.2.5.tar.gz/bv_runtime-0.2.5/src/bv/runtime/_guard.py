...
def require_bv_run() -> None:
    ...
    This is called by SDK APIs to verify that the code is running in a
    proper execution context (via 'bv run' or the Runner).
    ...
    if not runtime_context.is_runtime_initialized():
        raise RuntimeError(
            "bv.runtime is only available when running via 'bv run' or the BV Runner. "
