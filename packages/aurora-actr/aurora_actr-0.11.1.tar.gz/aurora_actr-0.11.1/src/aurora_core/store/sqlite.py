...
class SQLiteStore(Store):
    ...
    def save_doc_chunk(self, chunk: "DocChunk") -> bool:
        """Save a document chunk with hierarchy information.

        This method saves both the chunk data (via save_chunk) and the
        document hierarchy metadata to the doc_hierarchy table.

        Args:
            chunk: The DocChunk to save
        ...
        if not isinstance(chunk, DocChunk):
            raise ValidationError(
                f"Expected DocChunk, got {type(chunk).__name__}",
                details=f"chunk_id: {chunk.chunk_id if hasattr(chunk, 'chunk_id') else 'unknown'}",
            )

        # Save the chunk itself (handles validation and timestamps)
        ...
        with self._transaction() as conn:
            try:
                conn.execute(
                    """
                    INSERT OR REPLACE INTO doc_hierarchy
                        (chunk_id, parent_chunk_id, section_path, section_level, document_type)
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (
                        chunk.id,
    ...
    def get_section_children(self, parent_id: str) -> list["DocChunk"]:
        """Get all child sections of a parent document chunk.

        Args:
            parent_id: The chunk_id of the parent section

        Returns:
            List of DocChunk objects that are children of the parent
        ...
        try:
            ...
            cursor = conn.execute(
                """
                SELECT c.id, c.type, c.content, c.metadata, c.embeddings,
                       c.created_at, c.updated_at,
                       h.parent_chunk_id, h.section_path, h.section_level, h.document_type
                FROM chunks c
                JOIN doc_hierarchy h ON c.id = h.chunk_id
                WHERE h.parent_chunk_id = ?
                ORDER BY h.section_level ASC, c.id ASC
                """,
                (parent_id,),
            )
            ...
            for row in cursor.fetchall():
                ...
                # Create DocChunk from row data
                chunk = DocChunk(
                    chunk_id=row["id"],
                    file_path=content_data.get("file_path", ""),
                    page_start=content_data.get("page_start", 0),
                    page_end=content_data.get("page_end", 0),
                    element_type=content_data.get("element_type", "section"),
            ...
            raise StorageError(
                f"Failed to retrieve children for parent: {parent_id}",
                details=str(e),
            )

    def get_section_ancestors(self, chunk_id: str) -> list["DocChunk"]:
        """Get all ancestor sections of a document chunk (for verification).

        Traverses up the parent chain from the given chunk to the root.

        Args:
            chunk_id: The chunk_id to get ancestors for

        Returns:
            List of DocChunk objects representing ancestors, ordered from
            immediate parent to root (reversed hierarchy)
        ...
        try:
            ...
            while True:
                ...
                row = cursor.fetchone()
                if not row:
                    # Reached end of chain (no parent)
                    break
                ...
                parent_chunk = DocChunk(
                    ...
                    name=content_data.get("name", ""),
                    content=content_data.get("content", ""),
                    parent_chunk_id=parent_row["parent_chunk_id"],
                    section_path=section_path,
                    section_level=parent_row["section_level"],
                    document_type=parent_row["document_type"],
                    embeddings=parent_row["embeddings"],
                    metadata=json.loads(parent_row["metadata"]) if parent_row["metadata"] else {},
                    created_at=(
                        datetime.fromisoformat(parent_row["created_at"])
                        if parent_row["created_at"]
                        else None
                    ),
                    updated_at=(
                        datetime.fromisoformat(parent_row["updated_at"])
                        if parent_row["updated_at"]
                        else None
                    ),
                )
                ...
                if len(ancestors) > 100:
                    raise StorageError(
                        f"Ancestor chain too deep (>100) for chunk: {chunk_id}",
                        details="Possible circular reference in doc_hierarchy",
                    )
            ...
            raise StorageError(
                f"Failed to retrieve ancestors for chunk: {chunk_id}",
                details=str(e),
            )
