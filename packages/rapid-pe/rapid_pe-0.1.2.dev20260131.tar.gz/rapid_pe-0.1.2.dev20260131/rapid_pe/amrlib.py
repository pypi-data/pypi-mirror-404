...
def create_regular_grid_from_cell(cell, side_pts=5, return_cells=False):
    ...
    grid_pts = numpy.array(list(zip(*[grid.flatten() for grid in grid_pts])))
...
def etaToq(eta):
    ...
    tmp = 1. - 1. / (2. * eta)
    return - tmp - numpy.sqrt(tmp**2. - 1.)

def mcTopsi0(mc):
    """chirpmass to the 0PN phase coefficient"""
    return (3. / 4.) * (8. * numpy.pi * mc * MsunToTime * fref_mu)**(-5. / 3.)

def mcetaTopsi2(mc, eta):
    """chirpmass and eta=m1*m2/(m1+m2)**2 to the 1PN phase coefficient"""
    return mcTopsi0(mc) * (20. / 9.) * (743. / 336. + 11. * eta / 4.) * \
        eta**(-2. / 5.) * (numpy.pi * mc * MsunToTime * fref_mu)**(2. / 3.)

def qs1zs2zTobeta(q, s1z, s2z):
    """q=m2/m1, z-components of spins to beta: spin combination appearing in the 1.5PN phase correction"""
    return ((113. / 12. + 25. * q / 4.) * s1z + \
        q**2. * (113. / 12. + 25. / (4. * q)) * s2z) / ((1. + q)**2.)

def mcqs1zs2zTopsi3(mc, q, s1z, s2z):
    """chirpmass, eta=m1*m2/(m1+m2)**2, and z-components of spins to the 1.5PN phase coefficient"""
    beta = qs1zs2zTobeta(q, s1z, s2z)
    eta = qToeta(q)
    return mcTopsi0(mc) * (4. * beta - 16. * numpy.pi) * eta**(-3. / 5.) * numpy.pi * mc * MsunToTime * fref_mu
...
def mu2mcetas2zTos1z(mu2, mc, eta, s2z):
    """mu2, chirpmass, eta=m1*m2/(m1+m2)**2, z-component of secondary spin to z-component of primary spin"""
    psi0 = mcTopsi0(mc)
    beta = 4. * numpy.pi + (mu2 - mu_coeffs[1, 0] * psi0 - mu_coeffs[1, 1] * mcetaTopsi2(mc, eta)) / \
        (mu_coeffs[1, 2] * 4. * numpy.pi * mc * MsunToTime * fref_mu * eta**(-3. / 5.) * psi0)
    q = etaToq(eta)
    return ((1. + q)**2. * beta - q**2. * (113. / 12. + 25. / (4. * q)) * s2z) / (113. / 12. + 25. * q / 4.)
...
def check_mchirpeta(mchirp, eta):
    ...
    eta_arr = numpy.asarray(eta)
    return (eta_arr <= 0.25) & (eta_arr > 0) & (mchirp_arr > 0)
