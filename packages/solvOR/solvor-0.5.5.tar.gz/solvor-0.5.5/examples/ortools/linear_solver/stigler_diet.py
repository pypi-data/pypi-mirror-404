"""
OR-Tools Example: stigler_diet.py

This is a solvOR implementation of the Google OR-Tools example.
Original: https://github.com/google/or-tools/blob/stable/ortools/linear_solver/samples/stigler_diet.py
...
def main():
    ...
    # [START variables]
    # Variables: one per food item (77 foods)
    n_foods = len(data)
    print("Number of variables =", n_foods)
    # [END variables]

    # [START constraints]
    # Build constraint matrix A and bounds b
    # Original constraints are >= (minimum nutrients), convert to <= by negating
    # -sum(foods * nutrients) <= -minimum
    A = []
    b = []
    for i, nutrient in enumerate(nutrients):
        row = [-item[i + 3] for item in data]  # Negate for >= to <= conversion
        A.append(row)
        b.append(-nutrient[1])  # Negate minimum

    print("Number of constraints =", len(nutrients))
    # [END constraints]

    # [START objective]
    # Objective: Minimize sum of food quantities
    c = [1] * n_foods
    # [END objective]

    # [START solve]
    print("Solving with solvOR simplex")
    result = solve_lp(c, A, b, minimize=True)
    # [END solve]
    ...
    nutrients_result = [0.0] * len(nutrients)
    print("\nAnnual Foods:")
    for i, item in enumerate(data):
        if result.solution[i] > 0.0:
            print(f"{item[0]}: ${365.0 * result.solution[i]}")
            for j in range(len(nutrients)):
                nutrients_result[j] += item[j + 3] * result.solution[i]
    print(f"\nOptimal annual price: ${365.0 * result.objective:.4f}")

    print("\nNutrients per day:")
    for i, nutrient in enumerate(nutrients):
        print(f"{nutrient[0]}: {nutrients_result[i]:.2f} (min {nutrient[1]})")
    # [END print_solution]
