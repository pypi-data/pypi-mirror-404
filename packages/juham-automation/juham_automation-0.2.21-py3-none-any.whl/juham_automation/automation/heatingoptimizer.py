...
class HeatingOptimizer(Juham):
    ...
    def __init__(
        ...
        self.slots_per_hour : int = int(3600 / self.energy_balancing_interval)
        self.schedule_start_slot = start_hour * self.slots_per_hour
        self.spot_limit = spot_limit
        self.heating_duration_slots = num_hours * self.slots_per_hour
    ...
    def is_slot_within_schedule(self, slot: int, start_slot: int, duration_slots: int) -> bool:
        ...
        total_slots = self.slots_per_day()
        # The end slot could be > 96, so we use modulo
        end_slot = (start_slot + duration_slots) % total_slots
        if start_slot < end_slot:
            return start_slot <= slot < end_slot
        else: # This handles the 22:00 to 06:00 wrap-around
            return slot >= start_slot or slot < end_slot
    ...
    def compute_uoi(
        ...
        if not self.is_slot_within_schedule(
            slot, self.schedule_start_slot, self.heating_duration_slots
        ):
            return 0.0
    ...
    def align_forecast_to_slots(self, solar_forecast: list[dict]) -> list[dict]:
        ...
        slots_per_hour = int(3600 // self.energy_balancing_interval)
        expanded = []
    ...
    def calculate_target_temps(self,
        ...
        ts : float = timestamp()
        num_hours : float = self.heating_duration_slots * self.energy_balancing_interval / 3600.0
    ...
    def create_heating_plan(self) -> list[dict[str, Any]]:
        ...
        for hp in self.power_plan:
            ...
            fom = hp["FOM"]
            spot = hp["Spot"]
            end_slot: int = self.schedule_start_slot + self.heating_duration_slots
            slot: int = self.timestamp_slot(ts)
