...
### 2. 批量扫描 Excel 文件
指定一个 Excel 文件及其中的某一列，工具会自动识别并保存结果。你可以使用 `--match-mode` (或 `-mm`) 参数根据数据特征选择最合适的识别策略。

```bash
# 基本用法：指定列名（默认使用 overall 模式）
cndetect batch data.xlsx -c "姓名"

# 处理混合格式：从 wangxm133、zhang_test 等格式中提取姓氏
cndetect batch data.xlsx -c "用户ID" --match-mode component

# 严格过滤：在纯英文环境中过滤 He, She, Long 等常用词干扰
cndetect batch data.xlsx -c "Full_Name" -mm strict

# 组合使用：指定列名、切换模式并添加自定义姓氏
cndetect batch data.xlsx -c "Name" -mm component --custom-names "Wayne,Kent"
```

#### 模式在批量处理中的效果差异：
- **overall**：识别速度最快，适合标准 `Wang Xiaoming` 格式。
- **component**：**推荐用于数据清洗！** 能自动拆分 `li_123` 并精准提取出 `li`，大幅提升复杂数据的召回率。
- **strict**：误报率最低，适合对准确性要求极高的场景，但可能会跳过未首字母大写的合法拼音。

*执行完成后，会生成一个名为 `data_cn.xlsx` 的新文件，其中包含关键结果列：*
- **ChineseDetector**：核心输出列。如果识别为中文姓名（含中文字符或命中姓氏库），该列会保留**原始单元格的值**，方便你后续直接筛选或作为主键使用；未命中的行则为空。
