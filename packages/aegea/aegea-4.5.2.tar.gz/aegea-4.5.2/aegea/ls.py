...
def register_filtering_parser(function, **kwargs):
    ...
    parser.add_argument(
        "-t", "--tag", nargs="+", default=[], metavar="TAG_NAME=VALUE", help="Tag(s) to filter output by"
    )
    return parser
...
parser = register_listing_parser(
    taskdefs, help="List ECS task definitions", column_defaults=["family", "revision", "containerDefinitions"]
)
...
parser = register_listing_parser(
    subscriptions, help="List SNS subscriptions", column_defaults=["SubscriptionArn", "Protocol", "Endpoint"]
)
