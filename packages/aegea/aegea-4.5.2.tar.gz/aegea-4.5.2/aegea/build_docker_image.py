...
def get_cloud_config(args):
    cloud_config_data = OrderedDict(
        packages=args.packages, write_files=get_bootstrap_files(get_rootfs_skel_dirs(args)), runcmd=args.commands
    )
    cloud_config_data.update(dict(args.cloud_config_data))
    cloud_cfg_d = {
        "datasource_list": ["None"],
        "datasource": {"None": {"userdata_raw": encode_cloud_config_payload(cloud_config_data, gzip=False)}},
    }
...
def ensure_ecr_repo(name, read_access=None):
    ...
    policy = IAMPolicyBuilder(
        principal=dict(AWS=read_access),
...
def build_docker_image(args):
    ...
    args.tags += [
        "AegeaVersion={}".format(__version__),
    ...
    with tempfile.NamedTemporaryFile(mode="wt") as exec_fh:
        exec_fh.write(
            build_docker_image_shellcode.format(
                dockerfile=encode_dockerfile(args), use_cache=json.dumps(args.use_cache)
            )
        )
        exec_fh.flush()
