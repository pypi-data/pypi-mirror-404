...
class DNSZone(VerboseRepr):
    def __init__(self, zone_name=None, create_default_private_zone=True):
        ...
        try:
            ...
            if zone_name == config.dns.private_zone and create_default_private_zone:
                ...
                res = clients.route53.create_hosted_zone(
                    Name=config.dns.private_zone,
    ...
    def update(self, names, values, action="UPSERT", record_type="CNAME", ttl=60):
        ...
        return clients.route53.change_resource_record_sets(HostedZoneId=self.zone_id, ChangeBatch=dict(Changes=updates))

    def delete(self, name, value=None, record_type="CNAME", missing_ok=True):
        if value is None:
            res = clients.route53.list_resource_record_sets(
                HostedZoneId=self.zone_id, StartRecordName=name + "." + self.zone["Name"], StartRecordType=record_type
            )
