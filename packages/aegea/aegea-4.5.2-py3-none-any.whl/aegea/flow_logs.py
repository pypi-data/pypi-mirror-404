...
def create(args):
    if args.resource and args.resource.startswith("vpc-"):
        ...
        resource_type = "VPC"
    flow_logs_iam_role = ensure_iam_role(
        __name__, policies=["service-role/AmazonAPIGatewayPushToCloudWatchLogs"], trust=["vpc-flow-logs"]
    )
    try:
        return clients.ec2.create_flow_logs(
            ResourceIds=[args.resource],
