...
lines_group.add_argument(
    "--tail", type=int, nargs="?", const=10, help="Retrieve this number of lines from the end of the log (default 10)"
)


def stop(args):
    return clients.ecs.stop_task(cluster=args.cluster, task=args.task_id, reason="Stopped by {}".format(__name__))
...
def ssh(args):
    ...
    ecs_ci_desc = clients.ecs.describe_container_instances(
        cluster=task_desc["clusterArn"], containerInstances=[ecs_ci_arn]
