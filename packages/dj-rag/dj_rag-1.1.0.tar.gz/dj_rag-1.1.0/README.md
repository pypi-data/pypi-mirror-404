...
pip install dj-rag
dj-rag init my_rag_project
cd my_rag_project && # Edit .env with your credentials && uv sync && uv run main.py
...
```
dj-rag init folder_name         # Development (auto-reload)
# Edit .env with your credentials      # values in .env
uv sync             # Install deps
uv run main.py      # Start the Fastapi
...
import requests
```
#### After dj-rag init && upate values inside .env && uv sync && uv run main.py
```
...
cd my_project && update the values in .env && uv sync && uv run main.py
...
**Key improvements:**
- ✅ **CLI-first**: `dj-rag init folder_name` 
