...
class SourceTableAPI:
    ...
    def validate_table(
        self,
        request: SourceTableValidateRequest,
    ) -> SourceTableValidateResponse:
        response = self._client._request(
            SourceTableEndpoints.validate_table,
    ...
    def list_tables(
        self,
        request: SourceTableListRequest,
    ) -> SourceTableListResponse:
        response = self._client._request(
            SourceTableEndpoints.list_tables,
            json=to_json_dict(request),
        )
        raise_on_error(response)
    ...
    ) -> List[SourceTableDataResponse]:
        response = self._client._request(
            SourceTableEndpoints.get_table_data,
            json=to_json_dict(request),
        )
        raise_on_error(response)
    ...
    def get_table_config(
        self,
        request: SourceTableConfigRequest,
    ) -> SourceTableConfigResponse:
        response = self._client._request(
            SourceTableEndpoints.get_table_config,
            json=to_json_dict(request),
        )
        raise_on_error(response)
