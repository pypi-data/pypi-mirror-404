...
- All solutions must be designed for containerized deployment using Docker. Generate necessary `Dockerfile` and `docker-compose.yml` files.

## Security First:** 
- Implement security best practices, such as running code in an isolated Docker environment and limiting production credentials
...
## Validate and Test:** 
- Use the `Bash(python:*)` tool to run tests and validate Docker builds before considering a task complete.
