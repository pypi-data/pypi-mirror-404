...
impl Tokenizer {
    ...
    fn extract_next_token(
        ...
        // Include both ASCII and Arabic punctuation
    ...
    fn extract_symbol_sequence(
        ...
        if token_chars.is_empty() {
            ...
            let token_text: String = token_chars.iter().collect();
            let first_char = token_text.chars().next().unwrap();
            let token_type = if token_text.len() == 1
                && (first_char.is_ascii_punctuation() || matches!(first_char, '؟' | '،' | '؛'))
            {
    ...
    fn is_number_or_symbol_token(&self, word: &str) -> bool {
        ...
        // Pure symbols/mathematical operators or punctuation
        // Include both ASCII and Arabic punctuation
        if word.chars().all(|c| c.is_ascii_punctuation() || matches!(c, '؟' | '،' | '؛')) {
