...
impl ScriptGenerator {
    ...
    fn canonical_to_arabic<'a>(&self, canonical: &'a str, segment_type: SegmentType) -> &'a str {
        ...
        if matches!(segment_type, SegmentType::Number) {
            return canonical;
        }

        // Handle punctuation - map to Arabic equivalents
        if matches!(segment_type, SegmentType::Punctuation) {
            return match canonical {
                "?" => "؟", // Arabic question mark
                "," => "،", // Arabic comma
                ";" => "؛", // Arabic semicolon
                "." | "!" | ":" | "(" | ")" | "[" | "]" | "{" | "}" | "\"" | "'" | "-" | "/" => canonical, // Keep these as-is
                _ => canonical,
            };
        }
    ...
    fn canonical_to_latin<'a>(&self, canonical: &'a str, segment_type: SegmentType) -> &'a str {
        ...
        // Handle punctuation - map to Latin equivalents
        if matches!(segment_type, SegmentType::Punctuation) {
            return match canonical {
                "؟" => "?", // Arabic question mark → Latin
                "،" => ",", // Arabic comma → Latin
                "؛" => ";", // Arabic semicolon → Latin
                "." | "!" | ":" | "(" | ")" | "[" | "]" | "{" | "}" | "\"" | "'" | "-" | "/" => canonical, // Keep these as-is
                _ => canonical,
            };
        }
...
mod tests {
    ...
    #[test]
    fn test_punctuation_latin_to_arabic() {
        let generator = ScriptGenerator::new();

        // Test Latin punctuation → Arabic punctuation
        // Question mark: ? → ؟
        let icr_question = ICR {
            segments: vec![
                ICRSegment {
        ...
        let result = generator.generate(&icr_question, Script::Arabic).unwrap();
        assert!(result.contains('؟'), "Should contain Arabic question mark ؟, got: {}", result);
        ...
        let result = generator.generate(&icr_comma, Script::Arabic).unwrap();
        assert!(result.contains('،'), "Should contain Arabic comma ،, got: {}", result);
    }

    #[test]
    fn test_punctuation_arabic_to_latin() {
        let generator = ScriptGenerator::new();

        // Test Arabic punctuation → Latin punctuation
        // Arabic question mark: ؟ → ?
        let icr_question = ICR {
            segments: vec![
                ICRSegment {
                    canonical: "K".to_string(),
                    original: "ك".to_string(),
                    segment_type: SegmentType::Consonant,
                    confidence: 1.0,
                    word_context: None,
                },
        ...
        let result = generator.generate(&icr_question, Script::Latin).unwrap();
        assert!(result.contains('?'), "Should contain Latin question mark ?, got: {}", result);
        ...
        let result = generator.generate(&icr_comma, Script::Latin).unwrap();
        assert!(result.contains(','), "Should contain Latin comma ,, got: {}", result);
    }
}
