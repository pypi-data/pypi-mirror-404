...
def extract_span_info(span: ReadableSpan):
    ...
    # Note: 'llm.output_messages.output_messages' ...
    # Unflattened: llm -> output_messages -> output_messages
    assign(variables, 'llm_output_messages', to_str_or_none(attributes_json.get('llm', {}).get('output_messages', {}).get('output_messages')))
    assign(variables, 'llm_input_messages', to_str_or_none(attributes_json.get('llm', {}).get('input_messages', {}).get('input_messages')))
    assign(variables, 'llm_model', to_str_or_none(attributes_json.get('llm', {}).get('model_name')))
    assign(variables, 'llm_invocation_parameters', to_str_or_none(attributes_json.get('llm', {}).get('invocation_parameters')))
    ...
    assign(variables, 'gen_ai.provider.name', to_str_or_none(attributes_json.get('gen_ai', {}).get('provider', {}).get('name')))
    assign(variables, 'gen_ai.operation.name', to_str_or_none(attributes_json.get('gen_ai', {}).get('operation', {}).get('name')))
    assign(variables, 'server.address', to_str_or_none(attributes_json.get('server', {}).get('address')))
    assign(variables, 'server.port', attributes_json.get('server', {}).get('port'))
    assign(variables, 'error.type', to_str_or_none(attributes_json.get('error', {}).get('type')))

    # Request Configuration
    assign(variables, 'gen_ai.request.model', to_str_or_none(attributes_json.get('gen_ai', {}).get('request', {}).get('model') or attributes_json.get('llm', {}).get('model_name')))
    assign(variables, 'gen_ai.request.temperature', attributes_json.get('gen_ai', {}).get('request', {}).get('temperature'))
    assign(variables, 'gen_ai.request.top_p', attributes_json.get('gen_ai', {}).get('request', {}).get('top_p'))
    assign(variables, 'gen_ai.request.top_k', attributes_json.get('gen_ai', {}).get('request', {}).get('top_k'))
    assign(variables, 'gen_ai.request.max_tokens', attributes_json.get('gen_ai', {}).get('request', {}).get('max_tokens'))
    assign(variables, 'gen_ai.request.frequency_penalty', attributes_json.get('gen_ai', {}).get('request', {}).get('frequency_penalty'))
    assign(variables, 'gen_ai.request.presence_penalty', attributes_json.get('gen_ai', {}).get('request', {}).get('presence_penalty'))
    assign(variables, 'gen_ai.request.stop_sequences', to_str_or_none(attributes_json.get('gen_ai', {}).get('request', {}).get('stop_sequences')))
    assign(variables, 'gen_ai.request.seed', attributes_json.get('gen_ai', {}).get('request', {}).get('seed'))
    assign(variables, 'gen_ai.request.choice.count', attributes_json.get('gen_ai', {}).get('request', {}).get('choice', {}).get('count'))
    assign(variables, 'gen_ai.request.encoding_formats', to_str_or_none(attributes_json.get('gen_ai', {}).get('request', {}).get('encoding_formats')))

    # Response & Usage
    assign(variables, 'gen_ai.response.model', to_str_or_none(attributes_json.get('gen_ai', {}).get('response', {}).get('model')))
    assign(variables, 'gen_ai.response.id', to_str_or_none(attributes_json.get('gen_ai', {}).get('response', {}).get('id')))
    assign(variables, 'gen_ai.response.finish_reasons', to_str_or_none(attributes_json.get('gen_ai', {}).get('response', {}).get('finish_reasons')))
    assign(variables, 'gen_ai.usage.input_tokens', attributes_json.get('gen_ai', {}).get('usage', {}).get('input_tokens'))
    assign(variables, 'gen_ai.usage.output_tokens', attributes_json.get('gen_ai', {}).get('usage', {}).get('output_tokens'))
    assign(variables, 'gen_ai.usage.total_tokens', attributes_json.get('gen_ai', {}).get('usage', {}).get('total_tokens'))
    assign(variables, 'gen_ai.output.type', to_str_or_none(attributes_json.get('gen_ai', {}).get('output', {}).get('type')))

    # Content & Messages
    assign(variables, 'gen_ai.input.messages', to_str_or_none(attributes_json.get('gen_ai', {}).get('input', {}).get('messages')))
    assign(variables, 'gen_ai.output.messages', to_str_or_none(attributes_json.get('gen_ai', {}).get('output', {}).get('messages')))
    assign(variables, 'gen_ai.system_instructions', to_str_or_none(attributes_json.get('gen_ai', {}).get('system_instructions')))
    assign(variables, 'gen_ai.tool.definitions', to_str_or_none(attributes_json.get('gen_ai', {}).get('tool', {}).get('definitions')))

    # Agents & Frameworks
    assign(variables, 'gen_ai.agent.id', to_str_or_none(attributes_json.get('gen_ai', {}).get('agent', {}).get('id')))
    assign(variables, 'gen_ai.agent.name', to_str_or_none(attributes_json.get('gen_ai', {}).get('agent', {}).get('name')))
    assign(variables, 'gen_ai.agent.description', to_str_or_none(attributes_json.get('gen_ai', {}).get('agent', {}).get('description')))
    
    assign(variables, 'gen_ai.conversation.id', to_str_or_none(attributes_json.get('gen_ai', {}).get('conversation', {}).get('id')))
    assign(variables, 'gen_ai.data_source.id', to_str_or_none(attributes_json.get('gen_ai', {}).get('data_source', {}).get('id')))

    # Embeddings
    assign(variables, 'gen_ai.embeddings.dimension.count', attributes_json.get('gen_ai', {}).get('embeddings', {}).get('dimension', {}).get('count'))


    # Handling dict/list/string cases for Output -> ID 
    response_id = None
    
    # Check 'output.value' in attributes_json
    # It would be attributes_json.get('output', {}).get('value')
    # Or raw.output?
    
    # Let's check 'output' key directly.
    output_val = attributes_json.get('output') 
    
    # Let's also check 'output.value'
    output_value_val = attributes_json.get('output', {}).get('value')
    # output_value_val = attributes.get('raw.output') # THIS ONE WAS LEFT - but raw.output is strange.
    # original: output_value_val = attributes.get('raw.output')
    
    # Checking if attributes_json has 'raw' -> 'output'
    ...
    if isinstance(output_value_val, dict):
        ...
        # if it's a list of dicts, grab the first one (adjust if needed)
