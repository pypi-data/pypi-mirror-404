...
def _convert_assistant_message(msg: Message) -> dict[str, Any] | None:
    ...
    for part in msg.parts:
        if part.type == "text":
            ...
            content_parts.append(f"[Extended thinking: {len(part.text)} chars]")
            ...
            if tool_part.state.status == "completed":
                ...
                content_parts.append(
                    f"[Tool: {tool_part.tool}] {tool_part.state.title}\n{output}"
                )

            elif tool_part.state.status == "error":
                content_parts.append(
                    f"[Tool Error: {tool_part.tool}] {tool_part.state.error}"
                )

            elif tool_part.state.status in ["pending", "running"]:
                content_parts.append(
                    f"[Tool execution was interrupted: {tool_part.tool}]"
                )
