...
def run_async(coro: Coroutine[Any, Any, T], wait = False) -> T:
    """
    ...
    # global _tasks
    tasks = getattr(loop, '__tasks', None)
    if wait:
        if getattr(loop, '__tasks', None) is not None:
            loop.run_until_complete(tasks.__aexit__(None, None, None))
            loop.__tasks = None
        ...
        if tasks is None:
            ...
            loop.__tasks = tasks
            tasks._loop = loop
            loop.run_until_complete(tasks.__aenter__())
        # task = asyncio.create_task(coro)
        task = tasks.create_task(coro)
        return task
