...
def _build_filters_and_metadata(
    *,  # Enforce keyword-only arguments
    ...
    # set default source
    base_metadata_template['source'] = 'vector'
...
class AsyncMemory(MemoryBase):
    ...
    async def add(
        ...
        *,
        ...
        prompt: Optional[str] = None,
        llm=None,
        add_graph_results_to_vector_store: bool = False,
    ):
        """
        ...
        Args:
            ...
            add_graph_results_to_vector_store (bool, optional): Whether to add graph results to vector store. Defaults to False.
        Returns:
        ...
        if add_graph_results_to_vector_store:
            vector_store_result_from_graph = await self._add_graph_results_to_vector_store(graph_result, processed_metadata, effective_filters, infer)
            vector_store_result.extend(vector_store_result_from_graph)
    ...
    async def _add_graph_results_to_vector_store(self, graph_results, metadata, filter, infer):
        if not graph_results:
            return []
        
        added_entities = graph_results.get("added_entities", [])
        messages = []
        for entities in added_entities:
            for entity in entities:
                source = entity["source"]
                # Avoid duplicate user_id data in memory
                if source.startswith("user_id:"):
                    source = ""
                relationship = entity["relationship"]
                target = entity["target"]
                entity_data = f"{source} {relationship} {target}"
                message = {"role": "user", "content": entity_data}
                messages.append(message)

        if messages:
            copied_metadata = deepcopy(metadata) if metadata else {}
            copied_metadata['source'] = 'graph'
            return await self._add_to_vector_store(messages, copied_metadata, filter, infer)
