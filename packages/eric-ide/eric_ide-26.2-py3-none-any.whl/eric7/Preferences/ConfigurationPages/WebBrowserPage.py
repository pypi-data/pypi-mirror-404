...
from eric7.SystemUtilities.QtUtilities import qWebEngineVersionTuple
...
class WebBrowserPage(ConfigurationPageBase, Ui_WebBrowserPage):
    ...
    def __init__(self, configDialog):
        ...
        if qWebEngineVersionTuple() >= (6, 4, 0):
        ...
        if qWebEngineVersionTuple() >= (6, 5, 0):
        ...
        if qWebEngineVersionTuple() >= (6, 6, 0):
        ...
        if qWebEngineVersionTuple() >= (6, 7, 0):
        ...
        if qWebEngineVersionTuple() >= (6, 8, 0):
        ...
        if qWebEngineVersionTuple() >= (6, 9, 0):
            self.printHeaderAndFooterCheckBox.setChecked(
                Preferences.getWebBrowser("PrintHeaderAndFooter")
            )
        else:
            self.printHeaderAndFooterCheckBox.setEnabled(False)

        if qWebEngineVersionTuple() >= (6, 10, 0):
            self.backForwardCacheCheckBox.setChecked(
                Preferences.getWebBrowser("BackForwardCacheEnabled")
            )
        else:
            self.backForwardCacheCheckBox.setEnabled(False)
    ...
    def save(self):
        ...
        language = (
            ...
            if languageIndex > -1
            else QLocale.system().language()  # fall back to system default
        )
        ...
        if self.printHeaderAndFooterCheckBox.isEnabled():
            Preferences.setWebBrowser(
                "PrintHeaderAndFooter", self.printHeaderAndFooterCheckBox.isChecked()
            )

        if self.backForwardCacheCheckBox.isEnabled():
            Preferences.setWebBrowser(
                "BackForwardCacheEnabled", self.backForwardCacheCheckBox.isChecked()
            )
