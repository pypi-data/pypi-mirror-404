...
from PyQt6.QtCore import QPoint, QUrl, Qt, pyqtSignal, pyqtSlot
...
class FindFileWidget(QWidget, Ui_FindFileWidget):
    ...
    @pyqtSlot(str)
    def on_findtextCombo_editTextChanged(self, _text):
        """
    ...
    @pyqtSlot(str)
    def on_replacetextCombo_editTextChanged(self, _text):
        """
    ...
    @pyqtSlot(str)
    def on_dirPicker_editTextChanged(self, _text):
        """
    ...
    @pyqtSlot(int)
    def on_filterComboBox_currentIndexChanged(self, _index):
        """
    ...
    @pyqtSlot(int)
    def on_excludeFilterComboBox_currentIndexChanged(self, _index):
        """
    ...
    def __doSearch(self):
        ...
        if self.filterCheckBox.isChecked():
            ...
            fileFilterPattern = "|".join(
                self.__buildReFileFilter(f) for f in fileFilter
            )
        ...
        if self.excludeFilterCheckBox.isChecked():
            ...
            excludeFileFilterPattern = "|".join(
                self.__buildReFileFilter(e) for e in excludeFileFilter
            )
                            ...
                            if fileType == "SOURCES":
                                ...
                                filters.extend(
                                    [
                                        ...
                                        for ext in (
                                            self.__project.getFileCategoryExtension(
                                                fileType
                                            )
                                        )
                                    ]
        ...
        for progress, file in enumerate(files, start=1):
            ...
            try:
                text, _encoding, hashStr = Utilities.readEncodedFileWithHash(fn)
            ...
            for count, line in enumerate(lines, start=1):
                ...
                if contains:
                    ...
                    rline = search.sub(replTxt, line) if self.__replaceMode else ""
    ...
    @pyqtSlot(QPoint)
    def __contextMenuRequested(self, _pos):
        """
        Private slot to handle the context menu request.

        @param _pos position the context menu shall be shown
        @type QPoint
        """
