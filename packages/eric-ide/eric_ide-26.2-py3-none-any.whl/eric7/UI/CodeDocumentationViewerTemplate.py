...
def _stylesheet():
    ...
    if not _stylesheetsCache[stylesheetType]:
        ...
        with open(stylesheetFilePath) as f:
...
def prepareDocumentationViewerHtmlDocument(documentationInfo):
    ...
    if name:
        ...
        if documentationInfo.get("argspec"):
        ...
        if documentationInfo.get("typ"):
            typeInfo = typeTemplate.replace("@TYPE@", documentationInfo["typ"])
        else:
            typeInfo = ""

        if documentationInfo.get("note"):
    ...
    if documentationInfo.get("docstring"):
        docstring = (
