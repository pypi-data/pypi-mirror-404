...
class UserInterface(EricMainWindow):
    ...
    def __init__(
        ...
        splash.showMessage(self.tr("Preparing Main User Interface..."))
        logging.getLogger(__name__).debug("Preparing Main User Interface...")
        ...
        splash.showMessage(self.tr("Preparing Plugins Toolbars..."))
        logging.getLogger(__name__).debug("Preparing Plugins Toolbars...")
    ...
    def __createLayout(self):
        ...
        # Create layout with sidebar windows embedded in dock windows
            ...
            exc = "Wrong layout type given ({0})".format(self.__layoutType)
    ...
    def processInstallInfoFile(self):
        ...
        if not os.path.exists(installInfoFile):
            ...
            changed = False
            with open(installInfoFile) as infoFile:
                installInfo = json.load(infoFile)
    ...
    def __initMenus(self):
        ...
        self.__menus["plugins"] = QMenu(self.tr("P&lugins"))
        self.__menus["plugins"].setIcon(EricPixmapCache.getIcon("plugin"))
        ...
        self.__menus["extras"].addMenu(self.__menus["plugins"])
        self.__menus["extras"].addSeparator()
    ...
    def __updateExternalToolsActions(self):
        ...
        groupActionKeys = [
            key for key in self.toolGroupActions if key.startswith(groupkey)
        ]

        # step 2: build keys for all actions i.a.w. current configuration
        ckeys = [
            "{0}@@{1}".format(toolGroup[0], tool["menutext"])
            for tool in toolGroup[1]
            if tool["menutext"] != "--"
        ]
    ...
    def __showVersions(self):
        ...
        with contextlib.suppress(ImportError, AttributeError):
            ...
            versionInfo.append(
                "<tr><td><b>PyQt6-Charts</b></td><td>{0}{1}</td></tr>".format(
                    QtCharts.PYQT_CHART_VERSION_STR,
                    (
                        self.tr("<em> (not needed)</em>")
                        if PYQT_VERSION >= 0x60800
                        else ""
                    ),
                )
            )
        with contextlib.suppress(ImportError, AttributeError):
            from PyQt6 import QtGraphs  # __IGNORE_WARNING_I-10__

            versionInfo.append(
                "<tr><td><b>PyQt6-Graphs</b></td>"
                f"<td>{QtGraphs.PYQT_GRAPHS_VERSION_STR}</td></tr>"
            )
        with contextlib.suppress(ImportError, AttributeError):
    ...
    def __showSystemEmailClient(self, mode, attachFile=None, deleteAttachFile=False):
        ...
        subject = "[eric] "
        if attachFile is not None:
            with open(attachFile, encoding="utf-8") as f:
                body = f.read()
            if deleteAttachFile:
    ...
    def __compareFiles(self):
        ...
        fn = aw.getFileName() if aw else None
    ...
    def __editorClosing(self, editor):
        """
        Private method called, when an external eric editor is closing.
    ...
    def registerToolbar(self, name, text, toolbar, category=""):
        ...
        if name in self.__toolbars:
            exc = "Toolbar '{0}' already registered.".format(name)
            raise KeyError(exc)
    ...
    def __toolFinished(self, _exitCode, _exitStatus):
        ...
        for proc in exitedProcs:
            ...
            self.appendToStdout(t)
            proc[1].deleteLater()  # index 1 is the QProcess object
    ...
    def __showPythonDoc(self):
        ...
        if not pythonDocDir.startswith(("http://", "https://", "qthelp://")):
            pythonDocDir = pythonDocDir.removeprefix("file://")
    ...
    def __showQtDoc(self, version):
        ...
        if qtDocDir.startswith("qthelp://"):
            ...
            qtDocDir = qtDocDir.removeprefix("file://")
            if not os.path.splitext(qtDocDir)[1]:
    ...
    def __showPyQtDoc(self, variant=6):
        ...
        if not pyqtDocDir.startswith(("http://", "https://", "qthelp://")):
            home = ""
            if pyqtDocDir:
                pyqtDocDir = pyqtDocDir.removeprefix("file://")
    ...
    def __showPySideDoc(self, variant=6):
        ...
        if not pysideDocDir.startswith(("http://", "https://", "qthelp://")):
            pysideDocDir = pysideDocDir.removeprefix("file://")
    ...
    def __saveSessionToFile(self):
        ...
        """
        sessionFile = EricFileDialog.getSaveFileNameWithExtension(
            self,
            self.tr("Save Session"),
    ...
    def dragLeaveEvent(self, _event):
        ...
        @param _event the drag leave event
