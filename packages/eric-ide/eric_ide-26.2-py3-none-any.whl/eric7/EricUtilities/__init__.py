#
# Copyright (c) 2017 - 2026 Detlev Offenbach <detlev@die-offenbachs.de>
#
...
def toBool(value):
    ...
    if (isinstance(value, str) and value.lower() in ["true", "1", "yes"]) or (
        ...
        return True
    if (isinstance(value, str) and value.lower() in ["false", "0", "no"]) or (
...
def dataString(size, loc=None, metric=False):
    """
    ...
    @param metric flag indicating a metric conversion (defaults to False)
    @type bool (optional)
    ...
    if metric:
        if loc is None:
            if size < 1000:
                return QCoreApplication.translate(
                    "EricUtilities", "{0:4d} Bytes"
                ).format(size)
            if size < 1000 * 1000:
                size /= 1000
                return QCoreApplication.translate(
                    "EricUtilities", "{0:4.2f} kB"
                ).format(size)
            if size < 1000 * 1000 * 1000:
                size /= 1000 * 1000
                return QCoreApplication.translate(
                    "EricUtilities", "{0:4.2f} MB"
                ).format(size)
            if size < 1000 * 1000 * 1000 * 1000:
                size /= 1000 * 1000 * 1000
                return QCoreApplication.translate(
                    "EricUtilities", "{0:4.2f} GB"
                ).format(size)
            size /= 1000 * 1000 * 1000 * 1000
            return QCoreApplication.translate("EricUtilities", "{0:4.2f} TB").format(
                size
            )
        ...
        if size < 1000 * 1000:
            size /= 1000
            return QCoreApplication.translate("EricUtilities", "{0} kB").format(
                loc.toString(size, "f", 2)
            )
        if size < 1000 * 1000 * 1000:
            size /= 1000 * 1000
            return QCoreApplication.translate("EricUtilities", "{0} MB").format(
                loc.toString(size, "f", 2)
            )
        if size < 1000 * 1000 * 1000 * 1000:
            size /= 1000 * 1000 * 1000
            return QCoreApplication.translate("EricUtilities", "{0} GB").format(
                loc.toString(size, "f", 2)
            )
        size /= 1000 * 1000 * 1000 * 1000
        return QCoreApplication.translate("EricUtilities", "{0} TB").format(
            loc.toString(size, "f", 2)
        )
    if loc is None:
        if size < 1024:
        ...
        return QCoreApplication.translate("EricUtilities", "{0:4.2f} TiB").format(size)
    if not isinstance(size, float):
        size = float(size)
...
def decodeBytes(buffer):
    ...
    with contextlib.suppress(UnicodeError, LookupError):
        ...
        if buffer.startswith(codecs.BOM_UTF16):
            # UTF-16 with BOM
            return str(buffer[len(codecs.BOM_UTF16) :], encoding="utf-16")
        if buffer.startswith(codecs.BOM_UTF32):
            # UTF-32 with BOM
...
def html_uencode(text, pattern=_uescape):
    ...
    if not text:
        return ""
    return pattern.sub(escape_uentities, text)
...
def html_udecode(text, pattern=_uunescape):
    ...
    if not text:
        return ""
    return pattern.sub(unescape_uentities, text)
