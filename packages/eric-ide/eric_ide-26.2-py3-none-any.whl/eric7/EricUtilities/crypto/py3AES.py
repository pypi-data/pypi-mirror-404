...
def strip_PKCS7_padding(b):
    ...
    if len(b) % 16 or not b:
        err = "Data of len {0} can't be PCKS7-padded".format(len(b))
        raise ValueError(err)
    numpads = b[-1]
    if numpads > 16:
        err = "Data ending with {0} can't be PCKS7-padded".format(b[-1])
        raise ValueError(err)
    return b[:-numpads]
...
class AES:
    ...
    def __aes_invRound(self, state, roundKey):
        ...
        return self.__mixColumns(state, True)
    ...
    def __aes_invMain(self, state, expandedKey, nbrRounds):
        ...
        state = self.__shiftRows(state, True)
        state = self.__subBytes(state, True)
        return self.__addRoundKey(state, self.__createRoundKey(expandedKey, 0))
    ...
    def encrypt(self, iput, key, size):
        ...
        if size not in self.KeySize.values():
            err = "Wrong key size given ({0}).".format(size)
            raise ValueError(err)
...
class AESModeOfOperation:
    ...
    def encrypt(self, inputData, mode, key, size, IV):
        ...
        """
        if len(key) % size:
            err = "Illegal size ({0}) for key '{1}'.".format(size, key)
            raise ValueError(err)
        if len(IV) % 16:
            err = "IV is not a multiple of 16."
            raise ValueError(err)
        ...
        firstRound = True
        if inputData:
            for j in range(math.ceil(float(len(inputData)) / 16)):
                start = j * 16
                end = j * 16 + 16
...
def encryptData(key, data, mode=AESModeOfOperation.ModeOfOperation["CBC"]):
    ...
    if keysize not in AES.KeySize.values():
        err = "invalid key size: {0}".format(keysize)
        raise ValueError(err)
