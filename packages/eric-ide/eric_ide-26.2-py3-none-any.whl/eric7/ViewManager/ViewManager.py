...
class ViewManager(QWidget):
    ...
    def __init__(self):
        ...
        watcher.fileChanged.connect(self.__watchedFileChanged)
        watcher.error.connect(self.__watcherError)

        self.__watchedFilePaths = []

        self.__history = NavigationHistory(
            vm=self,
            offset=Preferences.getUI("NavigationHistoryLineOffset"),
            maxEntries=Preferences.getUI("NavigationHistoryMaximumSize"),
            parent=self,
        )
        self.editorLineChanged.connect(self.__history.recordPosition)
        self.__history.historyChanged.connect(self.__checkHistoryButtonStates)
        self.__history.historyCleared.connect(self.__checkHistoryButtonStates)
        self.__history.historyItemAdded.connect(self.__checkHistoryButtonStates)
    ...
    def _removeView(self, win):  # noqa: U-100
        """
    ...
    def showWindowMenu(self, windowMenu):  # noqa: U-100
        """
    ...
    def setEditorName(self, editor, newName):  # noqa: U-100
        """
    ...
    def _modificationStatusChanged(self, m, editor):  # noqa: U-100
        """
    ...
    @pyqtSlot()
    def removeSplit(self, index=-1):  # noqa: ARG002
        """
    ...
    def eventFilter(self, _qobject, _event):
        """
        Public method called to filter an event.

        @param _qobject object, that generated the event
        @type QObject
        @param _event the event, that was generated by object
        @type QEvent
    ...
    def __initEditActions(self):
        ...
        act = EricAction(
            ...
            QKeySequence(QCoreApplication.translate("ViewManager", "Ctrl+U")),
            self.editorActGrp,
        ...
        ###################################################################
        ## Below follow the actions for backward and forward history
        ## navigation commands.
        ###################################################################

        self.backAct = EricAction(
            QCoreApplication.translate("ViewManager", "Backward"),
            EricPixmapCache.getIcon("back"),
            QCoreApplication.translate("ViewManager", "Backward"),
            QKeySequence(self.tr("Meta+Ctrl+Left", "Go|Backward")),
            0,
            self,
            "vm_edit_go_backward",
        )
        self.backAct.setStatusTip(self.tr("Move to previous location"))
        self.backAct.setWhatsThis(
            self.tr(
                """<b>Backward</b>"""
                """<p>Moves to the previous global cursor location. If none is"""
                """ available, this action is disabled.</p>"""
            )
        )
        self.backAct.triggered.connect(self.__history.back)
        self.editActions.append(self.backAct)

        self.__backMenu = QMenu()
        self.__backMenu.aboutToShow.connect(self.__aboutToShowBackMenu)
        self.__backMenu.triggered.connect(self.__historyMenuTriggered)
        self.backAct.setMenu(self.__backMenu)

        self.forwardAct = EricAction(
            QCoreApplication.translate("ViewManager", "Forward"),
            EricPixmapCache.getIcon("forward"),
            QCoreApplication.translate("ViewManager", "Forward"),
            QKeySequence(self.tr("Meta+Ctrl+Right", "Go|Forward")),
            0,
            self,
            "vm_edit_go_forward",
        )
        self.forwardAct.setStatusTip(self.tr("Move to next location"))
        self.forwardAct.setWhatsThis(
            self.tr(
                """<b>Forward</b>"""
                """<p>Moves to the next global cursor location. If none is"""
                """ available, this action is disabled.</p>"""
            )
        )
        self.forwardAct.triggered.connect(self.__history.forward)
        self.editActions.append(self.forwardAct)

        self.__forwardMenu = QMenu()
        self.__forwardMenu.aboutToShow.connect(self.__aboutToShowForwardMenu)
        self.__forwardMenu.triggered.connect(self.__historyMenuTriggered)
        self.forwardAct.setMenu(self.__forwardMenu)
    ...
    def initEditToolbar(self, toolbarManager):
        ...
        tb.addSeparator()
        tb.addAction(self.backAct)
        tb.addAction(self.forwardAct)
    ...
    def __openFiles(self):
        ...
        progs = EricFileDialog.getOpenFileNames(
            self.ui,
    ...
    def openSourceFile(
        ...
        """
        Public method to display a file in an editor.
    ...
    def __setSbFile(
        ...
        if not fn:
            ...
            writ = "rw" if os.access(fn, os.W_OK) else "ro"
    ...
    def __editSpellingDictionary(self, dictionaryFile):
        ...
        if os.path.exists(dictionaryFile):
            try:
                with open(dictionaryFile, encoding="utf-8") as f:
                    data = f.read()
    ...
    def preferencesChanged(self):
        ...
        self.__history.setMinimumLineOffset(
            Preferences.getUI("NavigationHistoryLineOffset")
        )
        self.__history.setMaximumSize(Preferences.getUI("NavigationHistoryMaximumSize"))
    ...
    def __watchedFileChanged(self, filePath):
        """
        Private slot handling modification, renaming or deletion of a watched file.
        ...
        if editorList:
            ...
            editorList.pop(0).checkReloadFile()
    ...
    @pyqtSlot()
    def __checkHistoryButtonStates(self):
        """
        Private slot to set the enabled state of the history buttons.
        """
        self.backAct.setEnabled(self.__history.canGoBack())
        self.forwardAct.setEnabled(self.__history.canGoForward())

    @pyqtSlot()
    def __aboutToShowBackMenu(self):
        """
        Private slot to prepare the 'back' menu before it is shown.
        """
        self.__backMenu.clear()

        backItems = self.__history.backItems()
        for item in reversed(
            backItems[-Preferences.getUI("NavigationMaximumHistoryMenuItems") :]
        ):
            filepath = (
                self.__remotefsInterface.compactPath(
                    item.filename, self.ui.maxMenuFilePathLen
                )
                if FileSystemUtilities.isRemoteFileName(item.filename)
                else FileSystemUtilities.compactPath(
                    item.filename, self.ui.maxMenuFilePathLen
                )
            )
            act = self.__backMenu.addAction(
                self.tr("{0} : {1}", "file name : line number").format(
                    filepath, item.linenumber
                )
            )
            act.setData(item)

        self.__backMenu.addSeparator()
        self.__backMenu.addAction(self.tr("Clear History"), self.__history.clear)

    @pyqtSlot()
    def __aboutToShowForwardMenu(self):
        """
        Private slot to prepare the 'forward' menu before it is shown.
        """
        self.__forwardMenu.clear()

        forwardItems = self.__history.forwardItems()
        for item in forwardItems[
            : Preferences.getUI("NavigationMaximumHistoryMenuItems")
        ]:
            filepath = (
                self.__remotefsInterface.compactPath(
                    item.filename, self.ui.maxMenuFilePathLen
            ...
            act = self.__forwardMenu.addAction(
                self.tr("{0} : {1}", "file name : line number").format(
                    filepath, item.linenumber
                )
            )
        ...
        self.__forwardMenu.addAction(self.tr("Clear History"), self.__history.clear)

    @pyqtSlot(QAction)
    def __historyMenuTriggered(self, act):
        """
        Private slot handling the selection of an action of the 'back' menu.

        @param act reference to the triggered action
        @type QAction
        """
        navItem = act.data()
        if navItem is not None:
            self.__history.goToItem(navItem)
