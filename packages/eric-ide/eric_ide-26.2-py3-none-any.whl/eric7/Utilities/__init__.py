...
def __showwarning(
    ...
    filename,
    lineno,
    file=None,  # noqa: ARG001, U-100
    line=None,  # noqa: ARG001, U-100
):
    """
...
def decode(text):
    ...
    with contextlib.suppress(UnicodeError, LookupError):
        ...
        if text.startswith(codecs.BOM_UTF16):
            # UTF-16 with BOM
            return str(text[len(codecs.BOM_UTF16) :], "utf-16"), "utf-16"
        if text.startswith(codecs.BOM_UTF32):
            # UTF-32 with BOM
    ...
    if origEncoding == "utf-8-bom":
        ...
        if coding:
            try:
                ...
                raise CodingError(coding) from None
        else:
            if forcedEncoding:
                with contextlib.suppress(UnicodeError, LookupError):
            ...
            if encoding is None:
                ...
                if origEncoding and origEncoding.endswith(
                    ...
                    coding = origEncoding.rsplit("-", 1)[0]
                    with contextlib.suppress(UnicodeError, LookupError):
                        etext, encoding = text.encode(coding), coding
...
def linesep():
    ...
    if eolMode == QsciScintilla.EolMode.EolUnix:
        return "\n"
    if eolMode == QsciScintilla.EolMode.EolMac:
        return "\r"
    return "\r\n"
...
def extractFlagsFromFile(filename):
    ...
    """
    try:
        source, _encoding = readEncodedFile(filename)
...
def getPercentReplacement(code):
    ...
    if code in ["C", "%C"]:
        ...
        column = -1 if aw is None else aw.getCursorPosition()[1]
    ...
    if code in ["D", "%D"]:
        ...
        if aw is None:
            ...
            fn = aw.getFileName()
            dn = "not_available" if fn is None else os.path.dirname(fn)
        return dn
    if code in ["F", "%F"]:
    ...
    if code in ["L", "%L"]:
        ...
        aw = ericApp().getObject("ViewManager").activeWindow()
        line = 0 if aw is None else aw.getCursorPosition()[0] + 1
    ...
    if code in ["S", "%S"]:
        ...
        aw = ericApp().getObject("ViewManager").activeWindow()
        return "not_available" if aw is None else aw.selectedText()
    if code in ["U", "%U"]:
        # username
...
def generateVersionInfo(linesep="\n"):
    ...
    if PYQT_VERSION >= 0x60800:
        try:
            from PyQt6 import QtGraphs  # noqa: I-101, I-102

            info.append(f"  PyQt6-Graphs {QtGraphs.PYQT_GRAPHS_VERSION_STR}")
        except (AttributeError, ImportError):
            info.append("  PyQt6-Graphs not installed")
    try:
