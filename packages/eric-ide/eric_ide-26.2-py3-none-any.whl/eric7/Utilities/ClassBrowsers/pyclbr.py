...
|   (?P<Metadata>
        ^ \#\s///\s (?P<MetadataType> [a-zA-Z0-9-]+ ) $ \s
        (?P<MetadataContent> (?: ^ \# (?: |\s.*? ) $ \s )+? )
        ^ \# \s/// $
    )
...
class Metadata:
    """
    Class to represent the list of metadata sections.
    """
    ...
    def __init__(self, file):
        ...
        @param file file name containing the metadata
        @type str
        """
        self.name = "metadata"
        self.file = file
    ...
    def addMetadata(self, metadataType, metadataContent, lineno):
        """
        Public method to add the type name of a metadata section.

        @param metadataType type name of the metadata section
        @type str
        @param metadataContent content of the metadata section
        @type str
        @param lineno line number of the import
        @type int
        """
        if metadataType not in self.metadataEntries:
            content = "".join(
                line[2:] if line.startswith("# ") else line[1:]
                for line in metadataContent.splitlines(keepends=True)
            )
            meta = MetadataEntry(
                file=self.file, lineno=lineno, typ=metadataType, content=content
            )
            self.metadataEntries[metadataType] = meta

    def getMetadata(self, metadataType):
        """
        Public method to get a metadata item.
        ...
        @rtype MetadataEntry
        """
        return self.metadataEntries.get(metadataType)

    def getMetadataEntries(self):
        """
        Public method to get all imported module names.
        ...
        @rtype dict
        """
        return self.metadataEntries


@dataclass
class MetadataEntry:
    """
    Class defining the data structure for a metadata entry.
    """

    file: str
    lineno: int
    typ: str
    content: str  # content without leading '#' or '# '
...
def calculateEndline(lineno, lines, indent):
    ...
    inTripleQotedString = False

    while lineno < len(lines):
        line = lines[lineno]
        if line.strip() and not line.lstrip().startswith(("#", ")")):
            # line contains some text and does not start with
            # a comment sign or the closing bracket of a function
            # definition
            if bool(line.count('"""') % 2) or bool(line.count("'''") % 2):
                inTripleQotedString = not inTripleQotedString
                lineno += 1
                continue

            lineIndent = _indent(line.replace(line.lstrip(), ""))
            if lineIndent <= indent and not inTripleQotedString:
                return lineno
        lineno += 1
...
def scan(src, file, module):
    ...
    while True:
        ...
        elif m.captured("Metadata"):
            # a metadat section
            metadataType = m.captured("MetadataType")
            metadataContent = m.captured("MetadataContent")
            lineno += src.count("\n", last_lineno_pos, start)
            last_lineno_pos = start
            if "@@Metadata@@" not in dictionary:
                dictionary["@@Metadata@@"] = Metadata(file)
            dictionary["@@Metadata@@"].addMetadata(
