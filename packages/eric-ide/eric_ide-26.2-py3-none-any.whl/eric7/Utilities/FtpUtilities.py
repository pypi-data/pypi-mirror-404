#
# Copyright (c) 2012 - 2026 Detlev Offenbach <detlev@die-offenbachs.de>
#
...
class FtpDirLineParser(QObject):
    ...
    def __parseUnixMode(self, modeString, urlInfo):
        ...
        if len(modeString) != 10:
            exc = "invalid mode string '{0}'".format(modeString)
            raise FtpDirLineParserError(exc)
    ...
    def __parseUnixTime(self, monthAbbreviation, day, yearOrTime, urlInfo):
        ...
        try:
            ...
            exc = "illegal month abbreviation '{0}'".format(monthAbbreviation)
            raise FtpDirLineParserError(exc) from None
        day = int(day)
        if ":" in yearOrTime:
    ...
    def __splitUnixLine(self, line):
        ...
        if len(lineParts) < fieldCountWithoutUserID:
            exc = "line '{0}' cannot be parsed".format(line)
            raise FtpDirLineParserError(exc)
    ...
    def __parseWindowsTime(self, date, time, urlInfo):
        ...
        try:
            ...
            exc = f"illegal date string '{month}'"
            raise FtpDirLineParserError(exc) from None
        try:
            ...
            exc = f"illegal time string '{month}'"
            raise FtpDirLineParserError(exc) from None
    ...
    def __parseWindowsLine(self, line):
        ...
        try:
            ...
            exc = f"line '{line}' cannot be parsed"
            raise FtpDirLineParserError(exc) from None
