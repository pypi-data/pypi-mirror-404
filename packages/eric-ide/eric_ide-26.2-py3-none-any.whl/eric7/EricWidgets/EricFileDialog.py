...
def getSaveFileNameWithExtension(
    ...
    fname, selectedFilter = getSaveFileNameAndFilter(
        parent, caption, directory, filterStr, initialFilter, options
    )
    if fname:
        ...
        if not fpath.suffix:
            try:
                ex = selectedFilter.rsplit("(*", 1)[1].split(")", 1)[0]
            except IndexError:
                ex = ""
            if ex:
...
def getSaveFilePathWithSuffix(
    parent=None, caption="", directory="", filterStr="", initialFilter="", options=None
):
    """
    Module function to get the path of a file for saving ensuring it has a file
    suffix.
    ...
    fpath, selectedFilter = getSaveFilePathAndFilter(
        parent, caption, directory, filterStr, initialFilter, options
    )
    if fpath.name and not fpath.suffix:
        try:
            ex = selectedFilter.rsplit("(*", 1)[1].split(")", 1)[0]
        except IndexError:
            ex = ""
