...
class QtHelpGenerator:
    ...
    def __init__(
        ...
        self.filterAttributes = filterAttributes.split(":") if filterAttributes else []
    ...
    def __generateSections(self, package, level):
        ...
        s = indent + '<section title="{0}" ref="{1}">\n'.format(
            self.title if package == "00index" else package,
            FileSystemUtilities.joinext("index", ".html")
            if package == "00index"
            else FileSystemUtilities.joinext("index-{0}".format(package), ".html"),
        )
