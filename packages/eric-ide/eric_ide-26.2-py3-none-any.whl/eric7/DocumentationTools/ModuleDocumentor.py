...
class ModuleDocument:
        ...
        try:
            if self.module.type == RB_SOURCE:
                ...
                modBody = TemplatesListsStyleCSS.rbFileTemplate.format(
                    ...
                    ModuleDescription=self.__formatDescription(self.module.description),
    ...
    def __genGlobalsListSection(self, class_=None):
        ...
        s = (
            "\n".join(
                [
                    TemplatesListsStyleCSS.listEntrySimpleTemplate.format(Name=name)
    ...
    def __genDescriptionListSection(self, dictionary, template):
        ...
        lst = [
            template.format(
                Name=key, Description=html_uencode("\n".join(dictionary[key]))
            )
            ...
            if type_:
                ...
                lst.append(
                    TemplatesListsStyleCSS.parametersListEntryTemplate.format(
                        Name=name, Description=html_uencode("\n".join(lines))
                    )
                )
        return "".join(lst)
    ...
    def __formatCrossReferenceEntry(self, entry):
        ...
        if entry.startswith("<"):
            ...
            try:
                ...
                path = reference
                anchor = ""
            reference = "{0}.html".format(path) if path else ""
            if anchor:
    ...
    def __processInlineTags(self, desc):
        ...
        while start != -1:
            ...
            if tagText.startswith("@link"):
                ...
                if len(parts) < 2:
                    err = "Wrong format in inline tag {0}.\n{1}".format(parts[0], desc)
                    raise TagError(err)
                ...
                err = "Unknown inline tag encountered, {0}.\n{1}".format(tag, desc)
                raise TagError(err)
            ...
            if desc:
                if desc.startswith(("@param", "@keyparam")):
                    ...
                    if lastTag not in ["@param", "@keyparam"]:
                        err = "{0} line must be preceded by a parameter line\n".format(
                            parts[0]
                        )
                        raise TagError(err)
                    inTagSection = True
                    ...
                    err = "Tag '@ptype' is deprecated, use '@type' instead\n"
                    raise TagError(err)
                elif desc.startswith("@ireturn"):
                    err = "Tag '@ireturn' is deprecated, use '@return' instead\n"
                    raise TagError(err)
                    ...
                    parts = desc.split(None, 1)
                    if lastTag != "@return":
                        err = "{0} line must be preceded by a @return line\n".format(
                            parts[0]
                        )
                        raise TagError(err)
                    inTagSection = True
                    ...
                    parts = desc.split(None, 1)
                    if lastTag != "@yield":
                        err = "{0} line must be preceded by a @yield line\n".format(
                            parts[0]
                        )
                        raise TagError(err)
                    inTagSection = True
                    ...
                    err = "Tag '{0}' is deprecated, use '@exception' instead\n".format(
                        tag
                    )
                    raise TagError(err)
