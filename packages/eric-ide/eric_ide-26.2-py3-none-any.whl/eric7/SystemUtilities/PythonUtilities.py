...
def isPythonSource(filename, source, editor=None):
    ...
    if filename:
        ...
        if (
            ...
            return True
        if source:
            line0 = source.splitlines()[0] if isinstance(source, str) else source[0]
...
def __searchInterpreters_Windows(environments=None):
    ...
    minorVersions = range(10, 16)  # Py 3.8 until Py 3.15
    interpreters = set()
...
def getPythonInterpreterVersion(interpreter):
    """
    Function to determine the exact version of a given Python interpreter.

    @param interpreter path of the Python interpreter binary
    @type str
    @return version of the Python interpreter
    @rtype str
    """
    if os.path.exists(interpreter):
        ...
        if output:
            versionLineParts = output.splitlines()[0].split()
            return versionLineParts[1]  # assume that the second word is the version
