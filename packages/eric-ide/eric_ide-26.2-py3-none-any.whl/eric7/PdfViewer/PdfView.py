...
class PdfView(QPdfView):
    ...
    def __zoomFactorForMode(self, zoomMode):
        ...
        if zoomMode == QPdfView.ZoomMode.FitToWidth:
            ...
            pageSize = pageSize.scaled(viewportSize, Qt.AspectRatioMode.KeepAspectRatio)
        return (
            pageSize.width()
            / (self.document().pagePointSize(curPage) * self.__screenResolution).width()
        )
