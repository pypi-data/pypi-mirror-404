...
try:
    try:
        ...
        from .MicroPythonChartWidget import (
            MicroPythonChartWidget as MicroPythonGraphWidget,
        )

    HasQtChartsOrGraphs = True
except ImportError:
    HasQtChartsOrGraphs = False

try:
    from .MicroPythonSerialDeviceInterface import MicroPythonSerialDeviceInterface

    HasQtSerialport = True
except ImportError:
    HasQtSerialport = False
...
class MicroPythonWidget(QWidget, Ui_MicroPythonWidget):
    ...
    def __init__(self, parent=None, forMPyWindow=False):
        ...
        self.stopButton.setIcon(EricPixmapCache.getIcon("stop"))
    ...
    def setActionButtons(self, **kwargs):
        ...
        with contextlib.suppress(KeyError):
            ...
            self.stopButton.setEnabled(kwargs["repl"] and self.__linkConnected)
        ...
        with contextlib.suppress(KeyError):
            self.chartButton.setEnabled(
                kwargs["chart"] and HasQtChartsOrGraphs and self.__connected
            )
    ...
    def on_connectButton_clicked(self):
        ...
        if self.__linkConnected:
            ...
            if self.chartButton.isChecked():
                chartDisconnected = self.on_chartButton_clicked(False)
            else:
                chartDisconnected = True
            if self.runButton.isChecked():
                self.on_runButton_clicked(False)

            if chartDisconnected:
    ...
    def __connectToDevice(self, withAutostart=False):
        ...
        if interfaceType not in ("serial", "webrepl"):
            err = "Unsupported interface type detected ('{0}')".format(interfaceType)
    ...
    def __disconnectFromDevice(self):
        ...
        self.__device and self.__device.setButtons()
    ...
    def on_stopButton_clicked(self):
        """
        Private slot to stop the currently executed script.
        """
        # send Ctrl-C
        evt = QKeyEvent(
        ...
        QCoreApplication.sendEvent(self.replWidget.replEdit(), evt)
    ...
    def on_chartButton_clicked(self, checked):
        ...
        @param checked state of the button
        @type bool
        @return flag indicating a successful connect or disconnect
        @rtype bool
        """
        if not HasQtChartsOrGraphs:
        ...
        if checked:
            ...
            iconName = (
                "sbChart96" if self.__ui.getLayoutType() == "Sidebars" else "chart"
            )
            self.__ui.addSideWidget(
                UserInterfaceSide.Bottom,
                self.__chartWidget,
                EricPixmapCache.getIcon(iconName),
                self.tr("µPy Chart"),
            )
    ...
    def handleDataFlood(self):
        ...
        yes = EricMessageBox.yesNo(
            self,
            self.tr("Data Flooding Detected"),
            self.tr(
                "The connected device sent more data than can be handled by the"
                " graph widget. Shall the device be disconnected to keep eric-ide"
                " responsive?"
            ),
