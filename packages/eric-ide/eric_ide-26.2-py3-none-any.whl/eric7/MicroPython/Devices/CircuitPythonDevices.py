...
class CircuitPythonDevice(BaseDevice):
    ...
    def __init__(
        ...
        self.__securityCode2AuthModeString = {
            ...
            "SEC_WPA_WPA2": "[wifi.AuthMode.WPA, wifi.AuthMode.WPA2, wifi.AuthMode.PSK]",  # noqa: E501
            # noqa: E-501
            "SEC_WPA2_ENT": "[wifi.AuthMode.WPA2, wifi.AuthMode.ENTERPRISE]",
            "SEC_WPA3": "[wifi.AuthMode.WPA3, wifi.AuthMode.PSK]",
            "SEC_WPA2_WPA3": "[wifi.AuthMode.WPA2, wifi.AuthMode.WPA3, wifi.AuthMode.PSK]",  # noqa: E501
            # noqa: E-501
        }
    ...
    def __installLibraryFiles(self, packageMode=False):
        ...
        if packageMode:
            ...
            libraryFiles = EricFileDialog.getOpenFileNames(
                ...
                os.path.expanduser("~"),
                self.tr(
                    "Compiled Python Files (*.mpy);;Python Files (*.py);;All Files (*)"
                ),
            )
...
start_ap({0}, {1}, {2})
del start_ap
""".format(repr(ssid), repr(password), repr(hostname), authmode)

        out, err = self.executeCommands(command, mode=self._submitMode, timeout=15000)
        if err:
            return False, err
        if out and out.startswith(b"Error:"):
...
""".format(WiznetUtilities.cpyWiznetInit())
...
""".format(WiznetUtilities.cpyWiznetInit(), repr(server), tzOffset, timeout)
