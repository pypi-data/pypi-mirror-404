...
class MicroPythonChartWidget(QWidget):
    """
    ...
    def __init__(self, parent=None):
        ...
        self.__axisX.setLabelsVisible(False)
        ...
        self.__axisY.setRange(self.__minY, self.__maxY)
    ...
    def preferencesChanged(self):
        ...
        if chartColorTheme == -1:
            ...
            self.__chart.setTheme(QChart.ChartTheme(chartColorTheme))
    ...
    def __addData(self, values):
        ...
        for index, value in enumerate(values):
            ...
            maxRanges.append(max(self.__data[index]))
            minRanges.append(min(self.__data[index]))
            if len(self.__data[index]) > self.__maxX:
                self.__data[index].pop()
        if self.__yZeroBound:
            maxRanges.append(0)
            minRanges.append(0)
        ...
        minYRange = min(minRanges)
        yRange = bisect.bisect_right(self.__yRanges, minYRange)
        if yRange > 0:
            self.__minY = self.__yRanges[yRange - 1]
        elif minYRange < self.__minY:
            self.__minY += self.__minY
        elif minYRange > self.__minY / 2:
            self.__minY /= 2

        self.__axisY.setRange(self.__minY, self.__maxY)

        # ensure that floats are used to label the y-axis if the range is small
        if self.__maxY <= 5 and self.__minY >= -5:
    ...
    def __eraseData(self):
        ...
        if yes:
            ...
            # 2. remove all line series except the first one and clear the first one
            for series in self.__series[1:]:
                self.__chart.removeSeries(series)
            del self.__series[1:]
            self.__series[0].clear()

            # reset the x- and y-axis
            self.__maxY = 1000
            self.__minY = 0
            self.__axisX.setRange(0, self.__maxX)
            self.__axisY.setRange(self.__minY, self.__maxY)
    ...
    def __handleMaxXChanged(self, value):
        ...
        if delta > 0:
            ...
            self.__data = [deque(list(deq)[:value]) for deq in self.__data]
    ...
    def shutdown(self):
        ...
        Note: Nothing to do here for this graph widget variant.
        """
