...
class MicroPythonFileManagerWidget(QWidget, Ui_MicroPythonFileManagerWidget):
    ...
    def __init__(self, fileManager, parent=None):
        ...
        self.__openLocalFileAct = self.__localMenu.addAction(
            self.tr("Open Files"), self.__openLocalFiles
        )
        ...
        self.__localDelFileAct = self.__localMenu.addAction(
            self.tr("Delete Files"), self.__deleteLocalFiles
        )
        self.__localMenu.addSeparator()
        ...
        self.__openDeviceFileAct = self.__deviceMenu.addAction(
            self.tr("Open Files"), self.__openDeviceFiles
        )
        if not isMicrobitDeviceWithMPy:
        ...
        self.__devTouchFileAct = self.__deviceMenu.addAction(
            self.tr("Touch Files"), self.__touchDeviceFiles
        )
        self.__devDelFileAct = self.__deviceMenu.addAction(
            self.tr("Delete Files"), self.__deleteDeviceFiles
        )
        self.__deviceMenu.addSeparator()
    ...
    def __updateSaveButtonStates(self):
        ...
        """
        enable = len(self.deviceFileTreeWidget.selectedItems()) == 1
        if enable:
            enable &= not (
    ...
    def on_localFileTreeWidget_itemSelectionChanged(self):
        ...
        singleFileEnable = len(self.localFileTreeWidget.selectedItems()) == 1 and not (
            self.localFileTreeWidget.selectedItems()[0].text(0).endswith("/")
        )
        self.putButton.setEnabled(singleFileEnable)
        self.putAsButton.setEnabled(singleFileEnable)

        singleDirEnable = len(
            self.localFileTreeWidget.selectedItems()
        ) == 1 and self.localFileTreeWidget.selectedItems()[0].text(0).endswith("/")
        self.syncButton.setEnabled(
            singleDirEnable or len(self.localFileTreeWidget.selectedItems()) == 0
        )
    ...
    def on_deviceFileTreeWidget_itemSelectionChanged(self):
        ...
        singleEnable = len(self.deviceFileTreeWidget.selectedItems()) == 1 and not (
            self.deviceFileTreeWidget.selectedItems()[0].text(0).endswith("/")
        )
        self.getButton.setEnabled(singleEnable)
        self.getAsButton.setEnabled(singleEnable)

        selectedFilesEnable = (
            len(
                [
                    s
                    for s in self.deviceFileTreeWidget.selectedItems()
                    if not s.text(0).endswith("/")
                ]
            )
            > 0
        )
        self.openButton.setEnabled(selectedFilesEnable)
    ...
    def on_openButton_clicked(self):
        ...
        for fileItem in selectedFileItems:
            name = fileItem.data(0, Qt.ItemDataRole.UserRole)
            if self.__repl.deviceSupportsLocalFileAccess():
                if MimeTypes.isTextFile(name):
                    self.__viewmanager.getEditor(name)
            else:
    ...
    def __showLocalContextMenu(self, pos):
        ...
        @type QPoint
        """
        hasSingleSelection = len(self.localFileTreeWidget.selectedItems()) == 1
        hasMultiSelection = len(self.localFileTreeWidget.selectedItems()) > 1
        if hasSingleSelection:
            name = self.localFileTreeWidget.selectedItems()[0].text(0)
            isDir = name.endswith("/")
            isLink = name.endswith("@")
            isFile = not (isDir or isLink)
        elif hasMultiSelection:
            isFile = (
                len(
                    [
                        s
                        for s in self.localFileTreeWidget.selectedItems()
                        if not s.text(0).endswith("/")
                    ]
                )
                > 0
        ...
        self.__localRenameFileAct.setEnabled(isFile and hasSingleSelection)
        self.__localDelFileAct.setEnabled(isFile and hasMultiSelection)
        self.__openLocalFileAct.setEnabled(isFile and hasMultiSelection)
    ...
    @pyqtSlot()
    def __deleteLocalFiles(self, localDevice=False):
        """
        ...
        selectedFileNames = [
            s.data(0, Qt.ItemDataRole.UserRole)
            for s in fileTreeWidget.selectedItems()
            if not s.text(0).endswith("/")
        ]
        dlg = DeleteFilesConfirmationDialog(
            self,
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            for fileItem in [
                s for s in fileTreeWidget.selectedItems() if not s.text(0).endswith("/")
            ]:
                parentItem = fileItem.parent()
                filename = fileItem.data(0, Qt.ItemDataRole.UserRole)
                try:
                    os.remove(filename)
                    if parentItem:
                        self.__listLocalFiles(
    ...
    def __showDeviceContextMenu(self, pos):
        ...
        @type QPoint
        """
        hasSingleSelection = len(self.deviceFileTreeWidget.selectedItems()) == 1
        hasMultiSelection = len(self.deviceFileTreeWidget.selectedItems()) > 1
        if hasSingleSelection:
        ...
        if not self.__repl.isMicrobit():
            ...
            self.__devRenameFileAct.setEnabled(isFile and hasSingleSelection)
        self.__devTouchFileAct.setEnabled(isFile and hasMultiSelection)
        self.__devDelFileAct.setEnabled(isFile and hasMultiSelection)
        self.__openDeviceFileAct.setEnabled(isFile and hasMultiSelection)
        self.__devShowFileHashAct.setEnabled(isFile and hasSingleSelection)
    ...
    @pyqtSlot()
    def __deleteDeviceFiles(self):
        """
        Private slot to delete device files.
        """
        if self.__repl.deviceSupportsLocalFileAccess():
            self.__deleteLocalFiles(True)
        else:
            selectedFileNames = [
                s.data(0, Qt.ItemDataRole.UserRole)
                for s in self.deviceFileTreeWidget.selectedItems()
    ...
    @pyqtSlot()
    def __touchDeviceFiles(self):
        """
        Private slot to touch files on the device.
        """
        selectedFileNames = [
            s.data(0, Qt.ItemDataRole.UserRole)
            for s in self.deviceFileTreeWidget.selectedItems()
    ...
    @pyqtSlot()
    def __openLocalFiles(self):
        """
        Private slot to open the selected local files in editor windows.
        """
        for fileItem in self.localFileTreeWidget.selectedItems():
            self.on_localFileTreeWidget_itemActivated(fileItem, 0)
    ...
    def __openDeviceFiles(self):
        ...
        for fileItem in selectedFileItems:
            self.on_deviceFileTreeWidget_itemActivated(fileItem, 0)
