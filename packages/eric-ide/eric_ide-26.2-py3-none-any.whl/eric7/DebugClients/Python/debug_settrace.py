...
class DebugBase:
    ...
    def __init__(self, dbgClient):
        ...
        if DebugBase.timer_thread is None:
            DebugBase.timer_thread = _thread.start_new_thread(self.__eventPollTimer, ())
    ...
    def reset_debug_information(self):
        """
        Public dummy method to simplify switching between new and old debugger.
        """
    ...
    def profile(self, frame, event, _arg):
        ...
        @exception RecursionError raised to indicate too many recursions
        """
        if event == "return":
            self.__recursionDepth -= 1
            ...
            self.__recursionDepth += 1
            if self.__recursionDepth > _recursion_limit:
                err = (
        ...
        if event in ("line", "opcode"):  # handle both events identically
            if self.stop_here(frame) or self.break_here(frame):
                if (
                    ...
                    self.user_line(thread_id, frame)
    ...
    def set_trace(self, frame=None):
        ...
        """
        if frame is None:
            frame = sys._getframe(1)  # Skip set_trace method
    ...
    def set_stepinstr(self):
        ...
        """
        self._set_stopinfo(None, None, traceOpcodes=True)
    ...
    def user_line(self, thread_id, frame):
        ...
        @param thread_id ID of the thread executing a particular line
        @type int
        @param frame reference to the frame object
        @type frame object
        """
        ...
        while True:
            ...
            # Send information about the current stacks and selected thread
            stack = self.getStack(frame)
    ...
    def user_exception(self, excinfo, unhandled=False):
        ...
        ) or exctype is RecursionError:  # __IGNORE_WARNING__
        ...
        disassembly = None
        stack = []
        if exctb and not isinstance(exctb, str):
