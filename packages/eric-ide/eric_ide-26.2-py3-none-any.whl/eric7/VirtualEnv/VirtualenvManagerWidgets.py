...
class VirtualenvManagerWidget(QWidget, Ui_VirtualenvManagerWidget):
    ...
    def __init__(self, manager, parent=None):
        ...
        self.__manager.virtualEnvironmentTypeRegistered.connect(self.__refresh)
        self.__manager.virtualEnvironmentTypeUnregistered.connect(self.__refresh)
    ...
    def __updateButtons(self):
        ...
        if selectedItemsCount == 1:
            environmentData = self.venvList.selectedItems()[0].data(
                0, VirtualenvManagerWidget.MetadataRole
            )
            environmentTypeEntry = self.__manager.getEnvironmentTypeEntry(
                environmentType=environmentData.environment_type
            )
            self.upgradeButton.setEnabled(
                bool(environmentTypeEntry) and bool(environmentTypeEntry.upgradeFunc)
            )
        else:
    ...
    def on_searchNewButton_clicked(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            nameTemplate = "Environment #{0} added " + datetime.datetime.now().strftime(  # noqa: DTZ005, M-305
                "%Y-%m-%d %H:%M"
            )
    ...
    def on_removeButton_clicked(self):
        ...
        their directories.
        """
        selectedVenvs = [itm.text(0) for itm in self.venvList.selectedItems()]
        if selectedVenvs:
    ...
    def on_venvList_itemSelectionChanged(self):
        ...
        if len(selectedItems) == 1:
            ...
            self.interpreterVersionLabel.setText(
                PythonUtilities.getPythonInterpreterVersion(envMetaData.interpreter)
            )
        else:
            self.descriptionEdit.clear()
            self.interpreterEdit.clear()
            self.interpreterVersionLabel.clear()
