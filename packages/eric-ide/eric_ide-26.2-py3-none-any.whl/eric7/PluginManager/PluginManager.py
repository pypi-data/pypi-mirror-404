...
class PluginManager(QObject):
    ...
    def initOnDemandPlugin(self, name):
        ...
        try:
            ...
            if not self.__canActivatePlugin(module):
                raise PluginActivationError(  # noqa: TRY301
    ...
    def activatePlugin(self, name, onDemand=False):
        ...
        """
        try:
            logging.getLogger(__name__).debug("Activating Plugin '%s'...", name)
            try:
                module = (
                ...
                logging.getLogger(__name__).debug("No such plugin module: 's'", name)
            ...
            if not ok:
                logging.getLogger(__name__).debug(
                    "Error activating plugin '%s': %s", name, module.error
                )
                return None, module.error
        except PluginActivationError as err:
            logging.getLogger(__name__).debug(
                "Error activating plugin '%s': %s", name, str(err)
            )
            return None, str(err)
        else:
    ...
    def __canActivatePlugin(self, module):
        ...
        try:
            if not hasPluginHeaderEntry(module, "version"):
                raise PluginModuleFormatError(  # noqa: TRY301
            ...
            if not hasattr(pluginClass, "__init__"):
                raise PluginClassFormatError(  # noqa: TRY301
    ...
    def checkPluginUpdatesAvailable(self):
        ...
        if period in [1, 2, 3] and pathlib.Path(self.pluginRepositoryFile).exists():
    ...
    ########################################################################
    ## Method to clear private data of plug-ins below
    ########################################################################
    ...
    ########################################################################
    ## Method to convert passwords of plug-ins below
    ########################################################################

    def convertPasswords(self, oldPassword, newPassword):
        """
        Public method to convert passwords of plug-ins.
        ...
        for module in itertools.chain(
            ...
            if hasattr(module, "convertPasswords"):
                module.convertPasswords(oldPassword, newPassword)

    ########################################################################
    ## Method to install a plug-in module dependency via pip
    ########################################################################
