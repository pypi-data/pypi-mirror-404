...
class PluginRepositoryWidget(QWidget, Ui_PluginRepositoryDialog):
    ...
    def on_repositoryList_currentItemChanged(self, current, _previous):
        ...
        self.descriptionEdit.setPlainText(
            self.__formatDescription(current.data(0, PluginRepositoryWidget.DescrRole))
            if current.data(0, PluginRepositoryWidget.DescrRole)
            else ""
        )
    ...
    def __downloadPlugins(self):
        ...
        for itm in self.__selectedItems():
            ...
            pluginsToDownload.add((url, filename))
        self.__pluginsToDownload = list(pluginsToDownload)
