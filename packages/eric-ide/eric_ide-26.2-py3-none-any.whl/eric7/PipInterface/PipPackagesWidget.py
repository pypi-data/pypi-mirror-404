...
from PyQt6.QtCore import QUrl, QUrlQuery, Qt, pyqtSlot
...
class PipPackagesWidget(QWidget, Ui_PipPackagesWidget):
    ...
    def __init__(self, pip, parent=None):
        ...
        self.searchEdit.returnPressed.connect(self.on_searchButton_clicked)

        self.localCheckBox.clicked.connect(self.__refreshPackagesList)
        self.notRequiredCheckBox.clicked.connect(self.__refreshPackagesList)
        self.userCheckBox.clicked.connect(self.__refreshPackagesList)

        self.verboseCheckBox.clicked.connect(self.on_packagesList_itemSelectionChanged)
        self.installedFilesCheckBox.clicked.connect(
            self.on_packagesList_itemSelectionChanged
        )

        self.requiresButton.toggled.connect(self.__refreshDependencyTree)
        self.localDepCheckBox.clicked.connect(self.__refreshDependencyTree)
        self.userDepCheckBox.clicked.connect(self.__refreshDependencyTree)
    ...
    @pyqtSlot(bool)
    def on_includeGlobalsButton_clicked(self, _checked):
        """
        Private slot to handle a change of the 'Include Global Environments' button.

        @param _checked current check state
        @type bool
        """
        Preferences.setPip(
    ...
    def __allPackageNames(self):
        ...
        """
        return [
            self.packagesList.topLevelItem(index).text(0)
    ...
    def __refreshPackagesList(self):
        ...
        if venvName:
            ...
            if interpreter:
                # Inhibit the environment selector until the update is finished.
                self.environmentsComboBox.setEnabled(False)
    ...
    def __updateOutdatedInfo(self, outdatedPackages):
        ...
        self.environmentsComboBox.setEnabled(True)
    ...
    def __initPipMenu(self):
        ...
        self.__generateConstraintsAct = self.__requirementsSubenu.addAction(
            self.tr("Generate Constraints..."),
            lambda: self.__generateRequirements(doConstraints=True),
        )
        ...
        self.__pipMenu.addAction(
            self.tr("Edit User Configuration..."), self.__editConfiguration
        )
    ...
    @pyqtSlot()
    def __generateRequirements(self, doConstraints=False):
        """
        Private slot to generate the contents for a requirements or constraints file.

        @param doConstraints flag indicating to work on the constraints (defaults to
            False)
        @type bool (optional)
        """
        ...
        if venvName:
            self.__freezeDialog = PipFreezeDialog(
                self.__pip,
                mode=(
                    PipFreezeDialogModes.Constraints
                    if doConstraints
                    else PipFreezeDialogModes.Requirements
                ),
                parent=self,
            )
    ...
    def __editConfiguration(self, venvName=""):
        """
        Private slot to edit a configuration.
    ...
    @pyqtSlot(bool)
    def on_vulnerabilityCheckBox_toggled(self, checked):
        """
    ...
    def __updateVulnerabilityDbCache(self):
        ...
        """
        with EricOverrideCursor():
            ok = self.__pip.getVulnerabilityChecker().updateVulnerabilityDb()
            self.vulnerabilityCheckBox.setEnabled(ok)
    ...
    @pyqtSlot(QTreeWidgetItem, int)
    def on_dependenciesList_itemActivated(self, item, _column):
        """
        ...
        @param item reference to the activated item
        @type QTreeWidgetItem
        @param _column activated column
        @type int
        """
    ...
    @pyqtSlot(QTreeWidgetItem, int)
    def on_dependenciesList_itemPressed(self, item, _column):
        """
        Private slot reacting on a package item of the dependency tree being
        pressed.
