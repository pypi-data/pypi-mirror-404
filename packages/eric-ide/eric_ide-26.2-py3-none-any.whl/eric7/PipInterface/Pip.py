...
class Pip(QObject):
    ...
    def getVirtualenvConfig(self, venvName):
        ...
        return os.path.join(venvDirectory, pip) if venvDirectory else ""
    ...
    def __checkUpgradePyQt(self, packages):
        """
        Private method to check, if an upgrade of PyQt packages used by eric-ide
        is attempted.
    ...
    def installRequirements(self, venvName):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            requirements, _, _, user = dlg.getData()
    ...
    def installPyprojectDependencies(self, venvName):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            pyproject, allExtras, extras, user = dlg.getData()
            if pyproject and os.path.exists(pyproject):
                try:
                    with open(pyproject, encoding="utf-8") as f:
                        data = tomlkit.load(f)
                    dependencies = data.get("project", {}).get("dependencies", [])
                    optionalDependencies = data.get("project", {}).get(
                ...
                if optionalDependencies and (allExtras or bool(extras)):
                    for optDep in optionalDependencies:
                        if optDep in extras or allExtras:
                            dependencies += optionalDependencies.get(optDep, [])
