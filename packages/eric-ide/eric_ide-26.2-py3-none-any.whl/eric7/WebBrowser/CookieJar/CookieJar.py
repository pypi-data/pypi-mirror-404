...
class CookieJar(QNetworkCookieJar):
        ...
        if self.__acceptCookies == CookieAcceptPolicy.OnlyFromSitesNavigatedTo:
            ...
            if mainWindow is not None:
                ...
                if browser is not None:
                    url = browser.url()
                    host = url.host() if url.isValid() else ""
                    res = self.__matchDomain(cookieDomain, host)
                    if not res:
                        return True

        return self.__filterTrackingCookies and cookie.name().startsWith(b"__utm")
    ...
    def __matchDomain(self, cookieDomain, siteDomain):
        ...
        cookieDomain = cookieDomain.removeprefix(".")
        siteDomain = siteDomain.removeprefix(".")
