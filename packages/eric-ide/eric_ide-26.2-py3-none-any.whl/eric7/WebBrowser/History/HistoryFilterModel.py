...
class HistoryFilterModel(QAbstractProxyModel):
    ...
    @pyqtSlot(QModelIndex, int, int)
    def __sourceRowsRemoved(self, _parent, _start, _end):
        """
        Private slot to handle the removal of data in the source model.

        @param _parent reference to the parent index
        @type QModelIndex
        @param _start start row
        @type int
        @param _end end row
        @type int
        """
    ...
    def __frequencyScore(self, sourceIndex):
        ...
        if days <= 1:
            return 100
        if days < 8:  # within the last week
            return 90
        if days < 15:  # within the last two weeks
            return 70
        if days < 31:  # within the last month
            return 50
        if days < 91:  # within the last 3 months
            return 30
        return 10
