...
from PyQt6.QtWebEngineCore import QWebEngineWebAuthUxRequest as AuthUxRequest
...
from eric7.EricWidgets.EricPasswordEdit import EricPasswordEditMode
...
class WebBrowserWebAuthDialog(QDialog, Ui_WebBrowserWebAuthDialog):
    ...
    def __init__(self, uxRequest, parent=None):
        ...
        self.pinEdit.setMode(EricPasswordEditMode.Pin)
        self.pinEdit.setToolTip(self.tr("Enter the PIN"))
        ...
        self.pinEdit.visibilityToggled.connect(self.__togglePinConfirmationVisibility)
    ...
    @pyqtSlot(bool)
    def __togglePinConfirmationVisibility(self, checked):
        """
        Private slot to toggle the PIN confirmation entry visibility depending
        on the visibility of the new PIN contents.

        @param checked state of the new PIN visibility
        @type bool
        """
        pinRequestInfo = self.__uxRequest.pinRequest()
        if pinRequestInfo.reason != AuthUxRequest.PinEntryReason.Challenge:
    ...
    def __acceptRequest(self):
        ...
        if requestState == AuthUxRequest.WebAuthUxState.SelectAccount:
            checkedButton = self.selectAccountButtonGroup.checkedButton()
            if checkedButton:
                self.__uxRequest.setSelectedAccount(checkedButton.text())
        elif requestState == AuthUxRequest.WebAuthUxState.CollectPin:
    ...
    def updateDialog(self):
        ...
        elif requestState == AuthUxRequest.WebAuthUxState.FinishTokenCollection:
            self.__setupFinishCollectTokenUi()
        elif requestState == AuthUxRequest.WebAuthUxState.RequestFailed:
            self.__setupErrorUi()
    ...
    def __setupCollectPinUi(self):
        ...
        if pinRequestInfo.reason == AuthUxRequest.PinEntryReason.Challenge:
            ...
            if pinRequestInfo.reason == AuthUxRequest.PinEntryReason.Set:
        ...
        errorDetails = ""
        if pinRequestInfo.error == AuthUxRequest.PinEntryError.InternalUvLocked:
            errorDetails = self.tr("Internal User Verification Locked!")
        elif pinRequestInfo.error == AuthUxRequest.PinEntryError.WrongPin:
            errorDetails = self.tr("Wrong PIN!")
        elif pinRequestInfo.error == AuthUxRequest.PinEntryError.TooShort:
            errorDetails = self.tr("PIN Too Short!")
        elif pinRequestInfo.error == AuthUxRequest.PinEntryError.InvalidCharacters:
            errorDetails = self.tr("PIN Contains Invalid Characters!")
        elif pinRequestInfo.error == AuthUxRequest.PinEntryError.SameAsCurrentPin:
    ...
    def __setupErrorUi(self):
        ...
        if requestFailureReason == AuthUxRequest.RequestFailureReason.Timeout:
            errorMsg = self.tr("Request Timeout")
        elif (
            requestFailureReason == AuthUxRequest.RequestFailureReason.KeyNotRegistered
        ):
            errorMsg = self.tr("Security key is not registered.")
        elif (
            requestFailureReason
            == AuthUxRequest.RequestFailureReason.KeyAlreadyRegistered
        ):
            errorMsg = self.tr(
            ...
            retryVisible = True
        elif requestFailureReason == AuthUxRequest.RequestFailureReason.SoftPinBlock:
            errorMsg = self.tr(
                "The security key is locked because the wrong PIN was entered too"
                " many times. To unlock it, remove and reinsert it."
            )
            retryVisible = True
        elif requestFailureReason == AuthUxRequest.RequestFailureReason.HardPinBlock:
            errorMsg = self.tr(
                "The security key is locked because the wrong PIN was entered too"
            ...
            requestFailureReason
            == AuthUxRequest.RequestFailureReason.AuthenticatorRemovedDuringPinEntry
        ):
            errorMsg = self.tr(
            ...
            requestFailureReason
            == AuthUxRequest.RequestFailureReason.AuthenticatorMissingResidentKeys
        ):
            errorMsg = self.tr("Security key does not have resident key support.")
        elif (
            requestFailureReason
            == AuthUxRequest.RequestFailureReason.AuthenticatorMissingUserVerification
        ):
            errorMsg = self.tr("Security key is missing user verification.")
        elif (
            requestFailureReason
            == AuthUxRequest.RequestFailureReason.AuthenticatorMissingLargeBlob
        ):
            errorMsg = self.tr("Security key is missing Large Blob support.")
        elif (
            requestFailureReason
            == AuthUxRequest.RequestFailureReason.NoCommonAlgorithms
        ):
            errorMsg = self.tr("Security key does not provide a common algorithm.")
        elif requestFailureReason == AuthUxRequest.RequestFailureReason.StorageFull:
            errorMsg = self.tr("No storage space left on the security key.")
        elif (
            requestFailureReason == AuthUxRequest.RequestFailureReason.UserConsentDenied
        ):
            errorMsg = self.tr("User consent denied.")
        elif (
            requestFailureReason == AuthUxRequest.RequestFailureReason.WinUserCancelled
        ):
