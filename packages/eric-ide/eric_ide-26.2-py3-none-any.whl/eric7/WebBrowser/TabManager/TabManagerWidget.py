...
from PyQt6.QtCore import QPoint, QRect, QTimer, Qt, pyqtSignal, pyqtSlot
...
class TabManagerWidget(QWidget):
    ...
    def domainFromUrl(self, url, useHostName=False):
        ...
        if url.scheme() == "eric" or not urlString:
    ...
    def __createEmptyItem(self, parent=None, addToTree=True):
        ...
        """
        parentItem = (
            (parent if parent else self.__tree.invisibleRootItem())
            if addToTree
            else None
        )
