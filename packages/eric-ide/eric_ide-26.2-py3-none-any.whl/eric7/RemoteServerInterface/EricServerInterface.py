...
class EricServerInterface(QObject):
    ...
    def getServiceInterface(self, name):
        ...
        try:
            ...
            try:
                ...
                exc = f"no such service supported ({name})"
    ...
    def parseHost(self, host):
        ...
        if "]" in host:
            ...
            hostname = hostname[1:]
            port = int(rest[1:]) if rest and rest[0] == ":" else None
        else:
            if ":" in host:
    ...
    def __receiveJson(self):
        ...
        while self.__connection and self.__connection.bytesAvailable():
            ...
            logging.getLogger(__name__).debug(
                "<Remote Server Interface Rx> %s", jsonString
            )
            #: this is for debugging only
            #: print(f"Remote Server Interface Receive: {jsonString}")
    ...
    def sendJson(self, category, request, params, callback=None, flush=False):
        ...
        logging.getLogger(__name__).debug("<Remote Server Interface Tx> %s", jsonString)
        #: this is for debugging only
        #: print(f"Remote Server Interface Send: {jsonString}")
    ...
    def __handleServerVersionReply(self, reply, params):
        ...
        """
        if reply != "Versions":
            exc = f"unsupported reply received ({reply})"
        ...
        versionInfo.extend(
            [
                "<tr><td></td><td></td></tr>",
                f"<tr><td><b>eric7_server</b></td><td>{params['version']} </td></tr>",
                "<tr><td></td><td></td></tr>",
            ]
        )
    ...
    def initMenus(self):
        ...
        menu = QMenu(self.tr("eric-ide Server"))
