...
class EricServerFileDialog(QDialog, Ui_EricServerFileDialog):
    ...
    def setNameFilter(self, filter):  # noqa: A002, M-132
        """
    ...
    def on_listing_itemActivated(self, item, _column):
        """
    ...
    def on_listing_itemSelectionChanged(self):
        ...
        for itm in selectedItems:
            ...
            if (self.__fileMode == FileMode.Directory and isDir) or not isDir:
    ...
    def __getNames(self):
        ...
        namesStr = namesStr.removeprefix('"')
        namesStr = namesStr.removesuffix('"')
        return re.split(r'"\s+"', namesStr)
...
def getSaveFileNameAndFilter(
    ...
    @param withRemote flag indicating to create the file names with the remote
        indicator (defaults to True)
    @type bool (optional)
    @return name of file to be saved and selected filter
...
def getSaveFileNameWithExtension(
    parent=None,
    caption="",
    directory="",
    filterStr="",
    ...
    """
    Module function to get the name of a file for saving ensuring it has a file
    extension, if a file type filter was selected.
    ...
    @rtype tuple of (str, str)
    """
    fileName, selectedFilter = getSaveFileNameAndFilter(
        parent, caption, directory, filterStr, initialFilter, withRemote
    )
    if fileName:
        fileName = fileName.removesuffix(".")  # sanitize the file name
