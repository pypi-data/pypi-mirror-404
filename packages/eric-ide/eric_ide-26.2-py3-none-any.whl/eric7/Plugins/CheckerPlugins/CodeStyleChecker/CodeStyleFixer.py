...
class CodeStyleFixer:
    ...
    def __getLogical(self, line, pos):
        ...
        @return tuple of a tuple of two integers giving the start of the
            ...
            source lines
        @rtype tuple of ((int, int), (int, int), [str])
        """
        try:
    ...
    def __getIndentWord(self):
        ...
        @return string to be used for an indentation
        @rtype str
        """
        sio = StringIO("".join(self.__source))
    ...
    def __multilineStringLines(self):
        ...
        @rtype tuple of (set of int, set of int)
        """
    ...
    def __fixReindent(self, line, _pos, logical):
        ...
        @type str
        @return flag indicating a change was done
        @rtype bool
        @exception ValueError raised to indicate a bad 'logical' parameter
        """
        if not logical:
            err = "Bad value for 'logical' parameter."
    ...
    def __fixWhitespace(self, line, offset, replacement):
        ...
        @param line line to be corrected
        @type str
    ...
    def __fixD221(self, code, line, pos, apply=False):
        ...
        if apply:
            ...
            self.__source[line] = newText
            msg = "FIX-D-221" if code == "D-221" else "FIX-D-222"
    ...
    def __fixD242(self, code, line, pos, apply=False):
        ...
        if apply:
            line -= 1
            self.__source[line - 1] = ""
            msg = "FIX-D-242" if code == "D-242" else "FIX-D-244"
    ...
    def __fixD243(self, code, line, pos, apply=False):
        ...
        if apply:
            line -= 1
            self.__source[line + 1] = ""
            msg = "FIX-D-243" if code == "D-243" else "FIX-D-245"
    ...
    def __fixE101(self, code, line, _pos):
        ...
        if fixedLine is not None and fixedLine != self.__source[line - 1]:
            ...
            msg = "FIX-E-101" if code in ["E-101", "W-191"] else "FIX-E-111"
            # Tab converted to 4 spaces
            # Else: Indentation adjusted to be a multiple of four.
            return (1, msg, [], 0)
        return (0, "", [], 0)
    ...
    def __fixBlankLinesBefore(self, code, line, pos, apply=False):
        ...
        if apply:
            ...
            if delta < 0:
                ...
                return (1, "FIX-E-302+", blankLinesBefore - blanks, 0)
            if delta > 0:
                ...
                return (1, "FIX-E-302-", blanks - blankLinesBefore, 0)
            return (0, "", [], 0)
        fixId = self.__getID()
        self.__stack.append((fixId, code, line, pos))
        return (-1, "", [], fixId)
    ...
    def __fixE501(self, code, line, pos, apply=False):
        ...
        if apply:
            ...
            line -= 1
            text = self.__source[line]
            prevText = self.__source[line - 1] if line > 0 else ""
            nextText = self.__source[line + 1] if line < len(self.__source) - 1 else ""
    ...
    def __fixE711(self, _code, line, pos):
        ...
        self.__source[line] = f"{left} {center} {right}"
        # Comparison to None/True/False corrected.
        return (1, "FIX-E-711", [], 0)
    ...
    def __fixN804(self, code, line, pos, apply=False):
        ...
        if apply:
            line -= 1
            text = self.__source[line]
            arg = "cls" if code == "N-804" else "self"
            ...
            if text.rstrip().endswith("("):
                ...
                left = text[:index]
                right = text[index:]
                center = arg if right.startswith(")") else arg + ", "
                newText = left + center + right
            self.__source[line] = newText
            # '{0}' argument added.
            return (1, "FIX-N-804", [arg], 0)
        fixId = self.__getID()
        self.__stack.append((fixId, code, line, pos))
        return (-1, "", [], fixId)
...
class Reindenter:
    ...
    def run(self):
        ...
        @return flag indicating that a change was done
        @rtype bool
        """
        try:
                ...
                if add_second_chances:
                    ...
                    for col, what in indent_chances.items():
                        if col > min_indent and (
                            what is True
                            or (isinstance(what, str) and token_type == tokenize.STRING)
...
class LineShortener:
    ...
    def __init__(
        ...
        @param curLine text to work on
        @type str
        @param prevLine line before the text to work on
        @type str
        @param nextLine line after the text to work on
        @type str
        @param maxLength maximum allowed line length
        @type int
        @param eol eond-of-line marker
        @type str
        @param indentWord string used for indentation
        @type str
    ...
    def shorten(self):
        ...
        @return tuple of a flag indicating successful shortening, the
            shortened line and the changed next line
        ...
        if rs.endswith(("'", '"')) and " " in source:
            quote = rs[-3:] if rs.endswith(('"""', "'''")) else rs[-1]
            blank = source.rfind(" ")
    ...
    def __shortenComment(self, isLast):
        ...
        if isLast and re.match(r"\s*#+\s*\w+", newText):
    ...
    def __isProbablyInsideStringOrComment(self, line, index):
        ...
        @param line line to check
        @type str
        @param index position inside line to check
        @type int
    ...
    def __shortenLine(self, tokens, source, indent):
        ...
        @type list of Token
        @param source code string to work at
        @type str
        @param indent indentation string of the code line
        @type str
    ...
    def __normalizeMultiline(self, text):
        ...
        @param text code line to work on
        @type str
        @return normalized code line
        @rtype str
        """
        for quote in "'\"":
    ...
    def __lineShorteningRank(self, candidate):
        ...
        @param candidate candidate line to rank
        @type str
    ...
    def __checkSyntax(self, code):
        ...
        @param code code fragment to check
        @type str
        @return flag indicating syntax is ok
        @rtype bool
        """
        code = code.replace("\r\n", "\n").replace("\r", "\n")
        try:
