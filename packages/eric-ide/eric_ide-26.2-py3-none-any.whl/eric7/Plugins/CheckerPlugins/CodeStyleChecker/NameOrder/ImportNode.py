...
class ImportNode:
    ...
    def __init__(
        ...
        @param firstPartyNames list of first party package names
        @type list of str
        ...
        if not isinstance(astNode, (ast.Import, ast.ImportFrom)):
            err = "Node type {0} not recognized".format(type(astNode))
            raise ImportNodeError(err)
        ...
        if self.moduleName == "__future__":
            ...
            any(self.moduleName == fp for fp in firstPartyNames)  # first party module
            or self.moduleName.split(".")[0] in firstPartyNames  # first party package
        ):
            group = GroupEnum.FIRSTPARTY
        elif (
            level > 0
            or any(name == self.moduleName for name in appNames)
            or self.moduleName.startswith(tuple(f"{n}." for n in appNames))
        ):
        ...
        if group == GroupEnum.STDLIB:
            ...
            dotIndex = self.moduleName.find(".")
            topName = "." * level + (
                self.moduleName if dotIndex == -1 else self.moduleName[:dotIndex]
            )
            level = -level
    ...
    def __str__(self):
        ...
        if self.nodeType not in (NodeTypeEnum.IMPORT, NodeTypeEnum.IMPORT_FROM):
            err = "The node type {0} is not recognized.".format(self.nodeType)
            raise ImportNodeError(err)

        if self.nodeType == NodeTypeEnum.IMPORT:
            return "import {0}".format(self.moduleName)
        if self.nodeType == NodeTypeEnum.IMPORT_FROM:
