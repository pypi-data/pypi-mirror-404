...
###############################################################################
## adapted from: flake8-simplify v0.22.0
##
...
class SimplifyNodeVisitor(ast.NodeVisitor):
    ...
    def __isSameBody(self, body1, body2):
        ...
        if len(body1) != len(body2):
            return False
        for a, b in zip(body1, body2, strict=False):
            try:
    ...
    def __isStatementEqual(self, a, b):
        ...
        if isinstance(a, list):
            return all(map(self.__isStatementEqual, a, b))
        return a == b
        ...
        * Else must also just have a return
        ...
        if (
            ...
            if AstUtilities.isString(node.test.comparators[0]):
                ...
                keyValuePairs = {node.test.comparators[0].value: value}
            else:
    ...
    def __check117(self, node):
        ...
        if len(node.body) == 1 and isinstance(node.body[0], ast.With):
            withItems = [
                f"{unparse(withitem)}" for withitem in node.items + node.body[0].items
            ]
    ...
    def __check201(self, node):
        ...
        if not (
            ...
            or (
                isinstance(node.parent, ast.If) and self.__isExceptionCheck(node.parent)
            )
        ):
    ...
    def __check909(self, node):
        ...
        names.extend(unparse(target) for target in node.targets)
