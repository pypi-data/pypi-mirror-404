...
class LocalImportVisitor(ast.NodeVisitor):
    ...
    def __init__(self, args, checker):
        ...
        self.__firstPartyNames = (
            args.get("FirstPartyPackageNames", [])
            if args.get("AllowLocalImportOfFirstPartyPackages", False)
            else []
        )
    ...
    def __assertExternalModule(self, node, module):
        ...
        if getattr(node, "level", 0) != 0 or any(
            ...
            for appModule in self.__appImportNames + self.__firstPartyNames
        ):
            return
