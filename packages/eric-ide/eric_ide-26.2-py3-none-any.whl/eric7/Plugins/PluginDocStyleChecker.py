...
# Start-Of-Header
__header__ = {
    "name": "Code Documentation Style Checker Plugin",
    "author": "Detlev Offenbach <detlev@die-offenbachs.de>",
    "autoactivate": True,
    "deactivateable": True,
    "version": VersionOnly,
    "className": "DocStyleCheckerPlugin",
    "packageName": "__core__",
    "shortDescription": "Show the Python Code Documentation Style Checker dialog.",
    "longDescription": (
        "This plugin implements the Python Code Documentation Style Checker dialog."
        " It checks Python source files for compliance to docstring conventions given"
        " in PEP-257 or eric conventions."
    ),
    "pyqtApi": 2,
}
# End-Of-Header
...
class DocStyleCheckerPlugin(QObject):
    """
    Class implementing the code documentation style checker plug-in.

    @signal styleChecked(str, dict, int, list) emitted when the check was
        done for a file.
    ...
    def __init__(self, ui):
        ...
        path = os.path.join(
            os.path.dirname(__file__), "CheckerPlugins", "DocStyleChecker"
        )
    ...
    def serviceErrorPy3(self, fx, lang, fn, msg):
        ...
        @type str
        """
        if fx in ["docstyle", "batch_docstyle"] and lang == "Python3":
            if fx == "style":
    ...
    def batchJobDone(self, fx, lang):
        ...
        @type str
        """
        if fx in ["docstyle", "batch_docstyle"]:
    ...
    def __initialize(self):
        ...
        self.__projectDocStyleCheckerDialog = None
    ...
    def styleCheck(self, lang, filename, source, args):
        ...
        @param args arguments used by the docStyleCheck function (list of
            ...
            noFixCodes, fixIssues, maxLineLength, docType, errors, eol,
            encoding, backup)
        @type list of (str, str, bool, str, str, bool, int, str, list of str,
            str, str, bool)
        """
        if (lang is None and PythonUtilities.isPythonSource(filename, source)) or (
            ...
            data = [source, args]
            self.backgroundService.enqueueRequest("docstyle", "Python3", filename, data)
    ...
    def styleBatchCheck(self, argumentsList):
        ...
        if data["Python3"]:
            ...
            self.backgroundService.enqueueRequest(
                "batch_docstyle", "Python3", "", data["Python3"]
            )
    ...
    def cancelStyleBatchCheck(self):
        ...
        """
        self.backgroundService.requestCancel("batch_docstyle", "Python3")

    def __translateStyleCheck(self, fn, styleCheckerStats, results):
        """
        Private slot called after performing a style check on one file.

        @param fn filename of the just checked file
        @type str
        @param styleCheckerStats stats of documentation style check
        @type dict
        @param results dictionary containing the check result data
            (see DocStyleChecker.__checkDocStyle for details)
        @type dict
        """
        from eric7.Plugins.CheckerPlugins.DocStyleChecker.translations import (
        ...
        for result in results:
            ...
            if result["fixcode"]:
                ...
                msg += "\n" + QCoreApplication.translate(
                    "DocStyleCheckerDialog", "Fix: {0}"
                ).format(trFixedMsg)

            result["display"] = "\n".join(self.__wrapper.wrap(msg))
        self.styleChecked.emit(fn, styleCheckerStats, fixes, results)
    ...
    def activate(self):
        ...
        if menu:
            self.__projectAct = EricAction(
                self.tr("Check Documentation Style"),
                self.tr("&Documentation Style..."),
                0,
                0,
                self,
                "project_check_doc_style",
            )
            self.__projectAct.setStatusTip(self.tr("Check documentation style."))
            self.__projectAct.setWhatsThis(
                self.tr(
                    """<b>Check Documentation Style...</b>"""
                    """<p>This checks Python files for compliance to various"""
                    """ code documentation style conventions.</p>"""
                )
            )
            self.__projectAct.triggered.connect(self.__projectDocStyleCheck)
        ...
        self.__editorAct.triggered.connect(self.__editorDocStyleCheck)
        ...
        if menuName == "Checks" and ericApp().getObject(
            ...
            if self.__projectBrowserAct is None:
                ...
                self.__projectBrowserAct.triggered.connect(
                    self.__projectBrowserDocStyleCheck
                )
            if self.__projectBrowserAct not in menu.actions():
                menu.addAction(self.__projectBrowserAct)

    def __projectDocStyleCheck(self):
        """
        Private slot used to check the project files for code documentation style.
        """
        from eric7.Plugins.CheckerPlugins.DocStyleChecker import DocStyleCheckerDialog
        ...
        self.__projectDocStyleCheckerDialog = (
            DocStyleCheckerDialog.DocStyleCheckerDialog(self)
        )
        self.__projectDocStyleCheckerDialog.show()
        self.__projectDocStyleCheckerDialog.prepare(files, project)

    def __projectBrowserDocStyleCheck(self):
        """
        Private method to handle the code documentation style check context menu
        action of the project sources browser.
        """
        from eric7.Plugins.CheckerPlugins.DocStyleChecker import DocStyleCheckerDialog
        ...
        self.__projectBrowserDocStyleCheckerDialog = (
            DocStyleCheckerDialog.DocStyleCheckerDialog(self)
        )
        self.__projectBrowserDocStyleCheckerDialog.show()
        if isDir:
            self.__projectBrowserDocStyleCheckerDialog.start(fn, save=True)
        else:
            self.__projectBrowserDocStyleCheckerDialog.start(fn, save=True, repeat=True)
    ...
    def __editorDocStyleCheck(self):
        """
        Private slot to handle the code documentation style check context menu action
        of the editors.
        """
        from eric7.Plugins.CheckerPlugins.DocStyleChecker import DocStyleCheckerDialog
        ...
        if (
            ...
            self.__editorDocStyleCheckerDialog = (
                DocStyleCheckerDialog.DocStyleCheckerDialog(self)
            )
            self.__editorDocStyleCheckerDialog.show()
            self.__editorDocStyleCheckerDialog.start(
