...
from PyQt6.QtCore import QProcess, QSize, QTimer, Qt, pyqtSlot
...
class GitStatusDialog(QWidget, Ui_GitStatusDialog):
    ...
    def closeEvent(self, e):
        ...
        VcsGitPlugin.setPreferences("StatusDialogGeometry", self.saveGeometry())
    ...
    def show(self):
        ...
        geom = VcsGitPlugin.getPreferences("StatusDialogGeometry")
        if geom.isEmpty():
            s = QSize(900, 600)
            self.resize(s)
        else:
        ...
        states = VcsGitPlugin.getPreferences("StatusDialogSplitterStates")
        if len(states) == 2:
    ...
    def __sbsDiff(self):
        ...
        if workModified and stageModified:
            ...
            revisions = ["", ""] if result == messages[0] else ["HEAD", ""]
        else:
            revisions = ["HEAD", "Stage"]
