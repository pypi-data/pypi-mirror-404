...
class HgLogBrowserDialog(QWidget, Ui_HgLogBrowserDialog):
    ...
    def __init__(self, vcs, mode="", parent=None):
        ...
        if not mode:
            if VcsMercurialPlugin.getPreferences("LogBrowserShowFullLog"):
                mode = "full_log"
            else:
                mode = "log"
    ...
    def closeEvent(self, e):
        ...
        VcsMercurialPlugin.setPreferences("LogBrowserGeometry", self.saveGeometry())
    ...
    def __reloadGeometry(self):
        ...
        """
        geom = VcsMercurialPlugin.getPreferences("LogBrowserGeometry")
        if geom.isEmpty():
            s = QSize(1000, 800)
            self.resize(s)
        else:
    ...
    def __restoreSplitterStates(self):
        ...
        """
        states = VcsMercurialPlugin.getPreferences("LogBrowserSplitterStates")
        if len(states) == 3:
    ...
    def __resetUI(self):
        ...
        self.limitSpinBox.setValue(VcsMercurialPlugin.getPreferences("LogLimit"))
        self.stopCheckBox.setChecked(VcsMercurialPlugin.getPreferences("StopLogOnCopy"))
        ...
        if parents[0] != -1:
            for ecol, erev in enumerate(self.__revs):
                if erev in nextRevs:
                    ...
                    edges.extend(
                        (ecol, nextRevs.index(p), self.__revColors[p]) for p in parents
                    )
    ...
    def __getParents(self, rev):
        ...
        if int(rev) > 0:
            ...
            output, _errMsg = self.__hgClient.runcommand(args)
    ...
    def __generateLogItem(
        ...
        """
        logMessageColumnWidth = VcsMercurialPlugin.getPreferences(
        ...
        if not parents:
            parents = [int(rev) - 1]
        column, _color, edges = self.__generateEdges(int(rev), parents)
    ...
    def __showError(self, out):
        ...
        errLines = [
            ...
            if "no such file in rev" not in line
        ]
        if errLines:
            self.errorGroup.show()
            self.errors.insertPlainText("".join(errLines))
    ...
    @pyqtSlot(int)
    def on_branchCombo_activated(self, _index):
        """
    ...
    def __generateDiffs(self, parent=1):
        ...
        if len(selectedItems) == 1:
            ...
            parents = currentItem.data(0, self.__parentsRole)
            if len(parents) == 0:
                currentBranch = currentItem.text(self.BranchColumn)
                itemBelow = self.logTree.itemBelow(currentItem)
                while (
                    itemBelow is not None
                    and itemBelow.text(self.BranchColumn) != currentBranch
                ):
                    # try the next one
                    itemBelow = self.logTree.itemBelow(itemBelow)
                if itemBelow is None:
                    self.diffEdit.setPlainText(
                        self.tr("No differences available. Get some more log entries.")
                        if self.nextButton.isEnabled()
                        else self.tr("No differences available.")
                    )
                else:
                    rev1 = itemBelow.text(self.RevisionColumn).split(":", 1)[0]

                    self.__diffGenerator.start(
                        self.__filename, [rev1, rev2], self.__bundle
                    )

            elif len(parents) >= parent:
