...
class HgStatusDialog(QWidget, Ui_HgStatusDialog):
    ...
    def __init__(self, vcs, mq=False, parent=None):
        ...
        @type Hg
        @param mq flag indicating to show a queue repo status
        @type bool
    ...
    def show(self):
        ...
        geom = (
            VcsMercurialPlugin.getPreferences("MqStatusDialogGeometry")
            if self.__mq
            else VcsMercurialPlugin.getPreferences("StatusDialogGeometry")
        )
        if geom.isEmpty():
            s = QSize(800, 600)
            self.resize(s)
        ...
        diffSplitterState = (
            VcsMercurialPlugin.getPreferences("MqStatusDialogSplitterState")
            if self.__mq
            else VcsMercurialPlugin.getPreferences("StatusDialogSplitterState")
        )
