...
class Hg(VersionControl):
    ...
    def getEncoding(self):
        ...
        @rtype str
        """
        return VcsMercurialPlugin.getPreferences("Encoding")
    ...
    def initCommand(self, command):
        ...
        """
        args = [command]
        self.addArguments(args, VcsMercurialPlugin.getGlobalOptions())
        return args
    ...
    def vcsInit(self, _vcsDir, noDialog=False):  # noqa: ARG002, U-100
        """
    ...
    def vcsImport(
        ...
        @type str
        @param _noDialog flag indicating quiet operations (unused)
        @type bool
        @param addAll flag indicating to add all files to the repository
        @type bool
    ...
    def __vcsCommit_Step2(self):
        ...
        if not noDialog:
            ...
            nameList = name if isinstance(name, list) else [name]
            ok = True
            for nam in nameList:
    ...
    def vcsCommitMessages(self):
        ...
        if not messages:
            ...
            messages = VcsMercurialPlugin.getPreferences("Commits")
    ...
    def vcsAddCommitMessage(self, message):
        ...
        if not self._vcsAddProjectCommitMessage(message):
            ...
            VcsMercurialPlugin.setPreferences("Commits", commitMessages)
    ...
    def vcsClearCommitMessages(self):
        ...
        if not self._vcsClearProjectCommitMessages():
            VcsMercurialPlugin.setPreferences("Commits", [])
    ...
    def __getMostRecentCommitMessage(self):
        ...
        output, _error = self.__client.runcommand(args)

        return output

    def vcsUpdate(
        self,
        name=None,  # noqa: ARG002, U-100
        noDialog=False,
        revision=None,
    ):
        """
    ...
    def vcsAdd(self, name, isDir=False, noDialog=False):  # noqa: ARG002, U-100
        """
    ...
    def vcsRemove(self, name, project=False, noDialog=False):  # noqa: ARG002, U-100
        """
    ...
    def vcsTag(self, name=None, revision=None, tagName=None):  # noqa: ARG002, U-100
        """
    ...
    def vcsRevert(self, name):
        ...
        if not VcsMercurialPlugin.getPreferences("CreateBackup"):
    ...
    def vcsMerge(self, name, rev=""):  # noqa: ARG002, U-100
        """
        ...
        if force:
            args.append("--force")
        if VcsMercurialPlugin.getPreferences("InternalMerge"):
            args.append("--tool")
    ...
    def vcsAllRegisteredStates(
        self,
        names,
        dname,
        shortcut=True,  # noqa: ARG002, U-100
    ):
        """
    ...
    def vcsCleanup(self, name):
        ...
        @type str
        """
        patterns = VcsMercurialPlugin.getPreferences("CleanupPatterns").split()
    ...
    def vcsOptionsDialog(
        ...
        @param _project reference to the project object (unused)
        @type Project
        @param _archive name of the project in the repository (unused)
        @type str
        @param _editable flag indicating that the project name is editable (unused)
        @type bool
        @param parent parent widget
        @type QWidget
    ...
    def vcsSbsDiff(self, name, extended=False, revisions=None):
        ...
        @exception TypeError raised to indicate an invalid name parameter
        """
        ...
        if isinstance(name, list):
            err = "wrong parameter type, should be 'str'"
            raise TypeError(err)
        ...
        if error:
            ...
            return
        name1 = "{0} (rev. {1})".format(name, rev1 if rev1 else ".")
    ...
    def hgPull(self, revisions=None, noupdate=False):
        ...
        args.append("-v")
        if VcsMercurialPlugin.getPreferences("PullUpdate") and not noupdate:
            args.append("--update")
        if command == "unbundle":
    ...
    def hgEditConfig(self, repoName=None, withLargefiles=True, largefilesData=None):
        ...
        if not os.path.exists(cfgFile) or os.path.getsize(cfgFile) < 3:
            ...
            openMode = "w" if not os.path.exists(cfgFile) else "a"
            with contextlib.suppress(OSError):
                with open(cfgFile, openMode) as cfg:
                    if createContents:
    ...
    def hgBundle(self, bundleData=None):
        ...
        if bundleData is None:
            ...
            baseRevs = [bundleData["base"]] if bundleData["base"] else []
            compression = ""
    ...
    def hgRemoveSubrepositories(self):
        ...
        try:
            with open(hgsub) as f:
                subrepositories = [line.strip() for line in f]
        except OSError as err:
            EricMessageBox.critical(
                self.__ui,
                self.tr("Remove Sub-repositories"),
    ...
    def isExtensionActive(self, extensionName):
        ...
        return extensionName in self.__activeExtensions
