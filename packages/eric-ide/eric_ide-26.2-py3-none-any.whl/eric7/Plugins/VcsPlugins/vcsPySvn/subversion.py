...
from eric7.Plugins.PluginVcsPySvn import VcsPySvnPlugin
...
class Subversion(VersionControl):
        ...
        if not noDialog:
            dlg = SvnDialog(
                ...
                "import{0} --message {1} .".format(
                    " --non-recursive" if not recurse else "", msg
                ),
                client,
                parent=self.__ui,
            )
        ...
        if not noDialog:
            dlg = SvnDialog(
                ...
                "checkout{0} {1} {2}".format(
                    " --non-recursive" if not recurse else "", url, projectDir
                ),
                client,
                parent=self.__ui,
            )
        ...
        if not noDialog:
            dlg = SvnDialog(
                ...
                "commit{0}{1}{2}{3} --message {4} {5}".format(
                    " --non-recursive" if not recurse else "",
                    " --keep-locks" if keeplocks else "",
                    " --keep-changelists" if keepChangelists else "",
                    " --changelist ".join(["", *changelists]) if changelists else "",
                    msg,
                    " ".join(fnames),
                ),
                client,
    ...
    def vcsCommitMessages(self):
        ...
        if not messages:
            ...
            messages = VcsPySvnPlugin.getPreferences("Commits")
    ...
    def vcsAddCommitMessage(self, message):
        ...
        if not self._vcsAddProjectCommitMessage(message):
            ...
            VcsPySvnPlugin.setPreferences("Commits", commitMessages)
    ...
    def vcsClearCommitMessages(self):
        ...
        if not self._vcsClearProjectCommitMessages():
            VcsPySvnPlugin.setPreferences("Commits", [])
    ...
    def vcsUpdate(self, name, noDialog=False):
        ...
        if not noDialog:
            dlg = SvnDialog(
                ...
                "update{0} {1}".format(
                    " --non-recursive" if not recurse else "", " ".join(fnames)
                ),
                client,
                parent=self.__ui,
            )
    ...
    def vcsAdd(self, name, isDir=False, noDialog=False):
        ...
        if isinstance(name, list):
            if isDir:
                dname, _fname = os.path.split(name[0])
            else:
                dname, _fnames = self.splitPathList(name)
        else:
            if isDir:
                dname, _fname = os.path.split(name)
            else:
                dname, _fname = self.splitPath(name)
        names = []
        tree = []
        wdir = dname
        if self.__wcng:
    ...
    def vcsAddTree(self, path):
        ...
        if isinstance(path, list):
            dname, _fnames = self.splitPathList(path)
            for n in path:
                d = os.path.dirname(n)
                if self.__wcng:
                    repodir = d
            ...
            dname, _fname = os.path.split(path)
            if self.__wcng:
                repodir = dname
                while not os.path.isdir(os.path.join(repodir, self.adminDir)):
                    repodir = os.path.dirname(repodir)
    ...
    def vcsRemove(self, name, project=False, noDialog=False):  # noqa: ARG002, U-100
        ...
        if not noDialog:
            dlg = SvnDialog(
                ...
                "remove{0} {1}".format(" --force" if force else "", " ".join(name)),
                client,
                parent=self.__ui,
            )
            QApplication.processEvents()
        ...
        if accepted:
            ...
            if not noDialog:
                dlg = SvnDialog(
                    ...
                    "move{0}{1} {2} {3}".format(
                        " --force" if force else "",
                        " --message {0}".format(log) if log else "",
                        name,
                        target,
                    ),
                    client,
    ...
    def vcsRevert(self, name):
        ...
        if yes:
            ...
            dlg = SvnDialog(
                ...
                "revert {0} {1}".format(
                    " --non-recursive" if not recurse else "", " ".join(name)
                ),
                client,
                parent=self.__ui,
            )
    ...
    def vcsMerge(self, name):
        ...
        dlg = SvnDialog(
            ...
            "merge{0}{1} {2} {3} {4}".format(
                " --non-recursive" if not recurse else "",
                " --force" if force else "",
                "{0}{1}".format(url1, f"@{rev1}" if rev1 else ""),
                "{0}{1}".format(url2, f"@{rev2}" if rev2 else ""),
                fname,
            ),
            client,
            parent=self.__ui,
    ...
    def __vcsAllRegisteredStates_wcng(
        self,
        names,
        dname,
    ...
    def svnCopy(self, name, project):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            dlg = SvnDialog(
                ...
                "copy{0} {1} {2}".format(
                    " --message {0}".format(log) if log else "", name, target
                ),
                client,
                parent=self.__ui,
                log=log,
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            dlg = SvnDialog(
                ...
                "propset{0}{1} {2} {3} {4}".format(
                    ...
                    " --skip-checks" if skipchecks else "",
                    propName,
                    propValue,
                    " ".join(fnames),
                ),
    ...
    def vcsSbsDiff(self, name, extended=False, revisions=None):
        ...
        @exception TypeError raised to indicate an invalid name parameter type
        """
    ...
    def svnLock(self, name, stealIt=False, parent=None):
        ...
        dlg = SvnDialog(
            ...
            "lock{0}{1} {2}".format(
                " --force" if stealIt else "",
                " --message {0}".format(comment) if comment else "",
                " ".join(fnames),
            ),
            client,
            parent=parent,
    ...
    def svnUnlock(self, name, breakIt=False, parent=None):
        ...
        dlg = SvnDialog(
            ...
            "unlock{0} {1}".format(" --force" if breakIt else "", " ".join(fnames)),
            client,
            parent=parent,
        )
        QApplication.processEvents()
