...
class SvnDiffDialog(QWidget, SvnDialogMixin, Ui_SvnDiffDialog):
    ...
    def __getDiffSummaryKind(self, summaryKind):
        ...
        if summaryKind == pysvn.diff_summarize_kind.delete:
            return "D"
        if summaryKind == pysvn.diff_summarize_kind.modified:
            return "M"
        if summaryKind == pysvn.diff_summarize_kind.added:
            return "A"
        if summaryKind == pysvn.diff_summarize_kind.normal:
            return "N"
        return " "
    ...
    def on_saveButton_clicked(self):
        ...
        if isinstance(self.filename, list):
            if len(self.filename) > 1:
                ...
                fname = "{0}.diff".format(self.filename[0]) if fname != "." else dname
        else:
