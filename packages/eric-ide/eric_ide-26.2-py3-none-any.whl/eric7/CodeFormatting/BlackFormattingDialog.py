...
class BlackFormattingDialog(QDialog, Ui_BlackFormattingDialog):
    ...
    def __init__(
        ...
        @param lineRange tuple containing the start and end lines of the region to
            be treated (defaults to ())
        @type tuple of (int, int) (optional)
    ...
    def __formatOneFile(self, file):
        ...
        if self.__config["__action__"] is BlackFormattingAction.Diff:
            ...
            black.reformat_one(
                ...
                mode=mode,
                report=report,
                lines=[self.__lineRange] if self.__lineRange else [],
            )

        self.__finish()

    @staticmethod
    def __diffFormatFile(src, fast, mode, report, relSrc="", lineRange=()):
        """
        ...
        @type str (optional)
        @param lineRange range of lines to be processed (defaults to ())
        @type tuple of (int, int) (optional)
        """
        ...
        with open(src, "rb") as buf:
            try:
                # black >= 25.9.0
                srcContents, _, _ = black.decode_bytes(buf.read(), mode)
            except TypeError:
                # backward compatibility for black < 25.9.0
                srcContents, _, _ = black.decode_bytes(buf.read())
        try:
            dstContents = black.format_file_contents(
                srcContents,
                fast=fast,
                mode=mode,
                lines=[lineRange] if lineRange else [],
            )
...
class BlackReport(QObject, black.Report):
    ...
    def path_ignored(self, src, message=""):  # noqa: ARG002, U-100
        """
