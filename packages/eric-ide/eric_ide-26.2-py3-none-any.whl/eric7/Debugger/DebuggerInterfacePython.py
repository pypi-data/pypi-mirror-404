...
class DebuggerInterfacePython(QObject):
    ...
    def __identityTranslation(self, fn, remote2local=True):  # noqa: ARG002, U-100
        """
    ...
    def startRemote(
        ...
        useTracingDebugger = (
            "--settrace" if Preferences.getDebugger("PythonTracingDebugger") else ""
        )
        ...
        if Preferences.getDebugger("RemoteDbgEnabled") and not startViaServer:
            ...
            if rexec:
                ...
                args = [
                    *Utilities.parseOptionString(rexec),
                    rhost,
                    interpreter,
                    rdebugClient,
                ]
                ...
                if useTracingDebugger:
                    args.append(useTracingDebugger)
        ...
        if startViaServer and self.__ericServerDebuggerInterface is not None:
        ...
        if runInConsole or Preferences.getDebugger("ConsoleDbgEnabled"):
            ...
            if ccmd:
                args = [
                    *Utilities.parseOptionString(ccmd),
                    interpreter,
    ...
    def shutdown(self):
        ...
        while self.__connections:
            _debuggerId, sock = self.__connections.popitem()
    ...
    def remoteSetThread(self, debuggerId, tid, release_thread=False):
        """
        ...
        @type int
        @param release_thread flag to inform only to release the current thread
        @type bool
        """
        self.__sendJsonCommand(
            "RequestThreadSet",
    ...
    def __receiveJson(self, sock):
        ...
        while sock and sock.bytesAvailable():
            ...
            ##print("Server: ", jsonStr)    ## debug       # noqa: ERA001, M-891
    ...
    def handleJsonCommand(self, jsonStr, sock):
        ...
        if method == "DebuggerId":
            ...
            if self.__autoContinue and params["debuggerId"] not in self.__autoContinued:
                ...
                if not self.debugServer.getBreakPointModel().hasBreakpoint(
                    cf[0], int(cf[1])
                ):
                    # Do the automatic continue only if the line to be continued
                    # automatically does not contain a breakpoint.
                    QTimer.singleShot(
