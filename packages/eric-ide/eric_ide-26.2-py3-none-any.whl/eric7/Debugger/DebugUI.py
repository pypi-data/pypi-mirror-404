...
class DebugUI(QObject):
    ...
    def __init__(self, ui, vm, debugServer, debugViewer, project):
        ...
        self.clientType = ""
        self.lastAction = -1
        self.currentThreadID = -1  # -1=Main thread
        self.debugActions = [
            self.__continue,
            self.__step,
            self.__stepOver,
        ...
        debugServer.clientThreadList.connect(self.__setClientThreadList)
    ...
    def __resetUI(self, fullReset=True):
        ...
        if not self.passive:
            editor = self.viewmanager.activeWindow() if self.editorOpen else None
            self.__checkActions(editor)
    ...
    def __setClientThreadList(self, currentID, _threadList, _debuggerId):
        """
        Private method to store the current thread id.

        @param currentID id of the current thread
        @type int
        @param _threadList list of dictionaries containing the thread data
        @type list of dict
        @param _debuggerId ID of the debugger backend
        @type str
        """
        self.currentThreadID = currentID
    ...
    def __clientException(
        ...
        if (
            ...
            # Release the current thread to switch to the one with the exception. This
            # happens when already staying in a breakpoint while another thread runs
            # into an exception.
            self.debugServer.remoteSetThread(debuggerId, self.currentThreadID, True)

            # Now handle the user choice
            if res == EricMessageBox.Yes:
                ...
                self.debugActGrp.setEnabled(True)
                return
            if res == EricMessageBox.Ignore and exceptionType not in self.excIgnoreList:
                self.excIgnoreList.append(exceptionType)

        # Release the current thread in case of an ignored exception
        self.debugServer.remoteSetThread(debuggerId, self.currentThreadID, True)
