...
class BreakPointModel(QAbstractItemModel):
    ...
    def columnCount(self, parent=None):  # noqa: ARG002, U-100
        """
    ...
    def setData(self, index, value, role=Qt.ItemDataRole.EditRole):  # noqa: ARG002, U-100
        """
    ...
    def deleteBreakPoints(self, idxList):
        ...
        """
        rows = sorted(
            [index.row() for index in idxList if index.isValid()], reverse=True
        )
        for row in rows:
    ...
    def hasBreakpoint(self, fn, lineno):
        """
        Public method to test if a given line of a given file contains a breakpoint.

        @param fn filename to be tested
        @type str
        @param lineno line number to be tested
        @type int
        @return flag indicating a breakpoint
        @rtype bool
        """
        return any(bp[0] == fn and bp[1] == lineno for bp in self.breakpoints)
