...
class Editor(QsciScintillaCompat):
    ...
    def __init__(
        ...
        # reload the file automatically if there are no unsaved changes
        self.__ignoreExternalChanges = False
        # ignore all external changes to the loaded file
        self.line = -1
        ...
        # initialize the shared edit status object
        self.__sharedEditStatus = None
            ...
            if self.fileName:
                ...
                if fileExists:
                    if fileSizeKB > Preferences.getEditor("RejectFilesize"):
                        ...
                        raise OSError
                    if fileSizeKB > Preferences.getEditor("WarnFilesize"):
                    ...
                    self.loadFile(self.fileName, createIt=True, isRemote=fileIsRemote)
    ...
    def newBreakpointWithProperties(self, line, properties):
        ...
        if not properties[2]:
            ...
            marker = self.tcbreakpoint if properties[1] else self.cbreakpoint
        else:
            marker = self.tbreakpoint if properties[1] else self.breakpoint
    ...
    def __menuToggleTemporaryBreakpoint(self):
        ...
        """
        if self.line < 0:
            self.line, _index = self.getCursorPosition()
        self.line += 1
    ...
    def getBookmarks(self):
        ...
        @rtype list of int
        """
        return sorted([self.markerLine(handle) + 1 for handle in self.bookmarks])
    ...
    def readFile(self, fn, createIt=False, encoding="", isRemote=False):
        """
        ...
        @type bool (optional)
        @return tuple containing the file contents and the determine file encoding
        ...
        try:
            with EricOverrideCursor():
                if FileSystemUtilities.isRemoteFileName(fn) or isRemote:
                    ...
                    if encoding:
                        ...
                        txt, newEncoding = self.__remotefsInterface.readEncodedFile(
                            fn, create=True
                        )
                else:
                    ...
                    if encoding:
                        txt, newEncoding = Utilities.readEncodedFileWithEncoding(
                            fn, encoding
                        )
                    else:
                        txt, newEncoding = Utilities.readEncodedFile(fn)
            ...
            return txt, newEncoding

    def loadFile(self, fn, createIt=False, encoding="", noempty=False, isRemote=False):
        """
        Public method to load the text from a file.
        ...
        txt, self.encoding = self.readFile(
            fn=fn, createIt=createIt, encoding=encoding, isRemote=isRemote
        )
    ...
    def __getSaveFileName(self, path=None, remote=False):
        ...
        if remote or FileSystemUtilities.isRemoteFileName(path):
            title = self.tr("Save Remote File")
            fn = EricServerFileDialog.getSaveFileNameWithExtension(
                self,
                title,
                path,
            ...
            title = self.tr("Save File")
            fn = EricFileDialog.getSaveFileNameWithExtension(
                self,
                title,
                path,
                Lexers.getSaveFileFiltersList(True, True),
        ...
        if fn:
            if (
                ...
                and self.__remotefsInterface.exists(fn)
            ) or (FileSystemUtilities.isPlainFileName(fn) and os.path.exists(fn)):
                res = EricMessageBox.yesNo(
                    self,
                    title,
                    self.tr(
    ...
    def saveFile(self, saveas=False, path=None, remote=False):
        ...
        if self.__ignoreExternalChanges and not saveas:
            ok = EricMessageBox.yesNo(
                self,
                self.tr("Save File"),
                self.tr(
                    "<p>The file <b>{0}</b> has been changed while it was opened"
                    " in eric. Do you really want to save it?</p>"
                    "<p><b>Warning:</b> Saving the file now will overwrite these"
                    " changes.</p>"
    ...
    def __marginHoverStart(self, _pos, x, y):
        """
        Private slot showing the text of a syntax error or a warning marker.

        @param _pos mouse position into the document
        @type int
    ...
    def __commentSelection(self):
        ...
        lineFrom, _indexFrom, lineTo, indexTo = self.getSelection()
        ...
        if self.isPyFile() or self.isRubyFile():
            ...
            while True:
                if goUp and lineNo < 0:
                    ...
                    return
                if not goUp and lineNo == self.lines():
                    lineNo = self.lines() - 1
                ...
                if line.strip().startswith(("class ", "def ", "module ")):
                    ...
                    end = match.start() if match else self.lineLength(lineNo) - 1
    ...
    def autoComplete(self, auto=False, context=True):
        ...
        """
        if auto:
            if not Preferences.getEditor("AutoCompletionEnabled"):
                # auto-completion is disabled
                return

            style = self.styleAt(self.currentPosition() - 2)
            # Subtract two because the character was not added and no styling
            # has happened at the current position yet.
            if self.lexer_ is not None and (
                (
                    Preferences.getEditor("AutoCompletionDisabledInComments")
                    and self.lexer_.isCommentStyle(style)
                )
                or (
                    Preferences.getEditor("AutoCompletionDisabledInStrings")
                    and self.lexer_.isStringStyle(style)
                )
            ):
                # No auto-completion in strings and/or comments
                return
    ...
    def __callTip(self):
        ...
        ct = (
            self._encodeString("\n".join(callTips[:cv]))  # this is just a safe guard
            if cv > 0
            else self._encodeString(
                "\n".join(callTips)
    ...
    def __aboutToShowContextMenu(self):
        ...
        if not self.isResourcesFile:
            self.menuActs["Show"].setEnabled(bool(self.fileName) and self.isPyFile())
            self.menuActs["Diagrams"].setEnabled(
                bool(self.fileName) and (self.isPyFile() or self.isRubyFile())
            )
            self.menuActs["Formatting"].setEnabled(
                bool(self.fileName)
                and self.isPyFile()
                and not FileSystemUtilities.isRemoteFileName(self.fileName)
            )
        if not self.miniMenu:
            if self.lexer_ is not None:
    ...
    def __reopenWithEncodingMenuTriggered(self, act):
        ...
        """
        encoding = act.data()
        self.loadFile(self.fileName, encoding=encoding)
        self.__convertTabs()
        self.__checkEncoding()
    ...
    def macroSave(self):
        ...
        fname = EricFileDialog.getSaveFileNameWithExtension(
            self,
    ...
    def focusInEvent(self, event):
        ...
        if not self.dbs.isDebugging:
            self.updateReadOnly(False)
        self.setCursorFlashTime(QApplication.cursorFlashTime())

        if self.fileName and FileSystemUtilities.isRemoteFileName(self.fileName):
            self.checkReloadFile()
    ...
    def focusOutEvent(self, event):
        ...
        if (
            ...
            and not self.__assembly.isOutdatedShown()
        ):
            self.saveFile()
    ...
    def mousePressEvent(self, event):
        ...
        @type QMouseEvent
        """
        if event.button() == Qt.MouseButton.XButton1 and (
            event.modifiers() == Qt.KeyboardModifier.ControlModifier
        ):
            self.undo()
            event.accept()
        elif event.button() == Qt.MouseButton.XButton2 and (
            event.modifiers() == Qt.KeyboardModifier.ControlModifier
        ):
            self.redo()
            event.accept()
    ...
    def updateReadOnly(self, bForce=True):
        """
    ...
    @pyqtSlot()
    def setIgnoreExternalModification(self, ignore):
        """
        Public slot to set ignore external modifications of the loaded file.

        @param ignore flag indication to ignore external modifications
        @type bool
        """
        self.__ignoreExternalChanges = ignore

    @pyqtSlot()
    def setAutoReloadEnabled(self, enable):
        """
        Public slot to enable automatic reloading for this editor.

        @param enable flag indicating the automatic reloading state
        @type bool
        """
        self.__autoReload = enable

    @pyqtSlot()
    def checkReloadFile(self):
        """
        Public slot to check, if the file needs to be reloaded.
        """
        if (
            self.__checkExternalModification
            and self.checkModificationTime()
            and not self.__assembly.isOutdatedShown()
        ):
            if self.__ignoreExternalChanges:
                self.recordModificationTime()
            elif (
                self.__autoReload or Preferences.getEditor("AutoReopen")
            ) and not self.isModified():
                self.refresh()
            else:
                msg = self.tr(
                    """<p>The file <b>{0}</b> has been changed while it"""
                    """ was opened in eric.</p>"""
                ).format(self.fileName)
                msgType = "info"
                if self.isModified():
                    msg += self.tr(
                        """<p><b>Warning:</b> You will lose"""
                        """ your changes upon reloading it.</p>"""
                    )
                    msgType = "warning"
                self.__assembly.showOutdated(msg, msgType)
    ...
    def refresh(self):
        ...
        # reread the file
        try:
            self.loadFile(self.fileName, noempty=True)
        except OSError:
    ...
    def getDiffText(self):
        """
        Public method to generate a diff between the file on disk and the current text.

        @return string containing the unified diff output
        @rtype str
        """
        try:
            fileText = self.readFile(fn=self.fileName)[0]
        except OSError:
            fileText = ""
        return "".join(
            difflib.unified_diff(
                fileText.splitlines(keepends=True),
                self.text().splitlines(keepends=True),
                fromfile=self.fileName,
                tofile="editor contents",
            )
        )
    ...
    def __loadDiagram(self):
        ...
        if loadedDiagram.load():
            loadedDiagram.show(fromFile=True)
    ...
    def isSpellCheckRegion(self, pos):
        ...
        if self.__spellCheckStringsOnly:
            ...
            if self.lexer_ is not None:
                return self.lexer_.isCommentStyle(style) or self.lexer_.isStringStyle(
                    style
                )
    ...
    #######################################################################
    ## Shared edit status methods (Interface to the cooperation plugin)
    #######################################################################

    def getSharedEditStatus(self):
        """
        ...
        """
        return self.__sharedEditStatus

    def setSharedEditStatus(self, status):
        """
        Public method to set the editor's shared edit status.

        @param status shared edit status
        @type SharedEditStatus
        """
        self.__sharedEditStatus = status
    ...
    def __searchCurrentWord(self, forward=True):
        ...
        if matches:
            if forward:
                ...
                match = matchesAfter[0] if matchesAfter else matches[0]
            else:
                matchesBefore = [m for m in matches if m.start() < startPos]
                match = matchesBefore[-1] if matchesBefore else matches[-1]
    ...
    def sortLines(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            keyFun = str.lower if alnum and not caseSensitive else None
    ...
    @pyqtSlot(int, int, int)
    def __showMouseHoverHelp(self, pos, _x, y):
        """
        ...
        @type int
        @param _x x-value of mouse screen position
        @type int
        @param y y-value of mouse screen position
        @type int
        """
    ...
    def __performFormatWithBlack(self, action):
        ...
        if not self.isModified() or self.saveFile():
            ...
            if dlg.exec() == QDialog.DialogCode.Accepted:
                ...
                if self.hasSelectedText():
                    startLine, _, endLine, endIndex = self.getSelection()  # zero based
                    startLine += 1
                    if endIndex != 0:
                        endLine += 1
