...
from eric7.RemoteServerInterface.EricServerFileDialog import EricServerFileDialog
...
class PropertiesDialog(QDialog, Ui_PropertiesDialog):
    ...
    def __init__(self, project, new=True, isRemote=False, parent=None, name=None):
        ...
        environmentTypes = (
            ...
            .getEnvironmentTypesRegistry()
            .getCreatableEnvironmentTypes()
        )
        for environmentType in environmentTypes:
            self.embeddedVenvTypeComboBox.addItem(
                environmentType.visual_name, environmentType.name
            )
        ...
        projectTypes = [
            (projectTypeItem[1], projectTypeItem[0])
        ...
        if not new:
            ...
            self.embeddedVenvTypeComboBox.setCurrentIndex(
                self.embeddedVenvTypeComboBox.findData(
                    self.project.getProjectData(dataKey="EMBEDDED_VENV_TYPE")
                )
            )
        else:
    ...
    @pyqtSlot(str)
    def on_dirPicker_textChanged(self, _txt):
        """
        Private slot to handle a change of the project directory.

        @param _txt name of the project directory
        @type str
        """
        self.__updateOk()
    ...
    @pyqtSlot()
    def on_populateButton_clicked(self):
        """
        Private slot to populate the dialog with data of a 'pyproject.toml' file.
        """
        if self.dirPicker.text():
            if self.__remoteProject:
                initialPath = self.__remotefsInterface.join(
                    self.dirPicker.text(), "pyproject.toml"
                )
            else:
                initialPath = os.path.join(self.dirPicker.text(), "pyproject.toml")
        else:
            initialPath = "pyproject.toml"

        try:
            if self.__remoteProject:
                projectFile = EricServerFileDialog.getOpenFileName(
                    self,
                    self.tr("Populate From 'pyproject.toml'"),
                    initialPath,
                    self.tr("TOML Files (*.toml)"),
                    None,
                )
                if not bool(projectFile):
                    return

                pyprojectStr = self.__remotefsInterface.readEncodedFileWithEncoding(
                    projectFile, encoding="utf-8"
                )
            else:
                projectFile = EricFileDialog.getOpenFileName(
                    self,
                    self.tr("Populate From 'pyproject.toml'"),
                    initialPath,
                    self.tr("TOML Files (*.toml)"),
                ...
                with open(projectFile, encoding="utf-8") as f:
                    pyprojectStr = f.read()
            ...
            EricMessageBox.critical(
                ...
                self.tr("Populate From 'pyproject.toml'"),
                self.tr(
                    "<p>The 'pyproject.toml' file <b>{0}</b> could not be read.</p>"
                    "<p>Reason: {1}</p>"
                ).format(projectFile, str(err)),
            )
            return

        if pyprojectStr:
            try:
                data = tomlkit.loads(pyprojectStr)
            except tomlkit.exceptions.ParseError as err:
                EricMessageBox.critical(
                    self,
                    self.tr("Populate From 'pyproject.toml'"),
                    self.tr(
                        "<p>The 'pyproject.toml' file <b>{0}</b> has invalid"
                        " contents.</p><p>Reason: {1}</p>"
                    ).format(projectFile, str(err)),
                )
                return
            ...
            if not project:
                EricMessageBox.warning(
                    ...
                    self.tr("Populate From 'pyproject.toml'"),
                    self.tr(
                        "<p>The 'pyproject.toml' file <b>{0}</b> does not contain a"
                        " 'project' table.</p>"
                    ).format(projectFile),
                )
                return

            # now populate the various data fields
            # 1. data derived from the loaded 'pyproject.toml' file path
            self.dirPicker.setText(os.path.dirname(projectFile))
            if os.path.exists(os.path.join(os.path.dirname(projectFile), "src")):
                # common convention for packaged application structure
                self.srcDirPicker.setText("src")

            # 2. data derived from the 'pyproject.toml' file contents
            self.nameEdit.setText(project.get("name", ""))
            self.versionEdit.setText(project.get("version", ""))
            self.descriptionEdit.setText(project.get("description", ""))
            authors = project.get("authors", [])
            if authors:
                self.authorEdit.setText(authors[0].get("name", ""))
                self.emailEdit.setText(authors[0].get("email", ""))
    ...
    def storeData(self):
        ...
        self.project.setProjectData(
            self.embeddedVenvTypeComboBox.currentData(), dataKey="EMBEDDED_VENV_TYPE"
        )
