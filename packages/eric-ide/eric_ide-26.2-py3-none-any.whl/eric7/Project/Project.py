...
class Project(QObject):
    ...
    def addFileCategory(self, category, categoryItem):
        ...
        if not isinstance(categoryItem, FileCategoryRepositoryItem):
            err = "'categoryItem' must be an instance of 'FileCategoryRepositoryItem'"
            raise TypeError(err)
    ...
    def getFileCategoryExtension(self, category, reverse=False):
        ...
        if reverse:
            ...
            return extensions
        return self.__fileCategoriesRepository[category.upper()].fileCategoryExtensions[
            :
        ]
    ...
    def __readTasks(self):
        ...
        if FileSystemUtilities.isRemoteFileName(self.pfile):
            ...
            base, _ext = os.path.splitext(os.path.basename(self.pfile))
    ...
    def createNewProject(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            ...
            if (
                ...
                vcsSystemsDisplay.extend(
                    vcsSystemsDict[key] for key in sorted(vcsSystemsDict)
                )
            ...
            if self.vcs is not None and not FileSystemUtilities.isRemoteFileName(
                self.ppath
            ):
            ...
            if (
                ...
                and self.__isStandardEnvironment()
                and not FileSystemUtilities.isRemoteFileName(self.ppath)
            ):
    ...
    def saveProject(self):
        ...
        """
        ok = (
            (self.__writeProject() if len(self.pfile) > 0 else self.saveProjectAs())
            if self.isDirty()
            else True
        )
    ...
    def __closeAllWindows(self):
        ...
        """
        if self.codemetrics is not None:
            self.codemetrics.close()
            self.codemetrics = None
        if self.codecoverage is not None:
            self.codecoverage.close()
            self.codecoverage = None
        if self.profiledata is not None:
            self.profiledata.close()
            self.profiledata = None
        if self.applicationDiagram is not None:
            self.applicationDiagram.close()
            self.applicationDiagram = None
        if self.loadedDiagram is not None:
            self.loadedDiagram.close()
            self.loadedDiagram = None
    ...
    def getProjectFiles(self, fileType, normalized=False):
        ...
        if fileType not in self.getFileCategories():
            err = "Given file type has incorrect value."
    ...
    def getRelativeUniversalPath(self, fullpath):
        ...
        return FileSystemUtilities.fromNativeSeparators(self.getRelativePath(fullpath))
    ...
    def __checkProjectFileGroup(self, fn, group):
        ...
        if (
            ...
            newfn = newfn.lower()
            if any(entry.lower() == newfn for entry in self.__pdata[group]) or (
                group == "OTHERS"
                and any(
    ...
    def initVCS(self, vcsSystem=None, nooverride=False):
        ...
        if vcsSystem is not None:
            try:
                vcs = VCS.factory(vcsSystem)  # noqa: M-915
            except ImportError:
                if override:
        ...
        if forProject:
            if vcs is None:
                self.vcsProjectHelper = VCS.getBasicHelper(self)  # noqa: M-915
    ...
    def __pluginCreateArchives(self, snapshot=False):
        ...
        for count, pkglist in enumerate(selectedLists):
            ...
            try:
                with open(pkglist, encoding="utf-8") as pkglistFile:
    ...
    def __pluginExtractVersion(self, filename):
        ...
        for sourceline in sourcelines:
            if sourceline.startswith("version = "):
                ...
                break
            if sourceline.strip().startswith(('"version":', "'version':")):
    ...
    def __showContextMenuEnvironment(self):
        ...
        """
        self.installVenvAct.setEnabled(self.__isStandardEnvironment())
        self.upgradeVenvAct.setEnabled(
            self.__isStandardEnvironment() and bool(self.__findEmbeddedEnvironment())
        )
        self.recreateVenvAct.setEnabled(self.__isStandardEnvironment())
    ...
    def __createEmbeddedEnvironment(self, upgrade=False, force=False):
        ...
        if not self.__isStandardEnvironment():
            # not created by venv and maintained by pip
            return
    ...
    def __installProjectIntoEnvironment(self):
        ...
        development mode.
        """
        if self.__isStandardEnvironment():
            # created by venv and maintained by pip
            pip = ericApp().getObject("Pip")
            pip.installEditableProject(
                self.getProjectInterpreter(), self.getProjectPath()
            )
    ...
    def __loadEnvironmentConfiguration(self):
        ...
        try:
            with open(
                os.path.join(self.getProjectManagementDir(), "venv_config.json")
            ) as f:
    ...
    def __isStandardEnvironment(self):
        """
        Private method to check, if the embedded environment is a standard environment
        (i.e. created with 'venv' and managed with 'pip').

        @return flag indicating a standard environment
        @rtype bool
        """
        return self.__pdata["EMBEDDED_VENV_TYPE"] == "standard"
