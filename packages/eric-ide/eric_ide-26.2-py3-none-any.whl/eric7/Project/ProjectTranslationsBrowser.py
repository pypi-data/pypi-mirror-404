...
from xml.etree import ElementTree as ET  # noqa: N-815
...
def _escape_cdata_linguist(text):
    """
    Function to escape character data iaw. the way Qt Linguist does it.

    The difference to the one used by ElementTree is that the ' and " characters
    are escaped as well.
    ...
    try:
        ...
        ET._raise_serialization_error(text)  # noqa: M-915
    else:
        return text


def _elementTreeXmlString(xmlElement):
    """
    Function to generate an XML string from a given element using our specialized
    escape function.

    Therefore the ElementTree._escape_cdata() function is replaced by the specialized
    one before output for the .ts file is created and changed back to the original
    afterwards.

    @param xmlElement reference to the ElementTree Element object to be converted to
        a string
    @type xml.etree.ElementTree.Element
    @return string output of the element
    @rtype str
    """
    # patch ElementTree to adhere with Qt Linguist
    _escape_cdata_orig = ET._escape_cdata
    ET._escape_cdata = _escape_cdata_linguist

    xmlString = ET.tostring(xmlElement, encoding="unicode")  # noqa: M-915

    # set back ElementTree to its original
    ET._escape_cdata = _escape_cdata_orig
...
class ProjectTranslationsBrowser(ProjectBaseBrowser):
    ...
    def __writeTempProjectFile(self, langs, filterList):
        ...
        return len(self.__tmpProjects) != 0
    ...
    def __postProcessTSFile(self, tsFile):
        """
        Private method to post process the generated .ts file.

        @param tsFile project relative path of the .ts file
        @type str
        """
        tsFilePath = os.path.join(self.project.getProjectPath(), tsFile)
        root = ET.parse(source=tsFilePath).getroot()  # noqa: M-915
        ET.indent(root, space="")  # reset indentation  # noqa: M-915
        for contextElement in root.findall("context"):
            # indent all context elements with 4 spaces
            ET.indent(contextElement, space=4 * " ")  # noqa: M-915

        tsFileLines = [
            '<?xml version="1.0" encoding="utf-8"?>',
            "<!DOCTYPE TS>",
        ]
        for line in _elementTreeXmlString(root).splitlines():
            line = line.replace(" />", "/>")  # strip space before closing bracket
            if "<translation " in line and line.endswith("/>"):
                # adjust empty translation element
                line = line.replace("/>", "></translation>")
            tsFileLines.append(line)
        tsFileLines.append("")

        with open(tsFilePath, "w", encoding="utf-8") as f:
            f.write("\n".join(tsFileLines))
    ...
    def __generateTSFileDone(self, proc, exitCode, exitStatus):
        ...
        for processData in self.__pylupdateProcesses:
            if proc is processData[0]:
                ...
                if processData[3]:
                    self.__postProcessTSFile(processData[3])

                if processData[2]:
                    self.appendStdout.emit("".join(processData[2]))

                self.__pylupdateProcesses.remove(processData)  # noqa: M-569
                break
        ...
        if self.project.getProjectType() in ["PyQt6", "PyQt6C", "E7Plugin"]:
            ...
            for lang in langs:
                ...
                if procStarted:
                    ...
                    self.__pylupdateProcesses.append((proc, "", [], lang))
                else:
    ...
    def __releaseTSFileDone(self, proc, exitCode, exitStatus):
        ...
        for processData in self.__lreleaseProcesses:
            if proc is processData[0]:
                if processData[1]:
                    self.appendStdout.emit("".join(processData[1]))

                self.__lreleaseProcesses.remove(processData)  # noqa: M-569
                break
