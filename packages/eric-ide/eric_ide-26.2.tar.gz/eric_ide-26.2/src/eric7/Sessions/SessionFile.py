...
class SessionFile(QObject):
    ...
    def writeFile(self, filename: str, withServer: bool = True) -> bool:
        ...
        # ==================
        if self.__isGlobal:
            dbgScriptName = dbg.scriptsHistory[0] if bool(dbg.scriptsHistory) else ""
            dbgCmdline = dbg.argvHistory[0] if bool(dbg.argvHistory) else ""
            dbgWd = dbg.wdHistory[0] if bool(dbg.wdHistory) else ""
            dbgEnv = dbg.envHistory[0] if bool(dbg.envHistory) else ""
            dbgMultiprocessNoDebug = (
                dbg.multiprocessNoDebugHistory[0]
                if bool(dbg.multiprocessNoDebugHistory)
                else ""
            )
