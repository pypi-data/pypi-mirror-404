...
class MicroPythonDeviceInterface(QObject):
    ...
    def connectToDevice(self, connection):  # noqa: ARG002, U-100
        """
        ...
        """
        err = "This method needs to be implemented in a derived class."
    ...
    def write(self, data):  # noqa: U-100
        """
    ...
    def execute(self, commands, *, mode="raw", timeout=0):  # noqa: ARG002, U-100
        """
        ...
        if mode not in ("paste", "raw"):
            err = "Unsupported submit mode given ('{0}').".format(mode)
            raise ValueError(err)

        return b"", b""

    def executeAsync(self, commandsList, submitMode):  # noqa: ARG002, U-100
        """
        ...
        if submitMode not in ("raw", "paste"):
            err = "Unsupported submit mode given ('{0}').".format(submitMode)
            raise ValueError(err)
