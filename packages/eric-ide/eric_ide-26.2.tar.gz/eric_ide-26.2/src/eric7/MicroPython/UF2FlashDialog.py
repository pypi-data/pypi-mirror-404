...
from PyQt6.QtCore import QCoreApplication, QEventLoop, QThread, Qt, pyqtSlot
...
SupportedUF2Boards = {
    "UF2 Board": {
        "volumes": {
            ...
            (0x0456, 0xA010): [
                ("32650BOOT", "MAX32650EvKit"),
                ("32650BOOT", "MAX32650FTHR"),
                ("32666BOOT", "MAX32666EvKit"),
                ("32666BOOT", "MAX32666FTHR"),
                ("32690BOOT", "MAX32690EvKit"),
                ("32690BOOT", "AD-APARD32690-SL"),
                ("78002BOOT", "MAX78002EvKit"),
            ],
            (0x0483, 0x5740): [
                ("DaisyBOOT", "STM32FH750IB"),
            ],
            (0x04D8, 0x2402): [
...
def getFoundDevices(boardType=""):
    ...
    winVolumes = (
        FileSystemUtilities.winGetVolumes() if OSUtilities.isWindowsPlatform() else []
    )
        ...
        for vidpid, volumes in SupportedUF2Boards[board]["volumes"].items():
            for volume, description in volumes:
                if OSUtilities.isWindowsPlatform():
                    if [v for v, _ in winVolumes if v.startswith(volume)]:
    ...
    try:
        allUsbDevices = usb.core.find(find_all=True)
...
class UF2FlashDialog(QDialog, Ui_UF2FlashDialog):
        ...
        if os.path.exists(firmwarePath) and os.path.exists(volumePath):
            ...
            try:
                ...
                EricMessageBox.critical(
                    ...
                    self.tr(
                        "<p>The firmware file could not be copied to the device volume."
                        "</p><p>Reason: {0}</p>"
                    ).format(str(err)),
                )
    ...
    @pyqtSlot(str)
    def on_firmwarePicker_textChanged(self, _text):
        """
        Private slot handling a change of the firmware file.

        @param _text current text of the firmware edit
        @type str
        """
        self.__updateFlashButton()

    @pyqtSlot(str)
    def on_bootPicker_textChanged(self, _text):
        """
        Private slot handling a change of the boot volume.

        @param _text current text of the boot volume edit
        @type str
        """
        self.__updateFlashButton()
    ...
    def on_searchBootButton_clicked(self):
        ...
        if not OSUtilities.isWindowsPlatform():
            ...
            winVolumes = FileSystemUtilities.winGetVolumes()
                ...
                for volume, _ in volumes:
                    if OSUtilities.isWindowsPlatform():
                        foundBootVolumes.update(
                            d for v, d in winVolumes if v.startswith(volume)
                        )
                    else:
                        # check only user mounted devices on non-Windows systems
                        for userMount in userMounts:
        ...
        if len(foundBootVolumes) == 0:
            ...
            selectedVolume = next(iter[foundBootVolumes])
        else:
