...
class GraphicsRecursionError(OverflowError, ValueError):
    """
...
def sort(nodes, routes, noRecursion=False):
    ...
    @exception GraphicsRecursionError a recursion error was detected
    """
    ...
    stages = [stage]
    taken = []
    stage.extend(node for node in nodes if not parents.get(node))
    ...
    while nodes:
        ...
        for node in stage:
            for child in children.get(node, []):
                if child not in previousStageChildren and child not in taken:
                    ...
                    raise GraphicsRecursionError((child, node))
        ...
        for current in stage:
            ...
            for parent in currentParents:
                if (
                    ...
                    removes.append(current)  # noqa: PERF401
        ...
        if nodelen == len(nodes):
            if noRecursion:
                raise GraphicsRecursionError(nodes)
            stages.append(nodes[:])
            nodes = []
