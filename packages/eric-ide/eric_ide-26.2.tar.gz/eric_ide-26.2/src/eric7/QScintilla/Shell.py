...
class Shell(QsciScintillaCompat):
    ...
    def __showStartMenu(self):
        ...
        for venvName in sorted(
            venvManager.getVirtualenvNames(filterList=("-eric_server",))
        ):
    ...
    def __writePrompt(self):
        ...
        """
        self.queueText.emit(sys.ps2 if self.inContinue else sys.ps1)
    ...
    def paste(self, lines=None):
        ...
        if self.__isCursorOnLastLine():
            ...
            if self.hasSelectedText():
                lineFrom, indexFrom, _lineTo, indexTo = self.getSelection()
    ...
    def __indentLength(self, line):
        ...
        @rtype int
        """
        line = line.removeprefix(sys.ps1)
    ...
    def __QScintillaDelete(self):
        ...
        if self.__isCursorOnLastLine():
            if self.hasSelectedText():
                lineFrom, indexFrom, _lineTo, _indexTo = self.getSelection()
                if self.text(lineFrom).startswith(sys.ps1):
