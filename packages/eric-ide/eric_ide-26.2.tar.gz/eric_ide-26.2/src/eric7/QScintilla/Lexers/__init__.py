...
def registerLexer(
    ...
    if name in LexerRegistry:
        exc = 'Lexer "{0}" already registered.'.format(name)
        raise KeyError(exc)
    LexerRegistry[name] = [
        displayString,
        filenameSample,
...
def getOpenFileFiltersList(includeAll=False, asString=False, withAdditional=True):
    ...
    """
    openFileFiltersList = [
        QCoreApplication.translate("Lexers", "Python Files (*.py *.pyi *.py3)"),
        ...
        QCoreApplication.translate("Lexers", "JavaScript Files (*.js *.mjs *.cjs)"),
        ...
        QCoreApplication.translate(
            "Lexers", "Pascal Files (*.dpr *.dpk *.pas *.dfm *.inc *.pp *.lpr)"
        ),
...
def getSaveFileFiltersList(includeAll=False, asString=False, withAdditional=True):
    ...
    saveFileFiltersList = [
        ...
        QCoreApplication.translate("Lexers", "Python3 Stub Files (*.pyi)"),
