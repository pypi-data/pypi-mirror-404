...
from eric7.CodeFormatting.FormattingDiffWidget import FormattingDiffWidget
...
from .EditorOutdatedWidget import EditorOutdatedWidget
...
class EditorAssembly(QWidget):
    ...
    def __init__(self, dbs, fn="", vm=None, filetype="", editor=None, tv=None):
        ...
        self.__outdatedInfoWidget = EditorOutdatedWidget()
        ...
        self.__layout.addWidget(self.__outdatedInfoWidget, 0, 1, 1, -1)
        self.__layout.addWidget(self.__globalsCombo, 1, 1)
        self.__layout.addWidget(self.__membersCombo, 1, 2)
        self.__layout.addWidget(self.__buttonsWidget, 2, 0, -1, 1)
        self.__layout.addWidget(self.__editorSplitter, 2, 1, 1, 2)
        ...
        self.__outdatedInfoWidget.hide()

        ericApp().getObject("UserInterface").preferencesChanged.connect(
            self.__preferencesChanged
        )

        self.__outdatedInfoWidget.activateAutoReload.connect(self.__activateAutoReload)
        self.__outdatedInfoWidget.ignoreChanges.connect(self.__ignoreExternalChanges)
        self.__outdatedInfoWidget.reloadFile.connect(self.__refreshEditor)
        self.__outdatedInfoWidget.showDiff.connect(self.__showEditorDiff)
    ...
    #######################################################################
    ## Methods dealing with the outdated info widget below
    #######################################################################

    def showOutdated(self, message, messageType):
        """
        Public method to show the widget informing the user about external
        modifications.

        @param message text of a message to be show
        @type str
        @param messageType message type ('info' or 'warning')
        @type str
        """
        self.__outdatedInfoWidget.setMessage(message=message, messageType=messageType)
        self.__outdatedInfoWidget.show()

    def isOutdatedShown(self):
        """
        Public method to test, if the outdated widget is shown.

        @return flag indicating visibility
        @rtype bool
        """
        return self.__outdatedInfoWidget.isVisible()

    @pyqtSlot()
    def __showEditorDiff(self):
        """
        Private slot to show a diff dialog with the differences between the externally
        modified file and the current editor text.
        """
        diff = self.__editor.getDiffText()
        if self.__diffDialog is None:
            self.__diffDialog = FormattingDiffWidget()
        self.__diffDialog.showDiff(diff)

    @pyqtSlot()
    def __activateAutoReload(self):
        """
        Private slot to activate the automatic reloading feature of the editor.
        """
        self.__outdatedInfoWidget.hide()
        self.__editor.setAutoReloadEnabled(True)
        self.__editor.refresh()

    @pyqtSlot()
    def __ignoreExternalChanges(self):
        """
        Private slot to inform the editor, that this and all further external changes
        shall be ignored.
        """
        self.__outdatedInfoWidget.hide()
        self.__editor.setIgnoreExternalModification(True)
