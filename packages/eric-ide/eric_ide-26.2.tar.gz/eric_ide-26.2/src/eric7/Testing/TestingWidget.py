...
from eric7.Globals import (
    ...
    recentNameTestParametersHistory,
)
...
class TestingWidget(QWidget, Ui_TestingWidget):
    ...
    def __init__(self, testfile=None, parent=None, releaseMode=False):
        ...
        self.customParametersComboBox.lineEdit().setClearButtonEnabled(True)
        ...
        self.__failedTests = []
        self.__customParametersHistory = []
        ...
        self.customParametersComboBox.editTextChanged.connect(self.__resetResults)
    ...
    def __populateVenvComboBox(self):
        ...
        self.venvComboBox.addItems(
            sorted(self.__venvManager.getVirtualenvNames(filterList=("-eric_server",)))
        )
    ...
    def __insertHistory(self, widget, history, item):
        ...
        if history:
            if item != history[0]:
                # prepend the given item to the given history list
                if item is None:
                    item = ""
                if item in history:
                    history.remove(item)
    ...
    @pyqtSlot(str)
    def __insertCustomParameters(self, parameters):
        """
        Private slot to insert the custom parameters string into the
        customParametersComboBox object.

        @param parameters custom parameters string to be inserted
        @type str
        """
        self.__insertHistory(
            self.customParametersComboBox, self.__customParametersHistory, parameters
        )
    ...
    def __loadRecent(self):
        ...
        def uniq(list_):
            """
            Function to create a list containing unique values maintaining the
            original list order.

            @param list_ list to be made unique
            @type list
        ...
        maxRecent = Preferences.getDebugger("RecentNumber")
        ...
        # 2. discovery history
        self.__discoverHistory.clear()
        rs = Preferences.Prefs.rsettings.value(recentNameTestDiscoverHistory)
        if rs is not None:
            recent = [f for f in uniq(EricUtilities.toList(rs)) if os.path.exists(f)]
            self.__discoverHistory = recent[:maxRecent]

        # 3. test file history
        self.__fileHistory.clear()
        rs = Preferences.Prefs.rsettings.value(recentNameTestFileHistory)
        if rs is not None:
            recent = [f for f in uniq(EricUtilities.toList(rs)) if os.path.exists(f)]
            self.__fileHistory = recent[:maxRecent]

        # 4. test name history
        self.__testNameHistory.clear()
        rs = Preferences.Prefs.rsettings.value(recentNameTestNameHistory)
        if rs is not None:
            recent = [n for n in uniq(EricUtilities.toList(rs)) if n]
            self.__testNameHistory = recent[:maxRecent]

        # 5. custom parameters history
        self.__customParametersHistory.clear()
        rs = Preferences.Prefs.rsettings.value(recentNameTestParametersHistory)
        if rs is not None:
            recent = [p for p in uniq(EricUtilities.toList(rs)) if p]
            self.__customParametersHistory = recent[:maxRecent]
    ...
    def __saveRecent(self):
        ...
        Preferences.Prefs.rsettings.setValue(
            recentNameTestParametersHistory, self.__customParametersHistory
        )
    ...
    def clearRecent(self):
        ...
        self.customParametersComboBox.clear()
    ...
    @pyqtSlot(str)
    def on_discoveryPicker_editTextChanged(self, _txt):
        """
        Private slot to handle a change of the discovery start directory.

        @param _txt new discovery start directory
        @type str
        """
        self.discoveryList.clear()
    ...
    @pyqtSlot(int)
    def on_venvComboBox_currentIndexChanged(self, _index):
        """
        Private slot handling the selection of a virtual environment.

        @param _index index of the selected environment
        @type int
        """
    ...
    @pyqtSlot(int)
    def on_frameworkComboBox_currentIndexChanged(self, _index):
        """
        Private slot handling the selection of a test framework.

        @param _index index of the selected framework
        @type int
        """
        self.__resetResults()
        self.__updateCoverage()
    ...
    def __discoverTests(self):
        ...
        customParametersString = self.customParametersComboBox.currentText()
        self.__insertCustomParameters(customParametersString)
        customParameters = (
            shlex.split(customParametersString) if customParametersString else []
        )
    ...
    def __testsCollected(self, testNames):
        ...
        testResults = [
            ...
            for testId, name, desc, filename, lineno, _ in testNames
        ]
    ...
    @pyqtSlot(QTreeWidgetItem, int)
    def on_discoveryList_itemActivated(self, item, _column):
        """
        Private slot handling the user activating an item.

        @param item reference to the item
        @type QTreeWidgetItem
        @param _column column of the double click
        @type int
        """
        if item:
