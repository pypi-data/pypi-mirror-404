...
def getargvalues(frame):
    ...
    if not isframe(frame):
        err = "{0!r} is not a frame object".format(frame)
        raise TypeError(err)
...
def _getfullargs(co):
    ...
    """
    if not iscode(co):
        err = "{0!r} is not a code object".format(co)
        raise TypeError(err)
...
def isPythonProgram(program, withPath=False):
    ...
    prog = os.path.basename(str(program)).lower()
...
def patchArguments(debugClient, arguments, noRedirect=False, isPythonProg=False):
    ...
    program = os.path.basename(str(args[0])).lower()
    ...
    if useSettrace:
        modifiedArguments.append("--settrace")
    ...
    return quoteArgs(modifiedArguments)
...
def patchArgumentStringWindows(debugClient, argStr):
    ...
    return " ".join(patchArguments(debugClient, args))
