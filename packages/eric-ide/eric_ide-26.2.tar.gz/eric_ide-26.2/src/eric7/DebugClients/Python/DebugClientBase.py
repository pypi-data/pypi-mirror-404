...
from FlexModuleCompleter import ModuleCompleter
...
class DebugClientBase:
    ...
    def __compileCommand(self, statement, filename="<string>", mode="exec"):
        ...
        try:
            ...
            _exctype, excval, _exctb = sys.exc_info()
            try:
                message = str(excval)
    ...
    def handleJsonCommand(self, jsonStr):
        ...
        @type str
        """
        ##        printerr(jsonStr)          ## debug       # noqa: ERA001, M-891
        ...
        if method == "RequestVariables":
            ...
            if threadId in self.threads:
                ...
                if not params.get("releaseThread"):
            ...
            callTraceEnabled = self.profile if params["enable"] else None
            ...
            try:
                ...
                if code is None:
                    # store a potential from ... import statement for completion
                    if " import (" in params["statement"]:
                        self.__importLine = params["statement"]
                    elif self.__importLine:
                        self.__importLine += " " + params["statement"]
    ...
    def eventLoop(self, disablePolling=False):
        ...
        while not self.eventExit:
            ...
            try:
                rrdy, wrdy, _xrdy = select.select([self.readstream], wrdy, [])
    ...
    def eventPoll(self):
        ...
        @return polling delay in seconds
        @rtype float
        """
        ...
        rrdy = []
        if self.readstream:
            rrdy = [self.readstream]
        # immediate return if nothing is ready.
        try:
            rrdy, wrdy, _xrdy = select.select(rrdy, wrdy, [], 0)
        except (KeyboardInterrupt, OSError):
            return 1.0
    ...
    def __completionList(self, text):
        ...
        # Look for module completions if an import line is entered.
        if text.lstrip().startswith(("import ", "from ")) or self.__importLine:
            # Get completion for importing modules.
            try:
                namespace = self.currentThread.getFrameLocals(self.framenr)
            except AttributeError:
                cf = self.currentThread.getFrame(self.framenr)
                namespace = self.debugMod.__dict__ if cf is None else cf.f_globals
            moduleCompleter = ModuleCompleter(namespace)
            if text.lstrip().startswith(("import ", "from ")):
                compText = text
            elif self.__importLine:
                compText = f"{self.__importLine} {text}"
            for comp in moduleCompleter.get_completions(compText):
                if comp is not None:
                    completions.add(comp)
            if completions:
                text = compText.rsplit(None, 1)[-1].rsplit(",", 1)[-1]

        # Module completions did not determine anything, try keyword, function
        # and attribute completion.
        if not completions:
            completerDelims = " \t\n`~!@#$%^&*()-=+[{]}\\|;:'\",<>/?"
    ...
    def startDebugger(
        ...
        redirect=True,
        passive=True,
        multiprocessSupport=False,  # noqa: ARG002, U-100
    ):
        """
    ...
    def startProgInDebugger(
        ...
        if codeStr:
            ...
            _modName, modSpec, code = runpy._get_module_details(scriptModule)
        ...
        if "--" in sys.argv:
            ...
            while args[0]:
                if args[0] == "-h":
                    ...
                    del args[0]
            if not args:
                print("No program given. Aborting!")  # noqa: T201, M-801
            elif "-m" in args:
                print(  # noqa: T201, M-801
                    "Running module as a script is not supported. Aborting!"
                )
            else:
            ...
            if remoteAddress is None:
                print(f"Remote host '{ipOrHost}' could not be resolved.")  # noqa: T201, M-801
                sys.exit(1)
            ...
            if port >= 0:
                ...
                print("No network port given. Aborting...")  # noqa: T201, M-801
                sys.exit(2)
