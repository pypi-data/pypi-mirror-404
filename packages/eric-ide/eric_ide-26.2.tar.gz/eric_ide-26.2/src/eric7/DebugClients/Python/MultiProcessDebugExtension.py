...
def createForkExec(originalName):
    """
    Function to patch the '_posixsubprocess.fork_exec' process creation functions.
...
def createSubprocessForkExec(originalName):
    """
    Function to patch the 'subprocess._fork_exec' process creation functions.

    <ul>
        <li>subprocess._fork_exec(args, executable_list, close_fds, ... (13 more))</li>
    </ul>
    ...
    def newForkExec(args, *other_args):
        """
        Function replacing the '_fork_exec' functions of the subprocess module.
        """
        import subprocess  # __IGNORE_WARNING_I-103__

        if _shallPatch():
            args = patchArguments(_debugClient, args)
        return getattr(subprocess, originalName)(args, *other_args)
...
def patchNewProcessFunctions(multiprocessEnabled, debugClient):
    ...
    if isWindowsPlatform():
        ...
        with contextlib.suppress(ImportError):
            import subprocess  # __IGNORE_WARNING_I-10__

            patchModule(subprocess, "_fork_exec", createSubprocessForkExec)
