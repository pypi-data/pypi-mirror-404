"""
Word completion for the eric-ide shell.

<h4>NOTE for eric-ide variant</h4>

    This version is a re-implementation of rlcompleter as found in the Python3
...
class Completer:
    ...
    def __init__(self, namespace=None):
        ...
        if namespace and not isinstance(
            ...
            err = "namespace must be a dictionary or since 3.13 a FrameLocalsProxy"
            raise TypeError(err)
        ...
        while True:
            for word in words:
                if word[:n] == attr and not (noprefix and word[: n + 1] == noprefix):
                    attribute_match = "{0}.{1}".format(expr, word)
                    if isinstance(getattr(type(thisobject), word, None), property):
                        # thisobject.word is a method decorated by `@property`.
                        # What follows applies a postfix if thisobject.word is
                        # callable, but now we know that this is not callable
                        # (because it is a property). Also, getattr(thisobject, word)
                        # will evaluate the property method, which is not desirable.
                        attribute_matches.append(attribute_match)
                        continue
                    value = getattr(thisobject, word, None)
                    if value is not None:
                        attribute_matches.append(
                            self._callable_postfix(value, attribute_match)
                        )
                    else:
                        attribute_matches.append(attribute_match)
            if attribute_matches or not noprefix:
                break
