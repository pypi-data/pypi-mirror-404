...
class ThreadExtension:
    ...
    def __init__(self):
        ...
        # allowed threads to stop in
        self.exec_threads = [_thread.get_ident()]
    ...
    def attachThread(self, target=None, args=None, kwargs=None, mainThread=False):
        ...
        if mainThread:
            ...
            newThread.name = "Thread-{0}".format(self.threadNumber)
            self.threadNumber += 1
    ...
    def setCurrentThread(self, threadId):
        ...
        # If requested thread is running, just update the thread list
        if not self.currentThread.isBroken and not self.currentThread.isException:
            self.dumpThreadList()
            return

        # Requested thread is already active
        if threadId == _thread.get_ident():
            return

        # Set marker for the requested thread
        self.exec_threads.clear()
        self.exec_threads.append(threadId)

        self.eventExit = True  # Now exit this event loop

    def dumpThreadList(self, ignoreId=-1):
        """
        Public method to send the list of threads.

        @param ignoreId the id which should be removed because the thread has stopped
        @type int
        """
        self.ignore_thread_ids.add(ignoreId)
        ...
        for threadId, thd in list(self.threads.items()):
            d = {"id": threadId}
            try:
        ...
        currentThreadId = (
            -1
            if not self.currentThread or self.currentThread.id in self.ignore_thread_ids
            else self.currentThread.id
        )
    ...
    def updateThreadList(self):
        ...
        for threadId, frame in frames.items():
            ...
            if threadId in self.ignore_thread_ids:
                continue

            # Unknown thread
            if threadId not in self.threads:
                newThread = DebugBase(self)
                name = f"Thread-{self.threadNumber}"
                self.threadNumber += 1
        ...
        self.threads = {
            id_: thrd
            for id_, thrd in self.threads.items()
            if id_ in frames and id_ not in self.ignore_thread_ids
        }
        self.ignore_thread_ids &= frames.keys()
    ...
    def patchPyThreading(self, module):
        ...
        def _bootstrap(self, run):
            ...
            try:
                ...
                sys.settrace(None)
                _debugClient.dumpThreadList(self.ident)
    ...
    def patchQThread(self, module):
        ...
        def _bootstrapQThread(self, run):  # noqa: ARG001
            """
            ...
            try:
                ...
                sys.settrace(None)
                _debugClient.dumpThreadList(ident)
