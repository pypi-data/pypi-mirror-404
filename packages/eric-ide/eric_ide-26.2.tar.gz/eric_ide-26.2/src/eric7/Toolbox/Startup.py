...
def initializeResourceSearchPath(application):
    """
    Module function to initialize the default icon cache search paths.
...
# locale name of the loaded translations
_appLocale = None

# Keep a reference to the application object (just in case) to prevent garbage
# collection.
_application = None
...
def loadTranslators(qtTransDir, app, translationFiles=()):
    ...
    if loc != "C":
        ...
        for tf in ["{0}_{1}".format(tr, loc) for tr in translations]:
            ...
            _loadedTranslators[tf] = translator
            if ok:
...
def getLocale():
    """
    Function to get the name of the loaded locale.

    @return name of the loaded locale.
    @rtype str
    """
    return _appLocale
...
def appStartup(
    ...
    @rtype int
    """
    global _application, _appLocale
    ...
    _application = app
    _application.setQuitOnLastWindowClosed(quitOnLastWindowClosed)
    ...
    _application.setStyleSheetFile(Preferences.getUI("StyleSheet"))
    ...
    _appLocale = loadTranslators(qtTransDir, app, ("qscintilla",))
    # qscintilla needed for web browser

    w = mwFactory(args)
    if w is None:
        return 100
    _application.setMainWindow(mainWindow=w)

    if quitOnLastWindowClosed:
        _application.lastWindowClosed.connect(_application.quit)
    w.show()
    if raiseIt:
        w.raise_()
