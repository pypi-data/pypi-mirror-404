...
class DebugViewer(QWidget):
    ...
    def __init__(self, debugServer, parent=None):
        ...
        self.__debuggersList.itemClicked.connect(self.__debuggerSelected)
    ...
    def __findDebuggerItem(self, debuggerId):
        """
        Private method to find a debugger item given the debugger ID.

        Note: Only the host name and process ID are considered because the process name
        may change.

        @param debuggerId debugger ID to search for
        @type str
        @return reference to the top level item belonging to the given debugger ID
        @rtype QTreeWidgetItem
        """
        shortenedId = debuggerId.rsplit("/", 1)[0]
        for row in range(self.__debuggersList.topLevelItemCount()):
            itm = self.__debuggersList.topLevelItem(row)
            if itm.text(0).startswith(shortenedId):
                return itm
    ...
    def __clientDebuggerId(self, debuggerId):
        ...
        @type str
        """
        itm = self.__findDebuggerItem(debuggerId=debuggerId)
        if itm:
            itm.setText(0, debuggerId)
        else:
    ...
    def __setCurrentDebugger(self, debuggerId):
        ...
        if debuggerItems:
            ...
            currentParent = currentItem.parent() if currentItem else None
