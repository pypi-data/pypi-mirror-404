...
RE_WORDS = "(pas+wo?r?d|pass(phrase)?|pwd|token|secrete?|ken+wort|geheim|pin)"
...
def checkHardcodedPasswordAsDefault(reportError, context, _config):
    ...
    for key, val in zip(context.node.args.args, defs, strict=True):
