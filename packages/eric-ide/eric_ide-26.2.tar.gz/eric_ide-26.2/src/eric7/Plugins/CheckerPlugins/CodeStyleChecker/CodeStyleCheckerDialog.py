...
from PyQt6.QtCore import QCoreApplication, QTimer, Qt, pyqtSlot
...
class CodeStyleCheckerDialog(QDialog, Ui_CodeStyleCheckerDialog):
    ...
    def __getBanRelativeImportsValue(self):
        ...
        if self.banParentsButton.isChecked():
            return "parents"
        if self.banAllButton.isChecked():
            return "true"
        return ""
    ...
    def getDefaults(self):
        ...
        return {
            ...
            "ComprehensionsChecker": copy.deepcopy(
            ...
            "ImportsChecker": {
                ...
                "AllowLocalImportOfFirstPartyPackages": False,
                "BannedModules": [],
                "BanRelativeImports": "",
            },
            # Name Order
    ...
    def prepare(self, fileList, project):
        ...
        # Upgrading SecurityChecker from older data structures
        if "ExtendMarkupNames" not in self.__data["SecurityChecker"]:
            # 'ExtendMarkupNames' is the sentinel for the first extension
            self.__data["SecurityChecker"].update(
                {
                    "ExtendMarkupNames": defaultParameters["SecurityChecker"][
                        "ExtendMarkupNames"
                    ],
                    "AllowedEscapeFunctions": defaultParameters["SecurityChecker"][
                        "AllowedEscapeFunctions"
                    ],
                }
            )

        # Upgrading NameOrderChecker from older data structures
        if "FirstPartyPackageNames" not in self.__data["NameOrderChecker"]:
            self.__data["NameOrderChecker"]["FirstPartyPackageNames"] = (
                defaultParameters["NameOrderChecker"]["FirstPartyPackageNames"]
            )

        # Upgrading ImportsChecker from older data structures
        if "AllowLocalImportOfFirstPartyPackages" not in self.__data["ImportsChecker"]:
            self.__data["ImportsChecker"]["AllowLocalImportOfFirstPartyPackages"] = (
                defaultParameters["ImportsChecker"][
                    "AllowLocalImportOfFirstPartyPackages"
                ]
            )
        ...
        self.maxComprehensionsSpinBox.setValue(
            self.__data["ComprehensionsChecker"]["MaxComprehensions"]
        )
        ...
        self.appPackagesEdit.setPlainText(
            " ".join(sorted(self.__data["NameOrderChecker"]["ApplicationPackageNames"]))
        )
        self.firstPartyPackagesEdit.setPlainText(
            " ".join(sorted(self.__data["NameOrderChecker"]["FirstPartyPackageNames"]))
        )
        self.allowFPLocalImportCheckBox.setChecked(
            self.__data["ImportsChecker"]["AllowLocalImportOfFirstPartyPackages"]
        )
        ...
        if len(self.files) > 0:
            ...
            miscellaneousArgs = {
                ...
                "ComprehensionsChecker": {
                    "MaxComprehensions": self.maxComprehensionsSpinBox.value(),
                },
            }
            ...
            # 'ApplicationPackageNames' and 'FirstPartyPackageNames are used by
            # Imports Checker and Name Order Checker
            importsArgs = {
                "ApplicationPackageNames": sorted(
                    self.appPackagesEdit.toPlainText().split()
                ),
                "FirstPartyPackageNames": sorted(
                    self.firstPartyPackagesEdit.toPlainText().split()
                ),
                "AllowLocalImportOfFirstPartyPackages": (
                    self.allowFPLocalImportCheckBox.isChecked()
                ),
    ...
    def __modifyOptions(self, source):
        ...
        or '#~ eflag: noqa = M-601' at the end of the source in order to
        extend the list of excluded messages for one file only.
    ...
    def __processResult(self, fn, codeStyleCheckerStats, fixes, results):
        ...
        securityOk = 0
        if self.__itms:
            for itm, result in zip(self.__itms, results, strict=True):
            ...
            data = {
                ...
                "SecurityChecker": {
                    ...
                    "ExtendMarkupNames": [
                        n.strip()
                        for n in (
                            self.additionalMarkupNamesEdit.toPlainText().splitlines()
                        )
                    ],
                    "AllowedEscapeFunctions": [
                        f.strip()
                        for f in (
                            self.allowedEscapeFunctionsEdit.toPlainText().splitlines()
                        )
                    ],
                },
                # 'ApplicationPackageNames' and 'FirstPartyPackageNames' are used by
    ...
    def on_loadDefaultButton_clicked(self):
        ...
        self.maxComprehensionsSpinBox.setValue(
            int(
                settings.value(
                    "PEP8/MaxComprehensions",
                    defaultParameters["ComprehensionsChecker"]["MaxComprehensions"],
                )
            )
        )
        self.aggressiveCheckBox.setChecked(
                ...
                sorted(
                    EricUtilities.toList(
                        settings.value(
                            ...
                            defaultParameters["NameOrderChecker"][
                                "ApplicationPackageNames"
                            ],
                        )
                    )
        ...
        self.allowFPLocalImportCheckBox.setChecked(
            EricUtilities.toBool(
                settings.value(
                    "PEP8/AllowLocalImportOfFirstPartyPackages",
                    defaultParameters["ImportsChecker"][
                        "AllowLocalImportOfFirstPartyPackages"
                    ],
                )
    ...
    def on_storeDefaultButton_clicked(self):
        ...
        settings.setValue(
            "PEP8/MaxComprehensions", self.maxComprehensionsSpinBox.value()
        )
        ...
        settings.setValue(
            "PEP8/FirstPartyPackageNames",
            sorted(self.firstPartyPackagesEdit.toPlainText().split()),
        )
        settings.setValue(
            "PEP8/AllowLocalImportOfFirstPartyPackages",
            self.allowFPLocalImportCheckBox.isChecked(),
    ...
    def on_resetDefaultButton_clicked(self):
        ...
        settings.setValue(
            "PEP8/AllowLocalImportOfFirstPartyPackages",
            defaultParameters["ImportsChecker"]["AllowLocalImportOfFirstPartyPackages"],
        )
        settings.setValue(
            "PEP8/BannedModules",
            defaultParameters["ImportsChecker"]["BannedModules"],
        ...
        settings.setValue(
            "PEP8/ApplicationPackageNames",
            defaultParameters["NameOrderChecker"]["ApplicationPackageNames"],
        )
        settings.setValue(
            "PEP8/FirstPartyPackageNames",
            defaultParameters["NameOrderChecker"]["FirstPartyPackageNames"],
    ...
    def __getCommentedCodeCheckerWhiteList(self):
        ...
        """
        return [
            self.whitelistWidget.item(row).text()
