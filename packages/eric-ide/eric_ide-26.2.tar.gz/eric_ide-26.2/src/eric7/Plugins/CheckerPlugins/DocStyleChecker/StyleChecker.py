...
class StyleChecker:
    ...
    def __init__(
        ...
        @type int
        @param docType type of the documentation strings (one of 'eric', 'eric_black'
            or 'pep257')
        @type str
        """
        self.selected = tuple(select)
        self.ignored = tuple(ignore)
        ...
        for key, checkers in checkersWithCodes.items():
            for checker, codes in checkers:
                if any(
                    not (msgCode and self.__ignoreCode(msgCode)) for msgCode in codes
                ):
                    self.__checkers[key].append(checker)

    def __ignoreCode(self, msgCode):
        """
        Private method to check if the message code should be ignored.

        @param msgCode message code to check for
        @type str
        @return flag indicating to ignore the given code
        @rtype bool
        """
        return msgCode in self.ignored or (
            msgCode.startswith(self.ignored) and not msgCode.startswith(self.selected)
        )
    ...
    def __parseModuleDocstring(self, source):
        ...
        for kind, value, (line, _char), _, _ in tokenize.generate_tokens(
            ...
            if kind == tokenize.STRING:  # first STRING should be docstring
                return StyleContext(value, line - 1, "docstring")
            return None
    ...
    def __parseDocstring(self, context, what=""):
        ...
        with contextlib.suppress(StopIteration):
            ...
            if kind == tokenize.STRING:  # STRING after INDENT is a docstring
                return StyleContext(value, context.start() + line - 1, "docstring")
    ...
    def __parseTopLevel(self, keyword):
        ...
        try:
            while True:
                ...
                contexts.append(
                    StyleContext(self.source[start[0] : end[0]], start[0], keyword)
                )
    ...
    def __parseContexts(self, kind):
        ...
        if kind == "moduleDocstring":
            return [StyleContext(self.source, 0, "module")]
        ...
        if kind == "docstring":
            return [
                StyleContext(self.source, 0, "module"),
