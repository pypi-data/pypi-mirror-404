...
class Git(VersionControl):
    ...
    def vcsCommitMessages(self):
        ...
        if not messages:
            ...
            messages = VcsGitPlugin.getPreferences("Commits")
    ...
    def vcsAddCommitMessage(self, message):
        ...
        if not self._vcsAddProjectCommitMessage(message):
            ...
            VcsGitPlugin.setPreferences("Commits", commitMessages)
    ...
    def vcsClearCommitMessages(self):
        ...
        if not self._vcsClearProjectCommitMessages():
            VcsGitPlugin.setPreferences("Commits", [])
    ...
    def vcsRemove(
        ...
        @type str or list of str
        @param _project flag indicating deletion of a project tree (unused)
        @type bool
        @param noDialog flag indicating quiet operations
        @type bool
    ...
    def vcsMerge(self, name):
        ...
        dlg = GitMergeDialog(
            ...
            self.gitGetBranchesList(repodir, withMain=True),
    ...
    def vcsCleanup(self, name):
        ...
        @type str
        """
        patterns = VcsGitPlugin.getPreferences("CleanupPatterns").split()
    ...
    def vcsRepositoryInfos(self, ppath):
        ...
        @rtype str
        """
        formatTemplate = "format:%h%n%p%n%an%n%ae%n%ai%n%cn%n%ce%n%ci%n%d%n%s"
    ...
    def vcsSbsDiff(self, name, extended=False, revisions=None):
        ...
        if error:
            ...
            return
        name1 = "{0} (rev. {1})".format(name, rev1 if rev1 else "Stage")
    ...
    def gitGetBranchesList(
        ...
        if output:
            for line in output.splitlines():
                name = line[2:].strip()
                noMaster = VcsGitPlugin.getPreferences("PoliticalCorrectness")
                if (
                    (name != "master" or (withMain and not noMaster))
                    and (name != "main" or withMain)
                    and "->" not in name
    ...
    def gitBranch(self, name, revision=None, branchName=None, branchOp=None):
        ...
        dlg = GitBranchDialog(
            self.gitGetBranchesList(repodir, withMain=True, allBranches=True),
            revision=revision,
            branchName=branchName,
            branchOp=branchOp,
            correctness=VcsGitPlugin.getPreferences("PoliticalCorrectness"),
            parent=self.__ui,
        )
        if dlg.exec() == QDialog.DialogCode.Accepted:
            (
    ...
    def gitHouseKeeping(self, projectDir):
        ...
        args.append("--prune")
        if VcsGitPlugin.getPreferences("AggressiveGC"):
    ...
    def __gitSubmodulesList(self, repodir):
        ...
        try:
            with open(submodulesFile) as modulesFile:
