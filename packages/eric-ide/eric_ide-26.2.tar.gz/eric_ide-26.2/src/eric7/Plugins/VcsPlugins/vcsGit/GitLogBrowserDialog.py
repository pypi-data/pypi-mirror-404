...
from PyQt6.QtCore import QDate, QPoint, QProcess, QSize, QTimer, Qt, pyqtSlot
...
class GitLogBrowserDialog(QWidget, Ui_GitLogBrowserDialog):
    ...
    def __initLogTreeContextMenu(self):
        ...
        act.setChecked(VcsGitPlugin.getPreferences("ShowCommitIdColumn"))
        ...
        act.setCheckable(True)
        act.setChecked(VcsGitPlugin.getPreferences("ShowAuthorColumns"))
        ...
        act.setCheckable(True)
        act.setChecked(VcsGitPlugin.getPreferences("ShowCommitterColumns"))
        ...
        act.setCheckable(True)
        act.setChecked(VcsGitPlugin.getPreferences("ShowBranchesColumn"))
        ...
        act.setCheckable(True)
        act.setChecked(VcsGitPlugin.getPreferences("ShowTagsColumn"))
        act.triggered.connect(self.__showTagsColumn)

        # set column visibility as configured
        self.__showCommitIdColumn(VcsGitPlugin.getPreferences("ShowCommitIdColumn"))
        self.__showAuthorColumns(VcsGitPlugin.getPreferences("ShowAuthorColumns"))
        self.__showCommitterColumns(VcsGitPlugin.getPreferences("ShowCommitterColumns"))
        self.__showBranchesColumn(VcsGitPlugin.getPreferences("ShowBranchesColumn"))
        self.__showTagsColumn(VcsGitPlugin.getPreferences("ShowTagsColumn"))
    ...
    def closeEvent(self, e):
        ...
        VcsGitPlugin.setPreferences("LogBrowserGeometry", self.saveGeometry())
    ...
    def __reloadGeometry(self):
        ...
        """
        geom = VcsGitPlugin.getPreferences("LogBrowserGeometry")
        if geom.isEmpty():
            s = QSize(1000, 800)
            self.resize(s)
        else:
    ...
    def __restoreSplitterStates(self):
        ...
        """
        states = VcsGitPlugin.getPreferences("LogBrowserSplitterStates")
        if len(states) == 3:
    ...
    def __resetUI(self):
        ...
        self.limitSpinBox.setValue(VcsGitPlugin.getPreferences("LogLimit"))
        self.stopCheckBox.setChecked(VcsGitPlugin.getPreferences("StopLogOnCopy"))
    ...
    def __identifyProject(self):
        ...
        args.append(
            "--abbrev={0}".format(VcsGitPlugin.getPreferences("CommitIdLength"))
        )
    ...
    def __generateLogItem(
        ...
        logMessageColumnWidth = VcsGitPlugin.getPreferences("LogSubjectColumnWidth")
        msgtxt = subject
        ...
        column, _color, edges = self.__generateEdges(commitId, parents)
    ...
    def __getLogEntries(self, skip=0, noEntries=0):
        ...
        if VcsGitPlugin.getPreferences("FindCopiesHarder"):
    ...
    def __getTagsForCommit(self, commitId):
        ...
        if output:
            tagNames = [line.strip() for line in output.splitlines()]
    ...
    def on_stopCheckBox_clicked(self, _checked):
        ...
        @type bool
        """
        VcsGitPlugin.setPreferences("StopLogOnCopy", self.stopCheckBox.isChecked())
        self.nextButton.setEnabled(True)
        self.limitSpinBox.setEnabled(True)
    ...
    def __describeActTriggered(self):
        ...
        """
        commits = [
            itm.text(self.CommitIdColumn) for itm in self.logTree.selectedItems()
        ]
    ...
    def __showCommitterColumns(self, on):
        ...
        VcsGitPlugin.setPreferences("ShowCommitterColumns", on)
        self.__resizeColumnsLog()
    ...
    def __showAuthorColumns(self, on):
        ...
        VcsGitPlugin.setPreferences("ShowAuthorColumns", on)
        self.__resizeColumnsLog()
    ...
    def __showCommitIdColumn(self, on):
        ...
        VcsGitPlugin.setPreferences("ShowCommitIdColumn", on)
        self.__resizeColumnsLog()
    ...
    def __showBranchesColumn(self, on):
        ...
        VcsGitPlugin.setPreferences("ShowBranchesColumn", on)
        self.__resizeColumnsLog()
    ...
    def __showTagsColumn(self, on):
        ...
        VcsGitPlugin.setPreferences("ShowTagsColumn", on)
        self.__resizeColumnsLog()
    ...
    def on_modeComboBox_currentIndexChanged(self, index):
        ...
        if findMode:
            ...
            self.logTree.setColumnHidden(self.IconColumn, False)
        elif filterMode:
            self.__filterLogs()
    ...
    @pyqtSlot(QTreeWidgetItem, QTreeWidgetItem)
    def on_filesTree_currentItemChanged(self, current, _previous):
        """
        Private slot called, when the current item of the files tree changes.

        @param current reference to the new current item
        @type QTreeWidgetItem
        @param _previous reference to the old current item
        @type QTreeWidgetItem
        """
        if current:
    ...
    def on_saveLabel_linkActivated(self, link):
        ...
        if self.projectMode:
            ...
            fname = "{0}.diff".format(self.__filename) if fname != "." else dname
