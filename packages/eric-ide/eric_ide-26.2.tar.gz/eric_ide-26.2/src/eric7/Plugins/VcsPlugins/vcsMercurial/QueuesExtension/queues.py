...
class Queues(HgExtension):
    ...
    def __getPatchesList(self, listType, withSummary=False):
        ...
        @rtype list of str
        @exception TypeError raised to indicate an invalid patch list type
        """
        patchesList = []

        if not isinstance(listType, QueuePatchesListType):
            err = "illegal type for 'listType', should be 'QueuePatchesListType'"
            raise TypeError(err)
    ...
    def __getCurrentPatch(self):
        ...
        client = self.vcs.getClient()
        return client.runcommand(args)[0].strip()
    ...
    def __getCommitMessage(self):
        ...
        client = self.vcs.getClient()
        return client.runcommand(args)[0]
    ...
    def hgQueuePushPopPatches(self, operation, doAll=False, named=False, force=False):
        ...
        if operation not in (
            ...
            err = "illegal value for operation"
