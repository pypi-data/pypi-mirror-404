...
configTemplate1 = '''def createConfigurationPage(configDlg):
    """
...
configTemplate3 = '''
    ...
    def getPreferences(cls, key):
        """
        Class method to retrieve the various settings values.
        ...
        return Preferences.getSettings().value(
            f"{{{className}.PreferencesKey}}/{{key}}",
            {className}.DefaultPreferences[key]
        )

    @classmethod
    def setPreferences(cls, key, value):
        """
        Class method to store the various settings values.
        ...
        Preferences.getSettings().setValue(
            f"{{{className}.PreferencesKey}}/{{key}}", value
        )
'''
...
def convertPasswords(oldPassword, newPassword):
    """
    Function to convert all passwords.
    ...
    {className}.setPreferences(
        "Password",
        pwRecode(
            {className}.getPreferences("Password"), oldPassword, newPassword
        ),
    )
'''
