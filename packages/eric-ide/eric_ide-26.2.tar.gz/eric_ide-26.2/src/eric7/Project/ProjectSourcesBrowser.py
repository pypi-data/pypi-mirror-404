...
class ProjectSourcesBrowser(ProjectBaseBrowser):
    ...
    def __closeAllWindows(self):
        ...
        if self.classDiagram is not None:
            self.classDiagram.close()
            self.classDiagram = None
        if self.importsDiagram is not None:
            self.importsDiagram.close()
            self.importsDiagram = None
        if self.packageDiagram is not None:
            self.packageDiagram.close()
            self.packageDiagram = None
    ...
    def __createPythonPopupMenus(self):
        ...
        self.__startMenu = QMenu(self.tr("Start"))
        ...
        self.__gotoMenu = QMenu(self.tr("Goto"))
        self.__gotoMenu.aboutToShow.connect(self._showGotoMenu)
        self.__gotoMenu.triggered.connect(self._gotoAttribute)

        self.attributeMenu = QMenu(self)
        self.attributeMenu.addMenu(self.__gotoMenu)
    ...
    def _openItem(self):
        ...
        for itm in itmList:
            if isinstance(itm, BrowserFileItem):
                ...
                itm, (BrowserClassItem, BrowserMethodItem, BrowserClassAttributeItem)
            ):
    ...
    def __deleteFile(self):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted:
            for fn2, fn in zip(fullNames, files, strict=True):
