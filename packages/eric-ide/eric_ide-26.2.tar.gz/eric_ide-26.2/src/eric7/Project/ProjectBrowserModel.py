...
class ProjectBrowserFileItem(BrowserFileItem, ProjectBrowserItemMixin):
    ...
    def __init__(
        ...
        @type EricServerFileSystemInterface (optional)
        """
        BrowserFileItem.__init__(self, parent, finfo, sourceLanguage, fsInterface)
        ProjectBrowserItemMixin.__init__(self, projectType, bold)
...
class ProjectBrowserModel(BrowserModel):
    ...
    def populateItem(self, parentItem, repopulate=False):
        ...
        if parentItem.type() == BrowserItemType.PbDirectory:
    ...
    def findParentItemByName(
        self, type_, name, dontSplit=False, signalTypeChange=False
    ):
        """
        ...
        @type str
        @param dontSplit flag indicating the name should not be split (defaults to
            False)
        @type bool (optional)
        @param signalTypeChange flag indicating to emit a signal indicating a change of
    ...
    def itemIndexByName(self, name):
        ...
        """
        itm = self.findItem(name)
        return self.createIndex(itm.row(), 0, itm) if itm else QModelIndex()
