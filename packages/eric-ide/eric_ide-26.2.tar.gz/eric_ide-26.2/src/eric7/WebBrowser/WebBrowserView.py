...
class WebBrowserView(QWebEngineView):
    ...
    def __setRwhvqt(self):
        ...
        if self.__rwhvqt:
            ...
            print("Focus proxy is null!")  # noqa: T201, M-801
    ...
    def eventFilter(self, obj, evt):
        ...
        if obj is self:
            ...
            if evt.type() == QEvent.Type.Hide and self.isFullScreen():
                self.triggerPageAction(QWebEnginePage.WebAction.ExitFullScreen)
    ...
    def title(self):
        ...
        if not titleStr:
            url = self.__page.requestedUrl() if self.url().isEmpty() else self.url()
    ...
    def __getSavePageFileNameAndFormat(self):
        ...
        if fileName:
            ...
            if not extension:
                ...
                extensionsIndex = 1 if OSUtilities.isWindowsPlatform() else 0
    ...
    def createWindow(self, windowType):
        ...
        if windowType == QWebEnginePage.WebWindowType.WebBrowserBackgroundTab:
    ...
    @pyqtSlot(bool)
    def __printPageFinished(self, _success):
        """
        Private slot to handle the finishing of a print job.

        @param _success flag indicating success (not used)
        @type bool
        """
