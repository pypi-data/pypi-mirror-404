...
class AdBlockRule:
    ...
    def __parseFilter(self):
        ...
        parsedLine = parsedLine.removesuffix("*")
    ...
    def matchDomain(self, domain):
        ...
        if len(self.__allowedDomains) == 0:
            return all(
                not self.__isMatchingDomain(domain, dom)
                for dom in self.__blockedDomains
            )
        return all(
            not self.__isMatchingDomain(domain, dom) for dom in self.__blockedDomains
        ) and any(self.__isMatchingDomain(domain, dom) for dom in self.__allowedDomains)
