...
from .GreaseMonkeyDownloader import GreaseMonkeyDownloadType, GreaseMonkeyDownloader
        ...
        for urlStr in self.__require:
            if not self.__manager.requireScripts([urlStr]):
                ...
                downloader.finished.connect(
                    functools.partial(self.__requireDownloaded, downloader)
                )
                downloader.error.connect(
                    functools.partial(self.__requireDownloadError, downloader)
                )
                self.__downloaders.append(downloader)
