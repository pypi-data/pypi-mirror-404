...
def direntries(
    ...
    with (
        ...
        os.scandir(path) as dirEntriesIterator,
    ):
        for dirEntry in dirEntriesIterator:
            if checkStop and checkStop():
                break

            if any(fnmatch.fnmatch(dirEntry.name, ign) for ign in ignoreList):
                continue
...
def getDirTreeSize(path="."):
    """
    Function to calculate the directory tree size for a given path.

    @param path name of the directory (defaults to ".")
    @type str (optional)
    @return size in bytes
    @rtype int
    """
    total = 0
    with os.scandir(path) as pathIterator:
        for entry in pathIterator:
            if entry.is_file(follow_symlinks=False):
                total += entry.stat().st_size
            elif entry.is_dir(follow_symlinks=False):
                total += getDirTreeSize(entry.path)
    return total


def getDirSize(path=".", recurse=False):
    """
    Function to calculate the directory size for a given path.

    @param path name of the directory (defaults to ".")
    @type str (optional)
    @param recurse flag indicating a recursive calculation (i.e. calculate the
        size of the directory tree rooted at the given path) (defaults to False)
    @type bool (optional)
    @return size in bytes
    @rtype int
    """
    if recurse:
        return getDirTreeSize(path=path)
    total = 0
    with os.scandir(path) as pathIterator:
        for entry in pathIterator:
            if entry.is_file(follow_symlinks=False):
...
def getSize(path=".", recurse=False):
    """
    Function to calculate the size of a given file or directory.
    ...
    if os.path.isdir(path):
        return getDirSize(path=path, recurse=recurse)
...
def winGetVolumes():
    """
    Function to determine all volumes and associated drive letters that could possibly
    belong to a MicroPython device (Windows only).

    @return list of tuples containing the volume name and associated drive letter
    ...
    if OSUtilities.isWindowsPlatform():
        try:
            ...
            logging.getLogger(__name__).debug(
                "Could not determine the volumes; subprocess returned exit status %s.\n"
                "Output: %s",
                err.returncode,
                err.output if err.output else err.stderr,
            )
        else:
            for line in output:
                words = line.split()
                if len(words) >= 4 and words[1] == "2" and words[2] == "FAT":
                    volumes.append((" ".join(words[3:]), words[0]))
...
def findVolume(volumeName, findAll=False, markerFile=None):
    ...
    if OSUtilities.isWindowsPlatform():
        ...
        try:
            ...
            logging.getLogger(__name__).debug(
                "Could not determine the drives; subprocess returned exit status %s.\n"
                "Output: %s",
                err.returncode,
                err.output if err.output else err.stderr,
            )
