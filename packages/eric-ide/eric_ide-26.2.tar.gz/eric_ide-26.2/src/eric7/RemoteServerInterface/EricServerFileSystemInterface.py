...
class EricServerFileSystemInterface(QObject):
    ...
    def access(self, name, modes):
        ...
        """
        if not modes:
            exc = "At least one of 'read', 'write' or 'execute' must be specified."
    ...
    def populateFsCache(self, directory):
        ...
        """
        if not directory:
            exc = "The directory to be cached must not be empty."
            raise ValueError(exc)
        ...
        try:
            ...
            logging.getLogger(__name__).debug(
                "Remote Cache Size: %s entries", len(_RemoteFsCache)
            )
        except OSError as err:
            print("Error in 'populateFsCache()':", str(err))  # noqa: T201, M-801
