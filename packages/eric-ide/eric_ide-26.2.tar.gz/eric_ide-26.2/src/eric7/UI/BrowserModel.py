...
class BrowserModel(QAbstractItemModel):
    ...
    def entryDeleted(self, path, isDir=False):  # noqa: ARG002, U-100
        """
    ...
    def populateFileItem(self, parentItem, repopulate=False):
        ...
        if bool(dictionary):
            ...
            if "@@Metadata@@" in dictionary:
                node = BrowserMetadataItem(
                    parentItem,
                    QCoreApplication.translate("EditorOutlineModel", "Metadata"),
                )
                self._addItem(node, parentItem)
                for metadataEntry in (
                    dictionary["@@Metadata@@"].getMetadataEntries().values()
                ):
                    m_node = BrowserMetadataEntryItem(
                        node,
                        metadataEntry.typ,
                        metadataEntry.lineno,
    ...
    def populateClassItem(self, parentItem, repopulate=False):
        ...
        keys = [(name, "c") for name in cl.classes]
        keys.extend((name, "m") for name in cl.methods)
    ...
    def populateMethodItem(self, parentItem, repopulate=False):
        ...
        # build sorted list of names
        keys = [(name, "c") for name in fn.classes]
        keys.extend((name, "m") for name in fn.methods)
        keys.extend((name, "a") for name in fn.attributes)
...
class BrowserSimpleDirectoryItem(BrowserItem):
    ...
    def setName(self, dinfo, full=True):  # noqa: ARG002, U-100
        """
...
class BrowserDirectoryItem(BrowserItem):
    ...
    def setName(self, dinfo, full=True):
        ...
        @type bool (optional)
        """
        dn, _isRemote, host = self.__prepareInfo(dinfo, full=full)
        self.itemData[0] = dn
        self.remoteInfo = host
...
class BrowserFileItem(BrowserItem):
    ...
    def __init__(self, parent, finfo, sourceLanguage="", fsInterface=None):
        """
        Constructor
        ...
        if self.isPython3File():
            ...
            pixName = "fileLinguist" if self.fileext == ".ts" else "fileLinguist2"
    ...
    def setName(self, finfo, full=True):  # noqa: ARG002, U-100
        """
        Public method to set the directory name.
...
class BrowserClassAttributesItem(BrowserItem):
    ...
    def lessThan(self, other, column, order):
        ...
        if issubclass(other.__class__, BrowserCodingItem):
            return order == Qt.SortOrder.DescendingOrder
        if issubclass(other.__class__, (BrowserClassItem, BrowserMethodItem)):
            return order == Qt.SortOrder.AscendingOrder
...
class BrowserMetadataItem(BrowserItem):
    """
    Class implementing the data structure for browser metadata items.
    """
    ...
    def __init__(self, parent, text):
        ...
        self.type_ = BrowserItemType.Metadata
        self.icon = EricPixmapCache.getIcon("metadata")
...
class BrowserMetadataEntryItem(BrowserItem):
    """
    Class implementing the data structure for browser metadata entry items.
    """

    def __init__(self, parent, text, linenumber, content):
        """
        Constructor
        ...
        @param text text to be shown by this item
        @type str
        @param linenumber line number of the metadata entry
        @type int
        @param content content of the metadata entry
        @type str
        """
        super().__init__(parent, text)

        self.type_ = BrowserItemType.MetadataEntry
        self.icon = EricPixmapCache.getIcon("metadata")
    ...
    def lineno(self):
        """
        Public method returning the line number of the metadata entry.

        @return line number defining the metadata entry
        @rtype int
        """
        return self.__lineno
    ...
    def content(self):
        """
        Public method to get the content of the metadata entry.
    ...
    def boundaries(self):
        """
        Public method to get the boundaries of the metadata entry.

        @return tuple containing the first and last line of the metadata entry
        @rtype tuple of (int, int)
        """
        return self.__lineno, self.__lineno + self.__content.count("\n") + 1
