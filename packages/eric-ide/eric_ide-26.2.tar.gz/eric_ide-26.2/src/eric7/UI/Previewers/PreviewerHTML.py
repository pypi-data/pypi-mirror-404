...
from PyQt6.QtCore import QEventLoop, QPoint, QThread, QUrl, Qt, pyqtSignal, pyqtSlot
...
class PreviewerHTML(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.titleLabel.setText(
                self.tr(
                    "<b>HTML Preview is not available!<br/>Install PyQt6-WebEngine.</b>"
                )
            )
    ...
    def processEditor(self, editor=None):
        ...
        if editor is not None:
            ...
            rootPath = os.path.dirname(os.path.abspath(fn)) if fn else ""
...
class PreviewProcessingThread(QThread):
    ...
    def __getHtml(
        ...
        """
        if language == "HTML":
            html = self.__processSSI(text, filePath, rootPath) if ssiEnabled else text
    ...
    def __convertReSTSphinx(self, text):
        ...
        try:
            ...
            with open(basePath + ".html", encoding="utf-8") as fh:
                html = fh.read()
        finally:
