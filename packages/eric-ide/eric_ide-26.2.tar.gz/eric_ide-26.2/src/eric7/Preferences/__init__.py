...
class Prefs:
    ...
    debuggerDefaults = {
        ...
        # Always use the (older) tracing debugger instead of the modern
        # event based one.
        "PythonTracingDebugger": False,
    }
    ...
    uiDefaults = {
        ...
        "NavigationHistoryMaximumSize": 100,
        "NavigationHistoryLineOffset": 5,
        "NavigationMaximumHistoryMenuItems": 20,
    ...
    editorDefaults = {
        ...
        "AutoCompletionDisabledInStrings": True,
        "AutoCompletionDisabledInComments": True,
    ...
    def initWebEngineSettingsDefaults(cls):
        ...
        if QtUtilities.qWebEngineVersionTuple() >= (6, 4, 0):
            # Qt 6.4+
        ...
        if QtUtilities.qWebEngineVersionTuple() >= (6, 6, 0):
            # Qt 6.6+
        ...
        if QtUtilities.qWebEngineVersionTuple() >= (6, 7, 0):
            # Qt 6.7+
            cls.webBrowserDefaults["ForceDarkMode"] = webEngineSettings.testAttribute(
                QWebEngineSettings.WebAttribute.ForceDarkMode
            )

        if QtUtilities.qWebEngineVersionTuple() >= (6, 9, 0):
            # Qt 6.9+
            cls.webBrowserDefaults["PrintHeaderAndFooter"] = (
                webEngineSettings.testAttribute(
                    QWebEngineSettings.WebAttribute.PrintHeaderAndFooter
                )
            )

        if QtUtilities.qWebEngineVersionTuple() >= (6, 10, 0):
            # Qt 6.10+
            cls.webBrowserDefaults["BackForwardCacheEnabled"] = (
                webEngineSettings.testAttribute(
    ...
    microPythonDefaults = {
        ...
        "GraphColorScheme": 0,  # automatic
        "GraphColorTheme": 0,  # Qt Green
...
def exportPreferences():
    ...
    """
    filename = EricFileDialog.getSaveFileNameWithExtension(
        None,
...
def getPython(key):
    ...
    if key in ("DisViewerExpandCodeInfoDetails"):
        return EricUtilities.toBool(
...
def getUI(key):
    ...
    if key in [
        ...
        "NavigationHistoryMaximumSize",
        "NavigationHistoryLineOffset",
        "NavigationMaximumHistoryMenuItems",
    ...
    if key in ("LogStdErrColour", "IconBarColor"):
    ...
    if key in "ViewProfiles":
        ...
        return viewProfiles
    if key in ["ToolbarManagerState", "PreviewSplitterState"]:
...
def getPrinter(key):
    ...
    if key in ["Magnification", "Orientation", "PageSize", "Resolution"]:
...
def getMultiProject(key):
    ...
    if key in ["OpenMainAutomatically", "TimestampFile"]:
        return EricUtilities.toBool(
            Prefs.settings.value("MultiProject/" + key, Prefs.multiProjectDefaults[key])
        )
    return Prefs.settings.value("MultiProject/" + key, Prefs.multiProjectDefaults[key])
...
def getHelp(key):
    ...
    if key in ("QtHelpSearchNewOnStart", "ForceQTextBrowser"):
        return EricUtilities.toBool(
...
def getWebBrowser(key):
    ...
    if key in ["WebSearchKeywords"]:
        ...
        return keywords
    if key == "DownloadManagerDownloads":
    ...
    if key == "RssFeeds":
        ...
        Prefs.settings.endArray()
        return feeds
    if key in ["SyncFtpPassword", "SyncEncryptionKey"]:
        return pwConvert(
            Prefs.settings.value("WebBrowser/" + key, Prefs.webBrowserDefaults[key]),
            encode=False,
        )
    ...
    if key in ["AutoScrollDivider"]:
        return float(
            Prefs.settings.value("WebBrowser/" + key, Prefs.webBrowserDefaults[key])
        )
    if key in ["SafeBrowsingUpdateDateTime"]:
...
def getPluginManager(key):
    ...
    if key in ["UpdatesCheckInterval", "KeepGenerations"]:
        return int(
            Prefs.settings.value(
                "PluginManager/" + key, Prefs.pluginManagerDefaults[key]
            )
        )
    if key in ["HiddenPlugins"]:
        return EricUtilities.toList(
            Prefs.settings.value(
                "PluginManager/" + key, Prefs.pluginManagerDefaults[key]
            )
    ...
    return EricUtilities.toBool(
        Prefs.settings.value("PluginManager/" + key, Prefs.pluginManagerDefaults[key])
    )
...
def getPip(key):
    ...
    if key in ("VulnerabilityDbCacheValidity",):
...
def getMicroPython(key):
    ...
    if key in (
        ...
        return None
    if key in ("WifiPassword", "WifiApPassword"):
        return pwConvert(
            Prefs.settings.value("MicroPython/" + key, Prefs.microPythonDefaults[key]),
            encode=False,
        )
    return Prefs.settings.value("MicroPython/" + key, Prefs.microPythonDefaults[key])
...
def setGeometry(key, value):
    ...
    if key in ["MainMaximized"]:
        ...
        v = Prefs.geometryDefaults[key] if Prefs.resetLayout else value
...
def convertPasswords(oldPassword, newPassword):
    ...
    for key in ("WifiPassword", "WifiApPassword"):
        Prefs.settings.setValue(
            "MicroPython/" + key,
            pwRecode(
                Prefs.settings.value(
    ...
    try:
        ...
        pluginManager = PluginManager(ericApp().getMainWindow())
    pluginManager.convertPasswords(oldPassword, newPassword)
