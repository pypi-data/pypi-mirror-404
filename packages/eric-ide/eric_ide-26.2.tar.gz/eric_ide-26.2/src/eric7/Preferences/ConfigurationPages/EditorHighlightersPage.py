...
class EditorHighlightersPage(ConfigurationPageBase, Ui_EditorHighlightersPage):
    ...
    def on_addLexerButton_clicked(self):
        ...
        if lexer in self.extras:
            ...
            lexer = (
                "Pygments|{0}".format(pygmentsLexer) if pygmentsLexer else pygmentsLexer
            )
        if ext and lexer:
    ...
    def on_editorLexerList_itemClicked(self, itm, _column):
        """
        ...
        @param _column column the item was clicked or activated (ignored)
        @type int
        """
        if itm is None:
            self.editorFileExtEdit.clear()
...
def create(dlg):
    ...
    """
    return EditorHighlightersPage(dlg.getLexers())
