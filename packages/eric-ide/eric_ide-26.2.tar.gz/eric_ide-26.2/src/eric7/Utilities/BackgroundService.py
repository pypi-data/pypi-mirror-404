...
class BackgroundService(QTcpServer):
    ...
    def __init__(self, releaseMode=False, parent=None):
        ...
        if not releaseMode:
            ...
            print(f"Background Service listening on: {hostAddressStr}:{port:d}")  # noqa: T201, M-801
    ...
    def requestCancel(self, fx, lang):
        ...
        """
        entriesToRemove = [
            pendingArg for pendingArg in self.__queue if pendingArg[:2] == [fx, lang]
        ]
    ...
    def on_disconnectSocket(self, lang):
        ...
        if conn:
            conn.close()
            fx, lng, fn, _data = self.runningJob
