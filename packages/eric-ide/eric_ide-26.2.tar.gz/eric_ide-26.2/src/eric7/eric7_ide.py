...
try:
    ...
    messagebox.showerror(
        "eric7 Error",
        "PyQt could not be imported. Please make sure it is installed and accessible.",
    )
    sys.exit(100)
...
def handleSingleApplication():
    ...
    if res > 0:
        ...
        print("eric7: {0}".format(client.errstr()))  # noqa: T201, M-801
        sys.exit(res)
...
def main():
    ...
    QGuiApplication.setDesktopFileName("eric7_ide")
    ...
    splash = (
        NoneSplashScreen()
        if args.no_splash or not Preferences.getUI("ShowSplash")
        else SplashScreen()
    )
    ...
    splash.showMessage(
        QCoreApplication.translate("eric7_ide", "Preparing Main Window...")
    )
    logging.getLogger(__name__).debug("Preparing Main Window...")
