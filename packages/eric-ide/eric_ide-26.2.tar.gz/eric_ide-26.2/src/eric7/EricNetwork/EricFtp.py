    ...
    def connect(self, host="", port=0, timeout=-999):
        ...
        if self.__proxyType != EricFtpProxyType.NO_PROXY:
            if not self.__proxyHost:
                err = "990 Proxy usage requested, but no proxy host given."
                raise EricFtpProxyError(err)
        ...
        if self.__proxyType != EricFtpProxyType.NO_PROXY:
            if self.__proxyType != EricFtpProxyType.NON_AUTHORIZING:
                ...
                if not self.__proxyUser:
                    err = "991 Proxy usage requested, but no proxy user given"
                    raise EricFtpProxyError(err)
                if not self.__proxyPassword:
                    err = "992 Proxy usage requested, but no proxy password given"
                    raise EricFtpProxyError(err)
            ...
            if self.__proxyType in [
                ...
                if presp[0] != "2":
                    err = "9{0}0 Error authorizing at proxy\n{1}".format(
                        presp[0], presp
                    )
                    raise EricFtpProxyError(err)
                ...
                if self.__proxyType == EricFtpProxyType.SITE:
                    ...
                    if presp[0] != "2":
                        err = "9{0}0 Error sending SITE command\n{1}".format(
                            presp[0], presp
                        )
                        raise EricFtpProxyError(err)
                    ...
                    if presp[0] != "2":
                        err = "9{0}0 Error sending OPEN command\n{1}".format(
                            presp[0], presp
                        )
                        raise EricFtpProxyError(err)
