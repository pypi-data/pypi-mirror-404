...
class HexEditWidget(QAbstractScrollArea):
    ...
    def setAddressWidth(self, width):
        ...
        @type int
        """
        self.__addressWidth = math.ceil(width / 4) * 4
        self.__adjust()
        self.setCursorPosition(self.__cursorPosition)
        self.viewport().update()
    ...
    def goto(self, offset, fromCursor=False, backwards=False, extendSelection=False):
        ...
        if fromCursor:
            ...
            newPos = self.__chunks.size() - offset if backwards else offset
    ...
    def setData(self, dataOrDevice):
        ...
        if not isinstance(dataOrDevice, (bytes, bytearray, QByteArray, QIODevice)):
            err = "setData: parameter must be bytes, bytearray, QByteArray or QIODevice"
            raise TypeError(err)
            ...
            while row <= self.__rowsShown:
                ...
                while (
                    ...
                    if self.__asciiArea:
                        ...
                        ch = "." if by < 0x20 or (by > 0x7E and by < 0xA0) else chr(by)
                        rect.setRect(
                            pxPosAsciiX2,
                            pxPosY - self.__pxCharHeight + self.__pxSelectionSub,
                            self.__pxCharWidth,
    ...
    def resizeEvent(self, _evt):
        ...
        @param _evt reference to the resize event
        @type QResizeEvent
        """
        self.__adjust()
