...
class TemplateGroup(QTreeWidgetItem):
    ...
    def getEntryNames(self, beginning):
        ...
        @rtype list of str
        """
        return [name for name in self.entries if name.startswith(beginning)]
...
class TemplateViewer(QTreeWidget):
    ...
    def __getPredefinedVars(self):
        ...
        now = datetime.datetime.now()  # noqa: DTZ005, M-305
        sepchar = Preferences.getTemplates("SeparatorChar")
    ...
    def hasTemplate(self, entryName, groupName=None):
        ...
        """
        groups = (
            ([self.groups[groupName]] if self.hasGroup(groupName) else [])
            if groupName
            else list(self.groups.values())
        )
