...
from terminaluse.lib.sdk.config.agent_config import AgentConfig
from terminaluse.lib.sdk.config.build_config import BuildConfig
from terminaluse.lib.sdk.config.deployment_config import DeploymentConfig

logger = make_logger(__name__)


class SandboxMount(BaseModel):
    """Configuration for a single sandbox mount."""

    source: str = Field(description="Path relative to agent project root")
    target: str = Field(description="Absolute path in sandbox")
...
class SandboxConfig(BaseModel):
    ...
    code_dir: str = Field(
        default="src",
        description="Where agent code lives relative to WORKDIR (defaults to src)",
    )
    mounts: list[SandboxMount] | None = Field(
        default=None,
        description="Optional baked mounts from agent image. None = no baked mounts.",
    )
...
class AgentManifest(BaseModel):
    ...
    sandbox: SandboxConfig | None = Field(default=None, description="Optional sandbox configuration for agent execution")
