...
DEPRECATED: This module is deprecated in favor of using adk.messages.send()
...
adk.messages.send() now accepts Claude SDK message types directly and handles:
...
from terminaluse.types import TaskMessageDelta_Text
from terminaluse.types import TaskMessageUpdate_Delta

logger = make_logger(__name__)


class ClaudeMessageHandler:
    """Handles Claude SDK messages and streams them to TerminalUse UI.
    ...
    async def initialize(self):
        ...
        if self.task_id:
            ...
            self.streaming_ctx = await adk.streaming.streaming_task_message_context(
                task_id=self.task_id, initial_content=TaskMessageContent_Text(author="agent", content="", format="markdown")
            ).__aenter__()
    ...
    async def _handle_text_block(self, block: TextBlock):
        ...
        delta = TaskMessageDelta_Text(text_delta=block.text)

        try:
            await self.streaming_ctx.stream_update(
                TaskMessageUpdate_Delta(parent_task_message=self.streaming_ctx.task_message, delta=delta)
            )
