...
from terminaluse.lib.utils.model_utils import recursive_model_dump
...
class Trace:
    """
    Trace is a wrapper around the TerminalUse API for tracing.
    ...
    def __init__(
        ...
        Args:
            ...
            task_id: Optional task ID to associate with spans.
    ...
    def start_span(
        ...
        if not self.task_id:
            raise ValueError("Task ID is required to start a span")
    ...
    def span(
        ...
        If trace_id or task_id is falsy, acts as a no-op context manager.
        """
...
class AsyncTrace:
    """
    AsyncTrace is a wrapper around the TerminalUse API for tracing.
    It provides a context manager for spans and a way to start and end spans.
    It also provides a way to get spans by ID and list all spans in a trace.
    """
