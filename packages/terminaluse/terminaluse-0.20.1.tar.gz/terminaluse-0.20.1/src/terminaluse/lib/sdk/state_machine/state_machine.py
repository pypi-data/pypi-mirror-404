...
from terminaluse.lib.sdk.state_machine.state import State
from terminaluse.lib.sdk.state_machine.state_workflow import StateWorkflow
...
class StateMachine(ABC, Generic[T]):
    ...
    async def step(self) -> str:
        ...
        if self._trace_transitions and span is not None:
            ...
            assert self._task_id is not None, "Task ID required for tracing"
    ...
    async def load(cls, data: dict[str, Any], states: list[State]) -> "StateMachine[T]":
        ...
        try:
            ...
            trace_transitions = bool(data.get("trace_transitions", False))
