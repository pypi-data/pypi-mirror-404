...
# Using Any for optional import pattern - mypy doesn't track optional deps well
TemporalClient: Any
try:
...
def cleanup_agent_workflows(agent_name: str, force: bool = False, development_only: bool = True) -> None:
    ...
    try:
        ...
        for task in running_tasks:
            ...
            if force:
                # Force mode: Do both graceful SDK cancellation AND direct Temporal termination
                sdk_success = False
                temporal_success = False

                try:
                    # First: Graceful cancellation via SDK (handles database/agent cleanup)
                    client.tasks.cancel(task_id=task.id)
                    logger.debug(f"Completed SDK cancellation for task {task.id}")
                    sdk_success = True
                except Exception as e:
                    logger.warning(f"SDK cancellation failed for task {task.id}: {e}")
                ...
                task_cleanup_success = sdk_success or temporal_success

            else:
                # Normal mode: Only graceful cancellation via SDK
                try:
                    client.tasks.cancel(task_id=task.id)
