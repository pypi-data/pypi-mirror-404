...
from terminaluse.lib.cli.debug import DebugConfig, start_acp_server_debug, start_temporal_worker_debug
from terminaluse.lib.utils.logging import make_logger
from terminaluse.lib.cli.utils.path_utils import (
    get_file_paths,
    calculate_uvicorn_target_for_local,
)
from terminaluse.lib.environment_variables import EnvVarKeys
from terminaluse.lib.sdk.config.agent_manifest import AgentManifest
from terminaluse.lib.cli.handlers.cleanup_handlers import cleanup_agent_workflows, should_cleanup_on_restart
...
async def start_temporal_worker_with_reload(
    worker_path: Path, env: dict[str, str], process_manager: ProcessManager, config_dir: Path
) -> asyncio.Task[None]:
    ...
    try:
        ...
        worker_process = await start_temporal_worker(worker_path, env, config_dir)
    ...
    async def worker_runner() -> None:
        ...
        async def start_worker() -> asyncio.subprocess.Process:
            ...
            if current_process is not None:
                ...
                agent_name = env.get("TERMINALUSE_AGENT_NAME")
                console.print(f"FOUND TERMINALUSE_AGENT_NAME FROM ENV VARS: {agent_name} {agent_name is None}")
            ...
            current_process = await start_temporal_worker(worker_path, env, config_dir)
        ...
        try:
            ...
            async for changes in awatch(config_dir, recursive=True):
...
async def start_acp_server(
    acp_path: Path, port: int, env: dict[str, str], config_dir: Path
) -> asyncio.subprocess.Process:
    """Start the ACP server process"""
    # Use file path relative to manifest directory if possible
    uvicorn_target = calculate_uvicorn_target_for_local(acp_path, config_dir)
    ...
    return await asyncio.create_subprocess_exec(
        *cmd,
        cwd=config_dir,  # Always use manifest directory as CWD for consistency
        env=env,
...
async def start_temporal_worker(
    worker_path: Path, env: dict[str, str], config_dir: Path
) -> asyncio.subprocess.Process:
    """Start the temporal worker process"""
    run_worker_target = calculate_uvicorn_target_for_local(worker_path, config_dir)
    ...
    return await asyncio.create_subprocess_exec(
        *cmd,
        cwd=config_dir,  # Use worker directory as CWD for imports to work
        env=env,
        stdout=asyncio.subprocess.PIPE,
        stderr=asyncio.subprocess.STDOUT,
    )
...
async def run_agent(config_path: str, debug_config: "DebugConfig | None" = None):
    ...
    if not config_file.exists():
        raise RunError(f"Config file not found: {config_path}")

    # Parse config
    try:
        config_obj = AgentManifest.from_yaml(file_path=config_path)
    except Exception as e:
        raise RunError(f"Failed to parse config: {str(e)}") from e

    # Get and validate file paths
    try:
        file_paths = get_file_paths(config_obj, config_path)
    except Exception as e:
        raise RunError(str(e)) from e

    # Check if temporal agent and validate worker file
    if is_temporal_agent(config_obj):
    ...
    agent_env = create_agent_environment(config_obj)
    ...
    try:
        console.print(
            Panel.fit(
                f"🚀 [bold blue]Running Agent: {config_obj.agent.name}[/bold blue]",
                border_style="blue",
            )
        )
        ...
        acp_path = file_paths["agent"]
        if acp_path is None:
            raise RunError("Agent file path not found in configuration")
        ...
        if is_temporal_agent(config_obj) and file_paths["worker"]:
            if debug_config and debug_config.should_debug_worker():
                ...
                worker_task = await start_temporal_worker_with_reload(
                    file_paths["worker"], agent_env, process_manager, config_dir
                )
...
def create_agent_environment(manifest: AgentManifest) -> dict[str, str]:
    ...
    env_vars = {
        ...
        "TERMINALUSE_TEMPORAL_ADDRESS": "localhost:7233",
        "TERMINALUSE_AGENT_NAME": manifest.agent.name,
        "TERMINALUSE_ACP_TYPE": "async",  # Always async
        "TERMINALUSE_ACP_URL": "http://localhost",
        "TERMINALUSE_ACP_PORT": str(default_port),
    }

    # Add authorization principal if set - for local development, auth is optional
    from terminaluse.lib.cli.utils.auth_utils import _encode_principal_context

    encoded_principal = _encode_principal_context(manifest)
    if encoded_principal:
        env_vars[EnvVarKeys.TERMINALUSE_AUTH_PRINCIPAL_B64] = encoded_principal
    else:
        logger.info("No auth principal configured - agent will run without authentication context")

    # Add description if available
    if manifest.agent.description:
        env_vars["TERMINALUSE_AGENT_DESCRIPTION"] = manifest.agent.description
    ...
    if manifest.agent.is_temporal_agent():
        ...
        if temporal_config:
            env_vars["TERMINALUSE_WORKFLOW_NAME"] = temporal_config.name
            env_vars["TERMINALUSE_WORKFLOW_TASK_QUEUE"] = temporal_config.queue_name
        ...
        if manifest.agent.temporal and manifest.agent.temporal.health_check_port is not None:
            env_vars["TERMINALUSE_HEALTH_CHECK_PORT"] = str(manifest.agent.temporal.health_check_port)
