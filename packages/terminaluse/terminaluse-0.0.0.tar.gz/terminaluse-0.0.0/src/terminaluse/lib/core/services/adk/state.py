...
class StateService:
    ...
    async def create_state(
        ...
        async with trace.span(
            ...
            state_model = await self._terminaluse_client.states.create(
                task_id=task_id,
                agent_id=agent_id,
                state=state,
            )
    ...
    async def get(
        ...
        if trace is None:
            # Handle case without tracing - implement the core logic
            if state_id:
                return await self._terminaluse_client.states.retrieve(state_id=state_id)
            elif task_id and agent_id:
                states = await self._terminaluse_client.states.list(
                    task_id=task_id,
                    agent_id=agent_id,
                )
        ...
        async with trace.span(
            ...
            if state_id:
                state = await self._terminaluse_client.states.retrieve(state_id=state_id)
            elif task_id and agent_id:
                states = await self._terminaluse_client.states.list(
                    task_id=task_id,
                    agent_id=agent_id,
    ...
    async def update(
        ...
        async with trace.span(
            ...
            state_model = await self._terminaluse_client.states.update(
                state_id=state_id,
                task_id=task_id,
                agent_id=agent_id,
                state=state,
    ...
    async def delete_state(
        ...
        async with trace.span(
            ...
            state = await self._terminaluse_client.states.delete(state_id)
            if span:
