...
class ContextWorkflowInboundInterceptor(WorkflowInboundInterceptor):
    ...
    def init(self, outbound: WorkflowOutboundInterceptor) -> None:
        ...
        self.next.init(ContextWorkflowOutboundInterceptor(outbound, self._payload_converter))
...
class ContextWorkflowOutboundInterceptor(WorkflowOutboundInterceptor):
    ...
    def start_activity(self, input: StartActivityInput) -> workflow.ActivityHandle:
        ...
        if (
            "invoke_model_activity" in activity_name
            or "invoke-model-activity" in activity_name
            or "run_claude_agent_activity" in activity_name
        ):
            ...
            try:
                ...
                if task_id and trace_id and parent_span_id:
                    ...
                    logger.debug(
                        f"[OutboundInterceptor] Added task_id, trace_id, and parent_span_id to activity headers: {task_id}, {trace_id}, {parent_span_id}"
                    )
                else:
                    logger.warning(
                        "[OutboundInterceptor] No _task_id, _trace_id, or _parent_span_id found in workflow instance"
                    )
            except Exception as e:
                logger.error(
                    f"[OutboundInterceptor] Failed to get task_id, trace_id, or parent_span_id from workflow instance: {e}"
                )
...
class ContextActivityInboundInterceptor(ActivityInboundInterceptor):
    ...
    async def execute_activity(self, input: ExecuteActivityInput) -> Any:
        ...
        if input.headers and TASK_ID_HEADER in input.headers:
            task_id_value = self._payload_converter.from_payload(input.headers[TASK_ID_HEADER], str)
            trace_id_value = self._payload_converter.from_payload(input.headers[TRACE_ID_HEADER], str)
            parent_span_id_value = self._payload_converter.from_payload(input.headers[PARENT_SPAN_ID_HEADER], str)
            ...
            logger.info(
                f"[ActivityInterceptor] Set task_id, trace_id, and parent_span_id in context: {task_id_value}, {trace_id_value}, {parent_span_id_value}"
            )
        else:
