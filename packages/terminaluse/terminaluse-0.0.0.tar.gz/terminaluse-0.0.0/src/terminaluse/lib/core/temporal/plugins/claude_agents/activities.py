...
from terminaluse.lib.core.temporal.plugins.claude_agents.hooks import create_streaming_hooks
from terminaluse.lib.core.temporal.plugins.claude_agents.message_handler import ClaudeMessageHandler
...
@activity.defn
async def create_filesystem_directory(task_id: str, filesystem_root: str | None = None) -> str:
    """Create filesystem directory for task - runs as Temporal activity

    Args:
        task_id: Task ID for filesystem directory name
        filesystem_root: Root directory for filesystems (defaults to .claude-filesystem/ in cwd)

    Returns:
        Absolute path to created filesystem directory
    """
    if filesystem_root is None:
        # Default to .claude-filesystem in current directory
        # Follows Claude SDK's .claude/ convention
        filesystem_root = os.path.join(os.getcwd(), ".claude-filesystem")

    filesystem_path = os.path.join(filesystem_root, task_id)
    os.makedirs(filesystem_path, exist_ok=True)
    logger.info(f"Created filesystem directory: {filesystem_path}")
...
async def run_claude_agent_activity(
    ...
    options = ClaudeAgentOptions(
        ...
        agents=agent_defs,
        hooks=hooks,  # type: ignore[arg-type]  # Tool lifecycle hooks for streaming!
    )
