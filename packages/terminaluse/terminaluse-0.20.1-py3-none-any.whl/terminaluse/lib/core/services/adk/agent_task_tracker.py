...
from terminaluse.types.agent_task_tracker import AgentTaskTracker
...
class AgentTaskTrackerService:
    ...
    async def get_agent_task_tracker(
        ...
        async with trace.span(
            ...
            tracker = await self._terminaluse_client.tracker.retrieve(tracker_id)
            if span:
    ...
    async def get_by_task_and_agent(
        ...
        async with trace.span(
            ...
            trackers = await self._terminaluse_client.tracker.list(
                task_id=task_id,
                agent_id=agent_id,
            )
    ...
    async def update_agent_task_tracker(
        ...
        async with trace.span(
            ...
            tracker = await self._terminaluse_client.tracker.update(
                tracker_id=tracker_id,
                last_processed_event_id=last_processed_event_id,
                status=status,
                status_reason=status_reason,
