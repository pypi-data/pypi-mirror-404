...
for package_dir in "${packages_to_check[@]}"; do
    for forbidden_pattern in "${forbidden_patterns[@]}"; do
        # Check for imports, excluding playwright_async_api.py and sessions.py
        violations=$(grep -r "from $forbidden_pattern\|import $forbidden_pattern" "packages/$package_dir/" --include="*.py" --include="*.pyi" --exclude="playwright_async_api.py" --exclude="sessions.py")
...
# Check that playwright/patchright imports only exist in playwright_async_api.py or sessions.py files
...
for package_dir in "${all_packages[@]}"; do
    for forbidden_pattern in "${forbidden_patterns[@]}"; do
        ...
        if [ ! -z "$files_with_imports" ]; then
            # Check if all files are playwright_async_api.py or sessions.py
            for file in $files_with_imports; do
                if [[ ! "$file" =~ playwright_async_api\.py$ ]] && [[ ! "$file" =~ sessions\.py$ ]]; then
                    echo "ERROR: $forbidden_pattern import found in non-allowed file: $file"
                    found_violations=true
                fi
            done
        fi
...
if [ "$found_violations" = true ]; then
    echo ""
    echo "Playwright/patchright imports should only be used in playwright_async_api.py or sessions.py files."
