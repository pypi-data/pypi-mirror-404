...
def handle_commands(commands, auto_confirm=False, eval_mode=False, rich_to_stderr=False):
    ...
    # List of safe informational commands that can be auto-executed in interactive mode
    SAFE_INFORMATIONAL_COMMANDS = [
        'ls', 'wc', 'find', 'du', 'df', 'grep', 'cat', 'head', 'tail', 
        'whoami', 'pwd', 'date', 'hostname', 'uptime', 'free', 'top', 
        'ps', 'env', 'echo', 'which', 'whereis', 'locate'
    ]

    n_commands = len(commands)

    # Auto-execution logic for single informational commands
    if n_commands == 1 and not auto_confirm:
        cmd = commands[0]
        base_cmd = cmd.split()[0].lower()
        # If it's a safe informational command and NOT in eval_mode (which handles its own execution)
        if base_cmd in SAFE_INFORMATIONAL_COMMANDS and not eval_mode:
            console.print(Text("\n[Retrieving information...]", style="bold blue"))
            run_command(cmd)
            return
...
def run_command(command):
    ...
    try:
        ...
        if has_shell_operators:
            ...
            # Use shlex.split for regular commands
            process = subprocess.run(
                shlex.split(command),
                capture_output=True,
                text=True,
        ...
        # Show command output with a clear label in a panel
        if process.stdout:
            console.print(Panel(
