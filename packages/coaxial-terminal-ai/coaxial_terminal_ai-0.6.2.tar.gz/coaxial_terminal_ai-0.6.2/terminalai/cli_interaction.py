...
def interactive_mode(chat_mode=False):
    ...
    display_info = f"Provider: {current_provider}"
    if current_provider == 'ollama':
        model = config.get("providers", {}).get("ollama", {}).get("model", "default")
        display_info += f" ({model})"
    elif current_provider != "Not configured":
        # For other providers, maybe show model if available/relevant, or just provider
        # (Current config structure might not store model for others easily accessible here without more lookups, 
        # but let's stick to provider name for now unless it's Ollama where model is crucial)
        pass

    if chat_mode:
        console.print(Panel.fit(
            Text(f"TerminalAI AI Chat Mode: You are now chatting with the AI. [{display_info}]\nType 'exit' to quit.", style="bold magenta"),
            border_style="magenta"
        ))
        console.print("[dim]Type 'exit', 'quit', or 'q' to return to your shell.[/dim]")
    else:
        console.print(Panel.fit(
            Text(f"TerminalAI: What is your question? [{display_info}] (Type 'exit' to quit)", style="bold cyan"),
            border_style="cyan"
        ))
