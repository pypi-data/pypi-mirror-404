...
def main():
    ...
    # Force OS awareness at the very top of the context
    final_system_context = f"CRITICAL: THE USER IS ON {current_os}. ONLY SUGGEST {current_os} COMMANDS. DO NOT MENTION OTHER SYSTEMS.\n\n" + system_context
...
def handle_eval_mode_commands(commands, auto_confirm=False):
    ...
    if len(commands) == 1:
        ...
        # Auto-execution for safe informational commands in eval mode
        if base_cmd in SAFE_INFORMATIONAL_COMMANDS and not is_risky:
            # Print the command to stdout for shell execution immediately
            print(command)
            return
