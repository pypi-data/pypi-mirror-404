...
from kcwebsocket import __version__
confkcws={}
confkcws['name']='kcwebsocket'
confkcws['version']=__version__
confkcws['description']='kcwebsocket'
...
def get_file(folder='./',lists=[]):
    ...
    return lists
def start():
    b=get_file("kcwebsocket",['kcwebsocket'])
    setup(
        name = confkcws["name"],
        version = confkcws["version"],
        keywords = "kcwebsocket"+confkcws['version'],
