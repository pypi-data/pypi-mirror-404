...
def metropolis_hastings(
    ...
    # shape: (n_subtypes, n_stages)
    # Imagine this: the array of biomarker_int stays intact. we are randomizing the indices of each of them in the new order
    current_order = np.vstack([rng.permutation(np.arange(1, n_stages+1)) for _ in range(n_subtypes)])

    # shape: (n_subtypes, n_disease_stages)
    stage_prior = np.ones((n_subtypes, n_disease_stages))

    # shape: (n_subtypes, 1)
    subtype_prior = np.ones(n_subtypes)
