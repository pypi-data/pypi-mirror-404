...
def check_auto_approve(tool_name, tool_input, session_id=''):
    ...
    # DANGEROUS commands ALWAYS require manual approval - never auto-approve
    if tool_name == 'Bash':
        command = tool_input.get('command', '').strip()
        # Git commands - never auto-approve
        if command.startswith('git ') or command.startswith('git\t') or command == 'git':
            return False
        # rm -rf commands - NEVER auto-approve (catastrophic potential)
        if 'rm ' in command and ('-rf' in command or '-fr' in command or ' -r' in command):
            return False
    ...
    for pattern in patterns:
        ...
        if tool_name == 'Bash':
            ...
            # Check command_prefix - supports both first-word match and prefix match
            if 'command_prefix' in pattern:
                cmd_prefix = pattern['command_prefix']
                cmd_first_word = command.strip().split()[0] if command.strip() else ''
                # Try exact first-word match (new style - pattern is just first word like "ls")
                if cmd_first_word == cmd_prefix:
                    return True
                # Also try prefix match (old style - pattern is longer like "ls -la")
                if len(cmd_prefix) > len(cmd_first_word) and command.startswith(cmd_prefix):
                    return True
            # Check command_regex
