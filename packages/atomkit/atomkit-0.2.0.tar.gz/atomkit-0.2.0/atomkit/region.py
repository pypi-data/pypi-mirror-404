...
class Region:
    ...
    def snap_to_grid(
        ...
        """
        Snap spatial bounds to grid cell boundaries.

        Parameters
        ----------
        cell_size : tuple[float, float, float]
            Cell size per axis (cs_x, cs_y, cs_z).
        origin : tuple[float, float, float]
            Grid origin (xlo, ylo, zlo).
        mode : {"outward", "inward"}, default "outward"
            - "outward": expand to include any overlapping cell
            - "inward": shrink to only fully-contained cells

        Returns
        -------
        Region
            New region with bounds aligned to cell boundaries.
            Time bounds are preserved unchanged.
        """
        cs_x, cs_y, cs_z = cell_size
        xlo, ylo, zlo = origin

        def snap_axis(
            bounds: tuple[float, float], cs: float, lo: float
        ) -> tuple[float, float]:
            # Unbounded stays unbounded
            if bounds[0] == NEG_INF and bounds[1] == INF:
                return bounds
            if mode == "outward":
                return (
                    lo + math.floor((bounds[0] - lo) / cs) * cs,
                    lo + math.ceil((bounds[1] - lo) / cs) * cs,
                )
            else:  # inward
                new_min = lo + math.ceil((bounds[0] - lo) / cs) * cs
                new_max = lo + math.floor((bounds[1] - lo) / cs) * cs
                # If inward snap produces inverted bounds, return empty range
                # (indicates no fully interior cells on this axis)
                if new_min > new_max:
        ...
        return Region(
            x=snap_axis(self.x, cs_x, xlo),
            y=snap_axis(self.y, cs_y, ylo),
            z=snap_axis(self.z, cs_z, zlo),
            t=self.t,
        )
    ...
    def to_cell_slices(
        ...
        """
        Convert region bounds to cell index slices.

        Assumes region is already snapped to grid (or close to it).
        Clamps indices to valid grid bounds.
        ...
        -------
        tuple[slice, slice, slice]
            Slices for indexing into cell arrays (x, y, z order).
        """
        cs_x, cs_y, cs_z = cell_size
        xlo, ylo, zlo = origin
        nx, ny, nz = grid_shape
        ...
        def axis_slice(
            bounds: tuple[float, float], cs: float, lo: float, n: int
        ) -> slice:
            if bounds[0] == NEG_INF:
                i_min = 0
            else:
                i_min = max(0, int(round((bounds[0] - lo) / cs)))
            if bounds[1] == INF:
                i_max = n
            else:
                i_max = min(n, int(round((bounds[1] - lo) / cs)))
            return slice(i_min, i_max)

        return (
            axis_slice(self.x, cs_x, xlo, nx),
            axis_slice(self.y, cs_y, ylo, ny),
            axis_slice(self.z, cs_z, zlo, nz),
        )
    ...
    def is_grid_aligned(
        ...
        """
        Check if region bounds align with cell boundaries.
        ...
        -------
        bool
            True if all spatial bounds align with cell edges.
        """
        snapped = self.snap_to_grid(cell_size, origin, "outward")
        return (
            abs(self.x[0] - snapped.x[0]) < tol
            and abs(self.x[1] - snapped.x[1]) < tol
            and abs(self.y[0] - snapped.y[0]) < tol
            and abs(self.y[1] - snapped.y[1]) < tol
            and abs(self.z[0] - snapped.z[0]) < tol
            and abs(self.z[1] - snapped.z[1]) < tol
        )
