...
def parse_lammps(
    ...
    dict,                        # box info from first timestep
]:
    """
    ...
    box_info : dict
        Box information from first timestep:
        - "bounds": (xlo, xhi, ylo, yhi, zlo, zhi) - orthogonal bounds
        - "tilt": (xy, xz, yz) or None - tilt factors for triclinic boxes
        - "boundary": str - boundary conditions from header (e.g., "pp pp pp")
    """
    path = Path(path)
    ...
    with open(path, "rb") as f:
        ...
        n_cols = 0
        box_info: dict | None = None  # Extracted from first timestep

        pos = 0
        item_timestep = b"ITEM: TIMESTEP"
        item_atoms = b"ITEM: ATOMS"
        item_box = b"ITEM: BOX BOUNDS"
        item_marker = b"ITEM:"
        ...
        while pos < file_size:
            ...
            # Extract box bounds from first timestep only
            if box_info is None:
                box_pos = mm.find(item_box, ts_line_end)
                if box_pos != -1 and box_pos < num_atoms_pos:
                    # Parse header line for boundary conditions (e.g., "pp pp pp" or "pp pp pp xy xz yz")
                    box_header_end = mm.find(b"\n", box_pos)
                    box_header = mm[box_pos:box_header_end].decode("ascii")
                    # Header format: "ITEM: BOX BOUNDS [boundary_style] [xy xz yz]"
                    header_parts = box_header.split()[3:]  # Skip "ITEM:", "BOX", "BOUNDS"
                    boundary_style = " ".join(p for p in header_parts if p in ("pp", "ff", "ss", "fs", "sp", "fm", "sm", "pm"))
                    is_triclinic = "xy" in header_parts

                    # Read 3 lines of box bounds
                    line1_start = box_header_end + 1
                    line1_end = mm.find(b"\n", line1_start)
                    ...
                    box_line1 = mm[line1_start:line1_end].decode("ascii").split()
                    box_line2 = mm[line2_start:line2_end].decode("ascii").split()
                    box_line3 = mm[line3_start:line3_end].decode("ascii").split()

                    if is_triclinic:
                        # Triclinic: xlo_bound xhi_bound xy
                        xlo, xhi, xy = float(box_line1[0]), float(box_line1[1]), float(box_line1[2])
                        ylo, yhi, xz = float(box_line2[0]), float(box_line2[1]), float(box_line2[2])
                        zlo, zhi, yz = float(box_line3[0]), float(box_line3[1]), float(box_line3[2])
                        tilt = (xy, xz, yz)
                    else:
                        # Orthogonal: xlo xhi
                        xlo, xhi = float(box_line1[0]), float(box_line1[1])
                        ylo, yhi = float(box_line2[0]), float(box_line2[1])
                        zlo, zhi = float(box_line3[0]), float(box_line3[1])
                        tilt = None

                    box_info = {
                        "bounds": (xlo, xhi, ylo, yhi, zlo, zhi),
                        "tilt": tilt,
                        "boundary": boundary_style or "pp pp pp",
                    }
    ...
    # Provide default box_info if not found
    if box_info is None:
        box_info = {"bounds": None, "tilt": None, "boundary": ""}

    return coords_list, fields_list, selected_timesteps, box_info
