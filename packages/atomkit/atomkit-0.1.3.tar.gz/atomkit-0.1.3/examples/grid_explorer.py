...
@app.cell
def _(Path, SpatialGrid, cell_size_input, coord_type_select, file_select, get_prev_grid, mo, load_btn, set_prev_grid):
    # Wait for explicit button press
    if not load_btn.value:
        mo.stop(True, mo.md("👆 Select a file and click **Load/Build** to start"))
    ...
    # Fields available for visualization (cell aggregates, excluding internal fields)
    available_fields = ["counts"] + grid.cells.fields

    _info = f"**{_status}** | shape=`{grid.grid_shape}` | atoms=`{grid.n_atoms:,}` | timesteps=`{grid.n_timesteps}`"
    _info += f"\n**Cell aggregates:** {available_fields}"
...
def _(
    ...
    # Build 4D data: counts or aggregated field (using precomputed cell aggregates)
    if field_select.value == "counts":
        _data_4d = grid.counts.astype(np.float64)
    else:
        # Use precomputed aggregates (much faster)
        _agg = grid.cells[field_select.value]
        if cell_agg.value == "sum":
            _data_4d = _agg.sum
        elif cell_agg.value == "min":
            _data_4d = _agg.min
        elif cell_agg.value == "max":
            _data_4d = _agg.max
        else:  # mean
            _data_4d = _agg.mean
