"""
Legacy metadata migration utilities

This module provides utilities to migrate legacy metadata files from the flat format
...
Usage as module:
    from polystore.metadata_migration import migrate_plate_metadata, detect_legacy_format
...
Usage as script:
    python -m polystore.metadata_migration /path/to/plate/directory
    python -m polystore.metadata_migration /path/to/plate/directory --dry-run
"""
...
def main():
    ...
    parser.add_argument(
        "plate_directory",
        type=Path,
        help=f"Path to plate directory containing {METADATA_FILENAME}",
    )
