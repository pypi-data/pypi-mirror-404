...
- Model: `{model}`"""
                ...
                # /project - project management (interactive browser)
                    ...
                    if subcommand in ("create", "edit", "delete", "task-transition", "task-delete", "link-session", "add-session", "add-to-project"):
                        try:
                            ...
                            if subcommand == "add-session" and len(cmd_parts) > 2:
                                ...
                                payload = {"session_id": session_id or "unknown"}
                                if tid:
                                    payload["task_id"] = tid
                                resp = await client.post(url, json=payload, timeout=10.0)
                                resp.raise_for_status()
                                msg = "Session added to project."
                                if tid:
                                    msg += f" Linked to task {tid[:8]}..."
                            ...
                            # Alias: links current session to a project from any context
                                ...
                                proj_id = cmd_parts[2]
                                url = f"{server_url}/api/multiuser/project/{proj_id}/link-session"
                                resp = await client.post(url, json={"session_id": session_id or "unknown"}, timeout=10.0)
                                resp.raise_for_status()
                            ...
                            # Creates a new project
                                ...
                                resp = await client.post(url, json={"session_id": session_id or "unknown"}, timeout=10.0)
                                resp.raise_for_status()
                                yield {"type": "response", "data": {"content": "Current session linked to task."}}
                                return
