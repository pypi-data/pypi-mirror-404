...
from dttlib import TrendType, PipDuration
...
TREND_TRANS_THRESHOLD = {
    "raw/sec": PipDuration.from_sec(120),
    "sec/min": PipDuration.from_sec(3600),
}
...
TREND_MAX_SECONDS = {
    TrendType.Raw: 3600,
    TrendType.Second: 3600 * 24 * 20,
    TrendType.Minute: 3600 * 24 * 365 * 12,
}
...
# fill color for labels (0-255)
LABEL_ALPHA = 170


# alpha value for the trend min/max traces (0-255)
TREND_MINMAX_ALPHA = 120


# where to set the X axis in online mode (should be zero but that
# hides the "0" label)
ONLINE_X_MAX = PipDuration.from_sec(0)
