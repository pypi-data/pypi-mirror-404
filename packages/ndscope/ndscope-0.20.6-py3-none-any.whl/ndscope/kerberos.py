...
def kinit(
    ...
    if (
        ...
        raise KerberosError(
            "cannot generate kerberos ticket in a "
    ...
    if username is None:
        verbose = True
        username = input(
            "Please provide username for the {} kerberos realm: ".format(realm)
        )
    ...
    # execute command
    kget = subprocess.Popen(
        cmd, env=krbenv, stdout=subprocess.PIPE, stdin=subprocess.PIPE
    )
    if not keytab:
...
def parse_keytab(keytab):
    ...
    """
    try:
        out = subprocess.check_output(["klist", "-k", keytab], stderr=subprocess.PIPE)
    except OSError:
