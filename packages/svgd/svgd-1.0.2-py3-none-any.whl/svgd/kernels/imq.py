...
class IMQ(Kernel, Module):
    """
    IMQ Kernel: $k(x, y) = (1 + \\frac{1}{2\\sigma^2} ||x - y||^2)^{-\\frac{1}{2}}$
    """
    ...
    def forward(
        ...
        k = gamma.unsqueeze(-1).unsqueeze(-1).mul(squared_distance).add(1).pow(-0.5)
        k_xj = (
            gamma.unsqueeze(-1)
            .unsqueeze(-1)
            .mul(k.pow(3))
        ...
        if state.f_k_compute_tr_k_xj_xi_t_k_xk_xi:
            tr_k_xj_xi_t_k_xk_xi_1 = (
                ...
                .pow(2)
                .mul(k.pow(2).unsqueeze(-1))
                .mul(k.pow(2).unsqueeze(-2))
                .mul(gamma.pow(2).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
                .mul(9)
            )
            tr_k_xj_xi_t_k_xk_xi_2 = (
                squared_distance.mul(k.pow(2))
                .unsqueeze(-1)
                .add(squared_distance.mul(k.pow(2)).unsqueeze(-2))
                .mul(gamma.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1))
                .mul(-3)
            )
