...
impl Move {
    ...
    /// Move16からMoveを構築（Piece情報は不要）
    #[must_use]
    pub const fn from_move16(mv16: Move16) -> Self {
        Self(mv16.0 as u32)
    }

    /// USI文字列からMoveを生成（piece情報は未設定）
    #[must_use]
    pub fn from_usi(s: &str) -> Option<Self> {
        Move16::from_usi(s).map(Self::from_move16)
    }
    ...
    pub fn to_csa(self) -> Option<String> {
        ...
        let piece_type = if self.is_drop() {
            ...
            self.moved_after_piece().piece_type()
        };
...
mod tests {
    ...
    fn test_move32_to_csa() {
        ...
        let m = Move::make_promote(from, to, Piece::B_BISHOP);
        assert_eq!(m.to_csa(), Some("2233UM".to_string()));

        let drop_to = "5e".parse::<Square>().unwrap();
        let m = Move::make_drop(PieceType::PAWN, drop_to, Color::BLACK);
