...
use crate::types::{
    Color, File, Hand, Move16, Piece, PieceType, Rank, Square, SQ_51, SQ_55, SQ_59,
};
...
#[test]
fn position_move_from_csa_and_move16() {
    ...
    let mv16 = Move16::make_move("7g".parse().unwrap(), "7f".parse().unwrap());
    let mv = pos.move_from_move16(mv16);
    assert_eq!(mv.to_usi(), "7g7f");

    let mut promo_pos = Position::new();
    let from = "2b".parse::<Square>().unwrap();
    promo_pos.board.set(from, PackedPiece::from_piece(Piece::B_BISHOP));
