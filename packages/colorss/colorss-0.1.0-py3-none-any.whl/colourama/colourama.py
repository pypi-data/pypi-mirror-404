...
def colour(script_or_alias, background=None):
    ...
    if background:
        thread = threading.Thread(target=colour_sync, args=(script_or_alias,))
        thread.daemon = True
        thread.start()
        return thread
    else:
        return colour_sync(script_or_alias)

def colour_sync(script_or_alias):
    if not script_or_alias:
        return False
