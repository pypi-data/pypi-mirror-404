...
* Deno (JavaScript runtime required for modern YouTube extraction in yt-dlp)
* Python 3.8+
...
* ✅ Installs Deno system-wide (required for modern YouTube extraction in yt-dlp)
* ✅ Creates virtualenv at `/opt/yt-dlp-mullvad/venv`
* ✅ Installs `ytp-dl==0.5.8` into the virtualenv
* ✅ Installs `yt-dlp[default]` into the virtualenv (includes EJS solver scripts used by pip installs)
...
#   - Installs ytp-dl==0.5.8 + yt-dlp[default] + gunicorn + gevent in that venv
...
# IMPORTANT:
# - yt-dlp now relies on an external JS runtime (deno) for full YouTube support.
# - For pip installs, install yt-dlp with the [default] extras so EJS solver scripts are present.
pip install "ytp-dl==0.5.8" "yt-dlp[default]" gunicorn gevent
deactivate
