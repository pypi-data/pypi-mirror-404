...
def model_menu():
    ...
    while True:
        choice = input(f"  {LIME}>{RESET} Select module {GRAY}[1-3]{RESET}: ").strip()
        if choice in MODELS:
...
def main():
    ...
    try:
        while True:
            try:
                user_input = input(f"{GREEN}>{RESET} {LIME}VAULT DWELLER{RESET}{DARKGREEN}:{RESET} ").strip()
                ...
                if response is not None:
                    if response == "CONNECTION LOST":
                        ...
                        print(f"{LIME}>{RESET} {GREEN}PIP-BOY{RESET}{DARKGREEN}:{RESET} {WHITE}{response}{RESET}")
        ...
        stop_server()
        print(f"  {GREEN}[OK]{RESET} {LIME}Stay safe out there, Vault Dweller.{RESET}")
