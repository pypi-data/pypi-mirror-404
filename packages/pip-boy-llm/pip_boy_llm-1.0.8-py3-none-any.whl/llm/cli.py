...
MODELS = {
    ...
    "4": {"id": "deepseek-1.5b", "name": "DeepSeek R1 1.5B", "desc": "Slow/Reasoning", "mode": "FP16"},
}
...
def format_thinking(response):
    """Format <think>...</think> tags as italic dark grey."""
    def replace_think(match):
        thinking = match.group(1)
        return f"{ITALIC}{GRAY}{thinking}{RESET}{WHITE}"

    # Replace <think>...</think> with styled version
    formatted = re.sub(r'<think>(.*?)</think>\s*', replace_think, response, flags=re.DOTALL)
    return formatted
            ...
            try:
                ...
                if response is not None:
                    if response == "CONNECTION LOST":
                        ...
                        formatted_response = format_thinking(response)
                        print(f"{WHITE}>{RESET} {GREEN}PIP-BOY{RESET}{DARKGREEN}:{RESET} {WHITE}{formatted_response}{RESET}")
