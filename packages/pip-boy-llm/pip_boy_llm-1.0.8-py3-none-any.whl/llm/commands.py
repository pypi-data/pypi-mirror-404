...
from llm.config import get_theme_colors
...
# For backwards compatibility, initialize defaults
GREEN, DARKGREEN, LIME = get_colors()
...
def cmd_load(args: str, context: CommandContext) -> str:
    ...
    user_name = get_config_value("user_name", "VAULT DWELLER")
    ...
    for msg in messages[-10:]:  # Show last 10 messages
        ...
        if role == "user":
            print(f"  {LIME}{user_name}:{RESET} {GRAY}{content}{RESET}")
        else:
...
def cmd_history(args: str, context: CommandContext) -> str:
    ...
    for msg in messages[-20:]:
        ...
        if role == "user":
            print(f"  {LIME}{user_name}:{RESET} {WHITE}{content}{RESET}")
        else:
...
""")
    ...
    try:
        ...
        if choice == "1":
            ...
            print()
            color_choice = input(f"  {WHITE}>{RESET} Select theme [1-{len(color_options)}]: ").strip()
            ...
            try:
                ...
                if 0 <= idx < len(color_options):
                    ...
                    print(f"  {PRIMARY}[!]{RESET} {GRAY}Invalid selection{RESET}\n")
            except ValueError:
                print(f"  {PRIMARY}[!]{RESET} {GRAY}Invalid selection{RESET}\n")
            ...
            # Name input
            print()
            new_name = input(f"  {WHITE}>{RESET} Enter your name: ").strip()
            if new_name:
                set_config_value("user_name", new_name)
                print(f"\n  {PRIMARY}[OK]{RESET} {ACCENT}Name set to: {new_name}{RESET}")
                print(f"  {GRAY}The LLM will now address you by this name{RESET}\n")
            else:
                print(f"  {PRIMARY}[!]{RESET} {GRAY}Name not changed{RESET}\n")
