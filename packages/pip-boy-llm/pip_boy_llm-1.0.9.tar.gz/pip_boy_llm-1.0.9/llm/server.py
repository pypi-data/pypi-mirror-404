...
# Try to import bitsandbytes (optional, requires CUDA)
...
def load_model():
    ...
    print(f"[*] Loading {current_model_id} ({model_name}) in {mode} on {device}...")
    ...
    if use_4bit and BitsAndBytesConfig is not None:
        ...
        # Use float32 on CPU (better compatibility), float16 on GPU
        dtype = torch.float32 if not HAS_CUDA else torch.float16
        model = AutoModelForCausalLM.from_pretrained(
            model_name,
            torch_dtype=dtype,
        ...
        if not HAS_CUDA:
            model = model.to("cpu")
        print(f"[+] Model loaded in {'float16' if HAS_CUDA else 'float32'}!")
