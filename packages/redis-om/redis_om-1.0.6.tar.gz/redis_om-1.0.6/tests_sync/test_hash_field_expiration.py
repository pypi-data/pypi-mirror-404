...
@py_test_mark_sync
def test_field_with_expire_parameter(models):
...
@py_test_mark_sync
def test_save_applies_field_expiration(models, redis):
...
@py_test_mark_sync
def test_expire_field_sets_ttl(models, redis):
    ...
    result = simple.expire_field("value", 120)
    assert result == 1, "expire_field should return 1 on success"

    # Verify TTL was set
    ttl = simple.field_ttl("value")
    assert 0 < ttl <= 120


@py_test_mark_sync
def test_expire_field_nonexistent_field(models, redis):
    """expire_field() on non-existent field should return -2."""
    simple = models.SimpleModel(name="test", value="data")
    simple.save()

    result = simple.expire_field("nonexistent", 60)
...
@py_test_mark_sync
def test_field_ttl_returns_remaining_time(models, redis):
    """field_ttl() should return remaining TTL in seconds."""
    simple = models.SimpleModel(name="test", value="data")
    simple.save()

    simple.expire_field("value", 300)
    ttl = simple.field_ttl("value")
...
@py_test_mark_sync
def test_field_ttl_no_expiration(models, redis):
...
@py_test_mark_sync
def test_save_with_field_expirations_param(models, redis):
    """save(field_expirations={"field": ttl}) should apply TTLs."""
    simple = models.SimpleModel(name="test", value="important_data")
    simple.save(field_expirations={"value": 180})

    ttl = simple.field_ttl("value")
    assert 0 < ttl <= 180

    # name should have no TTL since it wasn't in field_expirations
    name_ttl = simple.field_ttl("name")
    assert name_ttl == -1


@py_test_mark_sync
def test_save_field_expirations_overrides_default(models, redis):
    ...
    session.save(field_expirations={"token": 10})
...
@py_test_mark_sync
def test_hexpire_not_available_raises_or_warns(models, redis):
...
@py_test_mark_sync
def test_check_hash_field_expiration_support():
...
@py_test_mark_sync
def test_update_preserves_field_expiration(models, redis):
    ...
    # Get initial TTL
    initial_ttl = session.field_ttl("token")
    ...
    session.update(user_id="user456")

    # Token TTL should still be set (possibly slightly lower)
    updated_ttl = session.field_ttl("token")
    assert updated_ttl > 0
    assert updated_ttl <= initial_ttl


@py_test_mark_sync
def test_save_preserves_manually_set_ttl(models, redis):
    """
    ...
    default_ttl = session.field_ttl("token")
    assert default_ttl > 0 and default_ttl <= 60

    # Manually extend TTL to 1 hour
    session.expire_field("token", 3600)
    extended_ttl = session.field_ttl("token")
    ...
    ttl_after_save = session.field_ttl("token")
    assert ttl_after_save > 60, f"TTL was reset to default! Got {ttl_after_save}"


@py_test_mark_sync
def test_field_expires_after_ttl(models, redis):
    ...
    key = simple.key()
    value_exists = redis.hexists(key, "value")
