...
def test_pagination_queries(members, m):
    ...
    actual = m.Member.find(m.Member.last_name == "Brookins").sort_by("id").page()

    assert actual == [member1, member2]

    actual = m.Member.find().sort_by("id").page(1, 1)

    assert actual == [member2]

    actual = m.Member.find().sort_by("id").page(0, 1)
...
@py_test_mark_sync
def test_only_method_basic(members, m):
    ...
    from redis_om.model.model import PartialModel
...
@py_test_mark_sync
def test_only_method_error_messages(members, m):
    """Test that .only() provides helpful error messages."""
    member1, member2, member3 = members

    # Test .only() with one field
    results = m.Member.find().only("first_name").all()
    result = results[0]
...
@py_test_mark_sync
def test_values_type_conversion(members, m):
    ...
    results = m.Member.find().values("first_name", "age").all()
    result = results[0]
...
@py_test_mark_sync
def test_only_type_conversion(members, m):
    ...
    results = m.Member.find().only("first_name", "age").all()
    result = results[0]
...
@py_test_mark_sync
def test_boolean_fields_work_with_hash_model(key_prefix, redis):
    ...
    # Test get() method
    retrieved = BoolTestModel.get(model.pk)
    ...
    active_users = BoolTestModel.find(BoolTestModel.is_active == True).all()
    assert len(active_users) == 1
    assert active_users[0].name == "test_user"
    assert active_users[0].is_active is True

    non_admin_users = BoolTestModel.find(BoolTestModel.is_admin == False).all()
    ...
    only_result = BoolTestModel.find().only("name", "is_active").all()
...
@py_test_mark_sync
def test_save_nx_only_saves_if_not_exists(m):
    ...
    fetched = m.Member.get(member.id)
    assert fetched.first_name == "Andrew"


@py_test_mark_sync
def test_save_xx_only_saves_if_exists(m):
...
@py_test_mark_sync
def test_save_nx_xx_mutually_exclusive(m):
...
@py_test_mark_sync
def test_save_nx_with_pipeline_raises_error(m):
    ...
    with m.Member.db().pipeline(transaction=True) as pipe:
        with pytest.raises(ValueError, match="Cannot use nx or xx with pipeline"):
            member.save(pipeline=pipe, nx=True)
...
@py_test_mark_sync
def test_hashmodel_vector_field_with_list(key_prefix, redis):
    ...
    # Retrieve and verify
    retrieved = VectorDocument.get(doc.pk)
    assert retrieved.name == "test"
