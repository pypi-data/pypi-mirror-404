...
def test_case_sensitive(members, m):
    ...
    actual = m.Member.find(m.Member.first_name == "Andrew").sort_by("pk").all()
...
@py_test_mark_sync
def test_values_method_with_specific_fields(members, m):
    member1, member2, member3 = members
    actual = (
        m.Member.find(
            (m.Member.first_name == "Andrew") & (m.Member.last_name == "Brookins")
...
@py_test_mark_sync
def test_values_method_all_fields(members, m):
    member1, member2, member3 = members
    actual = m.Member.find(m.Member.first_name == "Andrew").values().all()
...
def test_type_with_enum():
    ...
    assert TypeWithEnum.get(item.pk) == item


@py_test_mark_sync
def test_type_with_list_of_enums(key_prefix, redis):
    class TestEnum(Enum):
        FOO = "foo"
        BAR = "bar"
    ...
    assert TypeWithEnums.get(item.pk) == item
...
@py_test_mark_sync
def test_literals(key_prefix, redis):
...
@py_test_mark_sync
def test_child_class_expression_proxy():
    ...
    rematerialized = Child.find(Child.pk == m.pk).first()
...
@py_test_mark_sync
def test_child_class_expression_proxy_with_mixin():
...
@py_test_mark_sync
def test_model_raises_error_if_inherited_from_indexed_model():
...
@py_test_mark_sync
def test_model_inherited_from_indexed_model():
    class Model(JsonModel, index=True):
        name: str = "Steve"
...
@py_test_mark_sync
def test_non_indexed_model_raises_error_on_save():
    class Model(JsonModel):
        pass
...
@py_test_mark_sync
def test_model_with_alias_can_be_searched(key_prefix, redis):
    class Model(JsonModel, index=True):
    ...
    rematerialized = Model.find(Model.first_name == "Steve").first()
...
@py_test_mark_sync
def test_tag_field_sortability(key_prefix, redis):
    ...
    results = Product.find().sort_by("name").all()
    ...
    results = Product.find(Product.category == "Fruits").sort_by("name").all()
...
@py_test_mark_sync
def test_save_nx_only_saves_if_not_exists(m, address):
    ...
    result = member2.save(nx=True)
    assert result is None

    # Verify original data is unchanged
    fetched = m.Member.get(member.pk)
    assert fetched.first_name == "Andrew"


@py_test_mark_sync
def test_save_xx_only_saves_if_exists(m, address):
...
@py_test_mark_sync
def test_save_nx_xx_mutually_exclusive(m, address):
    ...
    with pytest.raises(ValueError, match="Cannot specify both nx and xx"):
        member.save(nx=True, xx=True)


@py_test_mark_sync
def test_save_nx_with_pipeline(m, address):
    ...
    with m.Member.db().pipeline(transaction=True) as pipe:
        member1.save(pipeline=pipe, nx=True)
        member2.save(pipeline=pipe, nx=True)
        pipe.execute()

    # Verify both were saved
    fetched1 = m.Member.get(member1.pk)
    fetched2 = m.Member.get(member2.pk)
    assert fetched1.first_name == "Andrew"
    assert fetched2.first_name == "Kim"


@py_test_mark_sync
def test_schema_for_fields_does_not_modify_dict_during_iteration(m):
    """
...
@py_test_mark_sync
def test_schema_for_fields_with_indexed_fields(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_with_optional_fields(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_with_inherited_fields(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_with_embedded_model(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_with_list_fields(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_field_info_has_annotation(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_with_primary_key(key_prefix, redis):
...
@py_test_mark_sync
def test_schema_for_fields_with_case_sensitive(key_prefix, redis):
...
def test_bytes_field_in_embedded_model(key_prefix, redis):
    ...
    retrieved = Document.get(doc.pk)
    assert retrieved.file.content == binary_content
    assert retrieved.file.mime_type == "image/png"


@py_test_mark_sync
def test_custom_primary_key_pk_property(key_prefix, redis):
    ...
    instance.save()
    retrieved = ModelWithCustomPK.get(42)
...
@py_test_mark_sync
def test_jsonmodel_vector_field_with_list(key_prefix, redis):
