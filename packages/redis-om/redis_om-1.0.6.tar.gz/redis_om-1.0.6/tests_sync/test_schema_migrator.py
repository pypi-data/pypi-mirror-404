...
from redis_om.model.migrations.schema import BaseSchemaMigration, SchemaMigrator
from redis_om.model.migrations.schema.legacy_migrator import (
    schema_hash_key,
    schema_text_key,
)
...
def clean_redis(redis):
    ...
    # Cleanup before test
    redis.delete(applied_migrations_key)
    keys = redis.keys(schema_key_pattern)
    if keys:
        redis.delete(*keys)
...
def test_create_migration_file_when_no_ops(redis, monkeypatch):
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fp = migrator.create_migration_file("noop")
            assert fp is None
    finally:
...
def test_create_and_status_empty(clean_redis):
    with tempfile.TemporaryDirectory() as tmp:
        migrator = _WorkerAwareSchemaMigrator(
...
class _WorkerAwareSchemaMigrator(SchemaMigrator):
    ...
    def mark_unapplied(self, migration_id: str):
        """Mark migration as unapplied using worker-specific key."""
        self.redis.srem(self.APPLIED_MIGRATIONS_KEY, migration_id)
...
class _TestSchemaMigration(BaseSchemaMigration):
    ...
    def down(self) -> None:
        ...
        for op in reversed(self.operations):
            ...
            try:
                self.redis.ft(index_name).dropindex()
            except Exception:
                pass
            if prev_schema:
...
def test_rollback_successful_single_operation(clean_redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Mark as applied
        migrator.mark_applied("001_add_description")

        # Verify new schema is active
        new_hash = redis.get(f"{schema_hash_key(index_name)}:{worker_prefix}")
        ...
        # Perform rollback
        success = migrator.rollback("001_add_description", verbose=True)
        ...
        applied_migrations = migrator.get_applied()
...
def test_rollback_with_empty_previous_schema(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Apply migration
        migration.up()
        migrator.mark_applied("002_create_index")
        ...
        # Perform rollback
        success = migrator.rollback("002_create_index", verbose=True)
        assert success is True
...
def test_rollback_multiple_operations(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        redis.execute_command(f"FT.CREATE {index1_name} {original_schema1}")
        redis.execute_command(f"FT.CREATE {index2_name} {original_schema2}")
        ...
        redis.ft(index1_name).dropindex()
        redis.ft(index2_name).dropindex()
        migration.up()
        migrator.mark_applied("003_update_multiple")
        ...
        # Perform rollback
        success = migrator.rollback("003_update_multiple", verbose=True)
        assert success is True
...
def test_rollback_not_supported(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        migrator.mark_applied("004_no_rollback")
        ...
        success = migrator.rollback("004_no_rollback", verbose=True)
...
def test_rollback_unapplied_migration(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        success = migrator.rollback("005_unapplied", verbose=True)
        assert success is False  # Should return False for unapplied migration


def test_rollback_dry_run(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        redis.execute_command(f"FT.CREATE {index_name} {new_schema}")
        new_hash = hashlib.sha1(new_schema.encode("utf-8")).hexdigest()
        redis.set(f"{schema_hash_key(index_name)}:{worker_prefix}", new_hash)
        redis.set(f"{schema_text_key(index_name)}:{worker_prefix}", new_schema)

        migrator.mark_applied("006_dry_run_test")
        ...
        current_hash = redis.get(f"{schema_hash_key(index_name)}:{worker_prefix}")
        current_text = redis.get(f"{schema_text_key(index_name)}:{worker_prefix}")
...
def test_rollback_with_redis_command_failure(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        migrator.mark_applied("007_redis_failure")
        ...
        def failing_execute_command(*args, **kwargs):
            if args[0] == "FT.CREATE":
        ...
        try:
            ...
            success = migrator.rollback("007_redis_failure", verbose=True)
...
def test_rollback_state_consistency(redis):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        redis.ft(index_name).dropindex()
        migration.up()
        migrator.mark_applied("008_consistency_test")

        # Verify new state
        new_hash = redis.get(f"{schema_hash_key(index_name)}:{worker_prefix}")
        new_text = redis.get(f"{schema_text_key(index_name)}:{worker_prefix}")
        ...
        # Perform rollback
        success = migrator.rollback("008_consistency_test", verbose=True)
        assert success is True
