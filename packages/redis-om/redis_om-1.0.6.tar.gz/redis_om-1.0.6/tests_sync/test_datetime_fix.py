...
@py_test_mark_sync
def test_hash_model_datetime_conversion(redis):
    ...
    try:
        ...
        raw_data = HashModelWithDatetime.db().hgetall(test_model.key())
        ...
        retrieved = HashModelWithDatetime.get(test_model.pk)
        ...
        # Clean up
        try:
            HashModelWithDatetime.db().delete(test_model.key())
        except Exception:
            pass
...
@py_test_mark_sync
def test_json_model_datetime_conversion(redis):
    ...
    try:
        ...
        # Get the raw data to check timestamp conversion
        raw_data = JsonModelWithDatetime.db().json().get(test_model.key())
        ...
        # Retrieve the model to ensure conversion back works
        retrieved = JsonModelWithDatetime.get(test_model.pk)
        assert isinstance(retrieved.created_at, datetime.datetime)
        ...
        # Clean up
        try:
            JsonModelWithDatetime.db().delete(test_model.key())
        except Exception:
            pass
