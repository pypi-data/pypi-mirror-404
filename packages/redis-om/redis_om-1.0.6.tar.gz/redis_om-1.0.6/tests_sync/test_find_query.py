...
@py_test_mark_sync
def test_find_query_in(members, m):
    # << means "in"
    member1, member2, member3 = members
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_not_in(members, m):
    # >> means "not in"
    member1, member2, member3 = members
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_sort_desc(m):
    model_name, fq = FindQuery(
        expressions=[m.Member.age > 0], model=m.Member, sort_fields=["-age"]
    ).get_query()
    assert fq == [
...
@py_test_mark_sync
def test_find_query_text_search(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_text_search_and(m, members):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_text_search_or(m, members):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_text_search_not(m):
    model_name, fq = FindQuery(
        expressions=[~(m.Member.first_name == "Andrew")], model=m.Member
    ).get_query()
    assert fq == ["FT.SEARCH", model_name, "-(@first_name:{Andrew})", "LIMIT", 0, 1000]
...
@py_test_mark_sync
def test_find_query_text_search_not_and(m, members):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_text_search_not_or(m, members):
    model_name, fq = FindQuery(
        expressions=[~((m.Member.first_name == "Andrew") | (m.Member.age < 40))],
        model=m.Member,
    ).get_query()
...
@py_test_mark_sync
def test_find_query_text_search_not_or_and(m, members):
    model_name, fq = FindQuery(
        expressions=[
            ~(
...
@py_test_mark_sync
def test_find_query_text_contains(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_text_startswith(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_text_endswith(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_test_fuzzy(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_limit_one(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_limit_offset(m):
    model_name, fq = FindQuery(
...
@py_test_mark_sync
def test_find_query_page_size(m):
