...
@py_test_mark_sync
def test_user_hashmodel_queries():
    ...
    try:
        # Exact match
        results = User.find(User.username == "alice").all()
        assert len(results) == 1
        assert results[0].username == "alice"

        # Numeric comparison
        results = User.find(User.age >= 30).all()
        assert len(results) >= 1
        assert all(r.age >= 30 for r in results)
    finally:
        User.delete(user1.pk)
        User.delete(user2.pk)


@py_test_mark_sync
def test_user_full_text_search():
    ...
    try:
        results = User.find(User.bio % "Redis").all()
        assert len(results) >= 1
...
@py_test_mark_sync
def test_product_jsonmodel_crud():
...
@py_test_mark_sync
def test_product_jsonmodel_queries():
    ...
    try:
        # Price range query
        results = Product.find(Product.price >= 100).all()
        assert len(results) >= 1
        assert all(r.price >= 100 for r in results)

        # Full-text search
        results = Product.find(Product.name % "Widget").all()
        assert len(results) >= 2
    finally:
        Product.delete(prod1.pk)
        Product.delete(prod2.pk)
...
@py_test_mark_sync
def test_order_with_embedded_address():
    ...
    try:
        ...
        results = Order.find(Order.shipping_address.city == "Boston").all()
        assert len(results) >= 1
    finally:
...
def test_pagination():
    ...
    try:
        ...
        page1 = query.copy(offset=0, limit=2).all()
        assert len(page1) >= 2

        # Get next 2 results
        page2 = query.copy(offset=2, limit=2).all()
        assert len(page2) >= 2
    finally:
        for u in users:
            User.delete(u.pk)
