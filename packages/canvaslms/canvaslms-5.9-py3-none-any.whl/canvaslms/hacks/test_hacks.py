...
# These are the actual Canvas classes we patch.
# We import them explicitly so we can call their patched dunder methods.
...
class TestCacheGetMethodsInit:
    ...
    def test_cache_key_parameter_default(self):
        """cache_key parameter should default to 'id'"""
        decorator = CacheGetMethods("page")

        assert decorator._CacheGetMethods__cache_key == 'id'

    def test_cache_key_parameter_url(self):
        """cache_key='url' should be stored for URL-based caching"""
        decorator = CacheGetMethods("page", cache_key='url')

        assert decorator._CacheGetMethods__cache_key == 'url'
class TestCacheKeyUrlParameter:
    """Test cache_key='url' functionality for URL-based caching"""

    def test_url_cache_stores_by_url(self):
        """Objects should be cached by URL when cache_key='url'"""
        class TestClass:
            def __init__(self):
                pass
            def get_page(self, url):
                obj = MagicMock()
                obj.url = url
                obj.title = f"Page: {url}"
                return obj
            def get_pages(self):
                return []

        CacheGetMethods("page", cache_key='url')(TestClass)
        instance = TestClass()

        page = instance.get_page("intro-page")

        assert "intro-page" in instance.page_cache
        cached_obj, _ = instance.page_cache["intro-page"]
        assert cached_obj.url == "intro-page"

    def test_url_cache_hit(self):
        """Second call with same URL should return cached object"""
        call_count = {"count": 0}
        ...
        page1 = instance.get_page("course-syllabus")
        page2 = instance.get_page("course-syllabus")

        assert call_count["count"] == 1
        assert page1 is page2

    def test_url_cache_different_urls(self):
        """Different URLs should result in separate cache entries"""
        class TestClass:
            def __init__(self):
                pass
            def get_page(self, url):
        ...
        page1 = instance.get_page("syllabus")
        page2 = instance.get_page("schedule")

        assert "syllabus" in instance.page_cache
        assert "schedule" in instance.page_cache
        assert page1.url == "syllabus"
        assert page2.url == "schedule"

    def test_bulk_fetch_populates_url_cache(self):
        """Bulk fetch should populate cache with URL keys"""
        class TestClass:
            def __init__(self):
                pass
            def get_page(self, url):
            ...
            def get_pages(self):
                return [
                    MagicMock(url="page-a"),
                    MagicMock(url="page-b"),
                    MagicMock(url="page-c")
                ]

        CacheGetMethods("page", cache_key='url')(TestClass)
        instance = TestClass()

        list(instance.get_pages())

        assert "page-a" in instance.page_cache
        assert "page-b" in instance.page_cache
        assert "page-c" in instance.page_cache

    def test_string_argument_accepted(self):
        """String URL argument should be accepted directly"""
        class TestClass:
            def __init__(self):
                pass
            def get_page(self, url):
        ...
        # Should not raise - string argument is valid for URL-based caching
        page = instance.get_page("my-page-url")
        assert page.url == "my-page-url"
...
class TestSingularMethodCaching:
    ...
    def test_rejects_string_for_id_based_cache(self):
        """Should reject string argument when cache_key='id' (default)"""
        class TestClass:
            def __init__(self):
                pass
            def get_item(self, item_id):
        ...
        CacheGetMethods("item")(TestClass)  # default cache_key='id'
        instance = TestClass()

        with pytest.raises(TypeError, match="must be int or Canvas object"):
            instance.get_item("some-string-id")
    ...
    def test_raises_typeerror_invalid_argument_type(self):
        """Should raise TypeError for invalid argument types like lists"""
        class TestClass:
            def __init__(self):
                pass
            def get_item(self, item_id):
        ...
        # Lists, dicts, etc. are invalid for ID-based caching
        with pytest.raises(TypeError, match="must be int or Canvas object"):
            instance.get_item([1, 2, 3])
...
class TestOutdated:
    ...
    def test_numeric_grade_is_outdated(self):
        ...
        assert outdated(submission)
class TestModuleItemResolution:
    """Test module item resolution to cached content objects"""

    def test_resolve_to_page_returns_cached_page(self):
        """resolve_to_page should return the same object as course.get_page"""
        # Create a mock course with page caching
        course = MagicMock()
        cached_page = MagicMock()
        cached_page.url = "intro-page"
        course.get_page.return_value = cached_page

        # Create a page module item
        module_item = MagicMock()
        module_item.type = "Page"
        module_item.page_url = "intro-page"

        # Apply the resolution method
        import canvasapi.module
        module_item.resolve_to_page = \
            canvasapi.module.ModuleItem.resolve_to_page.__get__(
                module_item, type(module_item))

        result = module_item.resolve_to_page(course)

        course.get_page.assert_called_once_with("intro-page")
        assert result is cached_page
        assert result.course is course

    def test_resolve_to_assignment_returns_cached_assignment(self):
        """resolve_to_assignment should return the same object as
        course.get_assignment"""
        course = MagicMock()
        cached_assignment = MagicMock()
        cached_assignment.id = 12345
        course.get_assignment.return_value = cached_assignment

        module_item = MagicMock()
        module_item.type = "Assignment"
        module_item.content_id = 12345

        import canvasapi.module
        module_item.resolve_to_assignment = \
            canvasapi.module.ModuleItem.resolve_to_assignment.__get__(
                module_item, type(module_item))

        result = module_item.resolve_to_assignment(course)

        course.get_assignment.assert_called_once_with(12345)
        assert result is cached_assignment
        assert result.course is course

    def test_resolve_to_page_raises_for_wrong_type(self):
        """resolve_to_page should raise TypeError for non-Page items"""
        course = MagicMock()
        ...
        with pytest.raises(TypeError, match="Cannot resolve Assignment"):
            module_item.resolve_to_page(course)

    def test_resolve_to_assignment_raises_for_wrong_type(self):
        """resolve_to_assignment should raise TypeError for non-Assignment
        items"""
        course = MagicMock()
        ...
        with pytest.raises(TypeError, match="Cannot resolve Page"):
            module_item.resolve_to_assignment(course)

    def test_resolve_dispatches_to_page(self):
        """resolve should call resolve_to_page for Page items"""
        course = MagicMock()
        cached_page = MagicMock()
        course.get_page.return_value = cached_page
        ...
        module_item = MagicMock()
        module_item.type = "Page"
        module_item.page_url = "some-page"

        import canvasapi.module
        module_item.resolve = canvasapi.module.ModuleItem.resolve.__get__(
            module_item, type(module_item))
        module_item.resolve_to_page = \
            canvasapi.module.ModuleItem.resolve_to_page.__get__(
                module_item, type(module_item))

        result = module_item.resolve(course)

        assert result is cached_page

    def test_resolve_dispatches_to_assignment(self):
        """resolve should call resolve_to_assignment for Assignment items"""
        course = MagicMock()
        cached_assignment = MagicMock()
        course.get_assignment.return_value = cached_assignment
    ...
    def test_resolve_returns_none_for_unsupported_types(self):
        """resolve should return None for File, ExternalUrl, SubHeader, etc."""
        course = MagicMock()

        for unsupported_type in ["File", "ExternalUrl", "SubHeader",
                                  "ExternalTool"]:
            module_item = MagicMock()
            module_item.type = unsupported_type
            ...
            assert result is None, \
                f"Expected None for type {unsupported_type}"

    def test_object_identity_page_via_module(self):
        """Page accessed via module item should be same object as direct
        cache access"""
        # This test verifies the key requirement: object identity
        # A page from module_item.resolve() should be the same Python object
        # as the page from course.get_page()

        course = MagicMock()
        the_page = MagicMock()
        the_page.url = "shared-page"
        course.get_page.return_value = the_page

        module_item = MagicMock()
        module_item.type = "Page"
        module_item.page_url = "shared-page"
        ...
        # Get page via module item
        page_via_module = module_item.resolve_to_page(course)

        # Get page directly (simulating cached access)
        page_direct = course.get_page("shared-page")

        # Must be the SAME object (object identity)
        assert page_via_module is page_direct
