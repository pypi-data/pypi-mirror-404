...
def _login_with_password(
    ...
    try:
        ...
        # Extract tenant_id from response if available (prefer UUID over user-provided name)
        tenant_info = data.get("tenant", {})
        tenant_id = tenant_info.get("tenant_id") or tenant_info.get("id") or tenant
        tenant_name = tenant_info.get("tenant_name") or tenant_info.get("subdomain") or tenant

        # Save profile with the resolved tenant ID
        profile = Profile(
            name=profile_name,
            server=server,
            tenant=tenant_id,
        ...
        if tenant_id != tenant_name:
            ...
            console.print(f"  [dim]Tenant:[/dim] {tenant_id}")
...
def _login_with_api_key(
    ...
    try:
        ...
        # Extract tenant_id from user response if available
        tenant_info = user_data.get("tenant", {})
        tenant_id = tenant_info.get("tenant_id") or tenant_info.get("id") or user_data.get("tenant_id") or tenant
        tenant_name = tenant_info.get("tenant_name") or tenant_info.get("subdomain") or tenant

        # Save profile with resolved tenant ID
        profile = Profile(
            name=profile_name,
            server=server,
            tenant=tenant_id,
