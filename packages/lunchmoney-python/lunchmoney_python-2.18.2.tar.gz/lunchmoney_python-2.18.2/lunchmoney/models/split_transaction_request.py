...
class SplitTransactionRequest(BaseModel):
    ...
    """ # noqa: E501
    child_transactions: Annotated[List[SplitTransactionObject], Field(min_length=2, max_length=500)] = Field(description="List of child transactions to create. The sum of the `amounts` must match the split transaction amount.")
    __properties: ClassVar[List[str]] = ["child_transactions"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in child_transactions (list)
        _items = []
        if self.child_transactions:
            for _item_child_transactions in self.child_transactions:
                if _item_child_transactions:
                    _items.append(_item_child_transactions.to_dict())
            _dict['child_transactions'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "child_transactions": [SplitTransactionObject.from_dict(_item) for _item in obj["child_transactions"]] if obj.get("child_transactions") is not None else None
        })
        return _obj
