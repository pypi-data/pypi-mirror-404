#!/usr/bin/env python3
"""
Developer utilities for souleyez.

Command: souleyez dev repair
...
def dev_repair():
    ...
    show = _run([py, "-m", "pip", "show", "souleyez"])
    print(show.stdout.strip() or "(not installed)")

    # 4) Uninstall any existing install
    _info("Uninstalling any existing 'souleyez'...")
    _run([py, "-m", "pip", "uninstall", "-y", "souleyez"])
    ...
    show2 = _run([py, "-m", "pip", "show", "souleyez"])
    print(show2.stdout)
