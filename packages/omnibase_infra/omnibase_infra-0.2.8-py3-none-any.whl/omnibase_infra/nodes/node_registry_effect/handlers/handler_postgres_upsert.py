...
"""Handler for PostgreSQL registration record upsert.
...
Architecture:
    HandlerPostgresUpsert is responsible for:
    - Executing upsert operations against the PostgreSQL adapter
...
Related:
    ...
    - ProtocolPostgresAdapter: Protocol defining the upsert interface
...
class HandlerPostgresUpsert:
    """Handler for PostgreSQL registration record upsert.

    Encapsulates all PostgreSQL-specific persistence logic extracted from
    NodeRegistryEffect for declarative node compliance. The handler provides
    a clean interface for executing upsert operations with proper timing
    ...
    def __init__(self, postgres_adapter: ProtocolPostgresAdapter) -> None:
        ...
        Args:
            postgres_adapter: Protocol-compliant PostgreSQL adapter for
                executing registration record upsert operations.
        """
    ...
    async def handle(
        ...
        Performs the upsert operation against the PostgreSQL adapter with:
        - Operation timing via time.perf_counter()
        - Protocol call to adapter.upsert()
        - Error sanitization for security
        - Structured result construction

        Args:
            request: Registration request with node details including
                node_id, node_type, node_version, endpoints, and metadata.
...
__all__: list[str] = ["HandlerPostgresUpsert"]
