...
class AsyncRuntime:
    ...
    def run(cls, coro):
        ...
        async def run_coro_with_context():
            # 在 Event Loop 线程中恢复上下文
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(run_coro_with_context(), cls._loop)
...
class TaskIdFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        from tomskit.celery.context import get_task_id
        task_id = get_task_id()
        if task_id:
...
def async_shared_task(*task_args, **task_kwargs):
    ...
    def decorator(func):
        ...
        def wrapper(task_instance, *args, **kwargs):
            tid = getattr(task_instance.request, "id", "")
            token = task_id_context_var.set(tid[-10])
            try:
                coro = func(*args, **kwargs)
                return AsyncRuntime.run(coro)
            finally:
                task_id_context_var.reset(token)
