...
AppType = Literal["fastapi", "celery", "alembic", "all"]
...
class SafeFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        try:
            ...
            at = getattr(record, "asctime", "")
            return f"[FORMAT ERROR] {at} | {record.levelname} | {record.getMessage()}"
...
class AsyncQueueHandler(logging.Handler):
    ...
    def __init__(
        ...
        formatter = SafeFormatter(fmt, datefmt)
...
class LoggingBase(ABC):
    ...
    def _file_logger(
        ...
        fmt_obj = SafeFormatter(fmt, self.config.LOG_DATE_FORMAT)
...
class FastAPILogging(LoggingBase):
    def _setup_loggers(self) -> None:
        loggers = [
            ("", self.config.LOG_NAME, self.config.LOG_FORMAT, self.config.LOG_LEVEL),
            ("uvicorn", self.config.LOG_ACCESS_NAME, self.config.LOG_ACCESS_FORMAT, "INFO"),
        ]
...
class CeleryLogging(LoggingBase):
    ...
    def _setup_loggers(self) -> None:
        loggers = [
            ("", self.config.LOG_CELERY_NAME, self.config.LOG_CELERY_FORMAT, self.config.LOG_CELERY_LEVEL),
            ("celery", self.config.LOG_CELERY_NAME, self.config.LOG_CELERY_FORMAT, self.config.LOG_CELERY_LEVEL),
        ]
        if self.config.LOG_SQL_ENABLED:
            loggers.append((
                "sqlalchemy",
                self.config.LOG_SQL_NAME,
                self.config.LOG_CELERY_FORMAT,
                self.config.LOG_SQL_LEVEL,
            ))
        for name, filename, fmt, level in loggers:
            self._file_logger(name, filename, fmt, level)
