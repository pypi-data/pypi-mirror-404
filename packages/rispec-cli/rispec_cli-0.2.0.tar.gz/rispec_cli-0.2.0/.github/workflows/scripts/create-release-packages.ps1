...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)\bmemory/', '.rispec/memory/'
    $Content = $Content -replace '(/?)\bscripts/', '.rispec/scripts/'
    $Content = $Content -replace '(/?)\btemplates/', '.rispec/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "rispec.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/rispec.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".rispec"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .rispec"
    }
    ...
    # Copy templates (excluding commands directory, rispecs directory, and vscode-settings.json)
    if (Test-Path "templates") {
        ...
        Get-ChildItem -Path "templates" -Recurse -File | Where-Object {
            ...
            $_.FullName -notmatch 'templates[/\\]rispecs[/\\]' -and 
        ...
        Write-Host "Copied templates -> .rispec/templates"
    }
    
    # Copy rispecs templates to rispecs/ directory in project root
    if (Test-Path "templates/rispecs") {
        $rispecsDestDir = Join-Path $baseDir "rispecs"
        Copy-Item -Path "templates/rispecs" -Destination $rispecsDestDir -Recurse -Force
        Write-Host "Copied rispecs templates -> rispecs/"
    }
    
    # Copy LLMS guidance files to src/llms/
    if (Test-Path "src/llms") {
        $llmsDestDir = Join-Path $baseDir "src/llms"
        New-Item -ItemType Directory -Path $llmsDestDir -Force | Out-Null
        Copy-Item -Path "src/llms/*" -Destination $llmsDestDir -Force
        Write-Host "Copied LLMS guidance -> src/llms/"
    }
