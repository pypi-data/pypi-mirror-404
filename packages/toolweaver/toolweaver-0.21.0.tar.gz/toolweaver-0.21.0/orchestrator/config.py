...
class ToolWeaverConfig:
    ...
    # Optional: Recursive Language Models (RLM)
    rlm_mode: str = "adaptive"  # off, adaptive, always
    rlm_context_threshold_tokens: int = 128000  # Use RLM if context >= threshold
    rlm_max_recursion_depth: int = 1  # Max recursive calls
    rlm_timeout_seconds: int = 60  # Timeout per recursive call
    rlm_debug: bool = False  # Enable detailed RLM logging
    ...
    def from_env(cls) -> "ToolWeaverConfig":
        ...
        if rlm_mode not in ["off", "adaptive", "always"]:
            rlm_mode = "adaptive"  # Default to adaptive if invalid

        try:
            rlm_context_threshold_tokens = int(os.getenv("RLM_CONTEXT_THRESHOLD_TOKENS", "128000"))
        except ValueError:
            rlm_context_threshold_tokens = 128000

        try:
            rlm_max_recursion_depth = int(os.getenv("RLM_MAX_RECURSION_DEPTH", "1"))
        except ValueError:
            rlm_max_recursion_depth = 1

        try:
            rlm_timeout_seconds = int(os.getenv("RLM_TIMEOUT_SECONDS", "60"))
        except ValueError:
            rlm_timeout_seconds = 60

        rlm_debug = os.getenv("RLM_DEBUG", "false").lower() == "true"
        ...
        return cls(
            ...
            rlm_mode=rlm_mode,
            rlm_context_threshold_tokens=rlm_context_threshold_tokens,
    ...
    def is_rlm_enabled(self) -> bool:
        """Check if RLM mode is enabled (adaptive or always)."""
        return self.rlm_mode in ["adaptive", "always"]
