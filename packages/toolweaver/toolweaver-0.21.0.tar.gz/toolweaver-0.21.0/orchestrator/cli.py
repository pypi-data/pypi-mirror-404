...
def browse_cmd(args: Any) -> int:
    ...
    for item in results:
        if detail_level == "name":
            ...
            if isinstance(item, dict):
                print(
                    f"  {item['name']:30} | {item['type']:12} | {item.get('description', '')[:50]}"
                )
            else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    browse_parser = subparsers.add_parser(
        "browse", help="Browse tools with progressive detail levels"
    )
    ...
    browse_parser.add_argument(
        "--limit", type=int, default=50, help="Pagination limit (default: 50)"
    )
    ...
    search_parser.add_argument(
        "--semantic",
        action="store_true",
        help="Use semantic search with vector embeddings (requires Qdrant)",
    )
    search_parser.add_argument(
        "--top-k", type=int, default=10, help="Number of results to return (default: 10)"
    )
    search_parser.add_argument(
        "--detail",
        choices=["name", "summary", "full"],
