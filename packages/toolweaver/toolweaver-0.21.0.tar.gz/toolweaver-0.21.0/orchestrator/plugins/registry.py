...
class PluginRegistry:
    ...
    def register(self, name: str, plugin: PluginProtocol, replace: bool = False) -> None:
        """
        Register a plugin.
        ...
        if not hasattr(plugin, "get_tools") or not callable(plugin.get_tools):
            raise InvalidPluginError(f"Plugin '{name}' must implement get_tools() method")

        if not hasattr(plugin, "execute") or not callable(plugin.execute):
            raise InvalidPluginError(f"Plugin '{name}' must implement execute() method")
        ...
        with self._lock:
            if name in self._plugins and not replace:
                raise PluginAlreadyRegisteredError(
                    f"Plugin '{name}' already registered. Use replace=True to override."
                )
    ...
    def _validate_plugin_tools(self, name: str, plugin: PluginProtocol) -> None:
        ...
        if not isinstance(tools, list):
            raise InvalidPluginError(
                f"Plugin '{name}' get_tools() must return a list, got {type(tools).__name__}"
            )
...
def register_plugin(name: str, plugin: PluginProtocol, replace: bool = False) -> None:
    """
