...
class ContextTracker:
    ...
    def print_summary(self) -> None:
        ...
        print("\nBreakdown:")
        print(
            f"  Tool definitions: {breakdown.tool_definitions:,} "
            f"({percentages['tool_definitions']:.1f}%)"
        )
        print(
            f"  Tool results:     {breakdown.tool_results:,} ({percentages['tool_results']:.1f}%)"
        )
        print(f"  User input:       {breakdown.user_input:,} ({percentages['user_input']:.1f}%)")
        print(
            f"  Model output:     {breakdown.model_output:,} ({percentages['model_output']:.1f}%)"
        )
        print("=" * 50 + "\n")
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "efficiency_score": (
                100 - percentages["tool_definitions"] if self.total_tokens > 0 else 0
            ),
        }
