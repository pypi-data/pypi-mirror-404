...
Configuration:
    ...
    1. TOOLWEAVER_LOG_LEVEL - Package-specific override (DEBUG, INFO, WARNING, ERROR, CRITICAL)
    2. LOG_LEVEL - Generic setting (applies to all logging, including LiteLLM)
    Default: INFO
...
Example .env:
    # All logging (ToolWeaver + LiteLLM)
    LOG_LEVEL=WARNING

    # Override just ToolWeaver (takes precedence over LOG_LEVEL)
    TOOLWEAVER_LOG_LEVEL=DEBUG
"""
...
def _is_silent_mode() -> bool:
    """
    Check if SILENT_MODE is enabled.

    When SILENT_MODE=true, all logging is suppressed (set to CRITICAL).
    This is useful for demos and samples that should output nothing but the result.

    Returns:
        True if SILENT_MODE is enabled, False otherwise
    """
    return os.getenv("SILENT_MODE", "").lower() in ("true", "1", "yes")
...
def _get_log_level_from_env() -> int:
    ...
    Priority:
    1. SILENT_MODE=true -> CRITICAL (suppress all logs)
    2. TOOLWEAVER_LOG_LEVEL (specific to this package)
    3. LOG_LEVEL (generic, applies to all logging)
    4. Default (INFO)

    Returns:
        Log level as int (logging.INFO, logging.DEBUG, etc.)
    """
    # Check SILENT_MODE first
    if _is_silent_mode():
        return logging.CRITICAL

    # Check TOOLWEAVER_LOG_LEVEL first (package-specific override)
    level_str = os.getenv("TOOLWEAVER_LOG_LEVEL")
    # Fall back to generic LOG_LEVEL if TOOLWEAVER_LOG_LEVEL not set
    ...
    result = level_map.get(level_str, logging.INFO)
    # Debug output to help troubleshoot
    if level_str not in level_map and os.getenv("_DEBUG_LOG_LEVEL"):
        print(f"[DEBUG] LOG_LEVEL lookup: '{level_str}' not in map, using default INFO", file=sys.stderr)
    return result
...
def _initialize_logging() -> None:
    ...
    - Timestamps in logs
    - Suppresses external library logs if SILENT_MODE is enabled
    """
    ...
    if _is_silent_mode():
        litellm_log_level = "CRITICAL"

    # Properly suppress LiteLLM using the correct logger name
    litellm_logger = logging.getLogger("LiteLLM")
    litellm_logger.setLevel(getattr(logging, litellm_log_level, logging.WARNING))

    # Also suppress other external library loggers if in silent mode
    if _is_silent_mode():
