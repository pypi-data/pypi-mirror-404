...
class TemplateSanitizer:
    ...
    LLM_INJECTION_PATTERNS: list[tuple[str, Pattern[str]]] = [
        ...
        (
            ...
            re.compile(r"disregard\s+(previous|prior|all|above|everything)", re.IGNORECASE),
        ),
        (
            "FORGET_INSTRUCTIONS",
            re.compile(r"forget\s+(previous|prior|all|above|everything)", re.IGNORECASE),
        ),
        # Role hijacking (avoid false positives like 'you are now viewing')
        (
            "ROLE_CHANGE",
            re.compile(
                r"(you\s+are\s+now\s+(a|an|the)\s+(admin|hacker|assistant|bot|system)|act\s+as\s+(a|an)|pretend\s+to\s+be|simulate\s+(a|an))",
                re.IGNORECASE,
            ),
        ),
        ...
        (
            "SYSTEM_PROMPT",
            re.compile(r"(system\s+prompt|system\s+message|initial\s+prompt)", re.IGNORECASE),
        ),
        (
            "REVEAL_PROMPT",
            re.compile(r"(show|reveal|display|print)\s+(the\s+)?(system\s+)?prompt", re.IGNORECASE),
        ),
        ...
        (
            "START_OVER",
            re.compile(
                r"(start\s+over|reset\s+context|new\s+conversation|clear\s+history)", re.IGNORECASE
            ),
        ),
        (
            "END_SESSION",
            re.compile(r"(end\s+of\s+prompt|end\s+of\s+instructions|###|---\s*end)", re.IGNORECASE),
        ),
        # Encoding tricks
