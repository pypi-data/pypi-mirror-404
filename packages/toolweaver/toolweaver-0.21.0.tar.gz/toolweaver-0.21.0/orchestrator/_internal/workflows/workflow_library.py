...
from orchestrator.observability import ToolCallMetric

from .workflow import WorkflowStep, WorkflowTemplate
...
class PatternDetector:
    ...
    def analyze_logs(
        self, logs: list[ToolCallMetric], max_sequence_length: int = 5
    ) -> list[ToolSequence]:
        """
        ...
        filtered_patterns.sort(key=lambda p: p.frequency * p.success_rate, reverse=True)
    ...
    def _extract_sequences(
        self, logs: list[ToolCallMetric], max_length: int
        ...
        sorted_logs = sorted(
            logs, key=lambda x: datetime.fromisoformat(x.timestamp.replace("Z", "+00:00"))
        )
        ...
        for length in range(2, min(len(tool_names) + 1, max_length + 1)):
            for i in range(len(tool_names) - length + 1):
                ...
                success = all(sorted_logs[j].success for j in range(i, i + length))

                # Calculate total duration (latency is in seconds, convert to ms)
                duration = sum(sorted_logs[j].latency * 1000 for j in range(i, i + length))
    ...
    def _calculate_pattern_stats(
        self, sequences: list[tuple[list[str], bool, float]], logs: list[ToolCallMetric]
    ) -> list[ToolSequence]:
        """Calculate statistics for each pattern"""
        pattern_data: dict[tuple[str, ...], dict[str, Any]] = defaultdict(
    ...
    def suggest_workflow(
        self, tools: list[str], patterns: list[ToolSequence]
...
class WorkflowLibrary:
    ...
    def _load_builtin_workflows(self) -> None:
        ...
        github_pr = WorkflowTemplate(
            ...
            metadata={"category": "github", "builtin": True},
        )
        ...
        slack_chain = WorkflowTemplate(
            ...
            steps=[
                WorkflowStep(
                    step_id="notify_dev",
                    tool_name="slack_send_message",
                    parameters={"channel": "#dev", "message": "{{message}}"},
                ),
                WorkflowStep(
                    step_id="notify_ops",
                    tool_name="slack_send_message",
                    parameters={"channel": "#ops", "message": "{{message}}"},
                ),
            ],
            metadata={"category": "slack", "builtin": True},
        )
    ...
    def search(
        self, query: str | None = None, category: str | None = None, tool_name: str | None = None
        ...
        # Filter by category
        if category:
            results = [w for w in results if w.metadata.get("category") == category]

        # Filter by tool name
        if tool_name:
            results = [w for w in results if any(step.tool_name == tool_name for step in w.steps)]
    ...
    def suggest_for_tools(self, tool_names: list[str]) -> list[WorkflowTemplate]:
        ...
        suggestions.sort(
            key=lambda w: len({step.tool_name for step in w.steps} & set(tool_names)), reverse=True
        )
    ...
    def save_to_disk(self) -> None:
        ...
        try:
            ...
            custom_workflows = [
                w for w in self.workflows.values() if not w.metadata.get("builtin", False)
            ]

            data = {"workflows": [self._serialize_workflow(w) for w in custom_workflows]}
