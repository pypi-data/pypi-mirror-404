...
async def function_call_worker(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not func:
        ...
        raise RuntimeError(
            f"Unknown function: {validated.name}. Available functions: {available or 'none'}"
        )
...
async def dispatch_step(
    ...
    for k, v in step.get("input", {}).items():
        if isinstance(v, str) and v.startswith("step:"):
            ...
            resolved_input[k] = [
                step_outputs.get(item.split("step:", 1)[1])
    ...
    if tool_type in mcp_client.tool_map:
        ...
        # Normalize responses to the underlying payload for consistent return type
        if hasattr(resp, "result"):
            result_value = cast(Any, resp.result)
            return cast(dict[str, Any], result_value if isinstance(result_value, dict) else {"result": result_value})
        if isinstance(resp, dict):
            return cast(dict[str, Any], resp.get("result", resp))
        # Convert non-dict response to dict
        return cast(dict[str, Any], {"result": resp})
        ...
        # Fallback: attempt to call tool directly from configured registry
        try:
            registry = get_tool_registry()
            func = registry.get_tool(tool_type)
            if func:
