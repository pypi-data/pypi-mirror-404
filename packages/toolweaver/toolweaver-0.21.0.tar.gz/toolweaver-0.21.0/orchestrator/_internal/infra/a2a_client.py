...
class A2AClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._idempotency_store: OrderedDict[str, tuple[float, AgentDelegationResponse]] = (
            OrderedDict()
        )
    ...
    async def discover_agents(
        ...
        *,
        ...
        if tags:
            agents = [a for a in agents if any(tag in a.metadata.get("tags", []) for tag in tags)]
    ...
    def _store_response(
        self, idempotency_key: str | None, response: AgentDelegationResponse
    ) -> None:
        if not idempotency_key:
            return
    ...
    async def _delegate_http(
        ...
        """Delegate to an HTTP agent endpoint using JSON-RPC 2.0 (A2A Spec)."""
        ...
        if auth_cfg and auth_cfg.get("type") in {"bearer", "api_key"}:
            ...
            if token:
                ...
                header_value = f"Bearer {token}" if auth_cfg.get("type") == "bearer" else token
                headers[header_name] = header_value

        # Construct JSON-RPC 2.0 Payload
        rpc_id = request.idempotency_key or str(int(time.time() * 1000))
        payload = {
            "jsonrpc": "2.0",
            "method": "execute_task",  # Standard A2A method
            "params": {
                "task": request.task,
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # A2A requires JSON response with "application/json" content type
                if not response.headers.get("Content-Type", "").startswith("application/json"):
                    raise RuntimeError(f"Invalid Content-Type from agent: {response.headers.get('Content-Type')}")

                parsed = await response.json()

                # Strict JSON-RPC 2.0 Validation
                if not isinstance(parsed, dict) or parsed.get("jsonrpc") != "2.0":
                    raise ValueError(f"Invalid JSON-RPC response from agent: {parsed}")

                if "error" in parsed and parsed["error"]:
                    err_code = parsed["error"].get("code", "unknown")
                    err_msg = parsed["error"].get("message", "Unknown Agent Error")
                    raise RuntimeError(f"Agent Error ({err_code}): {err_msg}")

                if "result" in parsed:
                    res = parsed["result"]
                    return res if isinstance(res, dict) else {"result": res}

                # Successful RPC must have 'result' member
                raise ValueError("JSON-RPC success response missing 'result' member")
    ...
    async def _delegate_http_stream(
        ...
        payload = {
            "jsonrpc": "2.0",
            "method": "execute_task_stream",  # Variant for streaming
            "params": {
                "task": request.task,
                "context": request.context,
                "metadata": request.metadata,
