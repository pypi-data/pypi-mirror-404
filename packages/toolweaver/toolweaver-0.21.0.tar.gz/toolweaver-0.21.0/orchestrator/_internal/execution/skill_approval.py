...
def _load_approvals() -> dict[str, ApprovalRecord]:
    ...
    try:
        ...
        return {name: ApprovalRecord(**record) for name, record in data.items()}
    except Exception:
        return {}
...
def review_pending_skills() -> None:
    ...
    if not pending:
        print("[OK] No pending skills to review")
        return
    ...
    for skill in pending:
        ...
        print(f"\nValidation: {'[OK] PASS' if is_valid else '[X] FAIL'}")
        if not is_valid:
        ...
        if action == "q":
            ...
            print(f"[OK] Approved: {skill.name}\n")
        else:
...
def promote_skill_to_git(skill_name: str, git_repo_path: str | None = None) -> bool:
    ...
    if skill_name not in approvals:
        print(f"[X] Skill '{skill_name}' is not approved. Run 'review' first.")
        return False

    skill = get_skill(skill_name)
    if not skill:
        print(f"[X] Skill '{skill_name}' not found")
        return False
    ...
    if not (git_repo / ".git").exists():
        print(f"[X] Not a Git repository: {git_repo}")
        return False
...
"""
    ...
    try:
        subprocess.run(
            ["git", "add", str(dest_file)], cwd=git_repo, check=True, capture_output=True
        )
        ...
        print(f"[OK] Promoted '{skill.name}' to Git")
        print(f"  Commit: {git_ref[:8]}")
        print(f"  File: {dest_file.relative_to(git_repo)}")
        return True

    except subprocess.CalledProcessError as e:
        print(f"[X] Git operation failed: {e.stderr}")
        return False
