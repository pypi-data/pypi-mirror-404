...
Storage:
  Uses centralized path management (AppPaths) for all directories
  - Approvals: ~/.toolweaver/approvals/
  - Audit logs: ~/.toolweaver/audit_logs/
  - Change tracking: ~/.toolweaver/change_tracking/
...
from orchestrator._internal.config.paths import (
    get_approvals_dir,
    get_audit_logs_dir,
    get_change_tracking_dir,
)
...
_APPROVAL_DIR = get_approvals_dir()
_AUDIT_DIR = get_audit_logs_dir()
_CHANGE_DIR = get_change_tracking_dir()
...
class ApprovalManager:
    ...
    def _load_approval_from_file(self, path: Path) -> ApprovalRequest | None:
        ...
        try:
            ...
            status = (
                ApprovalStatus[status_str.upper()]
                if isinstance(status_str, str)
                else ApprovalStatus(status_str)
            )
...
class ChangeTracker:
    ...
    @staticmethod
    def record_change(
        skill: Skill, version_from: str, changed_by: str, old_code: str = "", summary: str = ""
    ) -> SkillChange:
        """
        ...
        new_lines = (
            skill.code_path.splitlines(keepends=True) if isinstance(skill.code_path, str) else []
        )
...
def request_approval(
    ...
    return _get_approval_manager().request_approval(
        skill, submitter_id, submitter_name, description, target_org
    )
...
def provide_approval(
    ...
    return _get_approval_manager().provide_approval(
        approval_id, approver_id, approver_name, approver_role, approved, comment
    )


def add_approval_comment(
    approval_id: str, author_id: str, author_name: str, text: str, line_number: int | None = None
) -> Comment:
    """Add comment to approval."""
    return _get_approval_manager().add_comment(
        approval_id, author_id, author_name, text, line_number
    )
