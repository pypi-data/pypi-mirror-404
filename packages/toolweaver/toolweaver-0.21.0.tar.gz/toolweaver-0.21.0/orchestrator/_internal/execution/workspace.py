...
class WorkspaceSkill:
    ...
    def from_markdown(cls, md: str) -> "WorkspaceSkill":
        ...
        created_at = (
            created_match.group(1) if created_match else datetime.now(timezone.utc).isoformat()
        )

        updated_match = re.search(r"- Updated: (.+)", md)
        updated_at = (
            updated_match.group(1) if updated_match else datetime.now(timezone.utc).isoformat()
        )
...
class WorkspaceManager:
    ...
    def __init__(
        ...
        if workspace_root is None:
            from orchestrator._internal.config.paths import get_workspaces_dir

            workspace_root = get_workspaces_dir()
    ...
    def _check_quota(self, size: int, is_skill: bool = True) -> None:
        ...
        if total_files + 1 > self.quota.max_files:
            raise WorkspaceQuotaExceeded(f"File count limit exceeded: {self.quota.max_files} files")
