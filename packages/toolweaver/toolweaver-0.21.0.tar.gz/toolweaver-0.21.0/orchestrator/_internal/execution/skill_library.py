...
Stores skills using centralized path management (~/.toolweaver/skills).
Uses AppPaths for configuration instead of hardcoded paths.
...
from orchestrator._internal.config.paths import get_skills_dir
...
# Use centralized path management (AppPaths)
_ROOT = get_skills_dir()
...
class Skill:
    ...
    compatibility: dict[str, str] = field(default_factory=dict)  # engine/python requirements
...
def check_version_compatibility(required_range: str, actual_version: str) -> bool:
    """
    Check if a version satisfies a requirement range.

    Supported operators: >=, <=, >, <, ==, ~=
    Format: ">=1.0.0", "1.2.3", "~=1.2"

    Args:
        required_range: Requirement string
        actual_version: Version string to check
    ...
    op_match = re.match(r"([<>=~!]+)?\s*(.*)", required_range)
    if not op_match:
        return required_range == actual_version

    op, req_ver_str = op_match.groups()
    if not op:
        # Exact match required if no operator
        return required_range.strip() == actual_version.strip()

    req_ver = parse_version(req_ver_str)
    act_ver = parse_version(actual_version)

    # Pad shorter version with zeros for comparison
    length = max(len(req_ver), len(act_ver))
    req_ver = req_ver + (0,) * (length - len(req_ver))
    act_ver = act_ver + (0,) * (length - len(act_ver))
    ...
    if op == "==":
        ...
        return act_ver < req_ver
    elif op == "~=":  # Compatible release (same major/minor)
        # ~=1.2 means >=1.2 and <2.0
        # ~=1.2.3 means >=1.2.3 and <1.3.0
        parsed_req = parse_version(req_ver_str)
        if len(parsed_req) >= 3:
             # Patch level compatibility: >=1.2.3 and <1.3.0
             # We need to construct the next bracket manually
             req_base = list(parse_version(req_ver_str))
             req_base = req_base + [0] * (3 - len(req_base))

             upper = list(req_base)
             upper[1] += 1 # Increment minor
             upper[2] = 0
             upper_tuple = tuple(upper) + (0,) * (length - 3)
             return act_ver >= req_ver and act_ver < upper_tuple
        else:
             # Minor level compatibility: >=1.2 and <2.0
             req_base = list(parse_version(req_ver_str))
             req_base = req_base + [0] * (2 - len(req_base))

             upper = list(req_base)
             upper[0] += 1 # Increment major
             upper[1] = 0
             upper_tuple = tuple(upper) + (0,) * (length - 2)
             return act_ver >= req_ver and act_ver < upper_tuple
...
def _get_qdrant() -> tuple[Any, Any] | None:
    ...
    if not QDRANT_AVAILABLE:
        try:
            ...
            QdrantClient, Distance, PointStruct, VectorParams, SentenceTransformer = (
                QC,
                Dist,
                PS,
                VP,
            ...
            logger.debug(f"Qdrant/SentenceTransformers unavailable: {exc}")
            return None
    ...
    try:
        ...
        # Load embedding model from environment or default
        ...
        if backend == "sentence-transformers":
            ...
            # Other backends will be loaded lazily
        ...
        try:
            ...
            if VectorParams is None or Distance is None:
                raise ImportError("Qdrant models not available") from None
...
def _index_skill_in_qdrant(skill: Skill) -> None:
    ...
    try:
        ...
        if PointStruct is None:
            raise ImportError("PointStruct not available")
        client.upsert(
            collection_name=collection_name,
            points=[
                PointStruct(
...
def update_skill(
    ...
    *,
    ...
    skills = [
        s
        for s in skills
        if not (s.get("name") == safe_name and s.get("version") != current.version)
    ]
