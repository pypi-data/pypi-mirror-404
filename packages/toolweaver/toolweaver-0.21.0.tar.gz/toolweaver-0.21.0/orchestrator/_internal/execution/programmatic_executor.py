...
class ProgrammaticToolExecutor:
            ...
            try:
                ...
                async def _call_tool_proxy(
                    ...
                    try:
                        ...
                        )  # Use local start_ts, not dict access
                ...
                self.logger.warning(
                    f"{self.execution_id}: Failed to patch call_tool for stubs: {patch_exc}"
                )

        # Build execution environment / context
        exec_context = {"json": json, **(context or {})}
        ...
        try:
            if self.use_sandbox:
                ...
                try:
                    if self.stub_dir is not None:
                        ...
                        prelude_lines.append(
                            f"sys.path.insert(0, r'{str(self.stub_dir / 'tools')}')"
                        )
                        ...
                        prelude_lines.append(
                            "    _sd = r'" + str(self.stub_dir).replace("\\", "\\\\") + "'"
                        )
                ...
                with redirect_stdout(stdout_buffer):
                    result = await asyncio.wait_for(
                        self._exec_async(code, exec_globals), timeout=self.timeout
                    )
                execution_time = time.time() - start_time
                self.logger.info(
                    f"Programmatic execution {self.execution_id}: "
            ...
            if self.enable_stubs and patched_call_tool:
                try:
                    ...
                    self.logger.warning(
                        f"{self.execution_id}: Failed to restore call_tool: {restore_exc}"
                    )
    ...
    async def _execute_tool(self, tool_def: ToolDefinition, parameters: dict[str, Any]) -> Any:
        """
    ...
    def _validate_code_safety(self, parsed: ast.AST) -> None:
        ...
        forbidden_functions = [
            ...
            "delattr",
            "setattr",
            "getattr",  # Prevent reflection abuse
            "globals",
            "locals",
            "vars",
            "dir",  # Prevent introspection abuse
        ]
    ...
    def _get_safe_builtins(self) -> dict[str, Any]:
        ...
        # Note: __builtins__ can be dict or module depending on context
        builtins_obj: Any = __builtins__
        if isinstance(builtins_obj, dict):
            return {k: builtins_obj[k] for k in safe if k in builtins_obj}
        return {k: getattr(builtins_obj, k) for k in safe if hasattr(builtins_obj, k)}
    ...
    def _prepare_stub_environment(self) -> None:
        ...
        try:
            ...
            if self.stub_dir is None:
                ...
                stub_path = (
                    self.stub_dir if isinstance(self.stub_dir, Path) else Path(self.stub_dir)
                )
    ...
    def search_tools(self, query: str) -> list[str]:
        ...
        if not self.enable_stubs or not hasattr(self, "tool_filesystem"):
            ...
            return [
                name for name in self.tool_catalog.tools.keys() if query.lower() in name.lower()
            ]
...
async def execute_programmatic_code(
    code: str, tool_catalog: ToolCatalog, context: dict[str, Any] | None = None, timeout: int = 30
) -> dict[str, Any]:
    """
