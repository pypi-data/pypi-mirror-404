...
try:
    ...
    logging.warning(
        "OTLP metrics not available. Install: pip install opentelemetry-api opentelemetry-sdk opentelemetry-exporter-otlp-proto-http"
    )
...
def create_analytics_client(backend: str | None = None) -> Any:
    ...
    if backend == "otlp":
        ...
        raise ValueError(
            f"Unknown analytics backend: {backend}. Use 'sqlite', 'otlp', or 'prometheus'."
        )
