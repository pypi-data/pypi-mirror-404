...
class PrometheusMetrics:
    ...
    def _start_server(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Prometheus metrics server started: http://{self.config.host}:{self.config.port}/metrics"
            )
        except OSError as e:
            if "already in use" in str(e).lower():
                logger.warning(
                    f"Port {self.config.port} already in use, metrics server not started"
                )
            else:
                raise
    ...
    def _create_metrics(self) -> None:
        ...
        try:
            ...
            existing_executions = REGISTRY._names_to_collectors.get(
                "toolweaver_skill_executions_total"
            )
            if existing_executions:
                logger.debug("Reusing existing Prometheus metrics")
                self.skill_executions = cast(Counter, existing_executions)
                self.skill_success = cast(
                    Counter, REGISTRY._names_to_collectors["toolweaver_skill_success_total"]
                )
                self.skill_failures = cast(
                    Counter, REGISTRY._names_to_collectors["toolweaver_skill_failures_total"]
                )
                self.skill_latency = cast(
                    Histogram,
                    REGISTRY._names_to_collectors["toolweaver_skill_latency_milliseconds"],
                )
                self.skill_rating = cast(
                    Histogram, REGISTRY._names_to_collectors["toolweaver_skill_rating"]
                )
                self.skill_health_score = cast(
                    Gauge, REGISTRY._names_to_collectors["toolweaver_skill_health_score"]
                )
                return
        except Exception:
            pass
        ...
        self.skill_health_score = Gauge(
            "toolweaver_skill_health_score", "Skill health score (0-100)", ["skill_id"]
        )
    ...
    def record_skill_execution(
        ...
        logger.debug(
            f"Recorded execution: {skill_id} ({'success' if success else 'failure'}, {latency_ms}ms)"
        )

    def record_skill_rating(self, skill_id: str, rating: int, user_id: str | None = None) -> None:
        """
        ...
        labels = {"skill_id": skill_id, "user_id": user_id or "unknown"}
...
def create_prometheus_exporter(
    ...
    """
    if port or host:
        config = PrometheusConfig(port=port or 8000, host=host or "0.0.0.0")
...
if __name__ == "__main__":
    ...
    try:
        ...
        metrics.record_skill_execution(
            "test_skill", success=True, latency_ms=150, user_id="test_user"
        )
        metrics.record_skill_execution(
            "test_skill", success=True, latency_ms=200, user_id="test_user"
        )
        metrics.record_skill_execution(
            "test_skill", success=False, latency_ms=50, user_id="test_user"
        )
        ...
        print(f"\n[OK] Metrics exposed at: http://localhost:{metrics.config.port}/metrics")
        print("\nTest with:")
