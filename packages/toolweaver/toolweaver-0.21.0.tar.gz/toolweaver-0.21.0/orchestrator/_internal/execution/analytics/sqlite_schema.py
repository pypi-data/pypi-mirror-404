...
class SQLiteSchema:
    ...
    def __init__(self, db_path: str | None = None):
        ...
        """
        if db_path is None:
            from orchestrator._internal.config.paths import get_analytics_db

            path = get_analytics_db()
        else:
    ...
    def cleanup_old_data(self, retention_days: int) -> int:
        ...
        try:
            ...
            logger.info(
                f"Data cleanup complete: {total_deleted} rows deleted (retention: {retention_days} days)"
            )
