...
class SkillAnalytics:
        ...
        try:
            ...
            if row:
                result = {
                    ...
                    "last_used": row[7],
                    "success_rate": ((row[2] or 0) / (row[1] or 1) if row[1] else 0),
                    "period_days": days,
                }
                return result
            return {}
    ...
    def rate_skill(self, skill_id: str, rating: int, org_id: str | None = None) -> bool:
        ...
        try:
            ...
            cursor.execute("SELECT id FROM skill_metrics WHERE skill_id = ?", (skill_id,))
    ...
    def get_top_skills(
        ...
        """
        leaderboard = self.compute_leaderboard(MetricType.USAGE_COUNT, limit=limit, org_id=org_id)
        return leaderboard
