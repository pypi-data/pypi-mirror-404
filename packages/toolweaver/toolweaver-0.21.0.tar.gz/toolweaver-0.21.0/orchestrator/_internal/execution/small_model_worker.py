...
class SmallModelWorker:
    ...
    def _init_azure(self) -> None:
        ...
        if not self.use_azure_ad and not self.azure_key:
            raise ValueError(
                "AZURE_SMALL_MODEL_KEY required for Azure backend (or set AZURE_SMALL_MODEL_USE_AD=true)"
            )
    ...
    async def _generate_transformers(
        ...
        def _infer() -> str:
            ...
            response = decoded if isinstance(decoded, str) else "".join(decoded)

            # Extract only the assistant response
            if "<|assistant|>" in str(response):
                response = str(response).split("<|assistant|>")[-1].strip()
            elif "assistant" in str(response).lower():
...
JSON array of line items:"""
        ...
        for attempt in range(max_retries + 1):
            try:
                response = await self.generate(
                    prompt, system_prompt, max_tokens=1024, temperature=0.05
                )
                    ...
                    if end > start:
                        try:
                            ...
                            if isinstance(items, list):  # Verify it's actually an array
                                logger.info(
                                    f"Parsed {len(items)} line items with small model (attempt {attempt + 1})"
                                )
                                return items
    ...
    async def categorize_items(
        self, items: list[dict[str, Any]], max_retries: int = 2
    ) -> list[dict[str, Any]]:
        """
...
JSON array with category added:"""
        ...
        for attempt in range(max_retries + 1):
            try:
                response = await self.generate(
                    prompt, system_prompt, max_tokens=2048, temperature=0.05
                )
                ...
                if start >= 0 and end > start:
                    ...
                    categorized: list[dict[str, Any]] = (
                        categorized_raw if isinstance(categorized_raw, list) else []
                    )
                    logger.info(
                        f"Categorized {len(categorized)} items with small model (attempt {attempt + 1})"
                    )
                    return categorized
                else:
                    logger.warning(
                        f"No JSON array found in categorization response (attempt {attempt + 1})"
                    )
                    if attempt < max_retries:
                        continue
                    return items
    ...
    async def summarize(
        self, text: str, max_words: int = 100, focus: str | None = None
    ) -> str:
        """
        ...
        Returns:
            Summary text
        """
        system_prompt = f"Summarize the following text in under {max_words} words."
        if focus:
            system_prompt += f" Focus on: {focus}."

        prompt = f"Text to summarize:\n{text}\n\nSummary:"

        return await self.generate(
            prompt, system_prompt, max_tokens=max_words * 2, temperature=0.3
        )
