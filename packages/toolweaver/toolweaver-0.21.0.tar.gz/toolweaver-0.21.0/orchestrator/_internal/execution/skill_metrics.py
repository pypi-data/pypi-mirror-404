...
def record_skill_execution(skill_name: str, success: bool, latency_ms: float) -> None:
    """
    ...
    if skill_name not in metrics:
        metrics[skill_name] = SkillMetrics(
            skill_name=skill_name, created_at=datetime.now().isoformat(), ratings=[]
        )
...
def print_metrics_report(skill_name: str | None = None) -> None:
    ...
    if skill_name:
        ...
        # Success rates
        print("\n[OK] Most Reliable Skills (success rate):")
