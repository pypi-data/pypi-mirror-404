...
class AgentEvaluator:
    ...
    async def _evaluate_task(self, task: dict[str, Any]) -> TaskResult:
        """
        ...
        try:
            ...
            result = await self.orchestrator.execute(
                task["prompt"], context=task.get("context", {})
            )
    ...
    def _load_tasks(self, task_suite: str) -> list[dict[str, Any]]:
        """
    ...
    def _validate_result(self, result: Any, expected: dict[str, Any]) -> bool:
        """
    ...
    def compare_to_baseline(self, current: BenchmarkResults, baseline_name: str) -> dict[str, Any]:
        """
        ...
        comparison = {
            ...
            "context_usage": {
                ...
                "pct_change": (
                    ...
                    / baseline["avg_context"]
                    * 100
                    if baseline["avg_context"] > 0
                    else 0
                ),
            },
            "duration": {
    ...
    def print_comparison(self, comparison: dict[str, Any]) -> None:
        ...
        for metric, data in comparison.items():
            ...
            if metric == "context_usage":
                status = "[OK] Better" if data["change"] < 0 else "[X] Worse"
            elif metric == "duration":
                status = "[OK] Faster" if data["change"] < 0 else "[X] Slower"
