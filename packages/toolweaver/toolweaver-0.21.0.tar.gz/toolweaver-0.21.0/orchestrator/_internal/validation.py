...
def sanitize_string(
    ...
    if len(input_str) > max_length:
        raise InvalidInputError(f"Input too long: {len(input_str)} > {max_length} characters")
...
def sanitize_dict(
    input_dict: dict[str, Any], max_keys: int = 100, max_depth: int = 10
) -> dict[str, Any]:
    """
    ...
    if len(input_dict) > max_keys:
        raise InvalidInputError(f"Dictionary too large: {len(input_dict)} > {max_keys} keys")
...
def validate_file_path(
    ...
    if not allow_symlinks and path.is_symlink():
        raise PathTraversalError(f"Symbolic links not allowed: {file_path}")
...
def validate_url(
    url: str, allowed_schemes: set[str] | None = None, block_private_ips: bool = True
) -> str:
    """
    ...
    if parsed.scheme not in allowed_schemes:
        raise UnsafeURLError(
            f"Unsafe URL scheme: {parsed.scheme}\nAllowed schemes: {', '.join(allowed_schemes)}"
        )
...
def validate_code(
    code: str, allow_imports: bool = False, allow_file_io: bool = False, max_length: int = 50000
) -> str:
    """
    ...
    if len(code) > max_length:
        raise InvalidInputError(f"Code too long: {len(code)} > {max_length} characters")
    ...
    for node in ast.walk(tree):
        ...
        if not allow_imports and node_type in {ast.Import, ast.ImportFrom}:
            raise InvalidCodeError("Dangerous construct: import statement not allowed")
        ...
        if isinstance(node, ast.Call):
            ...
            if func_name in DANGEROUS_FUNCTIONS:
                if not allow_file_io and func_name in {"open", "file"}:
                    ...
                    raise InvalidCodeError(f"Dangerous function call: {func_name}")
...
def validate_tool_input(
    params: dict[str, Any], schema: type[BaseModel] | None = None, sanitize: bool = True
) -> dict[str, Any]:
    """
