...
class AgentDiscoverer(ToolDiscoveryService):
    ...
    def _coerce_agent(self, raw: object) -> AgentCapability:
        """Coerce dicts and compatible objects into AgentCapability instances."""
        # Accept already-constructed AgentCapability or any object with matching attributes
        if isinstance(raw, AgentCapability):
            return raw
        if hasattr(raw, "agent_id") and hasattr(raw, "name") and hasattr(raw, "endpoint"):
            # Assume it's a compatible AgentCapability-like object
        ...
        if isinstance(raw, dict):
            ...
            supports_http_streaming: bool = any(
                getattr(c, "supports_http_streaming", False) for c in caps
            )
    ...
    def _agent_to_tool_definition(self, agent: AgentCapability) -> ToolDefinition:
        ...
        return ToolDefinition(
            ...
            metadata={
                ...
                "supports_http_streaming": bool(
                    agent.metadata.get("supports_http_streaming", False)
                ),
