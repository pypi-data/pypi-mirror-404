...
from .._internal.infra.mcp_auth import MCPAuthConfig, MCPAuthManager
from ..plugins.registry import register_plugin
from ..shared.models import ToolDefinition


def _merge_headers(
    headers: dict[str, str] | None, auth: MCPAuthConfig | None
) -> dict[str, str]:
    """Combine user headers with auth-generated headers (auth wins on conflicts)."""

    combined = dict(headers or {})
    auth_headers = MCPAuthManager().get_headers(auth)
    combined.update(auth_headers)
    return combined
...
class MCPHttpAdapterPlugin:
    ...
    def __init__(
        ...
        *,
        ...
        self.headers: dict[str, str] = _merge_headers(headers, auth)
    ...
    async def execute_stream(self, tool_name: str, params: dict[str, Any], protocol: str = "http") -> Any:
    ...
    async def _stream_http(self, tool_name: str, params: dict[str, Any]) -> Any:
    ...
    async def _stream_sse(self, tool_name: str, params: dict[str, Any]) -> Any:
    ...
    async def _stream_websocket(self, tool_name: str, params: dict[str, Any]) -> Any:
...
def register_mcp_http_adapter(
    ...
    *,
    ...
    plugin = MCPHttpAdapterPlugin(
        base_url, headers=headers, auth=auth, timeout_s=timeout_s, verify_ssl=verify_ssl
    )
                    ...
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        try:
                            ...
                            if isinstance(payload, dict) and (
                                payload.get("result") is not None
...
def register_mcp_ws_adapter(
    ...
    *,
    ...
    plugin = MCPWebSocketAdapterPlugin(
        base_url, headers=headers, auth=auth, timeout_s=timeout_s, verify_ssl=verify_ssl
    )
    register_plugin(name, plugin)
    return plugin
...
class MCPJsonRpcHttpAdapterPlugin:
    ...
    def __init__(
        ...
        *,
        ...
        self.headers: dict[str, str] = _merge_headers(base_headers, auth)
        self.timeout_s = timeout_s
        self.verify_ssl = verify_ssl
        self._request_id = 0
...
def register_mcp_jsonrpc_http_adapter(
    ...
    *,
    ...
    plugin = MCPJsonRpcHttpAdapterPlugin(
        base_url, headers=headers, auth=auth, timeout_s=timeout_s, verify_ssl=verify_ssl
    )
    register_plugin(name, plugin)
    return plugin
