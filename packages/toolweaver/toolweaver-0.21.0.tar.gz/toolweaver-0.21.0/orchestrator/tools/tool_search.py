...
class ToolSearchEngine:
    ...
    def __init__(
        self,
        embedding_model: str | None = None,  # Fast, 384-dim, 80MB (default all-MiniLM-L6-v2)
        ...
        self.embedding_model: Any | None = None  # Lazy load
        self.bm25_weight = bm25_weight
        self.embedding_weight = embedding_weight

        # Set up cache directory - use centralized paths
        if cache_dir is None:
            from .._internal.config.paths import get_search_cache_dir

            cache_dir = get_search_cache_dir()
        ...
        if self.embedding_model is None:
            try:
                from sentence_transformers import (
                    SentenceTransformer as ST,  # local import to avoid heavy startup
                )
            except Exception as exc:
                logger.warning(
                    "SentenceTransformer not available; embedding search disabled (%s)", exc
                )
                return

            logger.info(f"Loading embedding model: {self.embedding_model_name}")
            self.embedding_model = ST(self.embedding_model_name)
            logger.info(
                f"Embedding model loaded (dim={self.embedding_model.get_sentence_embedding_dimension()})"
            )
            global SENTENCE_AVAILABLE
    ...
    def search(
        ...
        *,
        ...
        for i, tool in enumerate(tools):
            combined_score = (
                self.bm25_weight * bm25_scores[i] + self.embedding_weight * embedding_scores[i]
            )
    ...
    def _bm25_search(
        self, query: str, tools: list[ToolDefinition]
    ...
    def _load_from_cache(self, cache_key: str) -> list[tuple[ToolDefinition, float]] | None:
    ...
    def explain_results(self, query: str, results: list[tuple[ToolDefinition, float]]) -> str:
        """
