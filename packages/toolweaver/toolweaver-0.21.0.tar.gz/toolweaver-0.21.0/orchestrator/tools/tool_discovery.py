    ...
    async def discover(self) -> dict[str, ToolDefinition]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.registry_url, timeout=ClientTimeout(total=10)) as resp:
...
class ToolDiscoveryOrchestrator:
    ...
    def __init__(self, cache_dir: Path | None = None, cache_ttl_hours: int = 24):
        ...
        if cache_dir is None:
            from orchestrator._internal.config.paths import get_cache_dir
    ...
    async def discover_all(
        self, use_cache: bool = True, cache_ttl_hours: int | None = None
    ) -> ToolCatalog:
        """
        ...
        catalog = ToolCatalog(discovered_at=start_time, source="orchestrator", version="1.0")
        ...
        print(
            f"Discovery complete: {metrics.total_tools_found} tools found in {metrics.discovery_duration_ms:.0f}ms"
        )
