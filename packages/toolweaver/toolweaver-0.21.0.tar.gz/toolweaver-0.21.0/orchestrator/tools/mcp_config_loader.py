...
def _resolve_config_path(config_path: str | os.PathLike[str] | None) -> str | None:
    ...
    if os.path.exists(default_claude):
        ...
        for k, v in raw.items():
            if isinstance(v, str) and v.startswith("${") and v.endswith("}"):
                ...
                if env_val:
                    ...
                    if k.lower() == "authorization" and not env_val.lower().startswith(
                        ("bearer ", "token ")
                    ):
...
def _extract_auth(entry: dict[str, Any]) -> MCPAuthConfig | None:
    """Parse optional auth block supporting bearer/api_key/basic/none."""

    raw = entry.get("auth")
    if not isinstance(raw, dict):
        return None

    def _get(keys: list[str], default: str | None = None) -> str | None:
        for key in keys:
            val = raw.get(key)
            if isinstance(val, str) and val:
                return val
    ...
    auth_type = _get(["type", "authType"], "none")
    token_env = _get(["token_env", "tokenEnv", "tokenEnvVar", "token"])  # env name
    header_name = _get(["header", "header_name", "headerName"], "Authorization")
    username_env = _get(["username_env", "usernameEnv"], None)
    password_env = _get(["password_env", "passwordEnv"], None)

    try:
        # Cast auth_type to appropriate literal type
        valid_auth_type: str = auth_type if auth_type else "none"
        if valid_auth_type not in ("none", "bearer", "api_key", "basic"):
            valid_auth_type = "none"

        return MCPAuthConfig(
            type=valid_auth_type,  # type: ignore[arg-type]
            token_env=token_env,
            header_name=header_name if header_name else "Authorization",
