...
class ErrorRecoveryExecutor:
    ...
    async def execute_with_recovery(
        ...
        fallback_tools: list[Callable[..., Any]] | None = None,
    ) -> RecoveryResult:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning(f"{tool_name} attempt {attempt + 1} failed: {e}")
                if attempt < max_retries:
                    ...
                    # Try fallback tools
                    return await self._try_fallback_tools(fallback_tools, args, kwargs, attempt + 1)
    ...
    async def _try_fallback_tools(
        self,
        fallback_tools: list[Callable[..., Any]],
        args: tuple[Any, ...],
        kwargs: dict[str, Any],
