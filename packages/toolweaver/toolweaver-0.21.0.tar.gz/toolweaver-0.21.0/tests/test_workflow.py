...
class TestWorkflowStep:
    ...
    def test_create_simple_step(self) -> None:
        ...
        step = WorkflowStep(
            step_id="test_step", tool_name="test_tool", parameters={"param1": "value1"}
        )
    ...
    def test_create_step_with_dependencies(self) -> None:
        ...
        step = WorkflowStep(step_id="step2", tool_name="tool2", parameters={}, depends_on=["step1"])

        assert step.depends_on == ["step1"]

    def test_step_requires_id(self) -> None:
        """Test that step_id is required"""
        with pytest.raises(ValueError, match="step_id is required"):
            WorkflowStep(step_id="", tool_name="tool", parameters={})

    def test_step_requires_tool_name(self) -> None:
        """Test that tool_name is required"""
        with pytest.raises(ValueError, match="tool_name is required"):
            WorkflowStep(step_id="step1", tool_name="", parameters={})
...
class TestWorkflowTemplate:
    ...
    def test_create_simple_workflow(self) -> None:
        ...
        workflow = WorkflowTemplate(
            ...
            steps=[WorkflowStep(step_id="step1", tool_name="tool1", parameters={})],
        )
    ...
    def test_create_workflow_with_dependencies(self) -> None:
        ...
        workflow = WorkflowTemplate(
            ...
            steps=[
                ...
                WorkflowStep(
                    step_id="step2", tool_name="tool2", parameters={}, depends_on=["step1"]
                ),
            ],
        )
    ...
    def test_workflow_requires_steps(self) -> None:
        ...
        with pytest.raises(ValueError, match="must have at least one step"):
            WorkflowTemplate(name="empty", description="Empty workflow", steps=[])

    def test_workflow_validates_unique_step_ids(self) -> None:
    ...
    def test_workflow_validates_dependencies_exist(self) -> None:
...
class TestWorkflowContext:
    ...
    def test_create_context_with_variables(self) -> None:
    ...
    def test_substitute_step_result(self) -> None:
    ...
    def test_substitute_nested_field(self) -> None:
    ...
    def test_substitute_dict(self) -> None:
        ...
        template = {"repository": "{{repo}}", "branch": "{{branch}}", "static": "value"}

        result = context.substitute(template)

        assert result == {"repository": "myrepo", "branch": "main", "static": "value"}
...
class TestDependencyResolution:
    ...
    async def test_simple_linear_workflow(self) -> None:
        ...
        workflow = WorkflowTemplate(
            ...
            steps=[
                ...
                WorkflowStep(
                    step_id="step3", tool_name="tool3", parameters={}, depends_on=["step2"]
                ),
            ],
        )
    ...
    async def test_parallel_workflow(self) -> None:
        ...
        workflow = WorkflowTemplate(
            ...
            steps=[
                ...
                WorkflowStep(
                    step_id="step3", tool_name="tool3", parameters={}, depends_on=["step1"]
                ),
                WorkflowStep(
                    step_id="step4", tool_name="tool4", parameters={}, depends_on=["step2", "step3"]
                ),
            ],
        )
    ...
    async def test_circular_dependency_detection(self) -> None:
        ...
        workflow = WorkflowTemplate(
            ...
            steps=[
                WorkflowStep(
                    step_id="step1", tool_name="tool1", parameters={}, depends_on=["step2"]
                ),
                WorkflowStep(
                    step_id="step2", tool_name="tool2", parameters={}, depends_on=["step1"]
                ),
...
class TestWorkflowExecution:
    ...
    async def test_execute_simple_workflow(self) -> None:
        ...
        workflow = WorkflowTemplate(
            ...
            steps=[
                WorkflowStep(
                    step_id="step1", tool_name="tool1", parameters={"input": "{{start_value}}"}
                ),
                WorkflowStep(
                    step_id="step2",
                    tool_name="tool2",
    ...
    @pytest.mark.asyncio
    async def test_execute_workflow_with_variable_substitution(self) -> None:
        ...
        executor = WorkflowExecutor()
        context = await executor.execute(workflow, {"repo_name": "myrepo", "branch_name": "main"})
    ...
    @pytest.mark.asyncio
    async def test_conditional_step_execution(self) -> None:
    ...
    async def test_parallel_execution_faster_than_sequential(self) -> None:
        ...
        class MockToolExecutor:
            async def execute(self, tool_name: str, parameters: dict[str, Any]) -> dict[str, str]:
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_step_failure_stops_dependent_steps(self) -> None:
        """Test that failed step prevents dependent steps from running"""

        class FailingExecutor:
            async def execute(self, tool_name: str, parameters: dict[str, Any]) -> dict[str, bool]:
