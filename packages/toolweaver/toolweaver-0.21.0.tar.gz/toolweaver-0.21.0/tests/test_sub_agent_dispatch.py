...
@pytest.mark.asyncio
async def test_basic_dispatch_formats_templates() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
    ...
    results = await dispatch_agents(
        "Hello {name}", args, agent_name="helper", model="test", executor=exec_fn
    )
...
@pytest.mark.asyncio
async def test_respects_max_parallel() -> None:
    max_parallel = 2
    active = 0
    peak = 0
...
async def test_timeout_handling() -> None:
    ...
    args = [{"i": 1}]
    results = await dispatch_agents("Slow {i}", args, timeout_per_agent=0, executor=exec_fn)
...
@pytest.mark.asyncio
async def test_idempotency_cache_hits() -> None:
    ...
    assert results[1].duration_ms < 10.0, (
        f"Cache hit should be fast, got {results[1].duration_ms}ms"
    )


@pytest.mark.asyncio
async def test_template_sanitization_applied() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
        return {"output": prompt}
...
@pytest.mark.asyncio
async def test_pii_response_filtering() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
...
@pytest.mark.asyncio
async def test_rate_limiter_enforced_time_window() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
        return {"output": prompt}
...
@pytest.mark.asyncio
async def test_min_success_threshold_enforced() -> None:
...
@pytest.mark.asyncio
async def test_cost_limit_exceeded_returns_error() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
...
@pytest.mark.asyncio
async def test_custom_aggregation_function() -> None:
    get_global_cache().clear()
    ...
    args = [{"i": 1}, {"i": 3}, {"i": 2}]
    agg = await dispatch_agents(
        "{i}", args, executor=exec_fn, aggregate_fn=lambda res: max(r.output["score"] for r in res)
    )
    assert agg == 3


@pytest.mark.asyncio
async def test_large_dispatch_100_agents() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
        return {"output": prompt}
...
@pytest.mark.asyncio
async def test_a2a_style_executor_integration() -> None:
    async def exec_fn(prompt: str, args: dict[str, Any], agent_name: str, model: str) -> dict[str, Any]:
...
@pytest.mark.asyncio
async def test_min_success_with_aggregation_passes_and_returns_majority() -> None:
    get_global_cache().clear()
...
def test_secrets_redactor_auto_installed(caplog: Any) -> None:
...
def test_aggregation_helpers() -> None:
    results = [
        SubAgentResult(
            task_args={}, output={"score": 1, "value": "a"}, error=None, duration_ms=1, success=True
        ),
        SubAgentResult(
            task_args={}, output={"score": 3, "value": "b"}, error=None, duration_ms=1, success=True
        ),
        SubAgentResult(
            task_args={}, output={"score": 2, "value": "b"}, error=None, duration_ms=1, success=True
        ),
    ]
