...
def setup_tools() -> Generator[None, None, None]:
    """Create sample tools for testing."""
    with patch.dict(os.environ, {"ORCHESTRATOR_TOOL_REGISTRY": "local"}, clear=False):
        import orchestrator.tools.tool_search_tool as tst
        from orchestrator._internal.backends.tools import get_tool_registry

        # Clear both registries AND search engine cache to ensure clean state
        plugin_registry = get_registry()
        plugin_registry.clear()

        # Get the configured tool registry (respects ORCHESTRATOR_TOOL_REGISTRY env var)
        tool_registry = get_tool_registry()
        tool_registry.clear()  # type: ignore[attr-defined]

        # Clear search engine cache (critical to avoid stale results)
        tst._search_engine = None
        tst._full_catalog = None
        ...
        # Clear search engine cache to prevent cross-test contamination
        tst._search_engine = None
        tst._full_catalog = None


def test_browse_tools_name_level() -> None:
...
def test_browse_tools_summary_level() -> None:
...
def test_browse_tools_full_level() -> None:
...
def test_browse_tools_pagination() -> None:
    ...
    # Ensure no overlap
    names1 = {item["name"] for item in page1}  # type: ignore[index]
    names2 = {item["name"] for item in page2}  # type: ignore[index]
    assert len(names1 & names2) == 0


def test_browse_tools_with_domain_filter() -> None:
...
def test_browse_tools_with_type_filter() -> None:
    ...
    assert all(t["type"] == "function" for t in function_tools)  # type: ignore[index]


def test_search_tools_with_detail_level_name() -> None:
...
def test_search_tools_with_detail_level_summary() -> None:
...
def test_search_tools_with_detail_level_full() -> None:
...
def test_search_tools_without_detail_level() -> None:
...
def test_get_tool_info_name_level() -> None:
...
def test_get_tool_info_summary_level() -> None:
...
def test_get_tool_info_with_examples() -> None:
...
def test_get_tool_info_without_examples() -> None:
...
def test_browse_tools_invalid_detail_level() -> None:
...
def test_browse_tools_negative_offset() -> None:
...
def test_browse_tools_negative_limit() -> None:
    """Test browse_tools raises on negative limit."""
    with pytest.raises(ValueError, match="offset and limit must be non-negative"):
        browse_tools(limit=-1)


def test_progressive_workflow() -> None:
    ...
    assert tool["parameter_count"] == 3  # type: ignore[index]

    # 3. Get full details for selected tool
    full_info = get_tool_info(tool["name"], detail_level="full")  # type: ignore[index]
    assert isinstance(full_info, ToolDefinition)
    assert hasattr(full_info, "parameters")
    assert len(full_info.parameters) == 3


def test_detail_levels_token_reduction() -> None:
