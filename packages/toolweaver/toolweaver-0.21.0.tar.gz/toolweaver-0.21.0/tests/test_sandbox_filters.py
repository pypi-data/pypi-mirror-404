...
class TestPIITokenizer:
    ...
    def test_tokenize_email(self) -> None:
    ...
    def test_tokenize_phone(self) -> None:
        """Test phone number tokenization."""
        tokenizer = PIITokenizer()
        data = {"phone": "555-123-4567", "contact": "Call me at (555) 987-6543"}
    ...
    def test_tokenize_ssn(self) -> None:
    ...
    def test_tokenize_credit_card(self) -> None:
    ...
    def test_tokenize_nested_structures(self) -> None:
...
class TestDataFilter:
    ...
    def test_filter_large_list(self) -> None:
    ...
    def test_filter_long_strings(self) -> None:
    ...
    def test_filter_preserves_structure(self) -> None:
    ...
    def test_filter_nested_data(self) -> None:
        """Test filtering nested structures."""
        config = FilterConfig(max_rows=5)
        filter = DataFilter(config)

        data = {"records": [{"id": i, "data": "x" * 10} for i in range(20)]}
    ...
    def test_filter_generates_summary(self) -> None:
    ...
    def test_filter_no_truncation(self) -> None:
    ...
    def test_filter_max_bytes(self) -> None:
    ...
    def test_filter_deep_nesting(self) -> None:
...
class TestFilterAndTokenize:
    ...
    def test_filter_and_tokenize_combined(self) -> None:
        """Test applying both filtering and tokenization."""
        filter_config = FilterConfig(max_rows=10)

        data = {"users": [{"email": f"user{i}@example.com", "data": "x" * 100} for i in range(50)]}

        result = filter_and_tokenize(data, filter_config=filter_config, tokenize_pii=True)
    ...
    def test_filter_and_tokenize_no_pii(self) -> None:
        """Test filtering without PII tokenization."""
        filter_config = FilterConfig(max_rows=5)

        data = list(range(20))

        result = filter_and_tokenize(data, filter_config=filter_config, tokenize_pii=False)

        assert result["truncated"] is True
        assert "token_map" not in result
        assert "pii_detected" not in result

    def test_filter_and_tokenize_order(self) -> None:
        """Test that filtering happens before tokenization."""
        filter_config = FilterConfig(max_rows=5)

        data = [{"email": f"user{i}@example.com"} for i in range(20)]
    ...
    def test_filter_and_tokenize_realistic_scenario(self) -> None:
        """Test realistic scenario with database query results."""
        filter_config = FilterConfig(max_rows=100, max_string_length=500, summarize_truncated=True)

        tokenize_config = TokenizationConfig(enabled_types={PIIType.EMAIL, PIIType.PHONE})
        ...
        result = filter_and_tokenize(
            data, filter_config=filter_config, tokenize_config=tokenize_config, tokenize_pii=True
        )
