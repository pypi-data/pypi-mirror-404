...
@pytest.mark.asyncio
async def test_basic_linear_chain_execution() -> None:
    async def step1_tool(input_value: str) -> dict[str, Any]:
        return {"output": input_value.upper(), "length": len(input_value)}

    async def step2_tool(output: str) -> dict[str, Any]:
        return {"result": output + "!", "processed": True}

    executor = CompositionExecutor(
        tool_resolver=lambda ref: {"step1": step1_tool, "step2": step2_tool}.get(ref)  # type: ignore[arg-type]
    )
...
@pytest.mark.asyncio
async def test_chain_validation_empty() -> None:
    executor = CompositionExecutor()
...
async def test_step_timeout_handling() -> None:
    async def slow_tool(value: str) -> dict[str, Any]:
        await asyncio.sleep(1.0)
        return {"result": value}

    executor = CompositionExecutor(tool_resolver=lambda ref: slow_tool if ref == "slow" else None)  # type: ignore[arg-type]

    chain = CompositionChain(name="timeout_test").add_step(
        CompositionStep(
            name="slow_step",
            tool_ref="slow",
...
@pytest.mark.asyncio
async def test_step_error_propagation_raise() -> None:
    async def fail_tool(value: str) -> dict[str, Any]:
        raise ValueError("Expected failure")

    executor = CompositionExecutor(tool_resolver=lambda ref: fail_tool)  # type: ignore[arg-type]

    chain = CompositionChain(name="error_test").add_step(
        CompositionStep(
            name="fail_step",
            tool_ref="fail",
            input_schema={"value": str},
...
@pytest.mark.asyncio
async def test_step_error_continue() -> None:
    async def fail_tool(value: str) -> dict[str, Any]:
        raise ValueError("Expected failure")

    async def pass_tool(value: str | None = None) -> dict[str, Any]:
        return {"result": "recovered"}

    async def resolver(ref: str) -> Any:
        return {"fail": fail_tool, "pass": pass_tool}.get(ref)

    executor = CompositionExecutor(tool_resolver=resolver)  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_parameter_mapping_passthrough() -> None:
...
@pytest.mark.asyncio
async def test_parameter_mapping_explicit() -> None:
...
@pytest.mark.asyncio
async def test_composite_tool_decorator() -> None:
    @composite_tool(name="test_comp", description="Test composition")
    def my_chain() -> CompositionChain:
...
def test_composition_step_validates_on_error() -> None:
...
async def test_retry_on_transient_failure() -> None:
    ...
    executor = CompositionExecutor(tool_resolver=lambda ref: flaky_tool if ref == "flaky" else None)  # type: ignore[arg-type]

    chain = CompositionChain(name="retry_test").add_step(
        CompositionStep(
            name="flaky_step",
            tool_ref="flaky",
            input_schema={"value": str},
...
@pytest.mark.asyncio
async def test_sync_tool_support() -> None:
    def sync_tool(value: str) -> dict[str, Any]:
        return {"result": value.upper()}

    executor = CompositionExecutor(tool_resolver=lambda ref: sync_tool)  # type: ignore[arg-type]

    chain = CompositionChain(name="sync_test").add_step(
        CompositionStep(
            name="sync_step",
            tool_ref="sync",
            input_schema={"value": str},
...
@pytest.mark.asyncio
async def test_three_step_chain() -> None:
    ...
    async def extract(body: str) -> dict[str, Any]:
