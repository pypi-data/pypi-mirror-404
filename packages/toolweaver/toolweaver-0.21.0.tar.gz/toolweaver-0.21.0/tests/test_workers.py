...
from orchestrator._internal.dispatch.workers import (
    _mock_ocr,
...
class TestReceiptOCRWorker:
    ...
    @pytest.mark.asyncio
    async def test_mock_ocr_mode(self, monkeypatch: Any) -> None:
        """Test OCR in mock mode"""
        monkeypatch.setenv("OCR_MODE", "mock")

        payload: dict[str, str] = {"image_uri": "https://example.com/receipt.jpg"}
    ...
    @pytest.mark.asyncio
    async def test_mock_ocr_contains_receipt_data(self, monkeypatch: Any) -> None:
        """Test that mock OCR returns realistic receipt data"""
        monkeypatch.setenv("OCR_MODE", "mock")

        payload: dict[str, str] = {"image_uri": "https://example.com/test.jpg"}
        result = await receipt_ocr_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_mock_ocr_includes_image_uri(self, monkeypatch: Any) -> None:
        """Test that mock OCR includes the image URI in response"""
        monkeypatch.setenv("OCR_MODE", "mock")

        image_uri = "https://example.com/my-receipt.jpg"
        payload: dict[str, str] = {"image_uri": image_uri}
        result = await receipt_ocr_worker(payload)

        assert image_uri in result["text"]

    @pytest.mark.asyncio
    async def test_azure_fallback_without_endpoint(self, monkeypatch: Any) -> None:
    ...
    def test_mock_ocr_direct_call(self) -> None:
...
class TestLineItemParserWorker:
    ...
    @pytest.mark.asyncio
    async def test_parse_coffee_and_bagel(self, monkeypatch: Any) -> None:
        """Test parsing simple receipt text"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, str] = {"ocr_text": "Coffee Shop\n1x Coffee 3.50\n2x Bagel 5.00\nTotal: 8.50"}
    ...
    @pytest.mark.asyncio
    async def test_parse_with_text_field(self, monkeypatch: Any) -> None:
        """Test parsing when using 'text' field instead of 'ocr_text'"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, str] = {"text": "Coffee 3.50\nBagel 2.50"}
        result = await line_item_parser_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_parse_empty_text(self, monkeypatch: Any) -> None:
        """Test parsing with empty text"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, str] = {"ocr_text": ""}
        result = await line_item_parser_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_parse_dict_input(self, monkeypatch: Any) -> None:
        """Test parsing when ocr_text is a dict (from step reference)"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, dict[str, str]] = {"ocr_text": {"text": "Coffee 3.50\nBagel 5.00"}}
        result = await line_item_parser_worker(payload)

        assert "items" in result

    @pytest.mark.asyncio
    async def test_parse_skips_total_lines(self, monkeypatch: Any) -> None:
        """Test that TOTAL lines are skipped"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, str] = {"ocr_text": "Coffee 3.50\nTOTAL: 3.50"}
        result = await line_item_parser_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_parsed_items_structure(self, monkeypatch: Any) -> None:
...
class TestExpenseCategorizerWorker:
    ...
    @pytest.mark.asyncio
    async def test_categorize_coffee_items(self, monkeypatch: Any) -> None:
        """Test categorizing coffee items"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        items = [{"description": "Coffee", "quantity": 1, "total": 3.5}]
        payload: dict[str, list[Any]] = {"items": items}
    ...
    @pytest.mark.asyncio
    async def test_categorize_bagel_items(self, monkeypatch: Any) -> None:
        """Test categorizing bagel items"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        items = [{"description": "Bagel", "quantity": 2, "total": 5.0}]
        payload: dict[str, list[Any]] = {"items": items}
        result = await expense_categorizer_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_categorize_multiple_items(self, monkeypatch: Any) -> None:
    ...
    @pytest.mark.asyncio
    async def test_categorize_empty_items(self, monkeypatch: Any) -> None:
        """Test categorizing with empty items list"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, list[Any]] = {"items": []}
        result = await expense_categorizer_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_categorize_dict_input(self, monkeypatch: Any) -> None:
        """Test categorizing when items is nested in dict (from step reference)"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        payload: dict[str, dict[str, list[dict[str, Any]]]] = {"items": {"items": [{"description": "Coffee", "quantity": 1, "total": 3.5}]}}
        result = await expense_categorizer_worker(payload)

        assert "categorized" in result

    @pytest.mark.asyncio
    async def test_categorized_items_retain_original_fields(self, monkeypatch: Any) -> None:
        """Test that categorized items retain original fields"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        items = [{"description": "Coffee", "quantity": 1, "unit_price": 3.5, "total": 3.5}]
        payload: dict[str, list[Any]] = {"items": items}
        result = await expense_categorizer_worker(payload)
    ...
    @pytest.mark.asyncio
    async def test_default_category(self, monkeypatch: Any) -> None:
        """Test default category for unknown items"""
        monkeypatch.setenv("USE_SMALL_MODEL", "false")

        items = [{"description": "Unknown Item", "quantity": 1, "total": 10.0}]
        payload: dict[str, list[Any]] = {"items": items}
        result = await expense_categorizer_worker(payload)
...
class TestWorkerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_pipeline_mock_mode(self, monkeypatch: Any) -> None:
    ...
    @pytest.mark.asyncio
    async def test_pipeline_with_dict_references(self, monkeypatch: Any) -> None:
