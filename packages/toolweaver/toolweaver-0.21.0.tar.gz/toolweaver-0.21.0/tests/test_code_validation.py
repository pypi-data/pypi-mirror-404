...
def test_validate_code_blocks_import_and_eval() -> None:
...
def test_validate_code_blocks_file_io() -> None:
    with pytest.raises(InvalidCodeError):
        validate_code("open('x.txt','w')")


def test_validate_code_allows_import_when_flag_set() -> None:
    ...
    assert validate_code("import math\nx = math.sqrt(4)", allow_imports=True).startswith(
        "import math"
    )


def test_validate_code_length_limit() -> None:
    long_code = "x=1\n" * 60000
