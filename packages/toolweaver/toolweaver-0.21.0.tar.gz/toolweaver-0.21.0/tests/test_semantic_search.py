...
def use_local_embeddings() -> Generator[None, None, None]:
    """
    Use 'local' embedding provider for semantic search tests.

    Semantic search requires actual embeddings, not zero vectors from 'none' provider.
    """
...
class MockToolPlugin:
    ...
    async def execute(self, tool_name: str, params: dict[str, Any]) -> dict[str, str]:
        return {"result": "ok"}


@pytest.fixture
def sample_tools(monkeypatch: Any) -> Generator[list[ToolDefinition], None, None]:
    ...
    tools = [
        ...
        ToolDefinition(
            ...
            parameters=[
                ToolParameter(
                    name="to", type="string", required=True, description="Email recipient"
                ),
                ToolParameter(
        ...
        ToolDefinition(
            ...
            parameters=[
                ...
                ToolParameter(
                    name="project", type="string", required=True, description="Jira project"
                ),
            ],
        ),
    ]
    ...
    # Clear Qdrant collection and any cached data to ensure test isolation
    try:
        from orchestrator.tools.vector_search import VectorToolSearchEngine

        # Try to get any existing instances and clear them
        search_engine = VectorToolSearchEngine(fallback_to_memory=True)

        # Clear embeddings cache
        search_engine.memory_embeddings.clear()

        if search_engine.qdrant_available and search_engine.client:
            try:
                search_engine.client.delete_collection(search_engine.collection_name)
                import time
    ...
    # Also clear after test completes for next test's isolation
    try:
        from orchestrator.tools.vector_search import VectorToolSearchEngine
...
def test_semantic_search_basic(sample_tools: Any) -> None:
    """Test basic semantic search returns relevant tools"""
    results = semantic_search_tools("create github pull request", top_k=3, min_score=0.1)
...
def test_semantic_search_domain_filter(sample_tools: Any) -> None:
    """Test semantic search with domain filtering"""
    results = semantic_search_tools("send notification", top_k=5, domain="slack", min_score=0.1)
...
def test_semantic_search_top_k(sample_tools: Any) -> None:
    """Test top_k parameter limits results"""
    results = semantic_search_tools("create something", top_k=2, min_score=0.0)

    assert len(results) <= 2


def test_semantic_search_min_score(sample_tools: Any) -> None:
...
def test_semantic_search_conceptual_match(sample_tools: Any) -> None:
    """Test semantic search finds conceptual matches (not just keywords)"""
    # Query: "notify team" should match "send message" semantically
    results = semantic_search_tools("notify team members", top_k=3, min_score=0.1)
...
def test_semantic_search_fallback_to_substring(sample_tools: Any) -> None:
...
def test_semantic_search_no_fallback(sample_tools: Any) -> None:
    ...
    try:
        results = semantic_search_tools(
            "test query", top_k=3, min_score=0.1, fallback_to_substring=False
        )
...
def test_search_tools_with_semantic_flag(sample_tools: Any) -> None:
    """Test search_tools() with use_semantic=True"""
    results = search_tools(query="create pull request", use_semantic=True, top_k=3)

    assert len(results) > 0
    assert all(isinstance(r, ToolDefinition) for r in results)


def test_search_tools_semantic_with_type_filter(sample_tools: Any) -> None:
    """Test semantic search with type filter"""
    results = search_tools(
        query="create something", use_semantic=True, type_filter="function", top_k=5
    )
...
def test_search_tools_semantic_disabled(sample_tools: Any) -> None:
    """Test search_tools() defaults to substring when use_semantic=False"""
    results = search_tools(query="github", use_semantic=False)
...
def test_semantic_search_empty_catalog() -> None:
    """Test semantic search with no tools available"""
    # Clear all tools temporarily (if possible)
    results = semantic_search_tools("test query", top_k=5)

    # Should handle gracefully
    assert isinstance(results, list)


def test_semantic_search_scores_descending(sample_tools: Any) -> None:
    """Test that results are sorted by score (descending)"""
    results = semantic_search_tools("github repository management", top_k=5, min_score=0.0)
