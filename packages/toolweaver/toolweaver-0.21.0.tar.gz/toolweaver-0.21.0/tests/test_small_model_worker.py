...
def ollama_worker() -> Any:
    ...
    return SmallModelWorker(backend="ollama", model_name=os.getenv("WORKER_MODEL", "phi3:latest"))
...
async def test_ollama_connection(ollama_worker: Any) -> None:
...
@pytest.mark.asyncio
@pytest.mark.skipif(os.getenv("SMALL_MODEL_BACKEND") != "ollama", reason="Ollama not configured")
async def test_ollama_json_mode(ollama_worker: Any) -> None:
...
@pytest.mark.asyncio
@pytest.mark.skipif(os.getenv("SMALL_MODEL_BACKEND") != "ollama", reason="Ollama not configured")
async def test_ollama_function_calling(ollama_worker: Any) -> None:
...
async def test_ollama_streaming() -> None:
    ...
    worker = SmallModelWorker(backend="ollama", model_name=os.getenv("WORKER_MODEL", "phi3:latest"))

    # Test basic generation works
    response = await worker.generate(
        prompt="Count to 5", system_prompt="Count numbers", max_tokens=50, temperature=0.1
    )
...
@pytest.mark.asyncio
@pytest.mark.skipif(os.getenv("SMALL_MODEL_BACKEND") != "ollama", reason="Ollama not configured")
async def test_ollama_batch_processing() -> None:
    ...
    for prompt in prompts:
        response = await worker.generate(
            prompt=prompt, system_prompt="You are helpful", max_tokens=20, temperature=0.1
        )
...
def test_ollama_initialization() -> None:
    """Test Ollama worker initialization"""
    model_name = os.getenv("WORKER_MODEL", "phi3:latest")
    worker = SmallModelWorker(backend="ollama", model_name=model_name)
...
@pytest.mark.asyncio
@pytest.mark.skipif(os.getenv("SMALL_MODEL_BACKEND") != "ollama", reason="Ollama not configured")
async def test_ollama_error_handling(ollama_worker: Any) -> None:
...
@pytest.mark.asyncio
@pytest.mark.skipif(os.getenv("SMALL_MODEL_BACKEND") != "ollama", reason="Ollama not configured")
async def test_ollama_temperature_control(ollama_worker: Any) -> None:
    """Test temperature parameter affects randomness"""
    prompt = "Give me a creative idea"

    # Low temperature (more deterministic)
    response1 = await ollama_worker.generate(
        prompt=prompt, system_prompt="Be creative", max_tokens=50, temperature=0.1
    )

    # Higher temperature (more random)
    response2 = await ollama_worker.generate(
        prompt=prompt, system_prompt="Be creative", max_tokens=50, temperature=0.9
    )
