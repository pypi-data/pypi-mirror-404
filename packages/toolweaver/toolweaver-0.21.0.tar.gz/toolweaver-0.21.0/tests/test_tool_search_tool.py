...
def sample_catalog() -> Any:
    ...
    tools = [
        ...
        ToolDefinition(
            ...
            parameters=[
                ToolParameter(
                    name="json_str", type="string", description="JSON string", required=True
                )
            ],
        ),
        ToolDefinition(
...
def test_get_tool_search_definition() -> None:
...
def test_tool_search_not_initialized() -> None:
...
def test_tool_search_initialization(sample_catalog: Any) -> None:
...
def test_tool_search_github_tools(sample_catalog: Any) -> None:
...
def test_tool_search_communication_tools(sample_catalog: Any) -> None:
...
def test_tool_search_top_k_limit(sample_catalog: Any) -> None:
...
def test_tool_search_relevance_scores(sample_catalog: Any) -> None:
...
def test_tool_search_returns_tool_format(sample_catalog: Any) -> None:
...
def test_tool_search_empty_query(sample_catalog: Any) -> None:
...
def test_tool_search_no_matches(sample_catalog: Any) -> None:
...
def test_tool_search_case_insensitive(sample_catalog: Any) -> None:
...
def test_tool_search_multiple_keywords(sample_catalog: Any) -> None:
    ...
    communication_tools = [name for name in tool_names if "email" in name or "slack" in name]
    assert len(communication_tools) > 0


def test_tool_search_returns_metadata(sample_catalog: Any) -> None:
