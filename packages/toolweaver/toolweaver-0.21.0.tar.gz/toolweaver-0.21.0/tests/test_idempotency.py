...
class TestIdempotencyKeyGeneration:
    ...
    def test_generate_key_different_agent(self) -> None:
    ...
    def test_generate_key_empty_args(self) -> None:
        """Should handle empty arguments."""
        key = generate_idempotency_key("agent1", "template", {})
        assert len(key) == 16

    def test_generate_key_complex_args(self) -> None:
        """Should handle complex nested arguments."""
        args = {"user": {"name": "Alice", "age": 30}, "items": [1, 2, 3], "flag": True}
...
class TestAgentTask:
    ...
    def test_task_auto_generates_key(self) -> None:
    ...
    def test_task_same_params_same_key(self) -> None:
    ...
    def test_task_different_params_different_key(self) -> None:
...
class TestIdempotencyRecord:
    ...
    def test_record_not_expired(self) -> None:
        """Should not be expired if within TTL."""
        now = datetime.now()
        expires = now + timedelta(hours=1)
        record = IdempotencyRecord(
            key="test", result={"data": "value"}, created_at=now, expires_at=expires
        )
        assert not record.is_expired()
        assert record.is_valid()

    def test_record_expired(self) -> None:
    ...
    def test_record_failed_not_valid(self) -> None:
        """Should not be valid if status is failed."""
        now = datetime.now()
        expires = now + timedelta(hours=1)
        record = IdempotencyRecord(
            key="test", result=None, created_at=now, expires_at=expires, status="failed"
        )
        assert not record.is_expired()
        assert not record.is_valid()
...
class TestIdempotencyCache:
    ...
    def test_store_and_retrieve(self, cache: IdempotencyCache) -> None:
        """Should store and retrieve results."""
        cache.store("key1", {"result": "success"})
        result = cache.get("key1")
        assert result == {"result": "success"}

    def test_has_returns_true_for_cached(self, cache: IdempotencyCache) -> None:
        """Should return True for cached keys."""
        cache.store("key1", {"result": "success"})
        assert cache.has("key1")

    def test_has_returns_false_for_missing(self, cache: IdempotencyCache) -> None:
        """Should return False for missing keys."""
        assert not cache.has("nonexistent")

    def test_get_returns_none_for_missing(self, cache: IdempotencyCache) -> None:
        """Should return None for missing keys."""
        assert cache.get("nonexistent") is None

    def test_expired_entry_removed_on_get(self, cache: IdempotencyCache) -> None:
    ...
    def test_failed_status_not_returned(self, cache: IdempotencyCache) -> None:
        """Should not return results with failed status."""
        cache.store("key1", {"error": "failed"}, status="failed")
        assert cache.get("key1") is None

    def test_invalidate_removes_entry(self, cache: IdempotencyCache) -> None:
    ...
    def test_clear_removes_all(self, cache: IdempotencyCache) -> None:
    ...
    def test_cleanup_expired(self, cache: IdempotencyCache) -> None:
    ...
    def test_size(self, cache: IdempotencyCache) -> None:
    ...
    def test_get_stats(self, cache: IdempotencyCache) -> None:
...
class TestGlobalCache:
    ...
    def test_check_idempotency_missing(self) -> None:
        """Should return None for missing key."""
        # Clean slate
        get_global_cache().clear()
        assert check_idempotency("missing") is None

    def test_store_and_check_idempotency(self) -> None:
...
class TestIntegration:
    ...
    def test_prevent_duplicate_execution(self) -> None:
        ...
        assert task2.idempotency_key is not None
        key2 = task2.idempotency_key
        assert cache.has(key2)
        cached_result = cache.get(key2)
        assert cached_result == {"output": "Hello, World!"}

    def test_retry_with_idempotency(self) -> None:
        ...
        # First attempt fails
        cache.store(key, None, status="failed")
        ...
        cache.store(key, result, status="success")

        # Next access gets successful result
        assert cache.get(key) == result

    def test_concurrent_tasks_same_key(self) -> None:
        ...
        assert task1.idempotency_key is not None
        assert task2.idempotency_key is not None
        key1 = task1.idempotency_key
        key2 = task2.idempotency_key
        ...
        cache.store(key1, {"result": "done"})

        # Second task should get cached result
        assert cache.has(key2)
        assert cache.get(key2) == {"result": "done"}
