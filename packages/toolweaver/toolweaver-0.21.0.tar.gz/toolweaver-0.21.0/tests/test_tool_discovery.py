...
@pytest.mark.asyncio
async def test_tool_discovery_basic() -> None:
    print("=" * 60)
    ...
    assert len(mcp_tools) > 0 or len(function_tools) > 0, (
        "Should have at least MCP or function tools"
    )


@pytest.mark.asyncio
async def test_tool_discovery_caching() -> None:
    ...
    assert len(catalog1.tools) == len(catalog2.tools), (
        "Cached catalog should have same number of tools"
    )


@pytest.mark.asyncio
async def test_tool_discovery_force_refresh() -> None:
...
@pytest.mark.asyncio
async def test_tool_discovery_llm_format() -> None:
...
def test_tool_discovery_orchestrator() -> None:
...
@pytest.mark.asyncio
async def test_tool_discovery_by_type() -> None:
...
@pytest.mark.asyncio
async def test_discovery_includes_agent_and_streaming_metadata() -> None:
...
@pytest.mark.asyncio
async def test_discovery_marks_mcp_streaming_tools() -> None:
    async def streaming_worker(payload: object) -> Any:
        for i in range(2):
            yield f"chunk-{i}"

    async def plain_worker(payload: object) -> str:
        return "ok"

    mcp_client = MCPClientShim()
    mcp_client.tool_map = cast(
        dict[str, Any],
        {
...
@pytest.mark.asyncio
async def test_function_streaming_flag() -> None:
    async def streaming_func() -> Any:
        yield "x"
...
@pytest.mark.asyncio
async def test_unified_discovery_mcp_and_a2a() -> None:
    async def mcp_worker(payload: object) -> str:
        return "ok"

    mcp_client = MCPClientShim()
    mcp_client.tool_map = cast(dict[str, Any], {"mcp_tool": mcp_worker})
