...
def mock_env(monkeypatch: Any) -> Any:
...
class TestPlannerBackwardCompatibility:
    ...
    def test_planner_without_catalog(self) -> None:
    ...
    def test_default_catalog_has_expected_tools(self) -> None:
        ...
        # Check tool types
        receipt_ocr = catalog.get_tool("receipt_ocr")
        compute_tax = catalog.get_tool("compute_tax")
        code_exec = catalog.get_tool("code_exec")
        assert receipt_ocr is not None and receipt_ocr.type == "mcp"
        assert compute_tax is not None and compute_tax.type == "function"
        assert code_exec is not None and code_exec.type == "code_exec"
...
class TestPlannerWithCustomCatalog:
    ...
    def test_planner_with_custom_catalog(self) -> None:
    ...
    def test_custom_catalog_in_system_prompt(self) -> None:
...
class TestPlannerWithSemanticSearch:
    ...
    def test_planner_with_available_tools(self) -> None:
        """Planner should prioritize available_tools parameter"""
        # Create default catalog
        default_catalog = ToolCatalog(source="default", version="1.0")
        default_catalog.add_tool(
            ToolDefinition(
                name="default_tool", type="function", description="Default tool", parameters=[]
            )
        )
    ...
    def test_tool_priority_order(self) -> None:
...
class TestPlannerToolGrouping:
    ...
    def test_tools_grouped_by_type(self) -> None:
        """System prompt should group tools by type (mcp, function, code_exec)"""
        catalog = ToolCatalog(source="test", version="1.0")

        # Add one of each type
        catalog.add_tool(
            ToolDefinition(name="mcp_tool", type="mcp", description="MCP tool", parameters=[])
        )
        catalog.add_tool(
            ToolDefinition(
                name="func_tool", type="function", description="Function tool", parameters=[]
            )
        )
        catalog.add_tool(
            ToolDefinition(
                name="code_tool", type="code_exec", description="Code execution tool", parameters=[]
            )
        )
