    ...
    def __post_init__(self) -> None:
        ...
        if self.idempotency_key is None:
            self.idempotency_key = generate_idempotency_key(
                self.agent_name, self.template, self.arguments
            )
...
def generate_idempotency_key(agent_name: str, template: str, arguments: dict[str, Any]) -> str:
    ...
    canonical = {"agent": agent_name, "template": template, "arguments": arguments}
...
class IdempotencyCache:
    ...
    def store(self, key: str, result: Any, status: str = "success") -> None:
        """
        ...
        record = IdempotencyRecord(
            key=key, result=result, created_at=now, expires_at=expires_at, status=status
        )
    ...
    def cleanup_expired(self) -> int:
        ...
        """
        now = datetime.now()
        expired_keys = [key for key, record in self._cache.items() if record.expires_at <= now]
...
def store_idempotent_result(key: str, result: Any, status: str = "success") -> None:
    """
