...
class AuthManager:
    ...
    def get_headers(self, config: AuthConfig) -> dict[str, str]:
        ...
        if not config.token_env:
            raise ValueError(f"token_env is required for auth type '{config.type}'")

        token = os.getenv(config.token_env)
        if not token:
            raise ValueError(
                f"Authentication token not found in environment variable '{config.token_env}'"
            )
...
OPENAI_AUTH = AuthConfig(type="bearer", token_env="OPENAI_API_KEY", header_name="Authorization")

ANTHROPIC_AUTH = AuthConfig(type="api_key", token_env="ANTHROPIC_API_KEY", header_name="x-api-key")

GITHUB_AUTH = AuthConfig(type="bearer", token_env="GITHUB_TOKEN", header_name="Authorization")

AZURE_AUTH = AuthConfig(type="api_key", token_env="AZURE_API_KEY", header_name="api-key")
