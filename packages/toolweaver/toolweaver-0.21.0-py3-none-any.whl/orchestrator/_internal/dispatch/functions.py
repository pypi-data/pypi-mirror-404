...
def merge_items(items: list[dict[str, Any]] | dict[str, Any]) -> dict[str, Any]:
    """
    ...
    return {"total_sum": round(total_sum, 2), "count": count, "avg_total": round(avg_total, 2)}
...
def apply_discount(amount: float, discount_percent: float) -> dict[str, float]:
    ...
    return {"original": amount, "discount": discount, "final": final}
...
def compute_item_statistics(items: list[dict[str, Any]] | dict[str, Any]) -> dict[str, Any]:
    """
