    ...
    if _small_model_worker is None:
        ...
        if use_small_model:
            try:
                ...
                backend = cast(
                    Literal["transformers", "ollama", "azure"],
    ...
    if ocr_mode == "azure" and AZURE_CV_AVAILABLE:
        try:
            ...
            result = await asyncio.to_thread(
                client.analyze_from_url, image_url=image_uri, visual_features=[VisualFeatures.READ]
            )
...
async def expense_categorizer_worker(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if small_model and items:
        try:
            ...
            logger.warning(
                f"Small model categorization failed, falling back to keyword matching: {e}"
            )
