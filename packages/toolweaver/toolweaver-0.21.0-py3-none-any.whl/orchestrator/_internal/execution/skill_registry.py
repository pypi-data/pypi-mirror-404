...
from orchestrator._internal.config.paths import get_registry_cache_dir, get_registry_config

from .skill_library import Skill, save_skill

logger = logging.getLogger(__name__)

# Configuration - use centralized path management
REGISTRY_URL = get_registry_config()
_REGISTRY_CACHE_DIR = get_registry_cache_dir()
...
class SkillRegistry:
    ...
    def _verify_signature(self, skill_id: str, code_hash: str, signature: str, secret: str) -> bool:
    ...
    def publish_skill(
        ...
        try:
            response = self.session.post(
                self._api_url("/skills"), json=payload, timeout=self.config.timeout
            )
    ...
    def search(
        ...
        try:
            response = self.session.get(
                self._api_url("/skills"), params=params, timeout=self.config.timeout
            )
            response.raise_for_status()
    ...
    def get_skill(self, skill_id: str) -> RegistrySkill | None:
        ...
        try:
            response = self.session.get(
                self._api_url(f"/skills/{skill_id}"), timeout=self.config.timeout
            )
            response.raise_for_status()
    ...
    def update_skill_metadata(
        ...
        """
        Update skill metadata (PATCH /marketplace endpoint).

        Args:
            skill_id: ID of skill to update
            changelog: New changelog entries to merge
            compatibility: Updated compatibility requirements
            description: New description
            tags: Updated tags

        Returns:
            Updated RegistrySkill or None if failed
        """
        ...
        try:
            url = self._api_url(f"/skills/{skill_id}")
            response = self.session.patch(url, json=payload, timeout=self.config.timeout)
            response.raise_for_status()

            # Ensure we construct proper RegistrySkill from potentially partial response
            # Or refetch. Assuming server returns full object.
            return RegistrySkill(**response.json())
        except requests.RequestException as e:
            logger.error(f"Failed to update skill metadata: {e}")
            return None
            ...
            if self.config.verify_signature and registry_skill.signature:
                if "secret" in data:  # Registry provides shared secret in response
                    if not self._verify_signature(
                        skill_id, registry_skill.code_hash, registry_skill.signature, data["secret"]
                    ):
    ...
    def rate_skill(self, skill_id: str, rating: float, review: str | None = None) -> bool:
        """
        ...
        try:
            ...
            response = self.session.post(
                self._api_url(f"/skills/{skill_id}/rate"), json=payload, timeout=self.config.timeout
            )
            response.raise_for_status()
    ...
    def get_ratings(self, skill_id: str) -> dict[str, Any]:
        ...
        try:
            response = self.session.get(
                self._api_url(f"/skills/{skill_id}/ratings"), timeout=self.config.timeout
            )
            response.raise_for_status()
...
def search_registry(
    query: str = "", tags: list[str] | None = None, org: str | None = None, min_rating: float = 0.0
) -> list[RegistrySkill]:
    """Search registry for skills."""
