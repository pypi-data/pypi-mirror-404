...
class ToolDefinition(BaseModel):
    ...
    required_role: str | None = None  # Phase 9: RBAC (admin, user, etc.)

    @field_validator("parameters", mode="before")
    @classmethod
    def validate_parameters(cls, v: Any) -> Any:
        """Accept ToolParameter instances or dicts in parameters list."""
        ...
        for item in v:
            if isinstance(item, ToolParameter):
                ...
                result.append(ToolParameter(**item))
            else:
                result.append(item)  # Let Pydantic handle validation error
        return result

    @field_validator("examples", mode="before")
    @classmethod
    def validate_examples(cls, v: Any) -> Any:
        """Accept ToolExample instances or dicts in examples list."""
        if not isinstance(v, list):
            return v
        result = []
        for item in v:
            if isinstance(item, ToolExample):
                result.append(item)
            elif isinstance(item, dict):
                result.append(ToolExample(**item))
            else:
                result.append(item)  # Let Pydantic handle validation error
        return result
    ...
    # Accept both dicts and model instances for nested fields to avoid class identity issues
    @field_validator("parameters", mode="before")
    @classmethod
    def _coerce_parameters(cls, v: Any) -> list[ToolParameter]:
        if v is None:
            return []
        out: list[ToolParameter] = []
        for item in v:
            if isinstance(item, dict):
                out.append(ToolParameter.model_validate(item))
            elif hasattr(item, "name") and hasattr(item, "type"):
                # Duck typing: if it looks like ToolParameter, accept it
                # This avoids isinstance() failures from module reloading
                out.append(item)
            else:
                raise TypeError(f"Invalid parameter type: {type(item).__name__}")
        return out

    @field_validator("examples", mode="before")
    @classmethod
    def _coerce_examples(cls, v: Any) -> list[ToolExample]:
        if v is None:
            return []
        out: list[ToolExample] = []
        for item in v:
            if isinstance(item, dict):
                out.append(ToolExample.model_validate(item))
            elif hasattr(item, "scenario") and hasattr(item, "input"):
                # Duck typing: if it looks like ToolExample, accept it
                out.append(item)
            else:
                raise TypeError(f"Invalid example type: {type(item).__name__}")
        return out
...
class ToolCatalog(BaseModel):
    ...
    def to_llm_format(
        self, defer_loading: bool = False, include_examples: bool = True
    ) -> list[dict[str, Any]]:
        """
