...
async def call_tool(
    server: str, tool_name: str, parameters: dict[str, Any], timeout: int = 30
) -> Any:
    """
...
async def _execute_function(tool_name: str, parameters: dict[str, Any], timeout: int) -> Any:
    ...
    try:
        result = await asyncio.wait_for(asyncio.to_thread(func, **parameters), timeout=timeout)
        return result
...
async def _execute_mcp_tool(
    server: str, tool_name: str, parameters: dict[str, Any], timeout: int
) -> Any:
