...
# Cache registry-backed tool catalogs per backend
_registry_catalog_cache: dict[str, ToolCatalog] = {}


def _collect_all_tool_defs(catalog: ToolCatalog | None = None) -> list[ToolDefinition]:
    """Collect all tool definitions from all plugins and Phase-1 local registry, normalized to ToolDefinition.

    Plugins return lists of dicts or ToolDefinitions; we validate to ToolDefinition for consistent access.
    Also includes tools from Phase-1 local registry (registered via @tool decorators).
    """
    if catalog is not None:
        tools_list: list[Any] = list(catalog.tools.values())
        return [
            ToolDefinition.model_validate(t) if not isinstance(t, ToolDefinition) else t
            for t in tools_list
        ]
    ...
    # Collect from Phase-1 registry
    try:
        local_registry = get_tool_registry()
        for tool_name in local_registry.list_tools():
            # Check if this tool has a __tool_definition__ attribute (set by @tool decorator)
            tool_func = local_registry.get_tool(tool_name)
            if tool_func and hasattr(tool_func, "__tool_definition__"):
                tool_def = tool_func.__tool_definition__
                if tool_def.name not in seen_tool_names:
                    all_tools.append(tool_def)
                    seen_tool_names.add(tool_def.name)
    except Exception as e:
        logger.debug(f"Could not collect tools from local registry: {e}")
...
def get_available_tools(
    *,
    ...
    """List available tools across all plugins and Phase-1 local registry with optional filters."""
    out: list[ToolDefinition] = []
    seen_tool_names: set[str] = set()

    # Get from plugin registry
    registry = get_registry()
    ...
    # Get from Phase-1 registry (honors ORCHESTRATOR_TOOL_REGISTRY)
    try:
        registry = get_tool_registry()  # type: ignore[assignment]
        for tool_name in registry.list_tools():  # type: ignore[attr-defined]
            tool_func = registry.get_tool(tool_name)  # type: ignore[attr-defined]
            if tool_func and hasattr(tool_func, "__tool_definition__"):
                tool_def = tool_func.__tool_definition__
                if tool_def.name in seen_tool_names:
                    continue
                if type_filter and tool_def.type != type_filter:
                    continue
                if domain and tool_def.domain != domain:
                    continue
                out.append(tool_def)
                seen_tool_names.add(tool_def.name)
    except Exception as e:
        logger.debug(f"Could not collect tools from registry: {e}")

    return out


def get_registry_tool_catalog(*, use_cache: bool = True) -> ToolCatalog:
    """Build a ToolCatalog from the configured registry (local/sqlite/redis).

    Caches per backend name to avoid repeated hydration. Tools that cannot be hydrated
    (e.g., script-defined with import_path=__main__...) are skipped.
    """
    registry = get_tool_registry()
    backend_name = registry.__class__.__name__
    cache_key = backend_name

    if use_cache and cache_key in _registry_catalog_cache:
        return _registry_catalog_cache[cache_key]

    tools = get_available_tools()
    catalog = ToolCatalog(source="registry", metadata={"backend": backend_name})
    for tool in tools:
        catalog.add_tool(tool)

    if use_cache:
        _registry_catalog_cache[cache_key] = catalog
...
def get_tool_info(
    ...
    *,
    ...
    for td in _collect_all_tool_defs(catalog):
        if td.name == name:
            return _format_tool_view(
                td, detail_level=detail_level, include_examples=include_examples
            )
    return None


def list_tools_by_domain(
    domain: str, *, catalog: ToolCatalog | None = None
) -> list[ToolDefinition]:
...
def semantic_search_tools(
    ...
    *,
    ...
    try:
        ...
        logger.warning(
            "Vector search dependencies not available (qdrant-client, sentence-transformers)"
        )
        if fallback_to_substring:
            ...
            return [(tool, 1.0) for tool in substring_results if isinstance(tool, ToolDefinition)][
                :top_k
            ]
        return []
    ...
    search_engine = VectorToolSearchEngine(fallback_to_memory=True)
    ...
    try:
        results = search_engine.search(
            query=query, catalog=catalog, top_k=top_k, domain=domain, min_score=min_score
        )
        return results
    except Exception as e:
