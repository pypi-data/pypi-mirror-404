...
def _resolve_worker(import_path: str) -> Callable[..., Any]:
    ...
    if ":" in import_path:
        ...
        raise WorkerResolutionError(
            f"Invalid import path: {import_path}. Use 'module.function' or 'module:function' format"
        )

    try:
        module = importlib.import_module(module_path)
    except ImportError as e:
        raise WorkerResolutionError(f"Failed to import module '{module_path}': {e}") from e

    try:
        worker = getattr(module, func_name)
    except AttributeError as e:
        raise WorkerResolutionError(f"Module '{module_path}' has no function '{func_name}'") from e

    if not callable(worker):
        raise WorkerResolutionError(f"'{import_path}' is not callable")
