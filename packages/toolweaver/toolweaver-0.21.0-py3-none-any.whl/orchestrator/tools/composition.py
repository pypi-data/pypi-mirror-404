...
class CompositionStep:
    ...
    def __post_init__(self) -> None:
        if self.on_error not in ("raise", "continue", "fallback"):
            raise ValueError(
                f"on_error must be 'raise', 'continue', or 'fallback', got {self.on_error}"
            )
...
class ToolResolver(Protocol):
    ...
    async def __call__(self, tool_ref: str) -> Callable[..., Any]: ...
...
class CompositionExecutor:
    ...
    async def _default_resolver(self, tool_ref: str) -> Callable[..., Any]:
        ...
        for attempt in range(step.retry_count + 1):
            try:
                ...
                async def _make_call(
                    current_tool: Callable[..., Any] = tool, current_input: dict[str, Any] = step_input
                ) -> Any:
                ...
                if attempt == step.retry_count:
                    raise StepExecutionError(
                        step.name, f"Timeout after {step.timeout_sec}s", e
                    ) from e
    ...
    def decorator(fn: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> CompositionChain:
            ...
            if not isinstance(chain, CompositionChain):
                raise TypeError(
                    f"@composite_tool {name} must return a CompositionChain, got {type(chain)}"
                )
            chain.name = name
