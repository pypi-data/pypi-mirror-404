...
def _swiftui_text_node(node: Dict[str, Any], indent: int, parent_node: Dict[str, Any] = None) -> str:
    ...
    if w > 0:
        ...
        # CENTER-aligned text: check parent context for actual alignment
            ...
            # In auto-layout parent, Figma often reports CENTER for stretched text
            # that is actually left-aligned. Use .leading in auto-layout contexts.
            # Only use .center when there's no auto-layout parent (absolute positioning).
            if parent_layout:
...
def _icon_needs_flip(node: Dict[str, Any], icon_name: str) -> bool:
    ...
    # Check this node's rotation
    rotation = node.get('rotation', 0)
    if abs(rotation) > 90:
        return True
    # Check this node's relativeTransform for horizontal flip (scaleX: -1)
    transform = node.get('relativeTransform')
    if transform and len(transform) >= 1 and len(transform[0]) >= 1:
        if transform[0][0] < 0:
            return True
    # Also check children transforms (flip may be on inner vector/group)
    for child in node.get('children', []):
        child_rotation = child.get('rotation', 0)
        if abs(child_rotation) > 90:
            return True
        ct = child.get('relativeTransform')
        if ct and len(ct) >= 1 and len(ct[0]) >= 1:
            if ct[0][0] < 0:
                return True
    return False
