...
def _swiftui_text_node(node: Dict[str, Any], indent: int, parent_node: Dict[str, Any] = None) -> str:
    ...
    if w > 0:
        ...
        # CENTER-aligned text: expand only if parent is centered or no parent layout
        elif ts.text_align == 'CENTER':
            # If parent is left-aligned auto-layout, use .leading alignment
            if parent_layout and parent_counter_align == 'MIN':
...
def _swiftui_ellipse_ring(node: Dict[str, Any], indent: int) -> str:
    ...
    ending_angle = arc_data.get('endingAngle', 2 * 3.14159265)
    is_full_circle = abs(ending_angle - start_angle - 2 * 3.14159265) < 0.01

    lines.append(f'{prefix}Circle()')
    lines.append(f'{prefix}    .stroke({color_code}, lineWidth: {line_width})')

    if not is_full_circle:
        from_val = start_angle / (2 * 3.14159265)
        to_val = ending_angle / (2 * 3.14159265)
...
def _is_icon_container(node: Dict[str, Any]) -> bool:
    ...
    vector_types = {'VECTOR', 'BOOLEAN_OPERATION', 'STAR', 'POLYGON', 'ELLIPSE', 'LINE', 'REGULAR_POLYGON', 'GROUP'}
...
def _swiftui_container_node(node: Dict[str, Any], indent: int, depth: int, parent_node: Dict[str, Any] = None) -> str:
    ...
    needs_scroll = False
    if container == 'HStack' and len(visible_children) > 1:
        node_bbox = node.get('absoluteBoundingBox', {})
