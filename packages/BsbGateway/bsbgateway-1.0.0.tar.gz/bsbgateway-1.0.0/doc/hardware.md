...
⚠️ **WARNING: The circuit documented below is NOT RECOMMENDED for reuse.** It connects the Protective Earth (GND) wire of the bus to the "-15V wire" of the RS232 plug. This **might fail destructively** if your RS232 GND is connected to PE (i.e. the power line "ground" contact).
...
1. **Look at serial output**. Use `stty` and `hexdump` to directly print out the data coming from the serial port. Even in idle state, the heater always sends status messages in some seconds interval. They should be around 10-20 bytes long and begin with `0x23` (`0xDC` if your interface gives uninverted bytes).

2. **Check byte inversion**: Bus levels typically have inverted meanings with
respect to RS232 level.
    - Check Adapter / Invert Bytes and Adapter / Port Parity settings.
    - Usually, Invert bytes must be `True` and Port Parity `odd`.

3. **CTS check**: The CTS signal meaning depends on your circuit.
   - Try setting "Expect CTS State" to "None" temporarily
   - Measure CTS voltage level to see if busy-detection works correctly.

For more detailed troubleshooting, see the [Troubleshooting Guide](troubleshooting.md#connection-issues).
