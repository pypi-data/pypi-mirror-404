...
if sys.platform == "win32":
    SHIM_DIR = Path.home() / ".cc-vault" / "bin"
else:
...
def install_shim(force: bool = False) -> bool:
    ...
    print("Now when you run 'claude', sessions will auto-sync to CC-Vault.")
    return True
        ...
        try:
            ...
            for line in lines:
                if marker in line:
                    skip_next = True
                    continue
                if skip_next and line.startswith("export PATH=") and ".cc-vault" in line:
                    skip_next = False
                    continue
                skip_next = False
