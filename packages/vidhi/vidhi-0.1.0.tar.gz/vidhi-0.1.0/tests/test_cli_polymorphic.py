...
class TestBasicPolymorphicParsing:
    ...
    def test_select_variant_via_type_arg(self):
        ...
        test_args = ["script.py", "--cache_type", "redis"]
        with patch.object(sys, "argv", test_args):
            config = parse_cli_args(AppConfig)
    ...
    def test_invalid_variant_type(self):
        ...
        test_args = ["script.py", "--cache_type", "invalid"]
        with patch.object(sys, "argv", test_args):
            # Invalid type raises ValueError during reconstruction
...
class TestPolymorphicTypeCoercion:
    ...
    def test_int_coercion(self):
        ...
        test_args = ["script.py", "--cache_type", "redis", "--cache.port", "6380"]
        with patch.object(sys, "argv", test_args):
            config = parse_cli_args(AppConfig)
    ...
    def test_bool_coercion(self):
        ...
        # Test --no-flag syntax
        test_args = ["script.py", "--no-cache.enabled"]
        with patch.object(sys, "argv", test_args):
            config = parse_cli_args(AppConfig)
