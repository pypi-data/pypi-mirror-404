...
from vidhi.constants import CLI_ARG_SEPARATOR, FIELD_NAME_SEPARATOR
...
def get_cli_arguments(flat_dataclass_type: Type) -> List[Dict[str, Any]]:
    ...
    for field in fields(flat_dataclass_type):
        ...
        cli_name = field.name.replace(FIELD_NAME_SEPARATOR, CLI_ARG_SEPARATOR)
...
"""
    ...
    for arg in args:
        ...
        if arg["is_bool"]:
            script += f"        '--{cli_name}[{help_text}]'\n"
            script += f"        '--no-{cli_name}[Disable {cli_name}]'\n"
        elif arg["choices"]:
...
"""
    ...
    for arg in args:
        ...
        if arg["is_bool"]:
            script += f"complete -c {command_name} -l {cli_name} -d '{help_text}'\n"
            script += (
                f"complete -c {command_name} -l no-{cli_name} -d 'Disable {cli_name}'\n"
            )
        elif arg["choices"]:
            choices_str = " ".join(str(c) for c in arg["choices"])
