...
def _get_default_value_for_poly_field(field: Any, field_type: Type[Any]) -> str:
    ...
    Raises:
        ValueError: If the field has no default or default_factory
    """
    if field.default_factory is not MISSING:
        type_val = field.default_factory().get_type()
    ...
    # Use enum value if available for consistency with CLI usage
    if hasattr(type_val, "value"):
        return str(type_val.value)
    return str(type_val.name).lower()
...
def _handle_polymorphic_config_field(
    ...
    for subclass in subclasses:
        ...
        if hasattr(subclass_type, "value"):
            valid_types.append(str(subclass_type.value))
        else:
            valid_types.append(str(subclass_type.name).lower())
    ...
    for subclass in subclasses:
        ...
        subclass_type = subclass.get_type()
        if hasattr(subclass_type, "value"):
            type_key = str(subclass_type.value).lower()
        else:
            type_key = subclass_type.name.lower()
