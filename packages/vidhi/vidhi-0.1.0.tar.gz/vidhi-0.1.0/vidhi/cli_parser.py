...
class VidhiArgumentParser:
    ...
    def add_argument(
        ...
        if group_key not in self.groups:
            # Capitalize first letter only, replace underscores with spaces
            if group_key:
                formatted_name = group_key.replace("_", " ")
    ...
    def parse_args(
        ...
        while i < len(args):
            ...
            if arg.startswith("--no-"):
                cli_name = arg[5:]  # Remove "--no-"
                internal_name = self._find_argument_by_cli_name(cli_name)
                if internal_name and self.arguments[internal_name].is_boolean:
            ...
            if arg_def.is_list:
                ...
                try:
                    if arg_def.arg_type == bool:
                        result[internal_name] = value.lower() in ("true", "1", "yes")
    ...
    def _identify_polymorphic_groups(self) -> Dict[str, Dict[str, List[Argument]]]:
        ...
        # (the one without "__" separator, indicating top-level poly)
        primary_type_fields: Dict[str, str] = {}  # group -> primary type_field
        for arg in self.arguments.values():
            if arg.type_field and arg.group:
                # Check if this is a top-level type field (no "__" means direct child)
                if "__" not in arg.type_field:
                    if arg.group not in primary_type_fields:
                        primary_type_fields[arg.group] = arg.type_field
        ...
        for arg in self.arguments.values():
            if arg.type_field and arg.group:
                ...
                # Skip if this is a nested poly field (type_field has "__")
    ...
    def _find_parent_variants_for_nested_field(
        ...
        parent_cli_name = parent_path.replace("__", ".")
        ...
        # Fallback: look for the parent type selector (e.g., cache.pool_type)
        # and check which top-level variants it belongs to
        parent_type_selector = parent_cli_name + "_type"
        for name, other_arg in self.arguments.items():
    ...
    def _format_polymorphic_group(
        ...
        # Find the primary type selector (top-level, no "__" in name)
        type_arg = None
        type_field_name = None
        for arg in group.arguments:
            if arg.type_field and "__" not in arg.type_field:
                type_field_name = arg.type_field
                break
        ...
        primary_depth = 0  # Primary type field like "cache_type" has 0 "__"
        ...
        for variant_name in variant_names:
            ...
            # Variant header
            lines.append(f"  {c.variant('[' + variant_name + ']')}")
        ...
        for nested_tf, (parent_variants, nested_args) in all_nested_poly.items():
            for parent_variant in sorted(parent_variants):
                if parent_variant in top_level_variants:
                    lines.extend(
                        self._format_flattened_nested_variants(
                            nested_tf, nested_args, parent_variant
                        )
                    )
    ...
    def _format_flattened_nested_variants(
        ...
        variant_names = sorted(nested_variants.keys())
        choices_str = c.choices("{" + ",".join(variant_names) + "}")
        default_str = ""
        if nested_type_arg.default:
            default_str = f" {c.dim('[' + str(nested_type_arg.default) + ']')}"

        # Capitalize first letter of nested name
        capitalized_name = (
            nested_name[0].upper() + nested_name[1:] if nested_name else ""
        )
        lines.append(f"{base_indent}{c.header(capitalized_name + ' Options:')}")
        ...
        for nested_variant in variant_names:
            ...
            # Indented variant header: [parent.nested: variant]
            display_name = f"{parent_variant}.{nested_name}: {nested_variant}"
            lines.append(f"{content_indent}{c.variant('[' + display_name + ']')}")
            ...
            if (
                ...
                lines.append(
                    f"{content_indent}  {c.dim(self._variant_descriptions[nested_group][nested_variant])}"
                )
            ...
            for arg in variant_args:
                lines.extend(
                    self._format_argument_help(
                        ...
                        indent=len(content_indent) + 2,
                    )
                )

            # Check for even deeper nested poly within this variant
            for deeper_tf, (_, deeper_args) in deeper_nested.items():
                ...
                lines.extend(
                    self._format_flattened_nested_variants(
                        deeper_tf,
                        deeper_args,  # Pass all args, they'll be grouped by variant
                        deeper_parent,
                        depth + 1,
                    )
                )
    ...
    def _format_argument_help(
        ...
        if arg.is_boolean:
            arg_str = (
                f"{prefix}  {', '.join(option_names)}, "
                f"{c.option('--no-' + arg.cli_name)}"
            )
            if default_display:
                arg_str += f" {c.dim('[' + default_display + ']')}"
        elif arg.choices:
        ...
        if arg.help_text:
            ...
            # Add required marker inline with help
            if arg.required:
                help_text = f"{c.required('[REQUIRED]')} {help_text}"
    ...
    def _format_default(
        ...
        if isinstance(arg.default, dict):
            if variant and variant in arg.default:
                return f"default: {arg.default[variant]}"
            elif show_variants:
                default_strs = [f"{v}: {d}" for v, d in sorted(arg.default.items())]
                return f"default: {', '.join(default_strs)}"
            else:
                first_default = next(iter(arg.default.values()))
                return f"default: {first_default}"
        elif arg.default is not None:
