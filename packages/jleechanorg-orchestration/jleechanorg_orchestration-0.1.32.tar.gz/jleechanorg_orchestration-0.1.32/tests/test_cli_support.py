...
class TestCliValidation(unittest.TestCase):
    ...
    def test_all_cli_validations_skip_mcp_servers(self):
        ...
        with patch("orchestration.task_dispatcher.validate_cli_two_phase") as mock_validate:
            ...
            # Test Claude - doesn't have MCP-skip flag (--strict-mcp-config doesn't exist)
            # Validation will work but may take longer due to MCP server loading
            with patch("os.access", return_value=True):
                self.dispatcher._validate_cli_availability("claude", "/usr/bin/claude", "test-agent")
                call_args = mock_validate.call_args
                execution_cmd = call_args[1].get("execution_cmd", call_args[0][3] if len(call_args[0]) > 3 else [])
                # Claude should NOT have --strict-mcp-config (it doesn't exist)
                self.assertNotIn("--strict-mcp-config", execution_cmd,
                    "Claude validation should not use --strict-mcp-config (unsupported flag)")
