...
class TestCliValidation(unittest.TestCase):
    ...
    def test_all_cli_validations_skip_mcp_servers(self):
        ...
        with patch("orchestration.task_dispatcher.validate_cli_two_phase") as mock_validate:
            ...
            # Test Claude - should have --strict-mcp-config (with no --mcp-config, this skips all MCP)
            with patch("os.access", return_value=True):
                self.dispatcher._validate_cli_availability("claude", "/usr/bin/claude", "test-agent")
                call_args = mock_validate.call_args
                execution_cmd = call_args[1].get("execution_cmd", call_args[0][3] if len(call_args[0]) > 3 else [])
                self.assertIn("--strict-mcp-config", execution_cmd,
                    "Claude validation should skip MCP servers with --strict-mcp-config")
    ...
    def test_strict_answer_matching_avoids_false_positives(self):
        ...
        false_positive_cases = [
            ...
            "Error code: 429",              # HTTP error
            "2024-01-31",                   # Date
            "14:30:00",                     # Timestamp
            "Quota: 14/100",                # Quota message
            "Rate limit: retry in 45 seconds",  # 45 shouldn't match as "4"
            "Error 0x00000004",             # Hex code shouldn't match
        ]

        for case in false_positive_cases:
            match = re.search(strict_match_pattern, case)
            # For most cases, we expect no match, but some like "14:30" have a "4" at the end
            # The key is HTTP 429 should NOT match
            if "429" in case:
                self.assertIsNone(match, f"HTTP 429 should NOT trigger false positive: {case}")
