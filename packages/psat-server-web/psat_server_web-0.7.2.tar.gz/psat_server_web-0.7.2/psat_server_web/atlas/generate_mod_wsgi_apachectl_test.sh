...
# 
# REQUIREMENTS:
# Write a file in ~/.config/django/django_env_$CONDA_DEFAULT_ENV (e.g. atlas)
...
echo mod_wsgi-express setup-server --working-directory atlas --url-alias $PREFIX/static static --url-alias $PREFIX/media media --application-type module atlas.wsgi --server-root $APACHEPATH --port $PORT --mount-point $PREFIX
