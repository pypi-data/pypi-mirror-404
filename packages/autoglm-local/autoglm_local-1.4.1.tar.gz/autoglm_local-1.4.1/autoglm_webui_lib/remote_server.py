...
# 远程 API 配置（可通过环境变量 QUALITY_API_BASE 覆盖）
# Docker 中使用: docker run -e QUALITY_API_BASE=http://实际IP:8088/quality-service/ai/excute/case/result ...
REMOTE_API_BASE = os.environ.get(
    "QUALITY_API_BASE", 
    "http://qa.local:8088/quality-service/ai/excute/case/result"
)
...
@app.post("/api/proxy/case-result/insertOrUpdate")
async def proxy_insert_or_update(payload: CaseResultPayload):
    """代理转发：新增或更新测试结果到 qa.local:8088"""
    try:
        data = payload.dict(exclude_none=True)
        # 如果 id 为 None，移除 id 字段（新增时不传 id）
        if payload.id is None:
            data.pop('id', None)
        
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{REMOTE_API_BASE}/insertOrUpdate",
                json=data,
        ...
        return JSONResponse(
            status_code=504,
            content={"code": -1, "message": "请求远程服务器超时"}
        )
    except httpx.ConnectError as e:
        return JSONResponse(
            status_code=502,
            content={"code": -1, "message": f"无法连接到远程服务器: {str(e)}"}
        )
    except Exception as e:
        logger.error(f"代理请求失败: {e}")
        return JSONResponse(
            status_code=500,
            content={"code": -1, "message": f"代理请求失败: {str(e)}"}
        )


@app.post("/api/proxy/case-result/deleteById")
async def proxy_delete_by_id(payload: DeletePayload):
    """代理转发：删除测试结果"""
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{REMOTE_API_BASE}/deleteById",
                json={"id": payload.id},
                headers={'Content-Type': 'application/json'}
            )
            return response.json()
