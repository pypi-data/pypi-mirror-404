...
// Forward method for converting the input spherical harmonics
// coefficients of each Gaussian to a simple RGB color.
__device__ glm::vec3 computeColorFromSH(const int idx, const int* degs, const int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	// The implementation is loosely based on code for 
	// "Differentiable Point-Based Radiance Fields for 
	// Efficient View Synthesis" by Zhang et al. (2022)
...
// Used for inference with variable number of SH
template<int C>
__global__ void variableSHPreprocessCUDA(int P, int* D, const int* coeffsNum, 
	const int* perBandPrimitiveCount,
	const int* cumSumPrimitiveCount,
	...
	const float* opacities_unactivated,
	const float* shs,
	bool* clamped,
	...
	bool prefiltered)
{
	...
	// Activate opacity values
	const float opacity = sigmoid(opacities_unactivated[idx]);
	...
	{
		glm::vec3 result = computeColorFromSH(idx, coeffsNum, perBandPrimitiveCount, cumSumPrimitiveCount, (glm::vec3 *)orig_points, *cam_pos, shs, clamped);
...
void FORWARD::inferencePreprocess(int P, int* D, const int bandsNum, const int* coeffsNum,
    const int* perBandPrimitiveCount,
	const int* cumSumPrimitiveCount,
	const float* means3D,
	const glm::vec3* scales,
	...
	bool prefiltered)
{
	variableSHPreprocessCUDA<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, coeffsNum, perBandPrimitiveCount,
		cumSumPrimitiveCount,
		means3D,
		scales,
		scale_modifier,
		rotations,
