	...
	const bool debug)
{
	...
	if (P != 0)
	{
		...
		if (sh.size(0) != 0)
		{
			bandsNum = perBandPrimitiveCount.size(0);
		}

		rendered = CudaRasterizer::Rasterizer::inferenceForward(
			geomFunc,
			binningFunc,
			imgFunc,
			P, degrees.contiguous().data<int>(), bandsNum, coeffsNum.contiguous().data<int>(),
			perBandPrimitiveCount.contiguous().data<int>(),
			cumSumPrimitiveCount.contiguous().data<int>(),
	...
	const bool debug)
{
	...
	if (P != 0)
	{
		CudaRasterizer::Rasterizer::backward(P, degrees.contiguous().data<int>(), M, R,
											 background.contiguous().data<float>(),
											 W, H,
											 means3D.contiguous().data<float>(),
