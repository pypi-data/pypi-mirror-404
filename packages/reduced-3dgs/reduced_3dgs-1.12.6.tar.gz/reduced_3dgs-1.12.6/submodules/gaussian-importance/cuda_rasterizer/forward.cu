...
renderCUDA_count(
	...
	float* __restrict__ opacity_important_score,
	float* __restrict__ T_alpha_important_score)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
...
void FORWARD::count_gaussian(
	...
	const float* bg_color,
	int* gaussians_count,
	float* opacity_important_score,
	float* T_alpha_important_score,
	float* out_color)
{
	renderCUDA_count<NUM_CHANNELS> << <grid, block >> > (
		...
		out_color,
		gaussians_count,
		opacity_important_score,
		T_alpha_important_score);
}
