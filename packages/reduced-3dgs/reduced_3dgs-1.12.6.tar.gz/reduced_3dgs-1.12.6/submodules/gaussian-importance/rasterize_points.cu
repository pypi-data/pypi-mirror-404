	...
	const bool f_count)
{
  ...
  torch::Tensor opacity_important_score = torch::full({P}, 0.0, float_opts);
  torch::Tensor T_alpha_important_score = torch::full({P}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	...
	// overloaded forward, added gaussians_count, important_score
	  rendered = CudaRasterizer::Rasterizer::forwardCount(
		...
		opacity_important_score.contiguous().data<float>(),
		T_alpha_important_score.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(gaussians_count, opacity_important_score, T_alpha_important_score, rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer);
}
