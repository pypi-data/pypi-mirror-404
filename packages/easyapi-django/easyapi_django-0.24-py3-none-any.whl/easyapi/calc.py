        ...
        for key in groups:
            ...
            for i, nome_campo in enumerate(related_info):
                try:
                    ...
                    if i == len(related_info) - 1:  # Último item da lista
                        # print(f"Campo final: {field.name}, Tipo: {type(field).__name__}")
                        if field.choices:
                            fields[key] = {}
                            for choice in field.choices:
                                fields[key][choice[0]] = choice[1]
                    else:
                        tmp_model = field.related_model  # Navega para o próximo modelo relacionado
