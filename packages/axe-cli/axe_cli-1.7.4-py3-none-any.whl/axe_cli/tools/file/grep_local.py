...
from axe_cli.tools.utils import ToolResultBuilder, load_desc
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert axe_cli.__file__ is not None
    local_dep = Path(axe_cli.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    async with new_client_session() as session:
        with tempfile.TemporaryDirectory(prefix="axe-rg-") as tmpdir:
