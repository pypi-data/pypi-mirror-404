...
from axe_cli.config import Config, LLMModel, load_config, save_config
...
def _axe_code_base_url() -> str:
    if base_url := os.getenv("AXE_CODE_BASE_URL"):
        return base_url
    return "https://api.openrouter.ai/api/v1"  # Default to OpenRouter


PLATFORMS: list[Platform] = [
    Platform(
        id=AXE_CODE_PLATFORM_ID,
        name="Axe Code",
        base_url=_axe_code_base_url(),
    ),
    # Moonshot platforms removed - axe supports all providers via config
]
...
async def refresh_managed_models(config: Config) -> bool:
    ...
    for provider_key, provider in managed_providers.items():
        ...
        if not api_key and provider.oauth:
            from axe_cli.auth.oauth import load_tokens
