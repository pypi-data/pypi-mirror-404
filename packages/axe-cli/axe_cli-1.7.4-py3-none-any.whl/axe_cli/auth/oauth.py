...
from axe_cli.auth import AXE_CODE_PLATFORM_ID
from axe_cli.auth.platforms import (
    ModelInfo,
    get_platform_by_id,
    list_models,
    managed_model_key,
...
from axe_cli.utils.aiohttp import new_client_session
...
AXE_CODE_CLIENT_ID = "17e5f671-d194-4dfb-9706-5516cb48c098"
AXE_CODE_OAUTH_KEY = "oauth/axe-code"
DEFAULT_OAUTH_HOST = "https://auth.axe-code.dev"  # Placeholder - configure your own OAuth server
...
def _oauth_host() -> str:
    return os.getenv("AXE_CODE_OAUTH_HOST") or os.getenv("AXE_OAUTH_HOST") or DEFAULT_OAUTH_HOST
...
async def request_device_authorization() -> DeviceAuthorization:
    async with (
        ...
        session.post(
            ...
            data={"client_id": AXE_CODE_CLIENT_ID},
...
def _apply_axe_code_config(
    ...
    *,
    ...
    platform = get_platform_by_id(AXE_CODE_PLATFORM_ID)
    if platform is None:
        raise OAuthError("Axe Code platform not found.")

    provider_key = managed_provider_key(platform.id)
    config.providers[provider_key] = LLMProvider(
        type="openai_legacy",  # Changed from "kimi" to generic provider
    ...
    # Moonshot search/fetch services removed - no longer configuring


async def login_axe_code(
    ...
    platform = get_platform_by_id(AXE_CODE_PLATFORM_ID)
    if platform is None:
        yield OAuthEvent("error", "Axe Code platform is unavailable.")
        return
    ...
    oauth_ref = OAuthRef(storage="keyring", key=AXE_CODE_OAUTH_KEY)
...
async def logout_axe_code(config: Config) -> AsyncIterator[OAuthEvent]:
    if not config.is_from_default_location:
        yield OAuthEvent(
            "error",
    ...
    delete_tokens(OAuthRef(storage="keyring", key=AXE_CODE_OAUTH_KEY))
    delete_tokens(OAuthRef(storage="file", key=AXE_CODE_OAUTH_KEY))

    provider_key = managed_provider_key(AXE_CODE_PLATFORM_ID)
    ...
    # Moonshot services removed - no cleanup needed
...
class OAuthManager:
    ...
    def _iter_oauth_refs(self) -> list[OAuthRef]:
        ...
        # Moonshot services removed - no longer iterating their oauth refs
        return refs
    ...
    def _apply_access_token(self, runtime: Runtime, access_token: str) -> None:
        ...
        # Note: Kimi-specific provider removed - using standard OpenAI-compatible providers
