...
from axe_cli.ui.shell.keyboard import KeyboardListener, KeyEvent
from axe_cli.utils.aioqueue import QueueShutDown
from axe_cli.utils.diff import format_unified_diff
...
from axe_cli.utils.rich.syntax import AxeSyntax
from axe_cli.wire import WireUISide
...
class _ApprovalRequestPanel:
    def __init__(self, request: ApprovalRequest):
        ...
        self.options: list[tuple[str, ApprovalResponse.Kind]] = [
            ...
            ("Reject, tell Axe what to do instead", "reject"),
        ]
    ...
    def _render_block(
        ...
        if block.lexer:
            return AxeSyntax(text, block.lexer)
...
class _StatusBlock:
    ...
    def update(self, status: StatusUpdate) -> None:
        ...
        if status.context_usage is not None:
            parts.append(f"ctx: {status.context_usage:.1%}")
        
        # Add token counts if available
        if status.session_total_tokens is not None and status.session_total_tokens > 0:
            # Format tokens with thousands separator
            total = f"{status.session_total_tokens:,}"
            if status.session_input_tokens is not None and status.session_output_tokens is not None:
                input_tok = f"{status.session_input_tokens:,}"
                output_tok = f"{status.session_output_tokens:,}"
                parts.append(f"tokens: {input_tok}↑ {output_tok}↓ ({total})")
            else:
                parts.append(f"tokens: {total}")
        
        self.text.plain = " | ".join(parts) if parts else ""
