...
cli = typer.Typer(
    epilog="""\b\
Documentation:        https://github.com/SRSWTI/axe-code\n
LLM friendly version: https://github.com/SRSWTI/axe-code/blob/main/README.md""",
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"axe-cli, version {VERSION}")
        raise typer.Exit()
...
def axe_main(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.axe/config.toml.",
        ),
    ] = None,
    ...
    acp_mode: Annotated[
        ...
        typer.Option(
            "--acp",
            help="(Deprecated, use `axe acp` instead) Run as ACP server.",
        ),
    ] = False,
    wire_mode: Annotated[
        bool,
    ...
    from axe_cli.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from axe_cli.app import AxeCLI, enable_logging
    from axe_cli.config import Config, load_config_from_string
    from axe_cli.exception import ConfigError
    from axe_cli.metadata import load_metadata, save_metadata
...
@cli.command()
def login() -> None:
    """Login command (deprecated - use environment variables instead)."""
    from rich.console import Console

    console = Console()
    console.print("[yellow]Login is no longer supported.[/yellow]")
    console.print("Please configure your API key using environment variables:")
    console.print("  export AXE_API_KEY='your-api-key'")
    console.print("  export AXE_BASE_URL='https://your-llm-provider/v1'")
    console.print("  export AXE_MODEL_NAME='your-model-name'")


@cli.command()
def logout() -> None:
    """Logout command (deprecated - use environment variables instead)."""
    from rich.console import Console

    console = Console()
    console.print("[yellow]Logout is no longer supported.[/yellow]")
    console.print("To switch API keys, simply update your environment variables.")
...
def term(
    ...
    """Run Toad TUI backed by Axe Code CLI ACP server."""
...
@cli.command()
def acp():
    """Run Axe Code CLI ACP server."""
    from axe_cli.acp import acp_main
...
if __name__ == "__main__":
    if "axe_cli.cli" not in sys.modules:
        sys.modules["axe_cli.cli"] = sys.modules[__name__]
