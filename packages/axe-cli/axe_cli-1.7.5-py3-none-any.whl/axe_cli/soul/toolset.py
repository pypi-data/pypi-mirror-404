...
from axe_cli.exception import InvalidToolError, MCPRuntimeError
...
if TYPE_CHECKING:
    ...
    def type_check(axe_toolset: AxeToolset):
        _: Toolset = axe_toolset
...
class AxeToolset:
    ...
    def load_tools(self, tool_paths: list[str], dependencies: dict[type[Any], Any]) -> None:
        """
        Load tools from paths like `axe_cli.tools.shell:Shell`.
    ...
    async def load_mcp_tools(
        ...
        from axe_cli.ui.shell.prompt import toast
            ...
            for server_name, server_info in self._mcp_servers.items():
                ...
                if not await _check_oauth_tokens(server_url):
                    logger.warning(
                        ...
                        "Run 'axe mcp auth {server_name}' first.",
                        server_name=server_name,
                    )
...
class WireExternalTool(CallableTool):
    ...
    async def __call__(self, *args: Any, **kwargs: Any) -> ToolReturnValue:
        ...
        from axe_cli.soul import get_wire_or_none
