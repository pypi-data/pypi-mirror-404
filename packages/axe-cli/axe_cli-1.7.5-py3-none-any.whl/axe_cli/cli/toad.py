...
def _default_acp_command() -> list[str]:
    ...
    return [sys.executable, "-m", "axe_cli.cli", "acp"]


def _default_toad_command() -> list[str]:
    if sys.version_info < (3, 14):
        typer.echo("`axe term` requires Python 3.14+ because Toad requires it.", err=True)
        raise typer.Exit(code=1)
    if importlib.util.find_spec("toad") is None:
        typer.echo(
            "Toad dependency is missing. Install axe-cli with Python 3.14+ to use `axe term`.",
            err=True,
        )
        raise typer.Exit(code=1)
