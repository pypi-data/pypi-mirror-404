...
AXE_ANSI_THEME_NAME = "axe-ansi"
AXE_ANSI_THEME = ANSISyntaxTheme(
    {
...
def resolve_code_theme(theme: str | SyntaxTheme) -> str | SyntaxTheme:
    if isinstance(theme, str) and theme.lower() == AXE_ANSI_THEME_NAME:
        return AXE_ANSI_THEME
    return theme


class AxeSyntax(Syntax):
    def __init__(self, code: str, lexer: str, **kwargs: Any) -> None:
        if "theme" not in kwargs or kwargs["theme"] is None:
            kwargs["theme"] = AXE_ANSI_THEME
...
if __name__ == "__main__":
    ...
    for idx, (title, lexer, code) in enumerate(examples):
        ...
        console.print(AxeSyntax(code, lexer))
