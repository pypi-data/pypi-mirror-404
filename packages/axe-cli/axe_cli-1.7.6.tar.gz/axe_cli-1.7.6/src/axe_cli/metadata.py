...
def get_metadata_file() -> Path:
    """Get the metadata file path, migrating from legacy kimi.json if needed."""
    share_dir = get_share_dir()
    old_file = share_dir / "kimi.json"
    new_file = share_dir / "axe.json"
    
    if old_file.exists() and not new_file.exists():
        try:
            import shutil
            shutil.copy2(old_file, new_file)
            logger.info("Migrated metadata from kimi.json to axe.json")
        except Exception as e:
            logger.warning(f"Failed to migrate metadata file: {e}")
            return old_file  # Fallback to old file if migration fails
