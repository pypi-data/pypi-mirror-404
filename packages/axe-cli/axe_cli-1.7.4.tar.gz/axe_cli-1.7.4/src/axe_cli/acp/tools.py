...
from axe_cli.tools.shell import Params as ShellParams
from axe_cli.tools.shell import Shell
from axe_cli.tools.utils import ToolRejectedError, ToolResultBuilder
from axe_cli.wire.types import DisplayBlock
...
def replace_tools(
    ...
    if current_kaos not in (local_kaos.name, "acp"):
        # Only replace tools when running locally or under ACPKapos.
        return
...
class Terminal(CallableTool2[ShellParams]):
    ...
    async def __call__(self, params: ShellParams) -> ToolReturnValue:
        from axe_cli.acp.session import get_current_acp_tool_call_id_or_none
