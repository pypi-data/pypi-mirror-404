...
from axe_cli.soul.toolset import AxeToolset, WireExternalTool
from axe_cli.utils.aioqueue import Queue, QueueShutDown
...
from axe_cli.wire.types import ApprovalRequest, ApprovalResponse, Request, ToolCallRequest
...
class WireServer:
    ...
    async def _handle_initialize(
        ...
        toolset = None
        if isinstance(self._soul, AxeSoul) and isinstance(self._soul.agent.toolset, AxeToolset):
    ...
    def _apply_wire_client_info(self, client: ClientInfo | None) -> None:
        if not isinstance(self._soul, AxeSoul):
            return
        ...
        # Set custom user agent for providers that support it
        # Note: This works with OpenAI-compatible providers
        if hasattr(llm.chat_provider, 'client') and hasattr(llm.chat_provider.client, '_custom_headers'):
            client_obj = llm.chat_provider.client
            headers = dict(client_obj._custom_headers)  # pyright: ignore[reportPrivateUsage]
            headers["User-Agent"] = f"{USER_AGENT}{ua_suffix}"
            client_obj._custom_headers = headers  # pyright: ignore[reportPrivateUsage]
    ...
    async def _handle_prompt(
        ...
        try:
            await run_soul(
                ...
                self._cancel_event,
                self._soul.wire_file if isinstance(self._soul, AxeSoul) else None,
            )
            return JSONRPCSuccessResponse(
                id=msg.id,
