...
from cooperbench.agents.mini_swe_agent import Model
...
class GlobalModelStats:
    ...
    def __init__(self):
        ...
        if (self.cost_limit > 0 or self.call_limit > 0) and not os.getenv("MSWEA_SILENT_STARTUP"):
            logger.info(f"Global cost/call limit: ${self.cost_limit:.4f} / {self.call_limit}")
...
def get_model(input_model_name: str | None = None, config: dict | None = None) -> Model:
    ...
    if api_base := os.getenv("MSWEA_MODEL_API_BASE"):
        config.setdefault("model_kwargs", {})["api_base"] = api_base
...
_MODEL_CLASS_MAPPING = {
    "litellm": "cooperbench.agents.mini_swe_agent.models.litellm_model.LitellmModel",
}
