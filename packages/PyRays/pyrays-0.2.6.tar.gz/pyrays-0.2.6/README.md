...
6. To create the virtual environment, run `make virtualenv`. Note that this will also check for the requested Python version; if not available, it will automatically install it using `uv`.
...
> [!NOTE]
> Release notes are automatically generated by the `release.yml` workflow when you push a tag. The release notes include all commits since the previous tag, formatted as a list with commit messages and short hashes. You don't need to manually create or maintain a CHANGELOG.md file.
