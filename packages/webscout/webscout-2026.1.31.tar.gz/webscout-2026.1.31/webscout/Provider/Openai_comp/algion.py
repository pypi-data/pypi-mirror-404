...
class Completions(BaseCompletions):
    ...
    def _create_stream(
        ...
        try:
            ...
            print(f"Error during Algion stream request: {e}")
            raise IOError(f"Algion request failed: {e}") from e
    ...
    def _create_non_stream(
        ...
        try:
            ...
            print(f"Error during Algion non-stream request: {e}")
            raise IOError(f"Algion request failed: {e}") from e
...
class Algion(OpenAICompatibleProvider):
    required_auth = False
    AVAILABLE_MODELS = []
    ...
    def get_models(cls, api_key: Optional[str] = None):
        ...
        try:
            ...
            response = temp_session.get(
                "https://api.algion.dev/v1/models", headers=headers, impersonate="chrome110"
            )
