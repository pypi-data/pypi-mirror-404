"""
Success Rate Data Loader

Simple functions for loading play data and team data for success rate analysis.
...
from .models import TeamInfo, PlayData


def load_play_data(engine, season: int, max_week: int) -> List[PlayData]:
    """
    Load play-by-play data for success rate analysis.
    ...
    Returns:
        List of PlayData objects
    """
    logger = get_logger(__name__)
    
    try:
        # Read SQL file
        sql_path = Path("analytics/sql/success_rate_data.sql")
        ...
        for _, row in df.iterrows():
            play = PlayData(
                ...
                rush_attempt=bool(row['rush_attempt']),
                pass_attempt=bool(row['pass_attempt']),
                epa=float(row['epa']),
                is_success=bool(row['is_success'])
            )
            play_data.append(play)
        
        logger.info(f"Loaded {len(play_data)} plays for success rate analysis (season {season}, weeks 1-{max_week})")
        return play_data
...
def load_play_data_raw(engine, season: int, max_week: int) -> pd.DataFrame:
    """
    Load raw play-by-play data as DataFrame for success rate analysis.
    ...
    try:
        ...
        logger.info(f"Loaded {len(df)} raw plays for success rate analysis (season {season}, weeks 1-{max_week})")
        return df
        
    except Exception as e:
        logger.error(f"Failed to load raw play data: {e}")
        raise
