...
class OddsDataAnalyzer:
    ...
    def __init__(self, bucket_adapter, logger, output_dir: Path):
        """
    ...
    def generate_markdown_report(self) -> str:
        ...
        lines.append("")
        lines.append("### Play Ordering")
        lines.append("")
        lines.append("Play ordering for reconciliation checks follows:")
        lines.append("- Primary: `game_seconds_remaining` DESC (earlier in game first)")
        lines.append("- Secondary: `qtr` ASC")
        lines.append("- Tertiary: `play_id` ASC")
        lines.append("")
        lines.append("If reconciliation errors are observed, verify play ordering matches nflfastR's WPA calculation method.")
        lines.append("")
        lines.append("### Null Patterns")
        lines.append("")
        lines.append("See `null_patterns.csv` for detailed breakdown of why WPA values are missing (penalties, kickoffs, end-of-period, etc.).")
        lines.append("")
        ...
        lines.append("- `null_patterns.csv`: Root cause analysis of missing WPA values")
        lines.append("- `game_summary.csv`: Per-game reconciliation and leverage metrics")
        lines.append("- `full_data.csv`: Complete play-level dataset with odds columns")
    ...
    def export_csv(self) -> str:
        ...
        # Export
        filepath = self.output_dir / "full_data.csv"
    ...
    def export_game_summary_csv(self) -> str:
        ...
        # Sort by reconciliation error (highest first) before export
        if 'abs_recon_error_home' in self.game_summary.columns:
            export_df = self.game_summary.sort_values('abs_recon_error_home', ascending=False)
        else:
            export_df = self.game_summary
        
        filepath = self.output_dir / "game_summary.csv"
        self.logger.info(f"Exporting game summary ({len(export_df):,} games) to CSV...")
    ...
    def export_week_summary_csv(self) -> str:
        ...
        filepath = self.output_dir / "week_summary.csv"
    ...
    def analyze_null_patterns(self) -> pd.DataFrame:
        """
        Analyze patterns in null vegas_wpa plays to identify root causes.
        
        Returns:
            DataFrame with columns:
            - pattern_type: Category of null pattern
            - count: Number of plays matching pattern
            - pct_of_nulls: Percentage of all nulls
            - avg_per_game: Average occurrences per game
        """
        if self.df is None or 'vegas_wpa' not in self.df.columns:
            return pd.DataFrame()
        
        null_wpa = self.df[self.df['vegas_wpa'].isna()].copy()
        total_nulls = len(null_wpa)
        
        if total_nulls == 0:
            return pd.DataFrame()
        
        patterns = []
        total_games = self.df['game_id'].nunique() if 'game_id' in self.df.columns else 1
        
        # Pattern 1: Missing down (no normal play context)
        if 'down' in null_wpa.columns:
            no_down = null_wpa['down'].isna().sum()
            patterns.append({
                'pattern_type': 'missing_down',
                'count': int(no_down),
                'pct_of_nulls': round((no_down / total_nulls) * 100, 1),
                'avg_per_game': round(no_down / total_games, 1)
            })
        
        # Pattern 2: Penalty plays (no_play = true)
        if 'play_type' in null_wpa.columns:
            penalty = (null_wpa['play_type'] == 'no_play').sum()
            patterns.append({
                'pattern_type': 'penalty_no_play',
                'count': int(penalty),
                'pct_of_nulls': round((penalty / total_nulls) * 100, 1),
                'avg_per_game': round(penalty / total_games, 1)
            })
        
        # Pattern 3: Kickoffs (special teams context)
        if 'play_type' in null_wpa.columns:
            kickoff = (null_wpa['play_type'] == 'kickoff').sum()
            patterns.append({
                'pattern_type': 'kickoff',
                'count': int(kickoff),
                'pct_of_nulls': round((kickoff / total_nulls) * 100, 1),
                'avg_per_game': round(kickoff / total_games, 1)
            })
        
        # Pattern 4: End of period (< 10 seconds in quarter)
        if 'quarter_seconds_remaining' in null_wpa.columns:
            end_of_period = (null_wpa['quarter_seconds_remaining'] < 10).sum()
            patterns.append({
                'pattern_type': 'end_of_period',
                'count': int(end_of_period),
                'pct_of_nulls': round((end_of_period / total_nulls) * 100, 1),
                'avg_per_game': round(end_of_period / total_games, 1)
            })
        
        # Pattern 5: Special teams (punts, FGs, extra points)
        if 'play_type' in null_wpa.columns:
            special = null_wpa['play_type'].isin(['punt', 'field_goal', 'extra_point']).sum()
            patterns.append({
                'pattern_type': 'special_teams',
                'count': int(special),
                'pct_of_nulls': round((special / total_nulls) * 100, 1),
                'avg_per_game': round(special / total_games, 1)
            })
        
        return pd.DataFrame(patterns).sort_values('count', ascending=False)
    
    def export_null_patterns_csv(self) -> str:
        """
        Export null pattern analysis to CSV.
        
        Returns:
            str: Path to exported CSV file (or empty string if no patterns)
        """
        patterns = self.analyze_null_patterns()
        if patterns.empty:
            self.logger.info("No null patterns to analyze")
            return ""
        
        filepath = self.output_dir / "null_patterns.csv"
        
        self.logger.info(f"Exporting null patterns analysis...")
        patterns.to_csv(filepath, index=False)
        self.logger.info(f"✓ Exported null patterns CSV: {filepath}")
...
def main():
    ...
    output_dir = REPORTS_BASE_DIR / f"pbp_odds_analysis_{timestamp}"
    ...
    try:
        ...
        analyzer = OddsDataAnalyzer(bucket_adapter=bucket, logger=logger, output_dir=output_dir)
        ...
        for report_type, filepath in output_paths.items():
            if filepath:
                filename = Path(filepath).name
                logger.info(f"  - {report_type}: {filename}")
        logger.info("="*60)
