    ...
    def get_current_season(logger=None) -> int:
        ...
        try:
            ...
            if not schedule_df.empty and 'season' in schedule_df.columns:
                ...
                # Check if current season has any upcoming games
                current_date = datetime.now()
                schedule_df['gameday'] = pd.to_datetime(schedule_df['gameday'])
                
                # Check if there are any games in max_season that haven't happened yet
                max_season_games = schedule_df[schedule_df['season'] == max_season]
                upcoming_games = max_season_games[max_season_games['gameday'] >= pd.Timestamp(current_date)]
                
                if upcoming_games.empty:
                    # No upcoming games in max_season, season is over
                    logger.info(f"Season {max_season} is over (no upcoming games), returning {max_season + 1}")
                    return max_season + 1
                else:
                    # Still have upcoming games (including Super Bowl!)
                    logger.info(f"Current season determined from schedule data: {max_season} ({len(upcoming_games)} upcoming games)")
                    return max_season
