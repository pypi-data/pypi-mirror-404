...
class PurchaseOrderCreate(BaseModel):
    ...
    date_order: Optional[str] = Field(
        None, description="Fecha de la orden (YYYY-MM-DD)"
    )
...
class PurchaseOrderFilter(BaseModel):
    ...
    state: Optional[str] = Field(
        None, description="Estado de la orden (e.g., 'purchase', 'draft', 'done')"
    )
...
class SupplierPerformanceInput(BaseModel):
    ...
    supplier_ids: Optional[List[int]] = Field(
        None, description="Lista de IDs de proveedores (opcional)"
    )
...
class ProductAvailabilityInput(BaseModel):
    ...
    location_id: Optional[int] = Field(
        None, description="ID de la ubicación específica (opcional)"
    )
...
class InventoryAdjustmentCreate(BaseModel):
    ...
    adjustment_lines: List[InventoryLineAdjustment] = Field(
        description="Líneas de ajuste"
    )
...
class InventoryTurnoverInput(BaseModel):
    ...
    product_ids: Optional[List[int]] = Field(
        None, description="Lista de IDs de productos (opcional)"
    )
    category_id: Optional[int] = Field(
        None, description="ID de categoría de producto (opcional)"
    )
...
class JournalEntryCreate(BaseModel):
    ...
    lines: List[JournalEntryLineCreate] = Field(
        description="Líneas del asiento (debe y haber deben cuadrar)"
    )
...
class JournalEntryFilter(BaseModel):
    ...
    journal_id: Optional[int] = Field(
        None, description="Filtrar por diario contable ID"
    )
    state: Optional[str] = Field(
        None, description="Estado del asiento (e.g., 'posted', 'draft')"
    )
    limit: Optional[int] = Field(20, description="Límite de resultados")
    offset: Optional[int] = Field(0, description="Offset para paginación")
...
class FinancialRatioInput(BaseModel):
    ...
    ratios: List[str] = Field(
        description="Lista de ratios a calcular (e.g., ['liquidity', 'profitability', 'debt'])"
    )
