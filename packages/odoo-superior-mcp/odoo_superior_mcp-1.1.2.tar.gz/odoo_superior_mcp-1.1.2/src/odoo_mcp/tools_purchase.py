...
from .models import PurchaseOrderFilter, PurchaseOrderCreate, SupplierPerformanceInput
...
def register_purchase_tools(mcp: FastMCP) -> None:
    ...
    def search_purchase_orders(
        ctx: Context, filters: PurchaseOrderFilter
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logging.error("Tool 'search_purchase_orders'", e)
    ...
    def create_purchase_order(
        ...
        try:
            ...
            order_info = odoo.execute_method(
                "purchase.order", "read", [order_id], ["name"]
            )[0]
    ...
    def analyze_supplier_performance(
        ...
        try:
            ...
            for order in purchase_data:
                ...
                if order.get("effective_date") and order.get("date_planned"):
                    effective_date = datetime.strptime(
                        order["effective_date"].split(" ")[0], "%Y-%m-%d"
                    )
                    planned_date = datetime.strptime(
                        order["date_planned"].split(" ")[0], "%Y-%m-%d"
                    )
            ...
            for supplier_id, data in supplier_data.items():
                ...
                if total_deliveries > 0:
                    data["on_time_delivery_rate"] = (
                        data["on_time_delivery_count"] / total_deliveries
                    ) * 100
                else:
            ...
            top_suppliers = sorted(
                supplier_data.items(), key=lambda x: x[1]["total_amount"], reverse=True
            )
            ...
            result = {
                ...
                "summary": {
                    ...
                    "total_amount": sum(
                        order["amount_total"] for order in purchase_data
                    ),
                },
                "suppliers": [{"id": k, **v} for k, v in top_suppliers],
            }

            return {"success": True, "result": result}

        except Exception as e:
            logging.error("Tool 'analyze_supplier_performance'", e)
            return {"success": False, "error": str(e)}
