...
@tasks.command("quick-add")
@click.argument("title")
@click.option("--due-date", "-d", help="Due date (YYYY-MM-DD).")
@click.option("--no-assign", is_flag=True, help="Don't assign task to yourself.")
@click.pass_context
@handle_api_errors
@async_command
async def quick_add_task(
    ctx: click.Context, title: str, due_date: str | None, no_assign: bool
) -> None:
    """Quickly add a task to the default plan and assign to yourself.

    Requires default_plan_id in config.
    """
    settings = get_settings()
    if not settings.default_plan_id:
        click.echo("Error: default_plan_id not set in config.")
        click.echo("Set it in ~/.config/planer-cli/config.yaml")
        return
    ...
    try:
        ...
        if not no_assign:
            ...
            # Refresh task to get etag
            task = await planner_api.get_task(task.id)
            if task.etag:
                await planner_api.assign_task(task.id, me.id, task.etag)

        if ctx.obj.get("format") == "json":
            output_json(task)
        else:
            output_success(f"Created: {task.title}")
    finally:
        await client.close()
