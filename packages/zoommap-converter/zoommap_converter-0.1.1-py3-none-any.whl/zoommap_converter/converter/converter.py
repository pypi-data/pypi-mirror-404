...
def convert_leaflet_to_zoommap(
    ...
    if meters_per_px is None:
        logger.error(
            "Meters per pixel not calculated for Map ID '%s'", leaflet_block.data.id
        )
    ...
    if leaflet_shapes:
        for shape_data in leaflet_shapes.shapes:
            ...
            if shape_type == "rectangle":
                ...
                if size["w"] == 0 or size["h"] == 0:
                    logger.error(
                        "Invalid size dimensions for rectangle conversion: %s", size
                    )
                ...
                if size["w"] == 0 or size["h"] == 0:
                    logger.error(
                        "Invalid size dimensions for polygon conversion: %s", size
                    )
                ...
                if size["w"] == 0 or size["h"] == 0:
                    logger.error(
                        "Invalid size dimensions for polyline conversion: %s", size
                    )
