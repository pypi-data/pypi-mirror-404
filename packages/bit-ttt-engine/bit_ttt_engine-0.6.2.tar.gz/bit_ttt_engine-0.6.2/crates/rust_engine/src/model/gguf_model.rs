...
impl GgufLayer {
    ...
    fn ttt_forward(&mut self, hidden: &Tensor) -> Result<Tensor> {
        ...
        if self.ttt_w_state.is_none() || self.ttt_w_state.as_ref().unwrap().dim(0)? != batch {
            // Initialize W with small random values around identity
            // This ensures non-zero gradients on first pass (pure identity = zero gradients)
            let eye = Tensor::eye(d_small, DType::F32, device)?;
            let noise = Tensor::randn(0.0f32, 0.1f32, (d_small, d_small), device)?;
            let w_init = (eye + noise)?;
            self.ttt_w_state = Some(w_init.broadcast_left(batch)?);
        }
