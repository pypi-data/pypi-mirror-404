...
from kcwmysql import __version__
confkcws={}
confkcws['name']='kcwmysql'
confkcws['version']=__version__
confkcws['description']='kcwmysql'
...
def get_file(folder='./',lists=[]):
    ...
    return lists
def start():
    b=get_file("kcwmysql",['kcwmysql'])
    setup(
        name = confkcws["name"],
        version = confkcws["version"],
        keywords = "kcwmysql"+confkcws['version'],
        ...
        install_requires = ['PyMySQL==0.9.3','DBUtils==3.1.2','kcwcache>='+confkcws['version']], #第三方包
        package_data = {
