...
def test_get_project_metadata_fields():
    ...
    assert meta.name == "lib_shopify_graphql"
    assert meta.slug == "lib-shopify-graphql"
    assert meta.import_package == "lib_shopify_graphql"
    assert meta.coverage_source == "src/lib_shopify_graphql"
    assert meta.github_tarball_url("1.2.3").endswith("/bitranox/lib_shopify_graphql/archive/refs/tags/v1.2.3.tar.gz")
    assert meta.version
    assert meta.summary
    assert meta.author_name
    assert meta.metadata_module.as_posix().endswith("src/lib_shopify_graphql/__init__conf__.py")
