...
def clean_command(patterns: tuple[str, ...]) -> None:
    target_patterns = tuple(clean_module.get_clean_patterns()) + tuple(patterns)
...
@main.command(name="test-slow", help="Run integration tests against real Shopify shop (includes MySQL tests if configured)")
@click.option("--verbose", is_flag=True, help="Verbose pytest output")
@click.option("--coverage/--no-coverage", default=True, help="Enable/disable coverage collection")
def test_slow_command(verbose: bool, coverage: bool) -> None:
    raise SystemExit(test_module.run_tests_slow(verbose=verbose, coverage=coverage))
