...
def _run_pytest_step(
    ...
    if enable_coverage:
        ...
        pytest_result = run_fn(
            ["python", "-m", "pytest", "-m", "not integration", config.pytest_verbosity],
            capture=False,
            label="pytest-no-cov",
        )
        if pytest_result.code != 0:
...
def _check_mysql_configured() -> bool:
    """Check if MySQL is configured for integration tests."""
    return bool(os.environ.get("SHOPIFY__MYSQL__CONNECTION") or os.environ.get("SHOPIFY__MYSQL__HOST"))


def run_tests_slow(*, verbose: bool = False, coverage: bool = True) -> int:
    """Run slow/integration tests against a real Shopify shop.

    Runs all integration tests including MySQL tests if configured.
    Test execution order:
        1. JSON cache tests (test_integration.py)
        2. MySQL cache tests (test_integration_mysql.py) - if MySQL configured

    Requirements:
        SHOPIFY__SHOP_URL: Shop URL (e.g., mystore.myshopify.com)
        SHOPIFY__CLIENT_ID: OAuth client ID
        SHOPIFY__CLIENT_SECRET: OAuth client secret

    Optional (for MySQL tests):
        SHOPIFY__MYSQL__CONNECTION: MySQL connection string
        OR individual params: SHOPIFY__MYSQL__HOST, USER, PASSWORD, PORT, DATABASE
    ...
    required_vars = [
        "SHOPIFY__SHOP_URL",
        "SHOPIFY__CLIENT_ID",
        "SHOPIFY__CLIENT_SECRET",
    ]
    missing = [v for v in required_vars if not os.environ.get(v)]
    if missing:
        click.echo(f"[test-slow] Missing required environment variables: {', '.join(missing)}")
        click.echo("[test-slow] Set these in .env or environment before running")
        return 1

    shop_url = os.environ.get("SHOPIFY__SHOP_URL", "")
    mysql_configured = _check_mysql_configured()

    click.echo("[test-slow] ========================================")
    click.echo("[test-slow] Running integration tests against LIVE Shopify shop")
    click.echo(f"[test-slow] Shop URL: {shop_url}")
    click.echo(f"[test-slow] Coverage: {'enabled' if coverage else 'disabled'}")
    click.echo(f"[test-slow] MySQL tests: {'enabled' if mysql_configured else 'skipped (not configured)'}")
    click.echo("[test-slow] ========================================")

    config = TestConfig.from_pyproject(PROJECT_ROOT / "pyproject.toml")

    # Always use verbose pytest output to show what's being tested
    pytest_args = [
        "-v",  # Always verbose to show test names
        "--tb=short",
        "-s",  # No capture - show print statements live
    ]

    if coverage:
        click.echo("[test-slow] Setting up coverage collection...")
        _prune_coverage_data_files()
        _remove_report_artifacts(config.coverage_report_file)

        with tempfile.TemporaryDirectory() as tmpdir:
            coverage_file = Path(tmpdir) / ".coverage"
            env = os.environ | {"COVERAGE_FILE": str(coverage_file), "COVERAGE_NO_SQL": "1"}
            ...
            pytest_cmd = [
                ...
                f"--cov-report=xml:{config.coverage_report_file}",
                "--cov-report=term-missing",
                "--cov-fail-under=50",  # Lower threshold for integration tests
                *pytest_args,
            ]

            click.echo(f"[test-slow] Executing: {' '.join(pytest_cmd)}")
            click.echo("[test-slow] ----------------------------------------")
            result = subprocess.run(pytest_cmd, env=env, check=False)
            click.echo("[test-slow] ----------------------------------------")

            if result.returncode == 0:
                click.echo(f"[test-slow] All integration tests PASSED on shop '{shop_url}'")
                if Path(config.coverage_report_file).exists():
                    _ensure_codecov_token()
                    run_fn = _make_run_fn(verbose)
                    uploaded = _upload_coverage_report(run_fn=run_fn, coverage_report_file=config.coverage_report_file)
                    if uploaded:
                        click.echo("[test-slow] Coverage uploaded to Codecov")
                    _prune_coverage_data_files()
            else:
                click.echo(f"[test-slow] Integration tests FAILED on shop '{shop_url}'", err=True)
