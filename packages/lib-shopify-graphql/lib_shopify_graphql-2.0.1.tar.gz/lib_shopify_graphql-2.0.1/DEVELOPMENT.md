...
| `test-slow`           | Run integration tests against live Shopify (-m integration)                                |
| `coverage`            | Run pytest with coverage and generate report                                               |
...
| `dependencies`        | Check dependencies against latest PyPI versions                                            |
| `dependencies-update` | Update all outdated dependencies to latest versions                                        |
...
- **test-slow**
  - `TEST_VERBOSE=1` — verbose test runner output

- **coverage**
  - `TEST_VERBOSE=1` — verbose coverage run output
...
- **dependencies**
  - `VERBOSE=1` — show all dependencies (not just outdated)

- **dependencies-update**
  - `DRY_RUN=1` — preview changes without modifying pyproject.toml
...
- The library reads its own installed metadata at runtime via `importlib.metadata` (see `src/lib_shopify_graphql/__init__conf__.py`).
- Do not duplicate the version in code; bump only `pyproject.toml` and update `CHANGELOG.md`.
- Console script name is discovered from entry points; defaults to `lib-shopify-graphql`.
...
## Setting Up Shopify API Access for Development

This library uses **OAuth 2.0 Client Credentials Grant** for authentication. You need to create an app via the Shopify Dev Dashboard (not the store admin "custom apps" feature).

### Step 1: Become a Shopify Partner

Sign up for a free [Shopify Partner account](https://www.shopify.com/partners). This provides access to development stores (free, no expiration) and the Dev Dashboard.

### Step 2: Create a Development Store

1. From your Partner Dashboard, navigate to **Stores**
2. Click **Add store** → **Development store**
3. Fill in the required details and create the store

### Step 3: Create an App in the Dev Dashboard

1. Go to [https://dev.shopify.com/dashboard](https://dev.shopify.com/dashboard)
2. Click **Create an app**
3. Give it a name (e.g., "lib_shopify_graphql Development")
4. Select your development store as the target

### Step 4: Get Client Credentials

1. In your app's dashboard, go to **Client credentials**
2. Copy your **Client ID** and **Client secret**
3. **Important**: Store these securely - the client secret is shown only once

### Step 5: Configure API Scopes

1. In your app's dashboard, go to **Configuration** → **Admin API scopes**
2. Select the required permissions:
   - `read_products` — For reading product data
   - `write_products` — For modifying products
...
```bash
# .env
SHOPIFY__SHOP_URL=your-dev-store.myshopify.com
...
You now have API access configured for development. The library obtains OAuth tokens automatically (valid for 24 hours, refreshed as needed).
