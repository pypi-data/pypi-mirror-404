...
Note: Blender as a Python Module isn't provided on Blender's official download page.

  * A pre-compiled ``bpy`` module is
`available via PIP <https://pypi.org/project/bpy/>`_.

  * Or you may compile this yourself using the
`build instructions <https://developer.blender.org/docs/handbook/building_blender/python_module/>`_.
...
equivalents in Blender's Python API (``--threads`` and ``--log`` for example).
  ...
  Note: Access to these settings may be added in the future as needed.
...
Single Blend File Restriction
  ...
  Hint: As with the application it's possible to start multiple instances,
each with their own ``bpy`` and therefor Blender state.
Python provides the ``multiprocessing`` module to make communicating with sub-processes more convenient.In some cases the library API may be an alternative to starting separate processes,
