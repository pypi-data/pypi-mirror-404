...
**ServAI Model API**는 **화자 인식(Speaker Verification)** 및 **음성 인식(STT, Speech-to-Text)** 기능을 제공하는 고성능 AI 서버 애플리케이션.
...
* 입력된 음성이 `audio_source`에 등록된 기준 화자와 동일한지 판별
...
* `ngrok`이 내장되어 있어 로컬 서버를 외부에서 즉시 테스트 가능
...
이 프로젝트는 `src/core/config.py` 파일 하나로 **화자 인식 모델**과 **Whisper STT 모델**의 동작을 제어합니다.
...
### 3. 사용자 정의 모델 추가
새로운 모델을 추가하려면 `MODEL_REGISTRY`에 설정을 추가하면 됩니다.
...
```
*(기존 `pyproject.toml`이 있다면 `uv sync`로 한번에 설치 가능)*
...
1. 프로젝트 루트의 `resources/audio_source/` 디렉토리 확인 (없으면 생성)
2. 기준 화자의 깨끗한 음성 파일(.wav 권장)을 해당 폴더에 저장
3. API 실행 시 이 폴더의 오디오를 로드하여 임베딩 생성
...
```
> **Note**: `similarity_score`가 높을수록(1.0에 근접) 등록된 화자일 확률이 높음.
