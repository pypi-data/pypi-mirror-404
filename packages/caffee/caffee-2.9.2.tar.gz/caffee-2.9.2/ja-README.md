...
<div align="center">
【祝】PyPI 全期間でのダウンロードが1万を超えました🎉<br>
【祝】PyPI 全期間でのダウンロードが5千を超えました🎉
...
| `SET <var> <expr>` | 変数に式の結果を代入します。 |
| `IF <expr>` | 条件分岐を開始します。 `ENDIF` で閉じます。 |
| `ELSE` | `IF` 文内での別の条件ブロック（オプション）。 |
| `LOOP <count>` | 指定した回数だけブロックを繰り返します。 |
| `ENDLOOP` | `LOOP` ブロックの終了。 |

### 変数展開
マクロコマンド内では `{var_name}` という構文を使用して変数の値を参照できます。例えば、 `INSERT "Iteration {i}"` は変数 `i` の現在の値を挿入します。変数展開は、 `:` で始まるエディタコマンドでも有効です。
