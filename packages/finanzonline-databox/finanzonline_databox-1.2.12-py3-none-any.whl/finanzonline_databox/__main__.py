...
Provide the ``python -m finanzonline_databox`` path mandated by the
project's packaging guidelines. The wrapper delegates to
:func:`finanzonline_databox.cli.main` so that module execution mirrors the
...
def _module_main() -> int:
    ...
    Implements ``python -m finanzonline_databox`` by delegating to the
