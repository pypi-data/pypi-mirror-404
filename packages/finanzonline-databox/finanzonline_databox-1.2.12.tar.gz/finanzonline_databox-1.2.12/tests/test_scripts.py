...
def test_get_project_metadata_fields() -> None:
    ...
    assert meta.name == "finanzonline_databox"
    assert meta.slug == "finanzonline-databox"
    assert meta.import_package == "finanzonline_databox"
    assert meta.coverage_source == "src/finanzonline_databox"
    assert meta.github_tarball_url("1.2.3").endswith("/bitranox/finanzonline_databox/archive/refs/tags/v1.2.3.tar.gz")
    assert meta.version
    assert meta.summary
    assert meta.author_name
    assert meta.metadata_module.as_posix().endswith("src/finanzonline_databox/__init__conf__.py")
