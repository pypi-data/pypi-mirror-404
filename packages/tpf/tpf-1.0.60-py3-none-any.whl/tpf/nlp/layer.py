...
from tpf.nlp.att import MultiHead
...
class FullyConnectedOutput(torch.nn.Module):
    ...
    - [b, seq_len, feature_nums1] -> [b, seq_len, feature_nums2]
    """
    def __init__(self,features=32):
        super().__init__()
        self.fc = torch.nn.Sequential(
            torch.nn.Linear(in_features=features, out_features=features*2),
            torch.nn.ReLU(),
            torch.nn.Linear(in_features=features*2, out_features=features),
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self,features=32,head_num=4):
        ...
        super().__init__()
        # 多头注意力
        self.mh = MultiHead(feature_num=features,head_num=head_num)
...
#         # 自注意力提取输入的特征
#         self.mh1 = MultiHead(feature_num=features)
        
#         # 融合自己的输入和encoder的输出
#         self.mh2 = MultiHead(feature_num=features)
...
class DecoderLayer(torch.nn.Module):
    ...
    def __init__(self,features=32, head_num=4):
        ...
        # 自注意力提取输入的特征
        self.mh1 = MultiHead(feature_num=features,head_num=head_num)
        
        # 融合自己的输入和encoder的输出
        self.mh2 = MultiHead(feature_num=features,head_num=head_num)
...
class Encoder(torch.nn.Module):
    def __init__(self, features=32, head_num=4, layer_num=3):
        """编码器
        - features:序列元素特征维度个数，编码器不变换序列元素的位置，只变换元素特征的维度
        - head_num:多头的个数
        ...
        """
        super().__init__()
        # 使用 ModuleList 存储多个 DecoderLayer
        self.layers = torch.nn.ModuleList([
            EncoderLayer(features=features, head_num=head_num)
...
class Decoder(torch.nn.Module):
    ...
    def __init__(self, features=32, head_num=4, layer_num=3):
        ...
        # 使用 ModuleList 存储多个 DecoderLayer
        self.layers = torch.nn.ModuleList([
            DecoderLayer(features=features, head_num=head_num)
            for _ in range(layer_num)
        ])
