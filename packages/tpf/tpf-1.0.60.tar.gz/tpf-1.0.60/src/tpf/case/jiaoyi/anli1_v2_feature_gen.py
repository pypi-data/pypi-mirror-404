...
# 检查可用的列，用于调试
pc.lg(f"当前DataFrame列名: {list(df1.columns)[:10]}")
...
pc.lg(f"selected_features: {selected_features[:10]}")
...
pkl_save(selected_features, selected_features_file)

data_config_file = 'data_config_1.pkl'
data_config_file = os.path.join(os.getcwd(), data_config_file)
pkl_save(data_config, data_config_file)
pc.lg(f"保存数据配置文件: {data_config_file}")
