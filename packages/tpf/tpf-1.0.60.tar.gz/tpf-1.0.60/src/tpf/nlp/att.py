...
class MultiHead(nn.Module):
    ...
    """
    def __init__(self, feature_num=32, head_num=4):
        """多头注意力计算层
        - feature_num:特征数，最后是4的整数倍，会拆分成4个头，也叫hidden_size/embedding_dim 
        - 拆分的是特征向量n的维度 
        ...
        mhead = MultiHead(feature_num=pc.embedding_dim,head_num=pc.head_num)
        ...
        self.norm = nn.LayerNorm(normalized_shape=feature_num, elementwise_affine=True)

        # Q 矩阵
        self.fc_Q = nn.Linear(feature_num, feature_num)
        # K 矩阵
        self.fc_K = nn.Linear(feature_num, feature_num)
        # V 矩阵
        self.fc_V = nn.Linear(feature_num, feature_num)


        self.out_fc = nn.Linear(feature_num, feature_num)
    ...
    def attention(self, Q, K, V, mask=None):
        ...
        return 
        --------------------------
        - 上下文向量，shape=[batch_size, seq_len, feature_num]
        ...
        # b句话,每句话50个词,每个词编码成32维向量,4个头,每个头分到8维向量
        # Q, K, V = [b, 4, 50, 8]
        batch_size, head_n1, seq_len, head_n2 = Q.shape
        feature_num = head_n1*head_n2
        ...
        score = score.permute(0, 2, 1, 3).reshape(-1, seq_len, feature_num)
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        params
        -------------------------
        - K.shape=[batch_size,seq_len,embedding_dim],embedding_dim对应feature_num,
        ...
        feature_num = K.shape[2]
        head_n2 = int(feature_num/head_n1) 
        if head_n2 != feature_num/head_n1:
        ...
        Q = Q.reshape(bq, seq_len_q, head_n1, head_n2).permute(0, 2, 1, 3)
        K = K.reshape(bk, seq_len_k, head_n1, head_n2).permute(0, 2, 1, 3)
        V = V.reshape(bv, seq_len_v, head_n1, head_n2).permute(0, 2, 1, 3)
