...
def update_version(new_version=None):
    ...
    try:
        ...
        subprocess.run(['git', 'add', 'src/objectstate/__init__.py', 'pyproject.toml'], check=True)
...
def main():
    parser = argparse.ArgumentParser(description='Update ObjectState version number')
