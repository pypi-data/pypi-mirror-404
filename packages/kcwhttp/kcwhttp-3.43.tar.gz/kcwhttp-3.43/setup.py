...
from kcwhttp import __version__
confkcws={}
confkcws['name']='kcwhttp'
confkcws['version']=__version__
confkcws['description']='kcwhttp'
...
def get_file(folder='./',lists=[]):
    ...
    return lists
def start():
    b=get_file("kcwhttp",['kcwhttp'])
    setup(
        name = confkcws["name"],
        version = confkcws["version"],
        keywords = "kcwhttp"+confkcws['version'],
        ...
        install_requires = ['kcwredis>='+confkcws['version']], #第三方包
        package_data = {
