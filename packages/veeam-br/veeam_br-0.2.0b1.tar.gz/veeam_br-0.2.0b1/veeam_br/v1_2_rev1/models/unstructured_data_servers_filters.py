...
class UnstructuredDataServersFilters:
    ...
    Attributes:
        ...
        order_asc (bool | Unset): If `true`, sorts unstructured data servers in the ascending order by the `orderColumn`
            parameter.
        name_filter (str | Unset): Filters unstructured data servers by the `nameFilter` pattern. The pattern can match
            any unstructured data server parameter. To substitute one or more characters, use the asterisk (*) character at
