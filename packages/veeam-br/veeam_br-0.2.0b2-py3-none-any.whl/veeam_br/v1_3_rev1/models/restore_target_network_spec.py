...
class RestoreTargetNetworkSpec:
    """Network to which the restored VM will be connected. To get a network object, run the [Get Inventory
    Objects](Inventory-Browser#operation/GetInventoryObjects) request.

        Attributes:
            networks (list[RestoreTargetNetworkMappingModel]): Array of network mapping rules. To get a network object, run
                the [Get Inventory Objects](Inventory-Browser#operation/GetInventoryObjects) request.
            disconnected (bool | Unset): If `true`, the restored VMs are not connected to any virtual network.
    """
