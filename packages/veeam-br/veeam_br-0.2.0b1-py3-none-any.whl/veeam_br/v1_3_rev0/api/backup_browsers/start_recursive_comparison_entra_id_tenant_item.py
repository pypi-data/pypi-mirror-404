...
def sync_detailed(
    ...
    *,
    ...
    path allows you to start a recursive comparison of the properties of a Microsoft Entra ID
    Conditional Access policy available in a mount session that has the specified `sessionId`.<p>You can
    compare item properties either between two restore points or between a restore point and production.
    To compare the item to production, do not specify `newRestorePointId`.</p><p>This request is
    asynchronous, meaning that the request returns a comparison session immediately so the client can
    continue execution, and you can process the comparison results later when the session changes its
    state to `Success`. To get the comparison results, send the [Get Comparison Results for Microsoft
    Entra ID Conditional Access Policy](Backup-
    Browsers#operation/GetRecursiveComparisonEntraIdTenantItemResult) request. </p>**Available to**&#58;
