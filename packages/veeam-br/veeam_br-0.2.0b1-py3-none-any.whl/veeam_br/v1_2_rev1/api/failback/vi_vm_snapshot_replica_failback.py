...
def sync_detailed(
    *,
    ...
    """Start Failback
     ...
     The HTTP POST request to the `/api/v1/failback/vSphere/snapshot` path allows you to fail back from
    snapshot replicas to production VMware vSphere VMs.<p>Failback must be finalized. After you have
    switched over to production VMs (automatically or manually), do one of the following&#58;<ul><li>If
    a production VM works as expected, commit failback for the VM.</li><li>If a production VM does not
    work as expected, undo failback for the VM.</li></ul><p>**Available to**&#58; Veeam Backup
