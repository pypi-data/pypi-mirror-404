...
def sync_detailed(
    ...
    *,
    ...
    """Start Comparing Microsoft Entra ID Item Properties
     ...
     The HTTP POST request to the `/api/v1/backupBrowser/entraIdTenant/{sessionId}/startCompare` path
    allows you to start comparing the properties of a Microsoft Entra ID item available in a mount
    session that has the specified `sessionId`.<p>You can compare item properties either between two
    restore points or between a restore point and production. To compare the item to production, do not
    specify `newRestorePointId`.</p><p>This request is asynchronous, meaning that the request returns a
    ...
    results, send the [Get Comparison Results for Microsoft Entra ID
    Items](#tag/BackupBrowsers/operation/GetCompareEntraIdTenantItemResult) request. For details on
    synchronous request, see [Compare Microsoft Entra ID Item
    Properties](#tag/BackupBrowsers/operation/CompareEntraIdTenantItem).</p>**Available to**&#58; Veeam
