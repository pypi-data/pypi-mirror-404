...
class CapacityTierOverridePolicyModel:
    """Policy that overrides the move policy if the scale-out backup repository is reaching its capacity.

    Attributes:
        is_enabled (bool): If `true`, Veeam Backup & Replication moves oldest backup files sooner if the scale-out
            backup repository is reaching the threshold.
