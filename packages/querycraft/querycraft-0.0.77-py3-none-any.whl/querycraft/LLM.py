...
class OpenaiLLM(GenericLLM):
    def __init__(self, verbose, setHelp, sgbd, modele, api_key, base_url='https://api.openai.com/v1/chat/completions', bd=None):
        super().__init__(verbose, setHelp, sgbd, modele, bd)
        self.api_key = api_key
        self.base_url = base_url
        self.client = openai.OpenAI(api_key=self.api_key)
...
def manage_ia(cacheName, cfg, verbose, cacheKey, bd, erreur, sql, sol, intention, cmp):
    ...
    if rep == "":
        ...
        if cfg['IA']['service'] == 'ollama':
            ...
            rep = GoogleLLM(verbose, setHelp, cfg['Database']['type'], cfg['IA']['modele'], api_key=cfg['IA']['api-key'],
    ...
    if not rep:
        ...
        default_service = 'ollama_cloud'
        cache_file = f"{cacheName}_" + (default_service+ "_" + default_model).replace(':', '_') + "_default.json"

        if doCache:
            cache = loadCache(cache_file, duree)
            if cacheKey in cache:
                (rep, date) = cache[cacheKey]
                rep += buildCacheHint(cache_file, date, duree)

        if rep == "" and Ollama_API_KEY :

            rep = OllamaAPICloud(verbose, setHelp, cfg['Database']['type'], default_model, Ollama_API_KEY,
                            bd.tables2string()).run(erreur, sql, sol, intention, cmp)
            if doCache and rep: saveCache(cache_file, cache, cacheKey, rep)
        elif rep == "" and POE_API_KEY:
