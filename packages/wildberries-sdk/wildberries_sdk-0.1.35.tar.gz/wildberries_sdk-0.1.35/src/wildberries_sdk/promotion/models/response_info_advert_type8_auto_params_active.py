...
class ResponseInfoAdvertType8AutoParamsActive(BaseModel):
    """
    Места размещения
    """ # noqa: E501
    carousel: Optional[StrictBool] = Field(default=None, description="Карточка товара   - `false` — отключено   - `true` — включено ")
    recom: Optional[StrictBool] = Field(default=None, description="Рекомендации на главной   - `false` — отключено   - `true` — включено ")
    booster: Optional[StrictBool] = Field(default=None, description="Ручная ставка   - `false` — отключено   - `true` — включено ")
    __properties: ClassVar[List[str]] = ["carousel", "recom", "booster"]
