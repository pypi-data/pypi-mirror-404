...
class AdvV2AutoStatWordsGet200ResponseClustersInner(BaseModel):
    """
    AdvV2AutoStatWordsGet200ResponseClustersInner
    """ # noqa: E501
    cluster: Optional[StrictStr] = Field(default=None, description="Кластер — набор похожих ключевых фраз")
    count: Optional[StrictInt] = Field(default=None, description="Сколько раз товары показывались по всем фразам из кластера")
    keywords: Optional[List[StrictStr]] = Field(default=None, description="Ключевые фразы из кластера, по которым товары показывались хотя бы один раз")
    __properties: ClassVar[List[str]] = ["cluster", "count", "keywords"]
