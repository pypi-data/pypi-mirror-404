...
class Player:
    ...
    def get_camera_settings(self, camera_data: dict):
        ...
        for key, value in self.camera_settings.items():
            if value is None:
                logger.warning(
                    "Could not find " + key + " in camera settings for " + self.name
                )
    ...
    def get_data_from_car(self, car_data):
        ...
        for i in range(2):
            ...
            loadout["primary_color"] = (
                car_data["team_paint"]
                .get(i, {})
                .get("primary_color", 35 if i == 0 else 33)
            )
            loadout["accent_color"] = (
                car_data["team_paint"].get(i, {}).get("accent_color", 0)
            )
            loadout["primary_finish"] = (
                car_data["team_paint"].get(i, {}).get("primary_finish", 270)
            )
            loadout["accent_finish"] = (
                car_data["team_paint"].get(i, {}).get("accent_finish", 270)
            )
