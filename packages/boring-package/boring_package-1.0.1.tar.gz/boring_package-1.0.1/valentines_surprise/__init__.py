...
def text_to_ascii(text):
    """Convert text to 5-line ASCII block letters"""
    chars = {
        'A': [" ███  ", "█   █ ", "█████ ", "█   █ ", "█   █ "],
        'B': ["████  ", "█   █ ", "████  ", "█   █ ", "████  "],
        'C': [" ████ ", "█     ", "█     ", "█     ", " ████ "],
...
def show_message(name=None):
    ...
    print("\n\n\n\n\n")  # Space for countdown (5 lines)
    
    # Countdown from 10 to 1
    for i in range(10, 0, -1):
        # Move cursor up 5 lines to overwrite countdown
        print("\033[5A", end="", flush=True)
        countdown_lines = text_to_ascii(str(i))
        for line in countdown_lines:
            # Clear entire line first, then print centered
