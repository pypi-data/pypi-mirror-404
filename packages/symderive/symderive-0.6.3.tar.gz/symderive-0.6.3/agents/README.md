...
| **SymPy direct usage** | Flags `sympy.simplify()` etc. in `src/symderive/` | Yes |
...
Warns when source files in `src/symderive/` are modified without a corresponding test file in `tests/`.
