...
### Added
- All agent personas now available in `.claude/agents/` for Claude Code CLI
  ...
  - **jimsimons**: Quantitative researcher (statistical signals, backtesting, risk)
  - **billpress**: Numerical algorithms expert (stability, conditioning, error analysis)
  - **edwardtufte**: Information design critic (clarity, density, honest encoding)
  - **beavis**: Fast sanity checker for quick gut checks
  - **butthead**: Skeptical contrarian for stress-testing reasoning
  - Previously only `ed` and `steve` were available as Claude Code subagents

## [0.6.2] - 2026-01-31

### Changed
- **BREAKING**: Module renamed from `derive` to `symderive` - import with `from symderive import *` (previously `from derive import *`)
- Product name changed from "Derive" to "symderive" throughout documentation
- README reorganized: Installation section now appears before Quick Start
- Clarified package name vs import name in documentation

### Added
- `CLAUDE.md` project instructions file for AI coding agents
- Victor (Mingsy) Chua added to LICENSE

## [0.5.0] - 2026-01-31

### Changed
- **PyPI package name: `symderive`** - Install with `pip install symderive`, import with `from symderive import *`
