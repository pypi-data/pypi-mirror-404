...
class Project:
    ...
    def idea(
        ...
        **kwargs: Any,
        ...
        """
        result = self._run_step("idea", timeout=timeout, mode=mode, **kwargs)
        # Extract text from nested result
        inner = result.get("result", {})
        if isinstance(inner, dict):
            inner = inner.get("result", inner)
        return str(inner)

    def literature(self, timeout: float = 600, **kwargs: Any) -> str:
        """Run literature search."""
        result = self._run_step("literature", timeout=timeout, **kwargs)
        inner = result.get("result", {})
        if isinstance(inner, dict):
            inner = inner.get("result", inner)
        return str(inner)
    ...
    def methods(
        ...
        **kwargs: Any,
        ...
        result = self._run_step("methods", timeout=timeout, mode=mode, **kwargs)
        inner = result.get("result", {})
        if isinstance(inner, dict):
            inner = inner.get("result", inner)
        return str(inner)
    ...
    def paper(
        ...
        **kwargs: Any,
        ...
        """Run paper writing."""
        result = self._run_step("paper", timeout=timeout, journal=journal, **kwargs)
        return result.get("result", result)

    def review(self, timeout: float = 600, **kwargs: Any) -> str:
        """Run review."""
        result = self._run_step("review", timeout=timeout, **kwargs)
        inner = result.get("result", {})
        if isinstance(inner, dict):
            inner = inner.get("result", inner)
        return str(inner)
