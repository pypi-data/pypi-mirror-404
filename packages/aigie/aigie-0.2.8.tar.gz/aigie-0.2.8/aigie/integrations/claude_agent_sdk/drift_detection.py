...
class AgentPlan:
    ...
    # Planning turn outputs (from thinking/reasoning turns)
    planning_turn_outputs: List[Dict[str, Any]] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "planning_turn_outputs": self.planning_turn_outputs,
            "timestamp": self.timestamp.isoformat(),
            "model": self.model,
        }
...
class DriftDetector:
    ...
    def capture_planning_turn_output(self, turn_type: str, output: Any) -> None:
        """
        Capture output from a planning turn (e.g., thinking, reasoning).

        Args:
            turn_type: Type of turn (e.g., 'thinking', 'reasoning', 'planning')
            output: The output from the turn
        """
        # Detect if this is a planning turn by type
        planning_keywords = ['thinking', 'think', 'reason', 'reasoning', 'plan', 'planning', 'decide', 'strategy']
        is_planning_turn = any(kw in turn_type.lower() for kw in planning_keywords)

        if is_planning_turn:
            output_text = str(output) if output else ""
            self.plan.planning_turn_outputs.append({
                "turn_type": turn_type,
                "output_preview": output_text[:500],
                "timestamp": datetime.now().isoformat(),
            })

            # Extract planning from this turn's output
    ...
    def _extract_plan_from_prompt(self, text: str) -> None:
        ...
        # Look for tool mentions - comprehensive patterns
        tool_patterns = [
            r"use (?:the )?(\w+) tool",
        ...
        # Look for step indicators - enhanced patterns
        step_patterns = [
