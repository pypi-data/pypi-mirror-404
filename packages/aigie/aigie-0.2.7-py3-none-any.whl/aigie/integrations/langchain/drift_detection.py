...
class DriftDetector:
    ...
    def _extract_plan_from_response(self, text: str) -> None:
        """Extract planning from agent's/chain's first response."""
        # Look for "I'll first..." or "Let me..." patterns
        action_patterns = [
            r"(?:I'll|Let me|First,? I'll|I will)\s+(.+?)(?:\.|$)",
            r"(?:Step \d+[:.]\s*)(.+?)(?=Step \d+|$)",
        ]
        ...
        for pattern in action_patterns:
            matches = re.findall(pattern, text, re.IGNORECASE | re.DOTALL)
            if matches:
                for match in matches[:5]:  # Limit to first 5 steps
                    step = match.strip()[:100]
                    if len(step) > 10:
                        self.plan.planned_steps.append(step)

        # Look for tool mentions in planning
        tool_mentions = re.findall(r"\[(?:Using|Tool)[:\s]*(\w+)\]", text)
        self.plan.expected_tools.update(tool_mentions)

        # Look for explicit tool usage mentions
        tool_usage = re.findall(r"(?:use|using|call|calling)\s+(?:the\s+)?(\w+)\s+tool", text, re.IGNORECASE)
        self.plan.expected_tools.update(tool_usage)
