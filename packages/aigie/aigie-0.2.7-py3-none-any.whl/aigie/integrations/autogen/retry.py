"""
Retry and timeout utilities for AutoGen/AG2 integration.
...
class ConversationError(Exception):
    """Raised when an AutoGen conversation fails."""

    def __init__(self, message: str, agent: Optional[str] = None, turn: Optional[int] = None):
        super().__init__(message)
        self.agent = agent
        self.turn = turn
...
class RetryContext:
    ...
    Example:
        async with RetryContext(max_retries=3, operation="conversation") as ctx:
            result = await ctx.execute(agent.a_initiate_chat, recipient)
...
class ConversationRetryContext(RetryContext):
    """Specialized retry context for AutoGen conversations.

    Tracks additional conversation-specific metrics like turns and messages.

    Example:
        async with ConversationRetryContext(max_retries=3, operation="chat") as ctx:
            result = await ctx.execute(agent.a_initiate_chat, recipient)
            print(f"Turns completed: {ctx.turns_completed}")
    """
    ...
    def __init__(
        ...
        self.failed_turn: Optional[int] = None
    ...
    def record_failure(self, agent: Optional[str] = None, turn: Optional[int] = None) -> None:
        """Record where failure occurred."""
        self.failed_agent = agent
        self.failed_turn = turn

    def get_metrics(self) -> dict:
        """Get conversation-specific retry metrics for tracing."""
