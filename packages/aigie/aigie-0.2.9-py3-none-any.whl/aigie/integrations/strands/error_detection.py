...
ERROR_PATTERNS = [
    ...
    (r"name\s+['\"]?\w+['\"]?\s+is\s+not\s+defined", ErrorType.TOOL_EXECUTION, ErrorSeverity.MEDIUM, False),
    ...
    (r"api\s+call\s+failed|api\s+request\s+failed", ErrorType.API_ERROR, ErrorSeverity.MEDIUM, True),
    (r"failed\s+to\s+(?:fetch|get|retrieve|connect)", ErrorType.NETWORK, ErrorSeverity.MEDIUM, True),
]
