"""
Retry and timeout utilities for Strands Agent SDK integration.
...
class StrandsExecutionError(Exception):
    """Raised when Strands agent execution fails after all retries."""
...
class RetryContext:
    ...
    Example:
        async with RetryContext(max_retries=3, operation="strands_invoke") as ctx:
            result = await ctx.execute(my_agent, prompt)
