"""
Retry and timeout utilities for Google ADK integration.
...
class GoogleADKExecutionError(Exception):
    """Raised when Google ADK agent execution fails after all retries."""

    def __init__(self, message: str, agent_name: str = "", last_error: Optional[Exception] = None):
        super().__init__(message)
        self.agent_name = agent_name
        self.last_error = last_error


def _is_google_api_transient(exception: Exception) -> bool:
    """Check if a Google API exception is transient (should retry)."""
    exc_type = type(exception).__name__
    return exc_type in GOOGLE_API_TRANSIENT_ERRORS
    ...
    **kwargs,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Check if it's a transient Google API error
            if _is_google_api_transient(e):
                last_error = e
                if attempt < max_retries:
                    logger.warning(
                        f"{operation_name} Google API transient error (attempt {attempt + 1}/{max_retries + 1}): {e}. "
                        f"Retrying in {delay}s..."
                    )
                    await asyncio.sleep(delay)
...
class RetryContext:
    ...
    Example:
        async with RetryContext(max_retries=3, operation="google_adk_invoke") as ctx:
            result = await ctx.execute(runner.run_async, ...)
    ...
    async def execute(self, func: Callable[..., Any], *args, **kwargs) -> Any:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                # Check for transient Google API errors
                if _is_google_api_transient(e):
                    self.errors.append(e)
                    if attempt < self.max_retries:
                        logger.warning(
                            f"{self.operation} Google API error (attempt {attempt + 1}/{self.max_retries + 1}): {e}"
                        )
                        await asyncio.sleep(delay)
                        delay = min(delay * 2, self.max_delay)
                        continue
