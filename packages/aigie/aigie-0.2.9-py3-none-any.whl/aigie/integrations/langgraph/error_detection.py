...
ERROR_PATTERNS = [
    ...
    # Graph-specific errors
    (r"graph\s+(?:execution|invocation)\s+(?:error|failed)", ErrorType.GRAPH_ERROR, ErrorSeverity.HIGH, False),
    (r"node\s+(?:execution|invocation)\s+(?:error|failed)|node\s+error", ErrorType.NODE_ERROR, ErrorSeverity.MEDIUM, False),
    (r"edge\s+(?:condition|evaluation)\s+(?:error|failed)|invalid\s+edge", ErrorType.EDGE_ERROR, ErrorSeverity.MEDIUM, False),
    (r"state\s+(?:validation|update)\s+(?:error|failed)|invalid\s+state", ErrorType.STATE_ERROR, ErrorSeverity.MEDIUM, False),
]
