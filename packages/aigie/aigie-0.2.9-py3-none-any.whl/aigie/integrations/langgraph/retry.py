"""
Retry and timeout utilities for LangGraph integration.
...
class GraphExecutionError(Exception):
    """Raised when a LangGraph execution fails."""
...
class RetryContext:
    ...
    Example:
        async with RetryContext(max_retries=3, operation="graph_invoke") as ctx:
...
class GraphRetryContext(RetryContext):
    """Specialized retry context for LangGraph workflows.

    Tracks additional graph-specific metrics like nodes executed and state changes.

    Example:
        async with GraphRetryContext(max_retries=3, operation="workflow") as ctx:
            result = await ctx.execute(graph.ainvoke, inputs)
    ...
    def record_state_transition(self, from_state: dict, to_state: dict, node: str) -> None:
    ...
    def record_failure(self, node_name: str) -> None:
        """Record the node where failure occurred."""
        self.failed_node = node_name

    def get_metrics(self) -> dict:
        """Get graph-specific retry metrics for tracing."""
