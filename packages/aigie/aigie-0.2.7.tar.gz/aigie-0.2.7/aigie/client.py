...
class Aigie:
    ...
    async def _initialize_autonomous_features(self) -> None:
        ...
        try:
            ...
            await self._mode_controller.set_mode(mode)
            logger.debug(f"Mode controller initialized ({mode} mode)")

            # Initialize Gateway client for real-time validation
            from .gateway import GatewayWebSocketClient
            self._gateway_client = GatewayWebSocketClient()

            # Connect to gateway (non-blocking, will retry in background)
            try:
                gateway_url = self.api_url.replace("http://", "ws://").replace("https://", "wss://")
                await self._gateway_client.connect(f"{gateway_url}/gateway/ws")
