...
async def _handle_invoke_error_async(
    ...
    detected_error = error_detector.detect_from_exception(
        error, f"graph:{workflow_name}", {}
    )
    ...
    if aigie._buffer:
        ...
        try:
            ...
            if loop.is_running():
                ...
                asyncio.ensure_future(aigie._buffer.add(EventType.TRACE_UPDATE, update_data))
            else:
