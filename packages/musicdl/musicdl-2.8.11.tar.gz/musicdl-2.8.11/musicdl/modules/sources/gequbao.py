...
from ..utils import legalizestring, usesearchheaderscookies, resp2json, safeextractfromdict, searchdictbykey, extractdurationsecondsfromlrc, seconds2hms, cleanlrc, SongInfo, QuarkParser
...
class GequbaoMusicClient(BaseMusicClient):
    ...
    def _parsesearchresultsfromhtml(self, html_text: str):
        soup, base_url, search_results = BeautifulSoup(html_text, "lxml"), "https://www.gequbao.com", []
        for a in soup.select("a.music-link"):
            if not a.get("href", "").strip(): continue
    ...
    def _search(self, keyword: str = '', search_url: str = '', request_overrides: dict = None, song_infos: list = [], progress: Progress = None, progress_id: int = 0):
        ...
        try:
            ...
            for search_result in search_results:
                # --download results
                if not isinstance(search_result, dict) or ('href' not in search_result): continue
                song_info = SongInfo(source=self.source)
                # ----fetch basic information
                try: resp = self.get(search_result['href'], **request_overrides); resp.raise_for_status()
                except: continue
