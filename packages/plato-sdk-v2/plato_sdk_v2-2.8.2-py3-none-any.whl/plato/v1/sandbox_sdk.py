...
from plato.v1.models.sandbox import (
...
def _get_lib():
    ...
    if _lib is None:
        ...
        _lib.plato_proxytunnel_start.argtypes = [
            ctypes.c_char_p,
            ctypes.c_char_p,
            ctypes.c_int,
            ctypes.c_int,
        ...
        _lib.plato_gitea_merge_to_main.argtypes = [
            ctypes.c_char_p,
            ctypes.c_char_p,
            ctypes.c_char_p,
        ]
        ...
        _lib.plato_get_state.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int]
        _lib.plato_get_state.restype = ctypes.c_void_p
...
class PlatoSandboxClient:
    ...
    def create_sandbox(
        ...
        logger.info(
            f"Creating sandbox: artifact_id={artifact_id}, service={service}, dataset={dataset}, sandbox_timeout={sandbox_timeout}"
        )
        lib = _get_lib()
        ...
        if config is not None:
            self._sandbox_configs[sandbox.public_id] = {"config": config, "dataset": dataset}
    ...
    def close_sandbox(self, public_id: str) -> None:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_delete_sandbox(self._client_id.encode("utf-8"), public_id.encode("utf-8"))
    ...
    def create_snapshot(self, public_id: str, request: CreateSnapshotRequest) -> CreateSnapshotResponse:
        """
        ...
        result_ptr = lib.plato_create_snapshot(
            self._client_id.encode("utf-8"), public_id.encode("utf-8"), request_json.encode("utf-8")
        )
    ...
    def create_checkpoint(self, public_id: str, request: CreateSnapshotRequest) -> CreateSnapshotResponse:
        """
    ...
    def create_snapshot_with_cleanup(
        ...
        db_config: DBConfig | dict[str, Any] | None = None,
    ) -> CreateSnapshotResponse:
        """
    ...
    def start_worker(self, public_id: str, request: StartWorkerRequest | dict[str, Any]) -> StartWorkerResponse:
        """
    ...
    def list_simulators(self) -> list[SimulatorListItem]:
        ...
        """
        lib = _get_lib()
        result_ptr = lib.plato_list_simulators(self._client_id.encode("utf-8"))
    ...
    def get_simulator_versions(self, simulator_name: str) -> list[dict[str, Any]]:
        ...
        """
        lib = _get_lib()
        result_ptr = lib.plato_get_simulator_versions(self._client_id.encode("utf-8"), simulator_name.encode("utf-8"))
    ...
    def wait_until_ready(self, correlation_id: str, timeout: int = 600) -> None:
        """
        ...
        result_ptr = lib.plato_monitor_operation(
            self._client_id.encode("utf-8"), correlation_id.encode("utf-8"), ctypes.c_int(timeout)
        )
    ...
    def get_state(self, job_group_id: str, merge_mutations: bool = False) -> dict[str, Any]:
        """
        Get the current state of an environment.

        Args:
            job_group_id: Job group ID of the sandbox
            merge_mutations: If True, merge pending mutations into state (default: False)

        Returns:
            Dict containing the environment state

        Raises:
            RuntimeError: If getting state fails
        ...
        Example:
            ...
            >>> print(state)
        """
        logger.debug(f"Getting state for job_group_id={job_group_id}, merge_mutations={merge_mutations}")
        lib = _get_lib()
        result_ptr = lib.plato_get_state(
            self._client_id.encode("utf-8"),
            job_group_id.encode("utf-8"),
            ctypes.c_int(1 if merge_mutations else 0),
        )
        ...
        if isinstance(response, dict) and "error" in response and response["error"] is not None:
            logger.error(f"Failed to get state: {response['error']}")
            raise RuntimeError(f"Failed to get state: {response['error']}")

        # Handle null/None response (shouldn't happen with updated Go code, but be safe)
        if response is None:
            logger.debug("State is None, returning empty dict")
            return {}

        logger.info(f"Got state for job_group_id={job_group_id}")
        return response
    ...
    def get_gitea_credentials(self) -> dict[str, str]:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_gitea_get_credentials(self._client_id.encode("utf-8"))
    ...
    def list_gitea_simulators(self) -> list[dict[str, Any]]:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_gitea_list_simulators(self._client_id.encode("utf-8"))
    ...
    def get_gitea_repository(self, simulator_id: int) -> dict[str, Any]:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_gitea_get_simulator_repo(self._client_id.encode("utf-8"), ctypes.c_int(simulator_id))
        ...
        logger.info(
            f"Got repository for simulator {simulator_id}: {response.get('name')} (clone_url: {response.get('clone_url')})"
        )
        return response
    ...
    def create_gitea_repository(self, simulator_id: int) -> dict[str, Any]:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_gitea_create_simulator_repo(self._client_id.encode("utf-8"), ctypes.c_int(simulator_id))
        ...
        logger.info(
            f"Created repository for simulator {simulator_id}: {response.get('name')} (clone_url: {response.get('clone_url')})"
        )
        return response
    ...
    def stop_proxy_tunnel(self, tunnel_id: str) -> None:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_proxytunnel_stop(self._client_id.encode("utf-8"), tunnel_id.encode("utf-8"))
    ...
    def list_proxy_tunnels(self) -> list[dict[str, Any]]:
        ...
        lib = _get_lib()
        result_ptr = lib.plato_proxytunnel_list(self._client_id.encode("utf-8"))
