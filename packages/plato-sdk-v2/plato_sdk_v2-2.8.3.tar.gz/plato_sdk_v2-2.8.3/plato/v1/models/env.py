...
class PlatoEnvironment:
    ...
    async def evaluate(self, value: Any | None = None, agent_version: str | None = None) -> EvaluationResult:
        if not self._run_session_id:
            raise PlatoClientError("No active run session. Call reset() first.")
    ...
    def get_db_login_info(self, database: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "dest_port": int(cfg.get("dest_port", 5432 if cfg.get("db_type") == "postgresql" else 3306)),
    ...
    async def start_db_tunnel(
        ...
        if self._db_tunnel_process and self._db_tunnel_process.poll() is None:
            raise PlatoClientError("Database tunnel already running. Stop it before starting a new one.")
