    ...
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 medium pizza from papa johns with no toppings",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            type="custom",
            score_fn=lambda x: llm_judge_eval_fn(
                x, "There should be 1 order with nothing but 1 juicy pork bao from Dumpling story"
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            type="custom",
            score_fn=lambda x: llm_judge_eval_fn(
                x, "There should be 1 order with nothing but 1 Classic Grandma from pie punks"
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 2 fried chicken sandwhiches from The Bird",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            type="custom",
            score_fn=lambda x: llm_judge_eval_fn(
                x, "There should be 1 order with nothing but 10 piece buffalo wings from The Bird"
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 large chicken shawarma fries from Popstar shawarma",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 10 boneless wings from Wingstop. I want only original hot flavour",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 chicken pad thai from OSHA Thai Restaurant with medium spice level.",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 chicken over salad from Halal Cart SF",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            type="custom",
            score_fn=lambda x: llm_judge_eval_fn(
                x, "There should be 1 order with nothing but 1 Chicken Quesadilla from Chisme"
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 Gai Noodle Soup from Gai Chicken Rice",
            ),
        ),
    ),
]
...
multiple_items_specific_tasks = [
    ...
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 large poke bowl from Poke Bowl. For my sauce I want spicy mayo sauce. For protein I want crab. For my sides I want cucumber and carrot. For my base I want white rice. For toppings I want Sesame Seeds and Fried garlic chips.",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 chicken salad from Souvla. get dressing on the side and get no onions. also get a side of greek fries and a lemon soda.",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 2 burritos from Chipotle. The first one should be white rice, black beans and chicken. With medium salsa, cheese and lettuce. The second one should be a brown rice, pinto beans and steak with hot salsa, sour cream, guacamole, cheese and lettuce.",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 extra large pizza from Bellissimo's. I want 3 toppings. 1. pepperoni 2. mushroom 3. pineapple. I also want a side of garlic bread and 3 bottles of water.",
            ),
        ),
    ),
]
...
specific_restaurant_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 thin crust cheese pizza from Little Caesars",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            type="custom",
            score_fn=lambda x: llm_judge_eval_fn(
                x, "There should be 1 order with nothing but buffalo wings from Proposition Chicken"
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 extra large pepperoni pizza from North Beach pizza",
            ),
        ),
    ),
]
...
advanced_customization_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 10-piece Chicken McNuggets meal from McDonalds with large coke and hot mustard sauce",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 Double Cheeseburger from McDonalds with no onions and extra cheese",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 milk tea from Feng Cha with jasmine green tea base, Regular Ice, 50% sweet, oat milk, boba pearls and grass jelly",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 poke bowl from Ea Cafe with spicy crab meat cucumber, sweet corn, miso sauce, cabbage, and pineapple",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 3Pc spicy Tenders Combo from Popeyes with blackened ranch sauce, cajun fries and a diet coke and a biscuit",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 Fan Favorite Box from KFC with extra crispy chicken, 2x gravy, 2x ranch, 4 large Starrys, and a 10 piece cherry pie popper with a plate",
            ),
        ),
    ),
]
...
multiple_items_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 1 pizza with no toppings and a 2 liter bottle of pepsi from papa johns",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but Steamed BBQ Pork Bao, Shrimp & Pork Siu Mai, Tom Yum, and Crispy Sesame Tofu Squares from Dumpling Time",
            ),
        ),
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            ...
            score_fn=lambda x: llm_judge_eval_fn(
                x,
                "There should be 1 order with nothing but 2 Chicken Tikka Masalas, 2 Basmati Rices, 1 garlic naan and 1 plain naan from NEWA Nepalese Indian Resturant",
            ),
        ),
    ),
]
...
bonus_tasks = [
    ...
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
            type="custom",
            score_fn=lambda x: llm_judge_eval_fn(
                x, "There should be 1 order with Umami Fries from RT Rotisserie and a 30% tip"
            ),
        ),
    ),
]
