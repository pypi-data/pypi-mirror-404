...
class SyncPlatoEnvironment:
    ...
    def login(
        ...
        if from_api:
            ...
            if not os.path.exists(scripts_path):
                raise PlatoClientError(f"Flow scripts not found for env_id '{self.env_id}' at {scripts_path}")
    ...
    def reset(
        ...
        **kwargs,
        ...
        """
        response = self._client.reset_environment(self.id, task, agent_version, model, load_authenticated, **kwargs)
        if task:
    ...
    def get_state_mutations(self, merge_mutations: bool = False) -> list[dict[str, Any]]:
    ...
    def get_evaluation_result(self) -> EvaluationResult:
        ...
        if isinstance(eval_config, CustomEvalConfig):
            ...
            return EvaluationResult(success=False, reason=f"Unknown evaluation type: {eval_config.type}")

    def evaluate(self, value: Any | None = None, agent_version: str | None = None) -> EvaluationResult:
