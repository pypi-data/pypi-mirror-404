"""Synchronous flow execution engine for Plato v2."""
...
class FlowExecutor:
    ...
    def _wait_for_selector(self, step: WaitForSelectorStep) -> None:
        try:
    ...
    def _fill(self, step: FillStep) -> None:
        value = step.value
        try:
            self.page.wait_for_selector(step.selector, timeout=step.timeout)
    ...
    def _wait(self, step: WaitStep) -> None:
        try:
    ...
    def _navigate(self, step: NavigateStep) -> None:
        try:
    ...
    def _wait_for_url(self, step: WaitForUrlStep) -> None:
        try:
    ...
    def _check_element(self, step: CheckElementStep) -> None:
        try:
    ...
    def _verify(self, step: VerifyStep) -> None:
        handlers = {
    ...
    def _verify_element_exists(self, step: VerifyStep) -> None:
        try:
    ...
    def _verify_element_visible(self, step: VerifyStep) -> None:
        try:
            element = self.page.query_selector(step.selector) if step.selector else None
    ...
    def _verify_element_text(self, step: VerifyStep) -> None:
        try:
            element = self.page.query_selector(step.selector) if step.selector else None
            if not element:
                raise FlowExecutionError(f"Element not found: {step.selector}")
    ...
    def _verify_element_count(self, step: VerifyStep) -> None:
        try:
    ...
    def _verify_page_title(self, step: VerifyStep) -> None:
        try:
    ...
    def _screenshot(self, step: ScreenshotStep) -> None:
        try:
    ...
    def _verify_text(self, step: VerifyTextStep) -> None:
        try:
    ...
    def _verify_url(self, step: VerifyUrlStep) -> None:
        try:
    ...
    def _verify_no_errors(self, step: VerifyNoErrorsStep) -> None:
        try:
            errors_found = []
