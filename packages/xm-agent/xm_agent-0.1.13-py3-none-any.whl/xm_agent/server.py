...
async def get_config():
    """Get server configuration (read-only subset)."""
    settings = get_settings()
    return {
        "models_path": settings.models_path,
        "model_types": settings.model_types,
        "sd_output_path": settings.sd_output_path,
    }
...
class GenerateRequestBody(BaseModel):
    ...
    # Required
    prompt: str
    model: str  # Path to model file, or filename if model_type is specified
    model_type: str | None = None  # checkpoints, loras, etc. - if set, model is resolved relative to this
...
async def start_generation(request: GenerateRequestBody):
    ...
    # Resolve model path: absolute path, or relative to model_type dir
    model_path = Path(request.model)
    if not model_path.is_absolute():
        # Use model_type if specified, otherwise default to checkpoints
        model_type_dir = request.model_type or "checkpoints"
        model_path = Path(settings.models_path) / model_type_dir / request.model
