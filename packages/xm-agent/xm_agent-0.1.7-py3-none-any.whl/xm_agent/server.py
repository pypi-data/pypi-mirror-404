...
async def get_model_hash(model_type: str, filename: str):
...
async def delete_model_file(model_type: str, filename: str):
    """Delete a model file."""
    settings = get_settings()
    if model_type not in settings.model_types:
        raise HTTPException(
    ...
    deleted = delete_model(model_type, filename)
    if not deleted:
        raise HTTPException(status_code=404, detail="Model not found")
...
class DownloadRequest(BaseModel):
    ...
    url: str
    type: str  # model type (loras, checkpoints, etc.)
    filename: str
...
async def start_download(request: DownloadRequest):
    ...
    # Resolve CivitAI model ID to version URL if needed
    download_url = request.url
    filename = request.filename
    model_type = request.type
    expected_hash = request.expected_hash

    civitai_id = extract_civitai_id(request.url)
    if civitai_id:
        resolved = await resolve_civitai_model(civitai_id, request.api_key)
        if resolved:
            download_url = resolved["url"]
            filename = resolved["filename"]
            if resolved["type"]:
                model_type = resolved["type"]
            if resolved["expected_hash"]:
    ...
    dest = settings.get_model_path(model_type) / filename
