...
class MinimalExclusive(Constraint[T]):
    ...
    value: T
    """
    The lower exclusive limit for the value.
    """
    ...
    async def validate(self, value: T) -> bool:
        ...
        if value <= self.value:
            msg = f"Value '{value}' must be strictly greater than the minimal exclusive limit of '{self.value}'."
    ...
    def serialize(self, serializer: Serializer) -> None:
        serializer.write_str("MinimalExclusive", str(self.value))
