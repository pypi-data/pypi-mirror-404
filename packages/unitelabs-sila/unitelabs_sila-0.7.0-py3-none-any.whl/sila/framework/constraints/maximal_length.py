...
class MaximalLength(Constraint[T]):
    """
    A constraint that enforces a maximal length for a `String` or `Binary` value.
    ...
    value: int
    """
    The maximal allowed length for the value. Must be a non-negative
    integer and less than 2⁶³.
    """

    def __post_init__(self):
        if self.value < 0:
            msg = f"Maximal length must be a non-negative integer, received '{self.value}'."
            raise ValueError(msg)

        if self.value >= 2**63:
            msg = f"Maximal length must be less than 2⁶³, received '{self.value}'."
            raise ValueError(msg)
    ...
    async def validate(self, value: T) -> bool:
        ...
        if len(value.value) > self.value:
            msg = f"Expected value with maximal length '{self.value}', received '{len(value.value)}'."
            raise ValueError(msg)
    ...
    def serialize(self, serializer: Serializer) -> None:
        serializer.write_str("MaximalLength", str(self.value))
