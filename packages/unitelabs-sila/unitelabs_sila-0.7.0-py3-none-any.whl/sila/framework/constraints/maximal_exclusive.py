...
class MaximalExclusive(Constraint[T]):
    ...
    value: T
    """
    The upper exclusive limit for the value.
    """
    ...
    async def validate(self, value: T) -> bool:
        ...
        if value >= self.value:
            msg = f"Value '{value}' must be strictly less than the maximal exclusive limit of '{self.value}'."
    ...
    def serialize(self, serializer: Serializer) -> None:
        serializer.write_str("MaximalExclusive", str(self.value))
