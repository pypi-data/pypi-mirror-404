...
class Path__Handler__Key_Based(Path__Handler):                                  # Hash-based sharding path handler for references
    ...
    def generate_path(self, file_id : Safe_Str__Id          = None,
        ...
        if file_id is None:
            raise ValueError('In Path__Handler__Key_Based, file_id cannot be None')

        if file_key is None:
            raise ValueError('In Path__Handler__Key_Based, file_key cannot be None')
