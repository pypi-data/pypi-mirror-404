...
def CUSTOMIZED_select(
    ...
    **kwargs: Any,
    ...
    # CUSTOMIZED: 'default' is not passed to InquirerControl anymore
    # See https://github.com/tmbo/questionary/issues/473 for details
    ic = InquirerControl(
        choices,
        pointer=pointer,
        ...
        if ic.is_answered:
            if isinstance(ic.get_pointed_at().title, list):
                tokens.append(
                    (
                        "class:answer",
                        "".join([token[1] for token in ic.get_pointed_at().title]),  # type: ignore
                    )
                )
            else:
                tokens.append(("class:answer", ic.get_pointed_at().title))  # type: ignore
        else:
            if instruction:
