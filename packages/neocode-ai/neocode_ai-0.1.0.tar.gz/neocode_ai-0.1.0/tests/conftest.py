...
from neocode_ai import Neocode, AsyncNeocode, DefaultAioHttpClient
from neocode_ai._utils import is_dict
...
logging.getLogger("neocode_ai").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Neocode]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Neocode(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncNeocode]:
    ...
    async with AsyncNeocode(base_url=base_url, _strict_response_validation=strict, http_client=http_client) as client:
        yield client
