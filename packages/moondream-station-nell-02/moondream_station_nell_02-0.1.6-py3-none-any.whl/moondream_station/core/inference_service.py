...
class InferenceService:
    ...
    async def load_model(self) -> Dict[str, Any]:
        """Load model weights on all worker backends."""
        if not self.worker_backends:
            return {"error": "Inference service not started"}
        backend = self._get_next_backend()
        if not backend or not hasattr(backend, "load_model"):
            return {"error": "Backend does not support load_model"}
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, backend.load_model)
        return result

    async def unload_model(self) -> Dict[str, Any]:
        """Unload model weights from all worker backends."""
        if not self.worker_backends:
            return {"error": "Inference service not started"}
        backend = self._get_next_backend()
        if not backend or not hasattr(backend, "unload_model"):
            return {"error": "Backend does not support unload_model"}
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, backend.unload_model)
        return result
