...
class REPLSession:
    ...
    def load_and_prepare(self, model_name: str = None):
        """Load manifest and prepare a model without entering the REPL loop.

        Args:
            model_name: Model to switch to. If None, uses the default model.
        """
        if self.manifest_source:
            self._load_manifest(self.manifest_source)
            self.analytics._initialize_posthog()

        if model_name:
            if not self.models.switch_model(model_name, self.display):
