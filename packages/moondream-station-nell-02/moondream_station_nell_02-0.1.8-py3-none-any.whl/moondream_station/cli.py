...
def main(
    ...
    host: Optional[str] = typer.Option(None, "--host", help="Host to bind to (used with --serve, e.g. 127.0.0.1)"),
):
    ...
    if serve:
        _run_serve(manifest, model, port, host)
    else:
        session = REPLSession(manifest_source=manifest)
        session.start()


def _run_serve(manifest: str, model: Optional[str], port: Optional[int], host: Optional[str]):
    ...
    serve_host = host or session.config.get("service_host", "0.0.0.0")
    ...
    try:
        ...
        rprint(f"[red]Error: Port {serve_port} is already in use.[/red]")
        raise typer.Exit(1)
    model_name = active_model.name

    session.config.set("service_host", serve_host)
    ...
    rprint(f"  Endpoint: http://{serve_host}:{serve_port}/v1")
