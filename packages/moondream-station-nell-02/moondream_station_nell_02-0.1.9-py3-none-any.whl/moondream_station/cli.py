...
def main(
    ...
    model: Optional[str] = typer.Option(None, "--model", help="Model to load on startup"),
    port: Optional[int] = typer.Option(None, "--port", "-p", help="Port to serve on"),
    host: Optional[str] = typer.Option(None, "--host", help="Host to bind to (e.g. 127.0.0.1)"),
):
    ...
    session = REPLSession(manifest_source=manifest, port=port, host=host, model=model)
    session.start()
