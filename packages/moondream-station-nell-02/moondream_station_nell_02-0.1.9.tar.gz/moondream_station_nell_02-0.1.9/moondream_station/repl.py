...
class REPLSession:
    def __init__(self, manifest_source: str = None, port: int = None, host: str = None, model: str = None):
        self.console = Console()
        self.manifest_source = manifest_source
        self.config = ConfigManager()
        self.prompts = Prompts()

        # Apply CLI overrides for port and host
        if port is not None:
            self.config.set("service_port", port)
        if host is not None:
            self.config.set("service_host", host)

        self.cli_model = model  # Store for later use after manifest loads
    ...
    def _load_manifest(self, source: str):
        ...
        try:
            ...
            # Auto-switch to CLI model or default model if no model is selected
            if self.cli_model:
                # Switch to CLI-specified model
                if self.models.switch_model(self.cli_model, self.display):
                    self.display.success(f"Loaded model: {self.cli_model}")
                else:
                    self.display.error(f"Failed to load model: {self.cli_model}")
