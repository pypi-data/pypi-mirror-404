...
class InboxAlert(BaseModel):
    """An alert from the inbox (supports acknowledge/resolve/dismiss)."""

    id: str = Field(..., description="Alert public UUID")
    title: str = Field(..., description="Alert title")
    description: str | None = Field(None, description="Alert description")
    severity: str = Field(default="info", description="Severity level")
    alert_state: str = Field(
        ..., description="State: new, acknowledged, resolved, dismissed"
    )
    event_type: str = Field(..., description="Event type that triggered the alert")
    ...
    destination_name: str | None = Field(None, description="Destination name")
...
class ValidityCheckResult(BaseModel):
    ...
    invalid_samples: dict[str, Any] | None = Field(
        None, description="Sample invalid records"
    )
...
class ReferentialCheck(BaseModel):
    ...
    min_child_count: int | None = Field(
        None, description="Min child count (cardinality)"
    )
    max_child_count: int | None = Field(
        None, description="Max child count (cardinality)"
    )
    is_active: bool = Field(default=True, description="Whether active")
...
class ReferentialCheckResult(BaseModel):
    ...
    cardinality_sample: list[Any] | None = Field(
        None, description="Sample cardinality issues"
    )
...
class SchemaMonitoringStatus(BaseModel):
    ...
    baseline_captured_at: datetime | None = Field(
        None, description="When baseline was captured"
    )
...
class FreshnessRecommendationsResponse(BaseModel):
    ...
    tables_with_recommendations: int = Field(
        ..., description="Tables with recommendations"
    )
...
class MetricRecommendation(BaseModel):
    ...
    suggested_check_type: str = Field(
        ..., description="Check type: validity, referential, metric"
    )
...
class MetricsRecommendationsResponse(BaseModel):
    ...
    columns_with_recommendations: int = Field(
        ..., description="Columns with recommendations"
    )
...
class CoverageRecommendation(BaseModel):
    ...
    currently_monitored: MonitoringStatus = Field(
...
class CoverageRecommendationsResponse(BaseModel):
    ...
    coverage_percentage: float = Field(
        ..., description="Percentage of tables monitored"
    )
...
class ThresholdRecommendation(BaseModel):
    ...
    direction: str = Field(
        ..., description="Adjustment direction: increase or decrease"
    )
    ...
    projected_reduction: str = Field(
        ..., description="Estimated alert reduction (e.g., '80%')"
    )
...
class ThresholdsRecommendationsResponse(BaseModel):
    ...
    items_with_recommendations: int = Field(
        ..., description="Items with recommendations"
    )
