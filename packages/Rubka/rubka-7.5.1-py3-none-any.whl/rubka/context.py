...
class Message:
    def __init__(self, bot, chat_id, message_id, sender_id, text=None, raw_data=None):
        ...
        self.text: str = self.raw_data.get("text") or ""
        ...
        self.is_username = bool(re.search(r"@\w+", self.raw_data.get("text", text) or ""))
        self.is_id = self.is_username
        self.is_mention = bool("MentionText" in self.meta_types)
        ...
        if self.file and self.file.file_name:
            ...
            self.is_gif = (name.endswith(".mp4") and (1024 <= size <= 1_048_000)) or name.endswith(".gif")
    @property
    def session(self):
    ...
    async def copy_message(self, to_chat_id: Optional[str], message_id: Optional[str] = None):
        try:
            ...
            elif getattr(self, "is_gif", False):
                send_func = self.bot.send_gif
