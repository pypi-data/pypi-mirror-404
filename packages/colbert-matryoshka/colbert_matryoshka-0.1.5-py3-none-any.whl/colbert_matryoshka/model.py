...
class MatryoshkaColBERT(PyLateColBERT):
    ...
    def forward(
        ...
        # Ensure projection heads are on the same device as input
        if token_embeddings.device != next(self.projection_heads.parameters()).device:
            self.projection_heads = self.projection_heads.to(token_embeddings.device)
    ...
    # encode() is inherited from PyLate ColBERT - no need to reimplement
    # PyLate's encode() internally calls self.forward() which we override above
    # This automatically applies the Matryoshka projection head
