...
def _get_version() -> str:
    """Get version from installed package or pyproject.toml in dev mode."""
    try:
        return version("hearthfire")
    except PackageNotFoundError:
        pass

    # Dev mode: try to read from pyproject.toml
    try:
        import tomllib
    except ImportError:
        try:
    ...
    # Search for root pyproject.toml (contains name = "hearthfire")
    ...
    for parent in current.parents:
        ...
        if pyproject.exists():
            try:
                ...
                if project.get("name") == "hearthfire":
                    return project.get("version", "dev")
            except Exception:
                pass
    return "dev"
...
class HearthMCPServer:
    ...
    async def _handle_hearth_version(self, args: dict) -> dict:
        """Return hearthfire version information."""
        return {
            "version": __version__,
            "package": "hearthfire",
        }
