...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        module_path = "disassembler_output.spells.spells"
        ...
        if outputToText:
            shutil.rmtree("./src/assembler_output/spells/txt", ignore_errors=True)
            os.makedirs("./src/assembler_output/spells/txt", exist_ok=True)
        if outputToBin:
            shutil.rmtree("./src/assembler_output/spells/bin", ignore_errors=True)
            os.makedirs("./src/assembler_output/spells/bin", exist_ok=True)
        if outputToPatch:
            os.makedirs("./src/assembler_output/spells/bps", exist_ok=True)
        ...
        # Import the spell collection
        module = importlib.import_module(module_path)
        spell_collection = module.ALL_SPELLS

        # Render the spell collection
        collection = spell_collection.render()

        if collection:
            for start, bytes_ in collection.items():
                if outputToBin:
                    with open(f'./src/assembler_output/spells/bin/write_to_0x{start:06X}.img', 'wb') as f:
                        f.write(bytes_)
                if outputToText:
                    with open(f'./src/assembler_output/spells/txt/write_to_0x{start:06X}.txt', 'w') as f:
            ...
            if outputToPatch:
                ...
                with open(f'./src/assembler_output/spells/bps/smrpg-spells-{datetime.now().strftime("%Y%m%d%H%M%S")}.bps', 'wb') as f:
                    write_bps(bps_progress(iterable), f)

        self.stdout.write(self.style.SUCCESS("successfully assembled spell data"))
