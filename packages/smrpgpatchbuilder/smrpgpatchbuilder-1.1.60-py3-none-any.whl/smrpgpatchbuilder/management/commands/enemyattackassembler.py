...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        module_path = "disassembler_output.enemy_attacks.attacks"
        ...
        if outputToText:
            shutil.rmtree("./src/assembler_output/enemy_attacks/txt", ignore_errors=True)
            os.makedirs("./src/assembler_output/enemy_attacks/txt", exist_ok=True)
        if outputToBin:
            shutil.rmtree("./src/assembler_output/enemy_attacks/bin", ignore_errors=True)
            os.makedirs("./src/assembler_output/enemy_attacks/bin", exist_ok=True)
        if outputToPatch:
            os.makedirs("./src/assembler_output/enemy_attacks/bps", exist_ok=True)
        ...
        if combined_patch:
            for start, bytes_ in combined_patch.items():
                if outputToBin:
                    with open(f'./src/assembler_output/enemy_attacks/bin/write_to_0x{start:06X}.img', 'wb') as f:
                        f.write(bytes_)
                if outputToText:
                    with open(f'./src/assembler_output/enemy_attacks/txt/write_to_0x{start:06X}.txt', 'w') as f:
            ...
            if outputToPatch:
                ...
                with open(f'./src/assembler_output/enemy_attacks/bps/smrpg-enemy-attacks-{datetime.now().strftime("%Y%m%d%H%M%S")}.bps', 'wb') as f:
                    write_bps(bps_progress(iterable), f)

        self.stdout.write(self.style.SUCCESS("successfully assembled enemy attack data"))
