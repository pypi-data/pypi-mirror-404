...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if outputToText:
            shutil.rmtree("./src/assembler_output/items/txt", ignore_errors=True)
            os.makedirs("./src/assembler_output/items/txt", exist_ok=True)
        if outputToBin:
            shutil.rmtree("./src/assembler_output/items/bin", ignore_errors=True)
            os.makedirs("./src/assembler_output/items/bin", exist_ok=True)
        if outputToPatch:
            os.makedirs("./src/assembler_output/items/bps", exist_ok=True)
        ...
        # import the items module and get the itemcollection
        module = importlib.import_module(module_path)
        collection = module.ALL_ITEMS

        # render the collection to get the patch data
        try:
            patch_data = collection.render()
        except ValueError as e:
            self.stderr.write(self.style.ERROR(f"error rendering items: {e}"))
            exit(1)
        ...
        if patch_data:
            for start, bytes_ in patch_data.items():
                if outputToBin:
                    with open(
                        f"./src/assembler_output/items/bin/write_to_0x{start:06X}.img",
                        "wb",
                    ) as f:
                        f.write(bytes_)
                if outputToText:
                    with open(
                        f"./src/assembler_output/items/txt/write_to_0x{start:06X}.txt",
                        "w",
                    ) as f:
            ...
            if outputToPatch:
                ...
                with open(
                    f'./src/assembler_output/items/bps/smrpg-{datetime.now().strftime("%Y%m%d%H%M%S")}.bps',
                    "wb",
                ) as f:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"successfully created bps patch at ./src/assembler_output/items/bps/"
                    )
                )
            ...
            if outputToText:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"successfully created text files at ./src/assembler_output/items/txt/"
                    )
                )
            ...
            if outputToBin:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"successfully created binary files at ./src/assembler_output/items/bin/"
                    )
                )
