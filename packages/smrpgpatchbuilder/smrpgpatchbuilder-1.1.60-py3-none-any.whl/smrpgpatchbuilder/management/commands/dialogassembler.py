...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        module_path = "disassembler_output.dialogs.dialogs"
        ...
        if outputToText:
            shutil.rmtree("./src/assembler_output/dialogs/txt", ignore_errors=True)
            os.makedirs("./src/assembler_output/dialogs/txt", exist_ok=True)
        if outputToBin:
            shutil.rmtree("./src/assembler_output/dialogs/bin", ignore_errors=True)
            os.makedirs("./src/assembler_output/dialogs/bin", exist_ok=True)
        if outputToPatch:
            os.makedirs("./src/assembler_output/dialogs/bps", exist_ok=True)
        ...
        collection = module.data.render()
        
        if collection:
            for start, bytes_ in collection.items():
                if outputToBin:
                    with open(f'./src/assembler_output/dialogs/bin/write_to_0x{start:06X}.img', 'wb') as f:
                        f.write(bytes_)
                if outputToText:
                    with open(f'./src/assembler_output/dialogs/txt/write_to_0x{start:06X}.txt', 'w') as f:
        ...
        self.stdout.write(self.style.SUCCESS("successfully assembled dialog data"))
