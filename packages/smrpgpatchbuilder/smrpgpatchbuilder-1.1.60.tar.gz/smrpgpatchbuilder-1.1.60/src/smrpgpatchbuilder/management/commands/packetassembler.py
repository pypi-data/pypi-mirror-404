...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        module_path = "disassembler_output.packets.packets"
        ...
        if outputToText:
            shutil.rmtree("./src/assembler_output/packets/txt", ignore_errors=True)
            os.makedirs("./src/assembler_output/packets/txt", exist_ok=True)
        if outputToBin:
            shutil.rmtree("./src/assembler_output/packets/bin", ignore_errors=True)
            os.makedirs("./src/assembler_output/packets/bin", exist_ok=True)
        if outputToPatch:
            os.makedirs("./src/assembler_output/packets/bps", exist_ok=True)

        rom = bytearray()
        original_rom = bytearray()
        if outputToPatch:
            original_rom = bytearray(open(romPath, "rb").read())
            rom = deepcopy(original_rom)

        # Import the packet collection
        module = importlib.import_module(module_path)
        packet_collection = module.ALL_PACKETS

        # Render the packet collection
        collection = packet_collection.render()

        if collection:
            for start, bytes_ in collection.items():
                if outputToBin:
                    with open(f'./src/assembler_output/packets/bin/write_to_0x{start:06X}.img', 'wb') as f:
                        f.write(bytes_)
                if outputToText:
                    with open(f'./src/assembler_output/packets/txt/write_to_0x{start:06X}.txt', 'w') as f:
            ...
            if outputToPatch:
                ...
                with open(f'./src/assembler_output/packets/bps/smrpg-packets-{datetime.now().strftime("%Y%m%d%H%M%S")}.bps', 'wb') as f:
                    write_bps(bps_progress(iterable), f)

        self.stdout.write(self.style.SUCCESS("successfully assembled packet data"))
