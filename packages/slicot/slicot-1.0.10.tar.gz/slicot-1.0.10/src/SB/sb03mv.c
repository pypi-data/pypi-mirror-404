            ...
            f64* xnorm, i32* info)
{
    ...
    *info = 0;
    ...
    smin = fmax(fabs(t[0]), fabs(t[0 + 1*ldt]));
    smin = fmax(smin, fabs(t[1]));
    smin = fmax(smin, fabs(t[1 + 1*ldt]));
    smin = fmax(eps * smin, smlnum);

    // Build 3x3 coefficient matrix T9 (stored column-major)
    // T9(i,j) = t9[i + j*3]
    t9[0 + 0*3] = t[0] * t[0] - one;
    t9[1 + 1*3] = t[0] * t[1 + 1*ldt] + t[0 + 1*ldt] * t[1] - one;
    t9[2 + 2*3] = t[1 + 1*ldt] * t[1 + 1*ldt] - one;

    if (ltran) {
        t9[0 + 1*3] = t[0] * t[0 + 1*ldt] + t[0] * t[0 + 1*ldt];
        t9[0 + 2*3] = t[0 + 1*ldt] * t[0 + 1*ldt];
        t9[1 + 0*3] = t[0] * t[1];
        t9[1 + 2*3] = t[0 + 1*ldt] * t[1 + 1*ldt];
        t9[2 + 0*3] = t[1] * t[1];
        t9[2 + 1*3] = t[1] * t[1 + 1*ldt] + t[1] * t[1 + 1*ldt];
    } else {
        t9[0 + 1*3] = t[0] * t[1] + t[0] * t[1];
        t9[0 + 2*3] = t[1] * t[1];
        t9[1 + 0*3] = t[0] * t[0 + 1*ldt];
        t9[1 + 2*3] = t[1] * t[1 + 1*ldt];
        t9[2 + 0*3] = t[0 + 1*ldt] * t[0 + 1*ldt];
        t9[2 + 1*3] = t[0 + 1*ldt] * t[1 + 1*ldt] + t[0 + 1*ldt] * t[1 + 1*ldt];
    }
    ...
    *scale = one;
    ...
    // Apply column pivots in reverse
    for (i = 0; i < 2; i++) {
        if (jpiv[1 - i] != 1 - i) {
            temp = tmp[1 - i];
