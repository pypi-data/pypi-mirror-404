"""
Tests for MB03AG: Compute Givens rotations for Wilkinson shift polynomial (full evaluation).
...
Unlike MB03AF/MB03AE, this routine EVALUATES the full matrix product and computes
...
Key difference from MB03AI: AMAP(1) points to the Hessenberg matrix (first factor).
...
5. Multiple factors
6. K=1 case (single Hessenberg matrix)
...
def test_mb03ag_double_shift():
    ...
    c1, s1, c2, s2 = mb03ag('D', k, n, amap, s, sinv, a)

    assert_allclose(c1**2 + s1**2, 1.0, rtol=1e-14)
    assert_allclose(c2**2 + s2**2, 1.0, rtol=1e-14)


def test_mb03ag_negative_signature():
    """
    Validate with negative signature entries.

    Tests S(AI) != SINV branch in the algorithm (uses DGETC2/DGESC2 solve).
...
def test_mb03ag_givens_rotation_property():
    """
    ...
    for shft in ['D', 'S']:
        c1, s1, c2, s2 = mb03ag(shft, k, n, amap, s, sinv, a)
...
def test_mb03ag_mixed_signature():
    """
...
def test_mb03ag_eigenvalue_based_shift():
    """
    Validate that MB03AG computes proper Wilkinson shifts from eigenvalues.

    MB03AG explicitly evaluates the matrix product and uses DLAHQR to compute
    eigenvalues, selecting the two with smallest moduli for shifts.
