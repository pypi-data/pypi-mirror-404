...
def parse_PDB(
    ...
    S = [
        restype_3to1[AA] if AA in list(restype_3to1) else "X" for AA in list(S)
    ]
    ...
    output_dict["xyz_37"] = torch.tensor(
        xyz_37, device=device, dtype=torch.float32
    )
...
def get_nearest_neighbours(CB, mask, Y, Y_t, Y_m, number_of_ligand_atoms):
    ...
    Y = torch.zeros(
        [CB.shape[0], number_of_ligand_atoms, 3],
        dtype=torch.float32,
        device=device,
    )
    Y_t = torch.zeros(
