...
class ProductReviewResourceSchema(Schema):
    ...
    concern_type = fields.String(required=True)
    concern_detail = fields.String(required=True)
    note = fields.String(allow_none=True)
    references = fields.String(allow_none=True)
    assignee_id = fields.Integer(allow_none=True)
    reviewer_id = fields.Integer(allow_none=True)
    reviewed = fields.Boolean(allow_none=True)
    date_logged = fields.DateTime(required=True)
