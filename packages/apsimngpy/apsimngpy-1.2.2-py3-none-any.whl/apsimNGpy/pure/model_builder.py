...
class Tree:
    @dataclass(order=True)  # <-- init=True (default)
    class Simulations:
    ...
    def __init__(self, **kwargs):
        self.tree = self.Simulations(**kwargs)
    ...
    def build_tree(self):
        return json.dumps(obj=self.__dict__,
                          default=lambda x: x.__dict__, sort_keys=False, indent=4, ensure_ascii=False).replace('Schema',
                                                                                                               '$type')
