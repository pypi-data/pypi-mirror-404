...
class MasterMind:
    ...
    def __init__(self):
        ...
        legend = Text(
            self.window, wrap="word", width=bounds["width"], height=bounds["height"]
        )
        legend.pack(expand=False, side=BOTTOM, anchor="se")
        legend.insert(1.0, MasterMind.getLegend())
        legend.config(
    ...
    def getBoundsLegend():
        tab = MasterMind.getLegend().split("\n")
        max = 0
        for i in tab:
            current = len(i)
            if max < current:
