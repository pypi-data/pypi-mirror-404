...
class RefreshServer(CRUDEndpoint, MetadataMixin):
    ...
    def post(
        ...
        """
        Create new user/fsso/refresh_server object.
        ...
        Examples:
            ...
            >>> payload = RefreshServer.defaults()  # Start with defaults
            >>> payload['name'] = 'my-object'
            >>> result = fgt.api.monitor.user_fsso_refresh_server.post(payload_dict=payload)

        Note:
            Required fields: {{ ", ".join(RefreshServer.required_fields()) }}
            
            Use RefreshServer.help('field_name') to get field details.
        ...
        from ._helpers.refresh_server import DEPRECATED_FIELDS
        if DEPRECATED_FIELDS:
            from hfortix_core import check_deprecated_fields
            check_deprecated_fields(
                payload=payload_data,
                deprecated_fields=DEPRECATED_FIELDS,
                endpoint="monitor/user/fsso/refresh_server",
            )
