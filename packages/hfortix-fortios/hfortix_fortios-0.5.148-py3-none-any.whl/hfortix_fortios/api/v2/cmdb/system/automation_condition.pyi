""" - Type Stubs

Auto-generated stub file for type checking and IDE support.

Endpoint: system/automation_condition
Category: cmdb
"""

from __future__ import annotations

from typing import (
    Any,
    ClassVar,
    Literal,
    TypedDict,
    overload,
)

from hfortix_fortios.models import (
    FortiObject,
    FortiObjectList,
)


# ================================================================
# TypedDict Payloads
# ================================================================

class AutomationConditionPayload(TypedDict, total=False):
    """Payload type for AutomationCondition operations."""
    name: str
    description: str
    condition_type: Literal["cpu", "memory", "vpn"]
    cpu_usage_percent: int
    mem_usage_percent: int
    vdom: str
    vpn_tunnel_name: str
    vpn_tunnel_state: Literal["tunnel-up", "tunnel-down"]


# ================================================================
# Response Types (TypedDict for dict-style access)
# ================================================================

class AutomationConditionResponse(TypedDict, total=False):
    """Response type for AutomationCondition - use with .dict property for typed dict access."""
    name: str
    description: str
    condition_type: Literal["cpu", "memory", "vpn"]
    cpu_usage_percent: int
    mem_usage_percent: int
    vdom: str
    vpn_tunnel_name: str
    vpn_tunnel_state: Literal["tunnel-up", "tunnel-down"]


# ================================================================
# Response Types (Class for attribute access)
# ================================================================


class AutomationConditionObject(FortiObject):
    """Typed FortiObject for AutomationCondition with field access."""
    name: str
    description: str
    condition_type: Literal["cpu", "memory", "vpn"]
    cpu_usage_percent: int
    mem_usage_percent: int
    vpn_tunnel_name: str
    vpn_tunnel_state: Literal["tunnel-up", "tunnel-down"]


# ================================================================
# Main Endpoint Class
# ================================================================

class AutomationCondition:
    """
    
    Endpoint: system/automation_condition
    Category: cmdb
    MKey: name
    """
    
    # Class attributes for introspection
    endpoint: ClassVar[str] = ...
    path: ClassVar[str] = ...
    category: ClassVar[str] = ...
    mkey: ClassVar[str] = ...
    capabilities: ClassVar[dict[str, Any]] = ...
    
    def __init__(self, client: Any) -> None:
        """Initialize endpoint with HTTP client."""
        ...
    
    # ================================================================
    # GET Methods
    # ================================================================
    
    # CMDB with mkey - overloads for single vs list returns
    @overload
    def get(
        self,
        name: str,
        *,
        filter: str | list[str] | None = ...,
        count: int | None = ...,
        start: int | None = ...,
        payload_dict: dict[str, Any] | None = ...,
        range: list[int] | None = ...,
        sort: str | None = ...,
        format: str | None = ...,
        action: str | None = ...,
        error_mode: Literal["raise", "return", "print"] | None = ...,
        error_format: Literal["detailed", "simple", "code_only"] | None = ...,
    ) -> AutomationConditionObject: ...
    
    @overload
    def get(
        self,
        *,
        filter: str | list[str] | None = ...,
        count: int | None = ...,
        start: int | None = ...,
        payload_dict: dict[str, Any] | None = ...,
        range: list[int] | None = ...,
        sort: str | None = ...,
        format: str | None = ...,
        action: str | None = ...,
        error_mode: Literal["raise", "return", "print"] | None = ...,
        error_format: Literal["detailed", "simple", "code_only"] | None = ...,
    ) -> FortiObjectList[AutomationConditionObject]: ...
    
    def get_schema(
        self,
        format: str = ...,
    ) -> FortiObject: ...

    # ================================================================
    # POST Method
    # ================================================================
    
    def post(
        self,
        payload_dict: AutomationConditionPayload | None = ...,
        name: str | None = ...,
        description: str | None = ...,
        condition_type: Literal["cpu", "memory", "vpn"] | None = ...,
        cpu_usage_percent: int | None = ...,
        mem_usage_percent: int | None = ...,
        vpn_tunnel_name: str | None = ...,
        vpn_tunnel_state: Literal["tunnel-up", "tunnel-down"] | None = ...,
        error_mode: Literal["raise", "return", "print"] | None = ...,
        error_format: Literal["detailed", "simple", "code_only"] | None = ...,
    ) -> AutomationConditionObject: ...

    # ================================================================
    # PUT Method
    # ================================================================
    
    def put(
        self,
        payload_dict: AutomationConditionPayload | None = ...,
        name: str | None = ...,
        description: str | None = ...,
        condition_type: Literal["cpu", "memory", "vpn"] | None = ...,
        cpu_usage_percent: int | None = ...,
        mem_usage_percent: int | None = ...,
        vpn_tunnel_name: str | None = ...,
        vpn_tunnel_state: Literal["tunnel-up", "tunnel-down"] | None = ...,
        error_mode: Literal["raise", "return", "print"] | None = ...,
        error_format: Literal["detailed", "simple", "code_only"] | None = ...,
    ) -> AutomationConditionObject: ...

    # ================================================================
    # DELETE Method
    # ================================================================
    
    def delete(
        self,
        name: str | None = ...,
        error_mode: Literal["raise", "return", "print"] | None = ...,
        error_format: Literal["detailed", "simple", "code_only"] | None = ...,
    ) -> FortiObject[Any]: ...

    # ================================================================
    # Utility Methods
    # ================================================================
    
    def exists(
        self,
        name: str,
    ) -> bool: ...
    
    def set(
        self,
        payload_dict: AutomationConditionPayload | None = ...,
        name: str | None = ...,
        description: str | None = ...,
        condition_type: Literal["cpu", "memory", "vpn"] | None = ...,
        cpu_usage_percent: int | None = ...,
        mem_usage_percent: int | None = ...,
        vpn_tunnel_name: str | None = ...,
        vpn_tunnel_state: Literal["tunnel-up", "tunnel-down"] | None = ...,
        error_mode: Literal["raise", "return", "print"] | None = ...,
        error_format: Literal["detailed", "simple", "code_only"] | None = ...,
    ) -> FortiObject[Any]: ...
    
    # Helper methods
    @staticmethod
    def help(field_name: str | None = ...) -> str: ...
    
    @staticmethod
    def fields(detailed: bool = ...) -> list[str] | list[dict[str, Any]]: ...
    
    @staticmethod
    def field_info(field_name: str) -> FortiObject[Any]: ...
    
    @staticmethod
    def validate_field(name: str, value: Any) -> bool: ...
    
    @staticmethod
    def required_fields() -> list[str]: ...
    
    @staticmethod
    def defaults() -> FortiObject[Any]: ...
    
    @staticmethod
    def schema() -> FortiObject[Any]: ...


__all__ = [
    "AutomationCondition",
    "AutomationConditionPayload",
    "AutomationConditionResponse",
    "AutomationConditionObject",
]