...
Note: this is a sketch. The actual snapcraft.yaml will need testing. Key things
to verify:
- ctranslate2 manylinux wheel installs cleanly
- spaCy model lands in the right site-packages
- FFmpeg stage-package provides both `ffmpeg` and `ffprobe`
- The `bin/bristlenose` entry point is correctly generated by the python plugin
- Classic confinement linter passes
- Environment variables (LANG/LC_ALL) prevent encoding issues with non-ASCII filenames
...
These two workstreams are mostly independent but doctor should land first:

### Phase 1: `bristlenose doctor` (benefits all install methods)
...
### Phase 2: Snap packaging

1. Write `snap/snapcraft.yaml`
2. Test locally with Multipass
3. Register snap name and request classic confinement
4. Write `.github/workflows/snap.yml` CI workflow
5. Add SNAPCRAFT_STORE_CREDENTIALS secret
6. Build, test from edge channel
7. Promote to stable
