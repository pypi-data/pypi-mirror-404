...
class CollRes:
    def __init__(self, chr1, chr2):
        ...
        self._p_value_ls, self._score_ls = [], []
        # self._total_block:
        # loc1 loc2 bkidx
        # ......
        self._total_block = pd.DataFrame(columns=['loc1', 'loc2', 'bkidx'])

    def add_block(self, block: pd.DataFrame, p_value: float, score: float):
        block['bkidx'] = len(self._score_ls)
        self._total_block = pd.concat([self._total_block, block])
        self._p_value_ls.append(p_value)
        self._score_ls.append(score)

    def get_block(self, bkidx: int) -> tuple[pd.DataFrame, float, float]:
        return (self._total_block[self._total_block['bkidx'] == bkidx].loc[:,['loc1', 'loc2']],
                self._p_value_ls[bkidx],
                self._score_ls[bkidx],)

    @property
    def len(self):
        return len(self._p_value_ls)
...
class Collinearity:
    ...
    # Core input 2(Chr1, Chr2):
    chr_tuple: tuple[str, str]
    ...
    def run(self):
        ...
        self.get_matrix()
        self.score_matrix()
        result = CollRes(chr1=self.chr_tuple[0], chr2=self.chr_tuple[1])
        ...
        while (self.over_length >= self.over_gap) or (len(points1) >= self.over_gap):
            if self.max_path(points1):
                if self.p_value > self.pvalue_min:
                    continue
                result.add_block(self.path, self.p_value, self.score)
