...
class Sheet(WordFile, ComponentMixin):
    ...
    def process_image_data(self, xml_string, xml_file):
        ...
        Returns:
            str: 处理后的XML字符串
        """
        # 匹配包含 r:id 属性的 v:imagedata 标签的完整 w:pict 标签
        # 支持自闭合标签 (<v:imagedata ... />) 和非自闭合标签 (<v:imagedata ...></v:imagedata>)
        # 第一个捕获组是整个 w:pict 标签，第二个捕获组是 v:imagedata 标签，第三个捕获组是 r:id 的值
        pattern = r'(<w:pict\b[^>]*>.*?(<v:imagedata\s+[^>]*?r:id="([^"]+)"[^>]*?(?:\s*/>|>.*?</v:imagedata>)).*?</w:pict>)'
        matches = re.findall(pattern, xml_string, re.DOTALL)

        for full_pict, v_imagedata_tag, uri in matches:
            if '/' in uri:
                try:
                    ...
                    # 检查是否为自闭合标签
                    is_self_closing = v_imagedata_tag.rstrip().endswith('/>')

                    # 提取并保留所有其他属性
                    # 移除 r:id 属性，保留其他所有属性
                    # 匹配属性名（支持命名空间前缀如 o:title）和属性值
                    attrs_pattern = r'([\w:]+)\s*=\s*"([^"]+)"'
                    all_attrs = re.findall(attrs_pattern, v_imagedata_tag)
                    ...
                    # 构建新的 v:imagedata 标签，保留所有属性并更新 r:id，保持原始格式
                    if preserved_attrs:
                        attrs_str = ' ' + ' '.join(preserved_attrs) + ' '
                        if is_self_closing:
                            new_v_imagedata = f'<v:imagedata{attrs_str}r:id="{image_rid}"/>'
                        else:
                            new_v_imagedata = f'<v:imagedata{attrs_str}r:id="{image_rid}"></v:imagedata>'
                    else:
                        if is_self_closing:
                            new_v_imagedata = f'<v:imagedata r:id="{image_rid}"/>'
                        else:
                            new_v_imagedata = f'<v:imagedata r:id="{image_rid}"></v:imagedata>'

                    # 在完整的 w:pict 标签中替换 v:imagedata 标签
                    new_pict = full_pict.replace(v_imagedata_tag, new_v_imagedata)
                    xml_string = xml_string.replace(full_pict, new_pict)

                except Exception as e:
                    print(f"处理图片 {uri} 时出错: {e}")
                    # 如果处理失败，去除pict标签
                    xml_string = xml_string.replace(full_pict, '')
