    ...
    async def summarize_intelligence_debrief(self, brief_input: Any) -> IntelligenceReport:
        ...
        try:
            ...
            # This uses the LLM to 'read' the outcomes and explain them
                # 1. Manual Stringification (Bypassing the context bug)
            outcomes_str = "\n".join([
                f"- Step: {r.get('step_name')} | Status: {r.get('status')} | Result: {r.get('output')}"
                for r in brief.get('step_results', [])
            ])
            manual_user_input = f"""
            ...
            report = await self.llm.invoke_structured_model(
            ...
            return IntelligenceReport(
                ...
                narrative_summary=report.summary,
