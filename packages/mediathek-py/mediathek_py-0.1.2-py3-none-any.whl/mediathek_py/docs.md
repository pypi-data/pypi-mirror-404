...
**`exceptions.py`** defines `MediathekError` (base), `ApiError` (carries a `messages: list[str]` from the API `err` array), and `EmptyResponseError` (when `result` is null without an error).
...
- The download method uses `httpx.Client.stream()` with `iter_bytes()` and supports an optional progress callback that receives `(downloaded_bytes, total_bytes_or_none)`.
