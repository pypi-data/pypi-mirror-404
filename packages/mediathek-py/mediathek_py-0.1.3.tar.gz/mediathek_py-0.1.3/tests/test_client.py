...
class TestDownload:
    ...
    @respx.mock
    def test_download_403_raises_download_error(self, tmp_path):
        url = "https://media.tagesschau.de/video/expired.mp4"
        respx.get(url).respond(status_code=403)

        output_path = tmp_path / "test.mp4"
        with Mediathek() as m:
            with pytest.raises(DownloadError) as exc_info:
                m.download(url, output_path)

        assert exc_info.value.status_code == 403
        assert exc_info.value.url == url
        assert isinstance(exc_info.value, MediathekError)

    @respx.mock
    def test_download_404_raises_download_error(self, tmp_path):
        url = "https://media.tagesschau.de/video/gone.mp4"
    ...
    @respx.mock
    def test_download_500_raises_download_error(self, tmp_path):
        url = "https://media.tagesschau.de/video/broken.mp4"
