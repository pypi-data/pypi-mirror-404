...
Stateful Transforms:
    ...
    norm(x): x / std(x) — divide by std only (normalize magnitude)
    zscore(x): (x - mean(x)) / std(x) — traditional z-score (1 SD)
    scale(x): (x - mean(x)) / (2 * std(x)) — Gelman scaling (2 SD)
...
comparable to binary predictor coefficients in regression models.
