...
class BaseModel(ABC):
    ...
    def simulate(
        ...
        Returns:
            DataFrame with columns `sim_1`, `sim_2`, ..., `sim_{nsim}`.
        ...
        # Return as DataFrame with sim_1, sim_2, ... columns
        return pl.DataFrame({f"sim_{i + 1}": sims[:, i] for i in range(nsim)})
    ...
    def plot_resamples(self, **kwargs):
        """Plot distribution of resampled statistics.

        Convenience method that calls `bossanova.viz.plot_resamples(self, **kwargs)`.

        Visualizes bootstrap coefficient distributions or permutation null
        distributions. Requires prior call to .infer() with bootstrap or
        permutation method (samples are saved by default for n<=5000).

        Args:
            **kwargs (Any): Passed to `viz.plot_resamples`. Key options:
                which: "params" (default) or "mee" for marginal effects;
                include_intercept: Include intercept term (default False);
                terms: Subset of terms to plot;
                show_ci: Shade CI region for bootstrap (default True);
                show_pvalue: Annotate p-value for permutation (default True).

        Returns:
            object: Seaborn FacetGrid with density plots.

        Raises:
            RuntimeError: If no resampling results available.

        See Also:
            bossanova.viz.plot_resamples: Full documentation.
            infer: Method to run bootstrap/permutation inference.
        """
        from bossanova.viz.resamples import plot_resamples

        return plot_resamples(self, **kwargs)
    ...
    def infer(
        ...
        *,
        summary: bool = True,
        save_resamples: bool | Literal["auto"] = "auto",
        # Common
        n: int = 999,
        conf_int: float = 0.95,
        seed: int | None = None,
        ...
        Args:
            ...
            save_resamples: Whether to store raw resampling results:
                - "auto" (default): Enable when n <= 5000 for memory efficiency
                - True: Always store (for visualization with .plot_resamples())
                - False: Never store (memory-constrained scenarios)
                Results stored in: boot_samples_, perm_samples_, or cv_results_
        ...
        # Resolve "auto" based on resample count (before any dispatch)
        if save_resamples == "auto":
            save_resamples = n <= 5000
        ...
        # Store resolved save_resamples flag ("auto" already resolved above)
