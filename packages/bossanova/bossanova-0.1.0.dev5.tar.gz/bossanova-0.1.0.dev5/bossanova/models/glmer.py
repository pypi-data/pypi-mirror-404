    ...
    def fit(
        ...
        Args:
            ...
            weights: Column name for observation weights. If provided, must be
                a column in the data with non-negative values. For binomial
                grouped data (e.g., cbpp), weights specify the number of trials.
        ...
        Examples:
            ...
            >>> # Weighted GLMM (binomial grouped data)
            >>> model = glmer("y ~ x + (1|group)", data=df, family="binomial")
            >>> model.fit(weights="trials")
        """
        # Validate method
        if method != "ML":
            raise ValueError(
