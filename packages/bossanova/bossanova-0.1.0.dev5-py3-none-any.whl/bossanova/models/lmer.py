...
class lmer(BaseMixedModel):
    ...
    def fit(
        ...
        Args:
            ...
            weights: Column name for observation weights. If provided, must be
                a column in the data with non-negative values. Weights modify
                the residual covariance: Var(ε_i) = σ²/w_i.
        ...
        Examples:
            ...
            >>> # Weighted LMM
            >>> model = lmer("y ~ x + (1|group)", data=df).fit(weights="w")
        """
        ...
        # Apply sqrt(weights) transformation for weighted LMM
        # This transforms the problem so unweighted PLS applies to transformed data
        from bossanova.ops.lmer_core import apply_sqrt_weights

        X_w, Z_w, y_w, sqrtwts = apply_sqrt_weights(X_fit, Z_fit, y_fit, weights_fit)

        # Store fit-time matrices for inference computations
        # Original matrices for residual/fitted value interpretation
        self._X_fit = X_fit
        self._Z_fit = Z_fit
        self._y_fit = y_fit
        self._weights_fit = weights_fit
        self._sqrtwts = sqrtwts
        # Transformed matrices for Satterthwaite df computation (Hessian/Jacobian)
        # For unweighted models, these are the same as originals
        self._X_w = X_w
        self._Z_w = Z_w
        self._y_w = y_w
        ...
        # Use transformed matrices (X_w, y_w) for weighted LMM
        ...
        # Use transformed matrices for final solve (coefficients are valid for original scale)
        pls_result = solve_pls_sparse(X_w, Z_w, Lambda, y_w)
        ...
        # Use transformed matrices - weights are implicit in X_w, Z_w
        self._vcov = self._compute_vcov_schur(
            X=X_w,
            Z=Z_w,
            Lambda=Lambda,
            W=None,  # Weights implicit in transformed data
            sigma2=sigma2,
        )
    ...
    def _compute_satterthwaite_inference(self, conf_level: float) -> None:
        ...
        #
        # IMPORTANT: For weighted models, use the transformed matrices (X_w, Z_w, y_w)
        # so that the Hessian/Jacobian correctly reflect the weighted deviance.
        # For unweighted models, _X_w == _X_fit, so this works in both cases.
        X_fit = self._X_w
        Z_fit = self._Z_w
        y_fit = self._y_w
        sqrtwts = self._sqrtwts
        ...
        def deviance_varpar(varpar):
            ...
            # This matches lme4's deviance calculation for weighted models
        ...
        try:
            ...
            # Fall back to max_df for all coefficients
            warnings.warn(
            ...
            self._vcov_varpar = None
            self._jac_list = None
            self._satterthwaite_df = np.full(self._p, 1e6)
            # Fall back to Wald inference
            self._compute_wald_inference(conf_level)
            return
