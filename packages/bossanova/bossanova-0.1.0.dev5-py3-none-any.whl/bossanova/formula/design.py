...
class DesignMatrices:
    ...
    Attributes:
        ...
        transform_state: Dict storing state for stateful transforms (center, norm, zscore, scale).
...
class DesignMatrixBuilder:
    ...
    def __init__(
        ...
        # Storage for transformed data in random effects (e.g., center(x) in (center(x)|group))
        self._re_transform_data: dict[str, NDArray[np.float64]] = {}
    ...
    def _parse_re_term(self, term: Binary) -> dict:
        ...
        if isinstance(term.right, Binary) and term.right.operator.kind == "SLASH":
            ...
            for var in re_vars:
                ...
                if var in self._re_transform_data:
                    X_re_cols.append(self._re_transform_data[var])
    ...
    def _parse_re_expr(self, expr: object) -> tuple[list[str], bool]:
        ...
        def traverse(node: object) -> None:
            ...
            if isinstance(node, Literal):
                ...
                # Transform in RE term, e.g., center(x) in (center(x)|group)
                # Evaluate the transform and store data for RE matrix construction
                data_array, labels = self._evaluate_call(node)
                # Check for multi-column transforms (e.g., factor() with multiple dummies)
                n_cols = data_array.shape[1] if data_array.ndim > 1 else 1
                if n_cols > 1:
                    raise ValueError(
                        "Transforms producing multiple columns (e.g., factor()) "
                        "are not supported in random effects. Use the raw variable instead."
                    )
                synthetic_name = labels[0]  # e.g., "center(x)"
                self._re_transform_data[synthetic_name] = data_array.flatten()
                vars_found.append(synthetic_name)
    ...
    def _evaluate_call(self, call: Call) -> tuple[NDArray[np.float64], list[str]]:
        ...
        - center(), norm(), zscore(), scale(): Stateful transforms
        - I(): Identity function
        ...
        raise ValueError(
            ...
            f"Supported: factor, T, S, I, center, norm, zscore, scale, log, log10, sqrt, poly"
        )
    ...
    def _evaluate_stateful_transform(
        ...
        Args:
            func_name: Transform name (center, norm, zscore, scale).
