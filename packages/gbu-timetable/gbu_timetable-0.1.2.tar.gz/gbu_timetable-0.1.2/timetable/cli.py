...
def main():
    ...
    try:
        ...
        console.print(f"[red]Invalid JSON response:[/red] {e}")
        return 1
        ...
        while True:
            try:
                ...
                if key == "1":
                    ...
                    if not schools:
                        console.print("[red]No schools found in data[/red]")
                        readchar.readkey()
                        continue
                    ...
                    d1 = [x for x in DATA if x.get("school") == school]
                    depts = sorted(set(x["StudentDepartment"] for x in d1 if x.get("StudentDepartment")))
                    if not depts:
                    ...
                    d2 = [x for x in d1 if x.get("StudentDepartment") == dept]
                    codes = sorted(set(x["Code"] for x in d2 if x.get("Code")))
                    if not codes:
                        console.print("[red]No codes found[/red]")
                        readchar.readkey()
                        continue
                    ...
                    d3 = [x for x in d2 if x.get("Code") == code]
                    sec_map = {
                        f"{x['SectionName']} ({x['Section_Id']})": x["Section_Id"]
                        for x in d3 if x.get("SectionName") and x.get("Section_Id")
                    }
                    ...
                    if not sid:
                        console.print("[red]Section ID cannot be empty[/red]")
                        readchar.readkey()
                        continue
                    
                    rows = get_section_rows(sid)
                    if not rows:
                        console.print("[red]Invalid Section ID or no data found[/red]")
                        readchar.readkey()
                        continue
