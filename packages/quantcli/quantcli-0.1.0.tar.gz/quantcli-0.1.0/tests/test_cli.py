...
class TestLoadStrategy:
    ...
    def test_load_builtin_strategy(self):
        """Test loading built-in strategy"""
        strategy_cls = cli.load_strategy("ma_cross")
        assert strategy_cls is not None
        assert strategy_cls.__name__ == "MaCrossStrategy"

    def test_load_strategy_invalid_name(self):
        """Test loading non-existent strategy raises error"""
        with pytest.raises(ValueError) as exc_info:
            cli.load_strategy("invalid_strategy")
        assert "Unknown strategy" in str(exc_info.value)

    def test_load_strategy_file_not_found(self):
        """Test loading non-existent file raises error"""
        with pytest.raises((ValueError, FileNotFoundError)):
            cli.load_strategy("/nonexistent/path/strategy.py")
...
class TestEdgeCases:
    ...
    def test_backtest_run_no_data(self, runner):
        """Test backtest run with no data available"""
        with patch.object(cli.DataManager, '__init__') as mock_init, \
             patch.object(cli.DataManager, 'get_daily') as mock_get_daily:
            mock_init.return_value = None
            mock_get_daily.return_value = pd.DataFrame()
