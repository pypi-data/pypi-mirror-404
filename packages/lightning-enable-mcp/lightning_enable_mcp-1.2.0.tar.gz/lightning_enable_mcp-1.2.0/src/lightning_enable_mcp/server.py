...
from .license import LicenseService, detect_wallet_type
...
from .tools.license import unlock_l402_features, check_l402_license, get_budget_status
...
class LightningEnableServer:
    ...
    def __init__(self) -> None:
        ...
        self.license_service: LicenseService = LicenseService()
    ...
    def _setup_handlers(self) -> None:
        ...
        async def list_tools() -> list[Tool]:
            ...
            return [
                ...
                Tool(
                    name="unlock_l402_features",
                    description=(
                        "Purchase L402 feature license (one-time 6,000 sats payment, valid forever). "
                        "This unlocks access_l402_resource and pay_l402_challenge tools. "
                        "Requires an L402-compatible wallet (NWC with Alby, CoinOS, Mutiny, etc.)."
                    ),
                    inputSchema={
                        "type": "object",
                        "properties": {},
                ...
                Tool(
                    name="check_l402_license",
                    description="Check your L402 license status.",
                    inputSchema={
                        "type": "object",
                        "properties": {},
                    },
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            ...
            try:
                ...
                elif name == "unlock_l402_features":
                    result = await unlock_l402_features(
                        wallet=self.wallet,
                        license_service=self.license_service,
                        wallet_pubkey=self._nwc_config.wallet_pubkey if self._nwc_config else None,
                        relay_url=self._nwc_config.relay_url if self._nwc_config else None,
                    )

                elif name == "check_l402_license":
                    result = await check_l402_license(
                        license_service=self.license_service,
                    )
    ...
    async def _initialize_services(self) -> None:
        ...
        Supports three wallet backends (in priority order for L402):
        1. NWC (Nostr Wallet Connect) - Set NWC_CONNECTION_STRING (returns preimage - best for L402)
        2. OpenNode - Set OPENNODE_API_KEY (may not return preimage)
        3. Strike - Set STRIKE_API_KEY (does NOT return preimage - L402 will NOT work)

        For L402 support, use NWC. Strike and OpenNode are good for general payments.
        """
        ...
        try:
            ...
            if nwc_connection:
                ...
                # Set wallet pubkey on license service for license binding
                wallet_type = detect_wallet_type(self._nwc_config.relay_url)
                self.license_service.set_wallet_pubkey(
                    self._nwc_config.wallet_pubkey,
                    wallet_type
                )
                logger.info(f"License bound to NWC pubkey: {self._nwc_config.wallet_pubkey[:16]}...")
                ...
                logger.warning(
                    "Strike does NOT return preimage - L402 will NOT work. "
                    "Use NWC for L402 support."
                )
    ...
    async def run(self) -> None:
        """Run the MCP server."""
        logger.info("Starting Lightning Enable MCP server v1.2.0...")
        logger.info("L402 license purchase available (6,000 sats, valid forever)")

        # Log license status
        if self.license_service.is_licensed:
            logger.info(f"L402 license active: {self.license_service.license_key[:8]}...")
        else:
            logger.info("L402 license not active - use unlock_l402_features to purchase")
