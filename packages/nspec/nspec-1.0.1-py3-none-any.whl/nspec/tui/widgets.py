...
class EpicSelector(Static):
    ...
    def set_epics(self, epics: list[tuple[str, str, int, str]]) -> None:
        ...
        for epic_id, title, count, priority in epics:
            emoji = SPEC_PRIORITIES.get(priority, SPEC_PRIORITIES.get("P3")).emoji
            ellipsis = "..." if len(title) > 40 else ""
            display = f"{emoji} {epic_id}: {title[:40]}{ellipsis} ({count})"
...
class MoveSpecModal(Static):
    ...
    def set_epics(
        ...
        for epic_id, title, count, priority in epics:
            ...
            ellipsis = "..." if len(title) > 35 else ""
            display = f"{emoji} {epic_id}: {title[:35]}{ellipsis} ({count}){marker}"
            option_list.add_option(Option(display, id=epic_id))
    ...
    def show_spec(
        ...
        if impl:
            ...
            if impl.tasks_total > 0:
                task_progress = (
                    f"  Tasks: {impl.tasks_completed}/"
                    f"{impl.tasks_total} ({impl.completion_percent}%)"
                )
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            if (
                ...
                and isinstance(self.screen, DetailViewScreen)
            ):
...
[dim]Press Esc or h to close[/]"""
    ...
    def compose(self) -> ComposeResult:
        ...
        from nspec.statuses import get_status_legend

        legend = get_status_legend()
        text = f"""[bold]novaspec HELP[/]
