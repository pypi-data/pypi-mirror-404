...
class DetailViewScreen(Screen):
        ...
        with VerticalScroll(id="detail-scroll"):
            ...
            if self.impl and self.impl.tasks:
                ...
                with Vertical(id="tasks-content", classes="section-content"):
                    yield from self._compose_task_rows()
    ...
    def _render_header(self) -> str:
        ...
        if self.impl and self.impl.tasks_total > 0:
            progress = (
                f"Tasks: {self.impl.tasks_completed}/"
                f"{self.impl.tasks_total} ({self.impl.completion_percent}%)"
            )
    ...
    def _get_epic_info(self) -> tuple[str, str] | None:
        ...
        for epic_id, epic_fr in self.data.datasets.active_frs.items():
            if epic_fr.type == "Epic" and self.spec_id in epic_fr.deps:
    ...
    def _render_overview(self) -> str:
        ...
        if self.fr:
            ...
            if self.fr.ac_total > 0:
                ac_progress = (
                    f"[bold]AC Progress:[/] {self.fr.ac_completed}/"
                    f"{self.fr.ac_total} ({self.fr.ac_completion_percent}%)"
                )
                lines.append(ac_progress)
        ...
        if self.impl:
            ...
            if self.impl.tasks_total > 0:
                task_progress = (
                    f"[bold]Task Progress:[/] {self.impl.tasks_completed}/"
                    f"{self.impl.tasks_total} ({self.impl.completion_percent}%)"
                )
    ...
    def _render_deps(self) -> str:
        ...
        if self.upstream_specs:
            ...
            if display_limit and len(self.upstream_specs) > display_limit:
                more_count = len(self.upstream_specs) - display_limit
                lines.append(f"  [dim]... +{more_count} more (press 'd' to expand)[/]")
        else:
        ...
        if self.downstream_specs:
            ...
            if display_limit and len(self.downstream_specs) > display_limit:
                more_count = len(self.downstream_specs) - display_limit
                lines.append(f"  [dim]... +{more_count} more (press 'd' to expand)[/]")
        else:
    ...
    def _mark_task_with_marker(self, marker: str, marker_desc: str) -> None:
        ...
        if task_id.startswith("day") or (not is_numeric and not is_dod):
            msg = (
                f"Task {task_id} not in supported format "
                "(e.g., 1.1 or dod-1). Use MCP tool directly."
            )
            self.notify(
                msg,
                severity="warning",
    ...
    def action_copy_epic(self) -> None:
        ...
        if epic_fr.deps:
            ...
            for spec_id in epic_fr.deps:
                ...
                is_completed = False
                if not spec_fr and spec_id in self.data.datasets.completed_frs:
