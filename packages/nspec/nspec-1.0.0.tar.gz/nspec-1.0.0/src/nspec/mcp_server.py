...
def _find_epic_for_spec(spec_id: str, docs_root: Path) -> str | None:
    ...
    for fr_file in paths.active_frs_dir.glob("FR-*.md"):
        ...
        if spec_id in dep_ids:
            ...
            name_match = re.match(r"FR-(\d+)-", fr_file.name)
            if name_match:
...
def next_spec(epic_id: str | None = None, docs_root: str | None = None) -> dict[str, Any]:
    ...
    for spec_id, fr in datasets.active_frs.items():
        ...
        if epic_id and epic_id not in fr.deps:
            continue
...
def task_complete(
    ...
    from nspec.tasks import VERIFICATION_ROUTES, TaskParser
...
def create_spec(
    ...
    Args:
        ...
        priority: Priority level (P0-P3, E0-E3). Default: P2
    ...
    if epic_id:
        ...
        try:
            ...
            if spec_id not in datasets.active_frs:
                logger.warning(
                    "create_spec: spec %s created but not loadable — "
                    "skipping add_dep to epic %s",
                    spec_id, epic_id,
                )
                result["epic_warning"] = (
...
def set_priority(spec_id: str, priority: str, docs_root: str | None = None) -> dict[str, Any]:
    ...
    Args:
        ...
        priority: New priority (P0-P3 or E0-E3)
        docs_root: Path to docs directory (default: ./docs)
    """
