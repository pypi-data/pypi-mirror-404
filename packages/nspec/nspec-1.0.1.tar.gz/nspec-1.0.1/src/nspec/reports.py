...
def _is_epic(fr) -> bool:
    """Check if FR is an epic based on type."""
    return getattr(fr, "type", "") == "Epic"


def _is_done(status: str) -> bool:
    """Check if status indicates completion."""
    from nspec.statuses import is_completed_status, parse_status_text

    text = parse_status_text(status)
    return is_completed_status(status) or text in ("ready", "completed")


def _is_active(status: str) -> bool:
    """Check if status indicates active work."""
    from nspec.statuses import is_active_status
...
def epic_summary_report(ctx: ReportContext) -> dict[str, Any]:
    ...
    for spec_id, fr in ctx.datasets.active_frs.items():
        if _is_epic(fr):
            # Count specs in this epic (direct dependencies of the epic)
            children = [
                sid
                for sid in (fr.deps or [])
                if sid in ctx.datasets.active_frs and not _is_epic(ctx.datasets.active_frs[sid])
            ]
