#!/usr/bin/env python3
"""Migrate legacy ADR files to fRIMPL format (FR-S9XX/IMPL-S9XX).

This script:
1. Parses existing ADR-XXX.md files from docs/02-architecture/adrs/
2. Generates FR-S9XX and IMPL-S9XX pairs in fRIMPL format
3. Archives old ADR files to docs/02-architecture/adrs/archive/
4. Maps ADR-001 → FR-S901, ADR-002 → FR-S902, etc.
...
def generate_fr_content(adr_number: int, title: str, sections: dict[str, str]) -> str:
    """Generate FR-S9XX content from ADR sections."""
    fr_number = 900 + adr_number
    fr_id = f"S{fr_number:03d}"
...
**Status:** ✅ Completed
**Priority:** 🟡 P2
**Implementation:** [IMPL-{fr_id}](../impls/active/IMPL-{fr_id}-{_slugify(title)}.md)
...
def generate_impl_content(adr_number: int, title: str) -> str:
    """Generate IMPL-S9XX content (retrospective since ADR already implemented)."""
    fr_number = 900 + adr_number
    fr_id = f"S{fr_number:03d}"

    impl_content = f"""# Implementation Plan: {title}

**Implementation ID:** IMPL-{fr_id}
**Type:** ADR
**Feature Request:** [FR-{fr_id}](../frs/active/FR-{fr_id}-{_slugify(title)}.md)
**Status:** ✅ Completed
...
(FR-{fr_id}/IMPL-{fr_id}) for consistency with the unified nspec system.
...
- FR-{fr_id} for the architecture decision documentation
...
def migrate_adr(
    ...
    """Migrate a single ADR file to FR-S9XX/IMPL-S9XX format.
    ...
    print(f"\n{prefix}Migrating ADR-{adr_number:03d} → FR-{fr_id}/IMPL-{fr_id}")
    ...
    # Prepare paths
    slug = _slugify(title)
    fr_path = paths.active_frs_dir / f"FR-{fr_id}-{slug}.md"
    impl_path = paths.active_impls_dir / f"IMPL-{fr_id}-{slug}.md"
...
def main():
    ...
    if not args.dry_run and migrated:
        ...
        print("  3. Review generated FR-S9XX/IMPL-S9XX files")
        print("  4. Commit:")
        print("     git add docs/frs/active/FR-S9*.md")
        print("     git add docs/impls/active/IMPL-S9*.md")
        print("     git add docs/02-architecture/adrs/archive/")
        print('     git commit -m "feat: Migrate 17 ADRs to fRIMPL format (FR-S901 to FR-S917)"')
