...
import pandas as pd
import numpy as np
from rust_pyfunc.rust_pyfunc import rolling_window_stat_backward

StatType = Literal["mean", "sum", "max", "min", "std", "median", "count", "rank", "skew", "trend_time", "trend_oneton", "first", "last"]

@pd.api.extensions.register_dataframe_accessor("rolling_past")
@pd.api.extensions.register_series_accessor("rolling_past")
class RollingPastAccessor:
    """用于在pandas DataFrame或Series上实现向前滚动窗口计算的访问器。
    ...
    - median: 计算前面窗口内的中位数
    - count: 计算前面窗口内的数据点数量
    - rank: 计算当前值在前面窗口内的分位数（0到1之间）
    - skew: 计算前面窗口的偏度
    - trend_time: 计算前面窗口内数据序列与时间序列的相关系数
    - trend_oneton: 计算前面窗口内数据序列与1到n序列的相关系数（忽略时间间隔）
    - first: 计算前面窗口内的第一个值
    - last: 计算前面窗口内的最后一个值
    
    注意：所有计算都可以选择是否包括当前时间点的值
    
    使用方法：
    ---------
    >>> import pandas as pd
    >>> from rust_pyfunc import rolling_past
    >>> # DataFrame示例
    >>> df = pd.DataFrame({
    ...
    def __init__(self, pandas_obj: Union[pd.DataFrame, pd.Series]):
        ...
        self._window = None
        self._include_current = True  # 默认包含当前行，与pandas rolling一致
        
    def __call__(self, window: str, include_current: bool = True) -> "RollingPastAccessor":
        """设置滚动窗口的大小。
    ...
    def _apply_stat(self, column: str, stat_type: StatType) -> pd.Series:
        ...
        if not hasattr(self, '_window') or self._window is None:
            raise ValueError("必须先调用rolling_past(window)设置窗口大小")
        ...
        result = rolling_window_stat_backward(times, values, window_ns, stat_type, self._include_current)
    ...
    def trend_oneton(self) -> Union[pd.Series, pd.DataFrame]:
        """计算向前滚动窗口内数据序列与1到n序列的相关系数（忽略时间间隔）。
    ...
    @overload
    def first(self) -> Union[pd.Series, pd.DataFrame]:
        """计算向前滚动窗口内的第一个值。
    ...
    def first(self) -> Union[pd.Series, pd.DataFrame]:
        """计算向前滚动窗口内的第一个值。"""
        if isinstance(self._obj, pd.Series):
            return self._apply_stat("value", "first")
        else:
            return pd.DataFrame({col: self._apply_stat(col, "first") for col in self._obj.columns})
