...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'gamma', 'beta']
    output_names = ['y', 'rstd', 'x']
...
def _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, y_out_, rstd_out_, x_out_, epsilon):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, gamma_in__, beta_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, rstd_out_, x_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("AddRmsNormBias", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def add_rms_norm_bias(x1_in__, x2_in__, gamma_in__, beta_in__=None, y_out_=None, rstd_out_=None, x_out_=None, epsilon=1e-06, kernel_name="add_rms_norm_bias", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, y_out_, rstd_out_, x_out_, epsilon)
    ...
    origin_func_name = "add_rms_norm_bias"
    ascendc_src_dir_ex = "add_rms_norm_bias"
    ascendc_src_dir = "add_rms_norm_bias"
    ascendc_src_file = "add_rms_norm_bias.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator AddRmsNormBias, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "AddRmsNormBias"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, gamma_in__, beta_in__], origin_outputs = [y_out_, rstd_out_, x_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x1_in__, x2_in__, gamma_in__, beta_in__=None, y_out_=None, rstd_out_=None, x_out_=None, epsilon=1e-06, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, y_out_, rstd_out_, x_out_, epsilon)
    result = check_op_cap("op_select_format", "AddRmsNormBias", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, gamma_in__, beta_in__=None, y_out_=None, rstd_out_=None, x_out_=None, epsilon=1e-06, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, gamma_in__, beta_in__, y_out_, rstd_out_, x_out_, epsilon)
    result = check_op_cap("get_op_specific_info", "AddRmsNormBias", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
