...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['x1', 'x2', 'residual', 'gamma']
    output_names = ['y', 'add_out']
...
def _build_args(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon, is_trans_b, is_gather_add_out):
    __inputs__ = []
    for arg in [x1_in__, x2_in__, residual_in__, gamma_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, add_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if tp_rank_size != None:
        attr = {}
        attr["name"] = "tp_rank_size"
        attr["dtype"] = "int"
        attr["value"] = tp_rank_size
        __attrs__.append(attr)
    if tp_rank_id != None:
        attr = {}
    ...
    if is_trans_b != None:
        attr = {}
        attr["name"] = "is_trans_b"
        attr["dtype"] = "bool"
        attr["value"] = is_trans_b
        __attrs__.append(attr)
    if is_gather_add_out != None:
        attr = {}
        attr["name"] = "is_gather_add_out"
        attr["dtype"] = "bool"
        attr["value"] = is_gather_add_out
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("MatmulAllreduceAddRmsnorm", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_STR, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def matmul_allreduce_add_rmsnorm(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon=1e-06, is_trans_b=False, is_gather_add_out=False, kernel_name="matmul_allreduce_add_rmsnorm", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon, is_trans_b, is_gather_add_out)
    ...
    origin_func_name = "matmul_allreduce_add_rmsnorm"
    ascendc_src_dir_ex = "matmul_allreduce_add_rmsnorm"
    ascendc_src_dir = "matmul_allreduce_add_rmsnorm"
    ascendc_src_file = "matmul_allreduce_add_rmsnorm.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MatmulAllreduceAddRmsnorm, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "MatmulAllreduceAddRmsnorm"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x1_in__, x2_in__, residual_in__, gamma_in__], origin_outputs = [y_out_, add_out_out_],\
                param_type_dynamic = False, mc2_ctx = ["mc2_context_0"], param_type_list = ['required', 'required', 'required', 'required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon=1e-06, is_trans_b=False, is_gather_add_out=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon, is_trans_b, is_gather_add_out)
    result = check_op_cap("op_select_format", "MatmulAllreduceAddRmsnorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon=1e-06, is_trans_b=False, is_gather_add_out=False, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, residual_in__, gamma_in__, y_out_, add_out_out_, group_tp, tp_rank_size, tp_rank_id, epsilon, is_trans_b, is_gather_add_out)
    result = check_op_cap("get_op_specific_info", "MatmulAllreduceAddRmsnorm", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
