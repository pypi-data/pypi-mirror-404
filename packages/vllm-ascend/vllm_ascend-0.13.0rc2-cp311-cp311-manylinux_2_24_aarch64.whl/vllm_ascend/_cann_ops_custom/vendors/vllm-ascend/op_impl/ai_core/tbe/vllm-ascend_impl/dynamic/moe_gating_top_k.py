...
def _build_args(x_in__, bias_in__, y_out_, expert_idx_out_, out_out_, k, k_group, group_count, group_select_mode, renorm, norm_type, out_flag, routed_scaling_factor, eps):
    __inputs__ = []
    for arg in [x_in__, bias_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [y_out_, expert_idx_out_, out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if group_count != None:
        attr = {}
        attr["name"] = "group_count"
        attr["dtype"] = "int"
        attr["value"] = group_count
        __attrs__.append(attr)
    if group_select_mode != None:
        attr = {}
        attr["name"] = "group_select_mode"
        attr["dtype"] = "int"
        attr["value"] = group_select_mode
        __attrs__.append(attr)
    if renorm != None:
        attr = {}
        attr["name"] = "renorm"
    ...
    if out_flag != None:
        ...
        attr["value"] = out_flag
        __attrs__.append(attr)
    if routed_scaling_factor != None:
        attr = {}
        attr["name"] = "routed_scaling_factor"
        attr["dtype"] = "float"
        attr["value"] = routed_scaling_factor
        __attrs__.append(attr)
    if eps != None:
        attr = {}
        attr["name"] = "eps"
...
@tbe_register.register_operator("MoeGatingTopK", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def moe_gating_top_k(x_in__, bias_in__=None, y_out_=None, expert_idx_out_=None, out_out_=None, k=0, k_group=1, group_count=1, group_select_mode=0, renorm=0, norm_type=0, out_flag=False, routed_scaling_factor=1, eps=1e-20, kernel_name="moe_gating_top_k", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, bias_in__, y_out_, expert_idx_out_, out_out_, k, k_group, group_count, group_select_mode, renorm, norm_type, out_flag, routed_scaling_factor, eps)
    ...
    origin_func_name = "moe_gating_top_k"
    ascendc_src_dir_ex = "moe_gating_top_k"
    ascendc_src_dir = "moe_gating_top_k"
    ascendc_src_file = "moe_gating_top_k.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator MoeGatingTopK, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[x_in__, bias_in__], origin_outputs = [y_out_, expert_idx_out_, out_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'optional', 'required', 'required', 'required'], init_value_list = [None, None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(x_in__, bias_in__=None, y_out_=None, expert_idx_out_=None, out_out_=None, k=0, k_group=1, group_count=1, group_select_mode=0, renorm=0, norm_type=0, out_flag=False, routed_scaling_factor=1, eps=1e-20, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, bias_in__, y_out_, expert_idx_out_, out_out_, k, k_group, group_count, group_select_mode, renorm, norm_type, out_flag, routed_scaling_factor, eps)
    result = check_op_cap("op_select_format", "MoeGatingTopK", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, bias_in__=None, y_out_=None, expert_idx_out_=None, out_out_=None, k=0, k_group=1, group_count=1, group_select_mode=0, renorm=0, norm_type=0, out_flag=False, routed_scaling_factor=1, eps=1e-20, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, bias_in__, y_out_, expert_idx_out_, out_out_, k, k_group, group_count, group_select_mode, renorm, norm_type, out_flag, routed_scaling_factor, eps)
    result = check_op_cap("get_op_specific_info", "MoeGatingTopK", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
