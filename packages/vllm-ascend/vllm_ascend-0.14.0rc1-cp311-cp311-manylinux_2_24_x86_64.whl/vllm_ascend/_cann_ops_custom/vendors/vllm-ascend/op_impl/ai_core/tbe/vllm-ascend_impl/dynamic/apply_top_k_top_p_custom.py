...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['sorted_value', 'sorted_indices', 'p', 'k']
    output_names = ['out']
...
def _build_args(sorted_value_in__, sorted_indices_in__, p_in__, k_in__, out_out_):
    __inputs__ = []
    for arg in [sorted_value_in__, sorted_indices_in__, p_in__, k_in__]:
        if arg != None:
...
@tbe_register.register_operator("ApplyTopKTopPCustom", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def apply_top_k_top_p_custom(sorted_value_in__, sorted_indices_in__, p_in__=None, k_in__=None, out_out_=None, kernel_name="apply_top_k_top_p_custom", impl_mode = ""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(sorted_value_in__, sorted_indices_in__, p_in__, k_in__, out_out_)
    ...
    origin_func_name = "apply_top_k_top_p_custom"
    ascendc_src_dir_ex = "apply_top_k_top_p_custom"
    ascendc_src_dir = "apply_top_k_top_p_custom"
    ascendc_src_file = "apply_top_k_top_p_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Ascend C Operator ApplyTopKTopPCustom, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "ApplyTopKTopPCustom"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__ , impl_mode = impl_mode, origin_inputs=[sorted_value_in__, sorted_indices_in__, p_in__, k_in__], origin_outputs = [out_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'optional', 'optional', 'required'], init_value_list = [None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}', {'valueDepend': {}})

def op_select_format(sorted_value_in__, sorted_indices_in__, p_in__=None, k_in__=None, out_out_=None, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(sorted_value_in__, sorted_indices_in__, p_in__, k_in__, out_out_)
    result = check_op_cap("op_select_format", "ApplyTopKTopPCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(sorted_value_in__, sorted_indices_in__, p_in__=None, k_in__=None, out_out_=None, impl_mode = ""):
    __inputs__, __outputs__, __attrs__ = _build_args(sorted_value_in__, sorted_indices_in__, p_in__, k_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "ApplyTopKTopPCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
