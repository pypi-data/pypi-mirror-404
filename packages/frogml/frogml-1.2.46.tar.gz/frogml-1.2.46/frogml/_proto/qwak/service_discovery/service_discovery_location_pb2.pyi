"""
@generated by mypy-protobuf.  Do not edit manually!
isort:skip_file
"""

import builtins
import google.protobuf.descriptor
import google.protobuf.message
import typing

DESCRIPTOR: google.protobuf.descriptor.FileDescriptor

@typing.final
class ServiceLocationDescriptor(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    SERVICE_URL_FIELD_NUMBER: builtins.int
    LOCATION_FIELD_NUMBER: builtins.int
    service_url: builtins.str
    @property
    def location(self) -> global___ServiceLocation: ...
    def __init__(
        self,
        *,
        service_url: builtins.str = ...,
        location: global___ServiceLocation | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing.Literal["location", b"location"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing.Literal["location", b"location", "service_url", b"service_url"]) -> None: ...

global___ServiceLocationDescriptor = ServiceLocationDescriptor

@typing.final
class ServiceLocation(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    CORE_FIELD_NUMBER: builtins.int
    EDGE_FIELD_NUMBER: builtins.int
    @property
    def core(self) -> global___Core: ...
    @property
    def edge(self) -> global___Edge: ...
    def __init__(
        self,
        *,
        core: global___Core | None = ...,
        edge: global___Edge | None = ...,
    ) -> None: ...
    def HasField(self, field_name: typing.Literal["core", b"core", "edge", b"edge", "service_location", b"service_location"]) -> builtins.bool: ...
    def ClearField(self, field_name: typing.Literal["core", b"core", "edge", b"edge", "service_location", b"service_location"]) -> None: ...
    def WhichOneof(self, oneof_group: typing.Literal["service_location", b"service_location"]) -> typing.Literal["core", "edge"] | None: ...

global___ServiceLocation = ServiceLocation

@typing.final
class Core(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    def __init__(
        self,
    ) -> None: ...

global___Core = Core

@typing.final
class Edge(google.protobuf.message.Message):
    DESCRIPTOR: google.protobuf.descriptor.Descriptor

    def __init__(
        self,
    ) -> None: ...

global___Edge = Edge
