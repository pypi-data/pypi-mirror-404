...
class AsyncLalafoClient(BaseLalafoClient):
    """Асинхронный клиент Lalafo API"""
    ...
    async def get_categories_tree(self, with_duplicates: bool = True) -> CategoriesResponse:
    ...
    async def get_ad_details(self, ad_id: int) -> AdDetailsResponse:
        endpoint = f"/v3/ads/details/{ad_id}"
        
        data = await self._make_request("GET", endpoint)
        return AdDetailsResponse.from_api_response(data)
    
    async def search_ads(self, search_request: SearchRequest) -> SearchResponse:
    ...
    async def search_ads_all_pages(
        self,
        ...
        for page_num in range(max_pages):
            ...
            try:
                response = await self.search_ads(search_request)
    ...
    async def search_ads_simple(
        ...
        return await self.search_ads(search_request)
    ...
    async def get_feed_elements(self, category_id: int) -> Dict[str, Any]:
        endpoint = "/v3/feed-elements/scheme"
        params = {"category_id": category_id}
        
        return await self._make_request("GET", endpoint, params=params)
    
    async def get_user_status(self, user_ids: List[int]) -> List[Dict[str, Any]]:
    ...
    async def fetch_ad_from_url(self, url: str) -> AdDetailsResponse:
        ...
        return await self.get_ad_details(parsed['ad_id'])

    async def get_search_request_from_url(
            self, url: str,
            currency: Currency = Currency.KGS
        ) -> Optional[SearchRequest]:
        parsed = parse_search_url(url, (await self.get_filters(currency = currency)).filters[0].cities, await self.get_categories_tree())
        if not parsed:
            return None
        return SearchRequest(**parsed['search_params'])