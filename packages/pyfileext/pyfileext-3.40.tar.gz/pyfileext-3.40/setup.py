...
from pyfileext import __version__
confkcws={}
confkcws['name']='pyfileext'
confkcws['version']=__version__
confkcws['description']='pyfileext'
...
def get_file(folder='./',lists=[]):
    ...
    return lists
def start():
    b=get_file("pyfileext",['pyfileext'])
    setup(
        name = confkcws["name"],
        version = confkcws["version"],
        keywords = "pyfileext"+confkcws['version'],
        ...
        install_requires = ['pyquery==2.0.1',
                            # 'cssselect<=1.2.0',
                            'pillow>=10.0.0',
                            'xlrd==1.2.0',
                            'pdf2docx==0.5.8', #CentOS是依赖 sudo yum -y install mesa-libGL
                            'openpyxl>=3.0.9',
                            'Spire.Pdf==10.12.1','Spire.XLS==14.9.3','spire.Doc==12.6.0'], #第三方包
        package_data = {
