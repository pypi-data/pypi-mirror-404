...
def main_sifibridge():
    print("Starting Sifi Bridge thread")
...
def main_pylsl(streams_to_use: list | None = None):
...
def main():
    p = Process(target=main_sifibridge, daemon=True)
    p.start()

    time.sleep(10)  # Give some time for Sifi Bridge to get ready

    # ["emg", "ecg", "eda", "ax", "ay", "az", "ir", "r", "g", "b"]
    main_pylsl(["ecg"])
...
if __name__ == "__main__":
    # This example is an adapted copy-paste from pylsl's examples.
    ...
    print(
        "If you are having trouble with this example with VSCode's integrated terminal, try running it in a standalone terminal."
    )
    main()
