...
def jam_axi_intr_example():
    ...
    w = (Rbin == 0) & (zbin == 0)
    Rbin, zbin = Rbin[~w], zbin[~w]  # Avoid central singularity
    jam_dir = resources.files('jampy')
    filename = jam_dir / 'examples/cappellari2020_table1.txt'
    lg_dens_lum, lg_sigma_lum, qintr_lum = np.loadtxt(filename).T
    ...
    out = jam.axi.intr(dens_lum, sigma_lum, qintr_lum, dens_pot, sigma_pot, qintr_pot,
                       mbh, Rbin, zbin, beta=beta, gamma=gamma, align='cyl',
                       plot=1, data=data, errors=errors, ml=None, nodots=1)

    print(f"M/L = {out.ml:.3f}; chi2/DOF = {out.chi2:.3f}")
