...
async def put_organization(request: web.Request) -> web.Response:
    ...
    """
    try:
        obj = await new_heaobject_from_type(request, Organization)
    except (ValueError, DeserializeException) as e:
        raise response.status_bad_request(str(e))
    return await mongoservicelib.put(request, MONGODB_ORGANIZATION_COLLECTION, Organization, None, obj)
...
async def post_organization(request: web.Request) -> web.Response:
    """
    Creates a new organization, provided in the body.
    ...
    try:
        ...
        raise response.status_bad_request(str(e))
    return await mongoservicelib.post_desktop_object(request, obj, MONGODB_ORGANIZATION_COLLECTION)
