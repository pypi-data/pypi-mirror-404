...
From the project root, with [Poetry](https://python-poetry.org/) installed and PyPI credentials configured:

```bash
# Bump version in pyproject.toml and src/enable_ai/__init__.py first, then:
poetry build
poetry publish
# Or with twine (after poetry build):
# twine upload dist/*
```

For TestPyPI first: `poetry config repositories.testpypi https://test.pypi.org/legacy/` then `poetry publish -r testpypi`.
