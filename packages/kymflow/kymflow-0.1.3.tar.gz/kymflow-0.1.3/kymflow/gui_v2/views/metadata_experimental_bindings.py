"""Bindings between MetadataExperimentalView and event bus (state → view updates)."""
...
class MetadataExperimentalBindings:
    """Bind MetadataExperimentalView to event bus for state → view updates.

    This class subscribes to state change events from AppState (via the bridge)
    and updates the experimental metadata view accordingly. The view is reactive
    ...
    Attributes:
        ...
        _view: MetadataExperimentalView instance to update.
    ...
    def _on_metadata_update(self, e: MetadataUpdate) -> None:
        ...
        and the metadata type is "experimental". Wrapped in safe_call to handle
        deleted client errors gracefully.

        Args:
            e: MetadataUpdate event (phase="state") containing the file whose metadata was updated.
        """
        # Only refresh if this is an experimental metadata update
