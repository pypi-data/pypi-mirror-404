...
# ✅ Drawer width is controlled HERE (Tailwind width class).
# Was: w-[78rem]
# Now: w-[156rem]  (twice as wide)
# with ui.drawer(side='left', value=True) \
#     .props('behavior=desktop bordered') \
#     .classes('w-[20rem] min-w-[20rem] max-w-[20rem] p-4 overflow-auto') as drawer:
#     # .classes('w-[20vw] min-w-[20rem] max-w-[28rem] p-4 overflow-auto') as drawer:

with ui.drawer(side='left', value=True) \
    .props('behavior=desktop bordered width=300') \
    .classes('p-4 overflow-auto') as drawer:
