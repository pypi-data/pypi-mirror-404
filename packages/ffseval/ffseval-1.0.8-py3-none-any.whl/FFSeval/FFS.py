...
class L_2_f(Base):
    def __init__(self):
        super().SetTitle('周方向内表面半楕円表面亀裂')
        super().SetRefer('軸力P：Zahoor A. and Norris D. M.: ASME J. PVT, Vol.106, pp.399-404, 1984' \
                         '軸力Pと曲げモーメントM:Kanninen M. F., et al.: Nuclear Engineering and Design, Vol.48, pp.117-134,1978')

        # Applicable range:内圧： 0 <= lambda <= 5, 膜応力：き裂長さに比較して円筒が十分長く端部効果を無視できること 

    def Calc(self):
        df = super().GetData()
        theta = df['theta']        # half crack angle (radian)
        t = df['t']          # thickness
        Ri = df['Ri']        # inner radius
        Ro = df['Ro']        # outer radius
        a = df['a']
        ...
        Sb= M/(np.pi*Rm*Rm*t)
        x=a/t
        A1=x*((1-z)*(2-2*z+x*z)+(1-z+x*z)**2)/(2*(1+(2-z)*(1-z)))
        alpha=np.arccos(A1*np.sin(g))
        Pc=2*np.pi*Rm*t*Sy*(2*alpha/np.pi-(x*g/np.pi)*(2-2*z+x*z)/(2-z))
        Lr_P = P / Pc
        # 軸力Pと曲げモーメントM
        beta=0.5*np.pi*(1-(th/np.pi)*(a/t)-(Sm/Sy))
        if th+beta <= np.pi:
            Mc=2*Sy*Rm*Rm*t*(2*np.sin(beta)-(a/t)*np.sin(th))
        else:
            b1=np.pi*(1-a/t-(Sm/Sy))/(2-a/t)
            Mc=2*Sy*Rm*Rm*t*(2-a/t)*np.sin(b1)
        Lr_PM = M / Mc

        res = {'Lr_p': Lr_P, 'Lr_PM': Lr_PM}
        super().SetRes(res)
class L_2_k(Base):
    def __init__(self):
