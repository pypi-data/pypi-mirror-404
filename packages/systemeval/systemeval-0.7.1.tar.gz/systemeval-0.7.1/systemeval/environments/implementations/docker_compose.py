...
class DockerComposeEnvironment(Environment):
    ...
    def __init__(self, name: str, config: Dict[str, Any]) -> None:
        ...
        # Project directory is the host directory where compose file lives
        # (NOT the same as working_dir which is the container's working directory)
        self._project_dir = config.get("project_dir", Path.cwd())
        if isinstance(self._project_dir, str):
            self._project_dir = Path(self._project_dir).resolve()
        elif not isinstance(self._project_dir, Path):
    ...
    def setup(self) -> SetupResult:
        ...
        # Auto-detect running containers and attach if appropriate
        if not self.attach:
            from systemeval.utils.docker.preflight import check_containers_running
            compose_path = self._project_dir / self.compose_file
            logger.debug(f"Checking for running containers: compose={compose_path}, project_dir={self._project_dir}")
            running_services = check_containers_running(compose_path, self._project_dir, self.project_name)
            logger.debug(f"Running services detected: {running_services}")

            # If required services are already running, auto-attach
            if running_services:
                required = set(self.services) if self.services else {self.test_service}
                running = set(running_services)
                logger.debug(f"Required services: {required}, Running: {running}")

                if required.issubset(running):
                    logger.info(f"Containers already running: {', '.join(running_services)}")
                    logger.info("Auto-attaching to existing containers")
                    self.attach = True  # Auto-enable attach mode
                    self._is_up = True
                    return SetupResult(
                        success=True,
                        message=f"Attached to running containers: {', '.join(sorted(required))}",
                        duration=time.time() - total_start,
                        details={"mode": "auto-attach", "running_services": running_services},
                    )
                else:
                    logger.debug(f"Not all required services running. Missing: {required - running}")

        # Explicit attach mode: skip lifecycle, just verify containers are running
        if self.attach:
