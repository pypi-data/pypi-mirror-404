...
def create_snn_adjacency_matrix(coords1, coords2, k_neighbors):
    ...
    # Calculate Euclidean distances between coords1 and coords2
    distances_1_to_2 = np.sqrt(((coords1[:, np.newaxis] - coords2) ** 2).sum(axis=2))
    # Calculate Euclidean distances between coords2 and coords1 (for reverse neighbors)
    ...
    # Check for shared nearest neighbors
    for i in range(m):
        for j in range(n):
...
def preprogress_adata(adata_omics1, adata_omics2, method='knn', radius=2, k=10, use_obsm='spatial', return_mask=False):
...
def extract_spatial_region(adata: ad.AnnData, minx: float, miny: float, maxx: float, maxy: float,
                           retainx = 0, retainy = 0,
                           used_obsm: str = 'spatial') -> ad.AnnData:
    """
