...
def install_shutdown_hooks():
    ...
    Signal handlers for SIGTERM, SIGINT + atexit fallback
    Ensures cleanup happens before server stops.
    """
    # Signal handlers 등록
    signal.signal(signal.SIGTERM, shutdown_save_handler)
    signal.signal(signal.SIGINT, shutdown_save_handler)

    # atexit fallback (signal 놓쳤을 경우)
    atexit.register(lambda: shutdown_save_handler(signum=None, frame=None))

    print("[Cortex] Shutdown hooks installed (SIGTERM, SIGINT, atexit)", file=sys.stderr)
    logger.info("[Cortex] Shutdown hooks registered (SIGTERM, SIGINT, atexit)")
...
def cli_main():
    ...
    Usage:
        ...
        cortex-mcp --check   # Verify installation
        cortex-mcp verify    # Verify installation (alias)
        cortex-mcp install   # Run installation wizard
    """
    import asyncio
    ...
    if len(sys.argv) > 1:
        ...
        if command in ("--check", "verify"):
            ...
            print("")
            print("Usage:")
            print("  cortex-mcp           Start MCP server (default)")
            print("  cortex-mcp --check   Verify installation")
            print("  cortex-mcp verify    Verify installation (alias)")
            print("  cortex-mcp install   Run installation wizard")
            print("  cortex-mcp --help    Show this help message")
            print("")
            print("Documentation: https://cortexmcp.dev/docs")
            return

        elif command.startswith("-"):
            print(f"Unknown option: {command}")
            print("Use 'cortex-mcp --help' for available options")
            sys.exit(1)
