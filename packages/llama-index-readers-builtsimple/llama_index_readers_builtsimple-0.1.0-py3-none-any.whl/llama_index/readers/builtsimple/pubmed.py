...
class BuiltSimplePubMedReader(BaseReader):
    ...
    def load_data(
        ...
        **kwargs,
        ...
        fetch_full_text = include_full_text if include_full_text is not None else self.include_full_text
        
        limit = min(max(1, limit), 100)  # Clamp between 1 and 100
        
        logger.info(f"Searching PubMed for: {query} (limit={limit}, full_text={fetch_full_text})")
    ...
    def _result_to_document(self, result: dict, query: str, full_text_data: Optional[dict] = None) -> Document:
        ...
        if full_text_data:
            metadata["full_text_length"] = full_text_data.get("full_text_length", len(full_text_data.get("full_text", "")))
    ...
    def load_full_text(self, pmid: str) -> Optional[Document]:
        ...
        return self._result_to_document(result, query="direct_lookup", full_text_data=full_text_data)

    def lazy_load_data(self, query: str, limit: int = 10, include_full_text: Optional[bool] = None, **kwargs):
        ...
        """
        documents = self.load_data(query, limit, include_full_text=include_full_text, **kwargs)
        for doc in documents:
            yield doc
