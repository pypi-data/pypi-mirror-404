...
minPointsCloud = section.interactionDomainBuild2d(
    nbPoints=100, alpha=section.alphaPhi(AsteelMin, AsteelMax, 0), bounding=False
)
assert isinstance(minPointsCloud, est.PointCloud2d)
...
contained, minDistance, minPoint = minPointsCloud.contains(
    500.0 * 1e3, 191.2 * 1e6, convex=True, ro=[brox, broy]
)
...
contained, minDistance, minPoint = minPointsCloud.contains(
    -1.0 * 1e3 * 1e3, -4.0 * 1e2 * 1e6, convex=True, ro=[brox, broy]
)
section.addTensionPoint2d(minPoint.x, minPoint.y)
if contained:
...
contained, minDistance, minPoint = minPointsCloud.contains(
    +1.0 * 1e3 * 1e3, 0.0, convex=True, ro=[brox, broy]
)
section.addTensionPoint2d(minPoint.x, minPoint.y)
if contained:
