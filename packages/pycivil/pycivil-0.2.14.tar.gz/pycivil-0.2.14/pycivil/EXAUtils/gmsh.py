...
def logGmshFile(fileName):
    ...
    print("Model " + gm.model.getCurrent() + " (" + str(gm.model.getDimension()) + "D)")
    ...
    for e in entities:
        ...
        nodeTags, nodeCoords, nodeParams = gm.model.mesh.getNodes(dim, tag)

        # Get the mesh elements for the entity (dim, tag):
        elemTypes, elemTags, elemNodeTags = gm.model.mesh.getElements(dim, tag)
        ...
        type = gm.model.getType(e[0], e[1])
        name = gm.model.getEntityName(e[0], e[1])
        if len(name):
            name += " "
        ...
        up, down = gm.model.getAdjacencies(e[0], e[1])
        if len(up):
        ...
        physicalTags = gm.model.getPhysicalGroupsForEntity(dim, tag)
        if len(physicalTags):
            s = ""
            for p in physicalTags:
                n = gm.model.getPhysicalName(dim, p)
                if n:
                    n += " "
                s += n + "(" + str(dim) + ", " + str(p) + ") "
        ...
        partitions = gm.model.getPartitions(e[0], e[1])
        if len(partitions):
            print(
                " - Partition tags: "
                + str(partitions)
                + " - parent entity "
                + str(gm.model.getParent(e[0], e[1]))
            )

        # * List all types of elements making up the mesh of the entity:
        for t in elemTypes:
            name, dim, order, numv, parv, _ = gm.model.mesh.getElementProperties(t)
            print(
                " - Element type: "
                + name
                + ", order "
    ...
    # We can use this to clear all the model data but after i need to reinitialize gm:
    #
    # gm.clear()
    # gm.finalize()
