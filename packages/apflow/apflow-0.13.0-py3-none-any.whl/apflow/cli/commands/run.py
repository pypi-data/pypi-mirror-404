    ...
    try:
        ...
        if background:
            ...
            if len(root_task_ids) == 1:
                ...
                typer.echo(f"Monitor with: apflow tasks status {root_task_ids[0]}")
                typer.echo(f"Watch with: apflow tasks watch --task-id {root_task_ids[0]}")
            else:
                typer.echo(f"Task IDs: {', '.join(root_task_ids)}")
                typer.echo(f"Monitor with: apflow tasks status {' '.join(root_task_ids)}")
                typer.echo("Watch with: apflow tasks watch --all")
