//! Async Python visitor bridge implementation.
//!
//! This module provides the async bridge between Python async visitor objects
//! and the Rust HtmlVisitor trait.
...
/// Async PyO3 wrapper around a Python visitor object.
///
/// This struct bridges Python async callbacks to the Rust HtmlVisitor trait.
#[derive(Debug)]
...
// Macro for async bridge simple methods
macro_rules! impl_async_visit_ctx_only {
    ($self:ident, $ctx:ident, $method:literal) => {{
        Python::attach(|py| {
            ...
            $self
                .call_visitor_method_sync(py, $method, &args)
                .unwrap_or(VisitResult::Continue)
        })
    }};
}

macro_rules! impl_async_visit_ctx_text {
...
macro_rules! impl_async_visit_ctx_opt_src {
...
impl HtmlVisitor for PyAsyncVisitorBridge {
    fn visit_element_start(&mut self, ctx: &NodeContext) -> VisitResult {
        impl_async_visit_ctx_only!(self, ctx, "visit_element_start")
    }

    fn visit_element_end(&mut self, ctx: &NodeContext, output: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_element_end", output)
    }

    fn visit_text(&mut self, ctx: &NodeContext, text: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_text", text)
    }
    ...
    fn visit_code_inline(&mut self, ctx: &NodeContext, code: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_code_inline", code)
    }
    ...
    fn visit_table_start(&mut self, ctx: &NodeContext) -> VisitResult {
        impl_async_visit_ctx_only!(self, ctx, "visit_table_start")
    }
    ...
    fn visit_table_end(&mut self, ctx: &NodeContext, output: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_table_end", output)
    }
    ...
    fn visit_definition_list_start(&mut self, ctx: &NodeContext) -> VisitResult {
        impl_async_visit_ctx_only!(self, ctx, "visit_definition_list_start")
    }

    fn visit_definition_term(&mut self, ctx: &NodeContext, text: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_definition_term", text)
    }

    fn visit_definition_description(&mut self, ctx: &NodeContext, text: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_definition_description", text)
    }

    fn visit_definition_list_end(&mut self, ctx: &NodeContext, output: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_definition_list_end", output)
    }
    ...
    fn visit_button(&mut self, ctx: &NodeContext, text: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_button", text)
    }

    fn visit_audio(&mut self, ctx: &NodeContext, src: Option<&str>) -> VisitResult {
        impl_async_visit_ctx_opt_src!(self, ctx, "visit_audio", src)
    }

    fn visit_video(&mut self, ctx: &NodeContext, src: Option<&str>) -> VisitResult {
        impl_async_visit_ctx_opt_src!(self, ctx, "visit_video", src)
    }

    fn visit_iframe(&mut self, ctx: &NodeContext, src: Option<&str>) -> VisitResult {
        impl_async_visit_ctx_opt_src!(self, ctx, "visit_iframe", src)
    }
    ...
    fn visit_summary(&mut self, ctx: &NodeContext, text: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_summary", text)
    }

    fn visit_figure_start(&mut self, ctx: &NodeContext) -> VisitResult {
        impl_async_visit_ctx_only!(self, ctx, "visit_figure_start")
    }

    fn visit_figcaption(&mut self, ctx: &NodeContext, text: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_figcaption", text)
    }

    fn visit_figure_end(&mut self, ctx: &NodeContext, output: &str) -> VisitResult {
        impl_async_visit_ctx_text!(self, ctx, "visit_figure_end", output)
    }
}
