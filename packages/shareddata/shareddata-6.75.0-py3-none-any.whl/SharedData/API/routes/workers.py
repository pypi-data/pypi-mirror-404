"""
Worker routes blueprint for ServerAPI.
"""
...
from SharedData.API.constants import ERROR_SLEEP_SECONDS
from SharedData.API.utils import make_error_response
...
def init_workers_routes(shdata, authenticate_fn):
    """
    Initialize worker routes with required dependencies.
    ...
    @workers_bp.route('/workerpool', methods=['GET', 'POST'])
    def workerpool():
        """
    ...
    @workers_bp.route('/installworker')
    def installworker():
        """
        Generate and return a shell script to install and configure the SharedData worker service.
        """
        try:        
            if not authenticate_fn(request, shdata):
                return jsonify({'error': 'unauthorized'}), 401        
