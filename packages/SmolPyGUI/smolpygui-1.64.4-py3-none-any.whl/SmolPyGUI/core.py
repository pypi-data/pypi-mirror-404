...
class EventObject():
    ...
    def setActive(self,value:bool|Literal["toggle"]='toggle'):
        if(isinstance(value, (bool)) or value=="toggle"):
            self.active = not self.active if value=="toggle" else value
        else:
...
class ScreenObject():
    ...
    def setVisible(self,value:bool|Literal["toggle"]='toggle'):
        if(isinstance(value, (bool)) or value=="toggle"):
            self.visible = not self.visible if value=="toggle" else value
        else:
            raise ValueError(f"Improper value for `value` argument: {value} ({type(value)})")
