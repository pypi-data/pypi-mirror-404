...
class PdfRedactAnnotationError(Exception):
    """Exception raised by PdfRedactAnnotation operations."""
    pass
...
_lib.BridgePdfRedactAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfRedactAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfRedactAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfRedactAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfRedactAnnotationFreeErrorString.restype = None
_lib.BridgePdfRedactAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]

_lib.BridgePdfRedactAnnotationGetInteriorColor.restype = ctypes.c_void_p
_lib.BridgePdfRedactAnnotationGetInteriorColor.argtypes = [ctypes.c_void_p]

_lib.BridgePdfRedactAnnotationSetInteriorColorColor.restype = None
_lib.BridgePdfRedactAnnotationSetInteriorColorColor.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
...
class PdfRedactAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfRedactAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfRedactAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfRedactAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfRedactAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfRedactAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfRedactAnnotationError(f"PdfRedactAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfRedactAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfRedactAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfRedactAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_interior_color(self) -> 'Color':
        """
        Gets the interior color (fill color after redaction is applied).

        Returns:
            'Color': The value of the InteriorColor property.

        Raises:
            PdfRedactAnnotationError: If the operation fails
        """
        self._ensure_not_closed()

        result = _lib.BridgePdfRedactAnnotationGetInteriorColor(self._handle)
        self._check_error()
        return import_module('.color', package=__package__).Color._from_handle(result)

    def set_interior_color(self, value: 'Color') -> None:
        """
        Sets the interior color (fill color after redaction is applied).
        ...
        _lib.BridgePdfRedactAnnotationSetInteriorColorColor(self._handle, value._handle if value else None)
        self._check_error()
