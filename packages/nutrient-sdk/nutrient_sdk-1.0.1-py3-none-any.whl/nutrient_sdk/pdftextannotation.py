...
class PdfTextAnnotationError(Exception):
    """Exception raised by PdfTextAnnotation operations."""
    pass
...
_lib.BridgePdfTextAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfTextAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfTextAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfTextAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfTextAnnotationFreeErrorString.restype = None
_lib.BridgePdfTextAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]

_lib.BridgePdfTextAnnotationGetIsOpen.restype = ctypes.c_bool
_lib.BridgePdfTextAnnotationGetIsOpen.argtypes = [ctypes.c_void_p]

_lib.BridgePdfTextAnnotationSetIsOpenBoolean.restype = None
_lib.BridgePdfTextAnnotationSetIsOpenBoolean.argtypes = [ctypes.c_void_p, ctypes.c_bool]

_lib.BridgePdfTextAnnotationGetIconName.restype = ctypes.c_void_p
_lib.BridgePdfTextAnnotationGetIconName.argtypes = [ctypes.c_void_p]

_lib.BridgePdfTextAnnotationSetIconNameString.restype = None
_lib.BridgePdfTextAnnotationSetIconNameString.argtypes = [ctypes.c_void_p, ctypes.c_void_p]

_lib.BridgePdfTextAnnotationGetState.restype = ctypes.c_void_p
_lib.BridgePdfTextAnnotationGetState.argtypes = [ctypes.c_void_p]

_lib.BridgePdfTextAnnotationGetStateModel.restype = ctypes.c_void_p
_lib.BridgePdfTextAnnotationGetStateModel.argtypes = [ctypes.c_void_p]
...
class PdfTextAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfTextAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfTextAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfTextAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfTextAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfTextAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfTextAnnotationError(f"PdfTextAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfTextAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfTextAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfTextAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_is_open(self) -> bool:
        """
        Gets whether the annotation popup should initially be open.

        Returns:
            bool: The value of the IsOpen property.

        Raises:
            PdfTextAnnotationError: If the operation fails
        """
        self._ensure_not_closed()

        result = _lib.BridgePdfTextAnnotationGetIsOpen(self._handle)
        self._check_error()
        return result

    def set_is_open(self, value: bool) -> None:
        """
        Sets whether the annotation popup should initially be open.
        ...
        _lib.BridgePdfTextAnnotationSetIsOpenBoolean(self._handle, value)
        self._check_error()

    def get_icon_name(self) -> str:
        """
        Gets the icon name for the annotation. Valid values: Comment, Key, Note, Help, NewParagraph, Paragraph, Insert

        Returns:
            str: The value of the IconName property.
        ...
        result = _lib.BridgePdfTextAnnotationGetIconName(self._handle)
        self._check_error()
        return sdk_loader.convert_string_handle(result)

    def set_icon_name(self, value: str) -> None:
        """
        Sets the icon name for the annotation. Valid values: Comment, Key, Note, Help, NewParagraph, Paragraph, Insert
        ...
        _lib.BridgePdfTextAnnotationSetIconNameString(self._handle, value.encode('utf-8') if value else None)
        self._check_error()

    def get_state(self) -> str:
        """
        Gets the state of the annotation (for review workflows).

        Returns:
            str: The value of the State property.
        ...
        result = _lib.BridgePdfTextAnnotationGetState(self._handle)
        self._check_error()
        return sdk_loader.convert_string_handle(result)

    def get_state_model(self) -> str:
        """
        Gets the state model (e.g., "Marked", "Review").

        Returns:
            str: The value of the StateModel property.
        ...
        result = _lib.BridgePdfTextAnnotationGetStateModel(self._handle)
        self._check_error()
        return sdk_loader.convert_string_handle(result)
