...
class PdfFreeTextAnnotationError(Exception):
    """Exception raised by PdfFreeTextAnnotation operations."""
    pass
...
_lib.BridgePdfFreeTextAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfFreeTextAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfFreeTextAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfFreeTextAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfFreeTextAnnotationFreeErrorString.restype = None
_lib.BridgePdfFreeTextAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]

_lib.BridgePdfFreeTextAnnotationGetDefaultAppearance.restype = ctypes.c_void_p
_lib.BridgePdfFreeTextAnnotationGetDefaultAppearance.argtypes = [ctypes.c_void_p]

_lib.BridgePdfFreeTextAnnotationGetTextAlignment.restype = ctypes.c_void_p
_lib.BridgePdfFreeTextAnnotationGetTextAlignment.argtypes = [ctypes.c_void_p]

_lib.BridgePdfFreeTextAnnotationGetIsCallout.restype = ctypes.c_bool
_lib.BridgePdfFreeTextAnnotationGetIsCallout.argtypes = [ctypes.c_void_p]
...
class PdfFreeTextAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfFreeTextAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfFreeTextAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfFreeTextAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfFreeTextAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfFreeTextAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfFreeTextAnnotationError(f"PdfFreeTextAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfFreeTextAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfFreeTextAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfFreeTextAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_default_appearance(self) -> str:
        ...
        Returns:
            str: The value of the DefaultAppearance property.

        Raises:
            PdfFreeTextAnnotationError: If the operation fails
        """
        self._ensure_not_closed()

        result = _lib.BridgePdfFreeTextAnnotationGetDefaultAppearance(self._handle)
        self._check_error()
        return sdk_loader.convert_string_handle(result)

    def get_text_alignment(self) -> str:
        """
        Gets the text alignment (0=Left, 1=Center, 2=Right).

        Returns:
            str: The value of the TextAlignment property.
        ...
        result = _lib.BridgePdfFreeTextAnnotationGetTextAlignment(self._handle)
        self._check_error()
        return sdk_loader.convert_string_handle(result)

    def get_is_callout(self) -> bool:
        """
        Gets whether this is a callout annotation.

        Returns:
            bool: The value of the IsCallout property.
        ...
        result = _lib.BridgePdfFreeTextAnnotationGetIsCallout(self._handle)
        self._check_error()
        return result
    ...
    def text_alignment(self) -> str:
        ...
        Returns:
            str: The value of the TextAlignment property.
        """
        return self.get_text_alignment()
