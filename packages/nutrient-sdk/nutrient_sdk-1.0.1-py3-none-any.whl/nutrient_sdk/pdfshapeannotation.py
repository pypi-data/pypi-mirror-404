...
class PdfShapeAnnotationError(Exception):
    """Exception raised by PdfShapeAnnotation operations."""
    pass
...
_lib.BridgePdfShapeAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfShapeAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfShapeAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfShapeAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfShapeAnnotationFreeErrorString.restype = None
_lib.BridgePdfShapeAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfShapeAnnotation:
    """
    Base class for shape annotations (Line, Circle, Square).
    """

    def __init__(self):
        """Cannot instantiate PdfShapeAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfShapeAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfShapeAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfShapeAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfShapeAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfShapeAnnotationError(f"PdfShapeAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfShapeAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfShapeAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfShapeAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_interior_color(self) -> 'Color':
        """
        Gets the interior (fill) color of the shape annotation.

        Returns:
            'Color': The value of the InteriorColor property.

        Raises:
            PdfShapeAnnotationError: If the operation fails
        """
    ...
    def set_interior_color(self, value: 'Color') -> None:
        """
        Sets the interior (fill) color of the shape annotation.
