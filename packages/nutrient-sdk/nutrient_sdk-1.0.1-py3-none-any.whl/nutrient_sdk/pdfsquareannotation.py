...
class PdfSquareAnnotationError(Exception):
    """Exception raised by PdfSquareAnnotation operations."""
    pass
...
_lib.BridgePdfSquareAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfSquareAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfSquareAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfSquareAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfSquareAnnotationFreeErrorString.restype = None
_lib.BridgePdfSquareAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfSquareAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfSquareAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfSquareAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfSquareAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfSquareAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfSquareAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfSquareAnnotationError(f"PdfSquareAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfSquareAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfSquareAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfSquareAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_interior_color(self) -> 'Color':
        ...
        Raises:
            PdfSquareAnnotationError: If the operation fails
        """
