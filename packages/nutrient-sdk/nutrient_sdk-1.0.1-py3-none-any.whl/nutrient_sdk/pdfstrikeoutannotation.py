...
class PdfStrikeOutAnnotationError(Exception):
    """Exception raised by PdfStrikeOutAnnotation operations."""
    pass
...
_lib.BridgePdfStrikeOutAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfStrikeOutAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfStrikeOutAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfStrikeOutAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfStrikeOutAnnotationFreeErrorString.restype = None
_lib.BridgePdfStrikeOutAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfStrikeOutAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfStrikeOutAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfStrikeOutAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfStrikeOutAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfStrikeOutAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfStrikeOutAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfStrikeOutAnnotationError(f"PdfStrikeOutAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfStrikeOutAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfStrikeOutAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfStrikeOutAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_creation_date(self) -> str:
        ...
        Raises:
            PdfStrikeOutAnnotationError: If the operation fails
        """
