...
class PdfHighlightAnnotationError(Exception):
    """Exception raised by PdfHighlightAnnotation operations."""
    pass
...
_lib.BridgePdfHighlightAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfHighlightAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfHighlightAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfHighlightAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfHighlightAnnotationFreeErrorString.restype = None
_lib.BridgePdfHighlightAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfHighlightAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfHighlightAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfHighlightAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfHighlightAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfHighlightAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfHighlightAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfHighlightAnnotationError(f"PdfHighlightAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfHighlightAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfHighlightAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfHighlightAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_creation_date(self) -> str:
        ...
        Raises:
            PdfHighlightAnnotationError: If the operation fails
        """
