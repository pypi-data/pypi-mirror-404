...
class PdfMarkupAnnotationError(Exception):
    """Exception raised by PdfMarkupAnnotation operations."""
    pass
...
_lib.BridgePdfMarkupAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfMarkupAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfMarkupAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfMarkupAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfMarkupAnnotationFreeErrorString.restype = None
_lib.BridgePdfMarkupAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfMarkupAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfMarkupAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfMarkupAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfMarkupAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfMarkupAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfMarkupAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfMarkupAnnotationError(f"PdfMarkupAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfMarkupAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfMarkupAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfMarkupAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_creation_date(self) -> str:
        ...
        Raises:
            PdfMarkupAnnotationError: If the operation fails
        """
    ...
    def get_opacity(self) -> float:
        """
        Gets the opacity of the annotation (0.0 to 1.0).

        Returns:
            float: The value of the Opacity property.
    ...
    def set_opacity(self, value: float) -> None:
        """
        Sets the opacity of the annotation (0.0 to 1.0).
    ...
    def get_rich_text_contents(self) -> str:
        """
        Gets the rich text contents of the annotation.

        Returns:
            str: The value of the RichTextContents property.
