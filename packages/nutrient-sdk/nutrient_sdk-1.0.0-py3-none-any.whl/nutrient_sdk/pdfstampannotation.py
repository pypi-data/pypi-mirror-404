...
class PdfStampAnnotationError(Exception):
    """Exception raised by PdfStampAnnotation operations."""
    pass
...
_lib.BridgePdfStampAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfStampAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfStampAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfStampAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfStampAnnotationFreeErrorString.restype = None
_lib.BridgePdfStampAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]

_lib.BridgePdfStampAnnotationGetStampName.restype = ctypes.c_void_p
_lib.BridgePdfStampAnnotationGetStampName.argtypes = [ctypes.c_void_p]

_lib.BridgePdfStampAnnotationGetStampStyle.restype = ctypes.c_int32
_lib.BridgePdfStampAnnotationGetStampStyle.argtypes = [ctypes.c_void_p]

_lib.BridgePdfStampAnnotationSetStampStylePdfRubberStampIcon.restype = None
_lib.BridgePdfStampAnnotationSetStampStylePdfRubberStampIcon.argtypes = [ctypes.c_void_p, ctypes.c_int32]
...
class PdfStampAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfStampAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfStampAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfStampAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfStampAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfStampAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfStampAnnotationError(f"PdfStampAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfStampAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfStampAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfStampAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
        return sdk_loader.convert_string_handle(result)

    def get_stamp_name(self) -> str:
        """
        Gets the stamp name/type (e.g., "Approved", "Draft", "Confidential").

        Returns:
            str: The value of the StampName property.

        Raises:
            PdfStampAnnotationError: If the operation fails
        """
        self._ensure_not_closed()

        result = _lib.BridgePdfStampAnnotationGetStampName(self._handle)
        self._check_error()
        return sdk_loader.convert_string_handle(result)

    def get_stamp_style(self) -> Any:
        """
        Gets the stamp icon style.

        Returns:
            Any: The value of the StampStyle property.
        ...
        result = _lib.BridgePdfStampAnnotationGetStampStyle(self._handle)
        self._check_error()
        return result

    def set_stamp_style(self, value: Any) -> None:
        """
        Sets the stamp icon style.
        ...
        _lib.BridgePdfStampAnnotationSetStampStylePdfRubberStampIcon(self._handle, value.value if isinstance(value, Enum) else value)
        self._check_error()
    ...
    def stamp_name(self) -> str:
        ...
        Returns:
            str: The value of the StampName property.
        """
        return self.get_stamp_name()

    @property
    def stamp_style(self) -> Any:
        """
        Gets the stamp icon style.
        ...
        Returns:
            Any: The value of the StampStyle property.
        """
        return self.get_stamp_style()

    @stamp_style.setter
    def stamp_style(self, value: Any) -> None:
        """
        Sets the stamp style.

        Args:
            value (Any): The value to set.
        """
        self.set_stamp_style(value)
