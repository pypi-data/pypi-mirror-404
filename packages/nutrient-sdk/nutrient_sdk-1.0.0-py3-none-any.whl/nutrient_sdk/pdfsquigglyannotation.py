...
class PdfSquigglyAnnotationError(Exception):
    """Exception raised by PdfSquigglyAnnotation operations."""
    pass
...
_lib.BridgePdfSquigglyAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfSquigglyAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfSquigglyAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfSquigglyAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfSquigglyAnnotationFreeErrorString.restype = None
_lib.BridgePdfSquigglyAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfSquigglyAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfSquigglyAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfSquigglyAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfSquigglyAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfSquigglyAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfSquigglyAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfSquigglyAnnotationError(f"PdfSquigglyAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfSquigglyAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfSquigglyAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfSquigglyAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_creation_date(self) -> str:
        ...
        Raises:
            PdfSquigglyAnnotationError: If the operation fails
        """
