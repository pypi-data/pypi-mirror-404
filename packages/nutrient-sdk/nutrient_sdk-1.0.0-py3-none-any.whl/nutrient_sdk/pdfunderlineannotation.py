...
class PdfUnderlineAnnotationError(Exception):
    """Exception raised by PdfUnderlineAnnotation operations."""
    pass
...
_lib.BridgePdfUnderlineAnnotationGetLastErrorCode.restype = ctypes.c_int
_lib.BridgePdfUnderlineAnnotationGetLastErrorCode.argtypes = []

_lib.BridgePdfUnderlineAnnotationGetLastErrorMessage.restype = ctypes.c_void_p
_lib.BridgePdfUnderlineAnnotationGetLastErrorMessage.argtypes = []

_lib.BridgePdfUnderlineAnnotationFreeErrorString.restype = None
_lib.BridgePdfUnderlineAnnotationFreeErrorString.argtypes = [ctypes.c_void_p]
...
class PdfUnderlineAnnotation:
    ...
    def __init__(self):
        """Cannot instantiate PdfUnderlineAnnotation directly. Use static factory methods instead."""
        raise TypeError("PdfUnderlineAnnotation cannot be instantiated directly. Use static factory methods to obtain instances.")

    def _check_error(self):
        error_code = _lib.BridgePdfUnderlineAnnotationGetLastErrorCode()
        if error_code != 0:
            message_ptr = _lib.BridgePdfUnderlineAnnotationGetLastErrorMessage()
            if message_ptr:
                message = ctypes.string_at(message_ptr).decode('utf-8')
                _lib.BridgePdfUnderlineAnnotationFreeErrorString(message_ptr)
            else:
                message = "Unknown error"
            raise PdfUnderlineAnnotationError(f"PdfUnderlineAnnotation: {message} (code: {error_code})")
    
    def _ensure_not_closed(self):
        if self._closed:
            raise ValueError("PdfUnderlineAnnotation instance has been closed")
    ...
    def _get_sub_type_from_handle(handle):
        ...
        """
        result = _lib.BridgePdfUnderlineAnnotationGetSubType(handle)
        error_code = _lib.BridgePdfUnderlineAnnotationGetLastErrorCode()
        if error_code != 0:
            return None
    ...
    def get_creation_date(self) -> str:
        ...
        Raises:
            PdfUnderlineAnnotationError: If the operation fails
        """
