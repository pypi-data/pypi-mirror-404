...
class CodexGitHubMentionsAutomation:
    ...
    async def ensure_openai_login(self):
        ...
        try:
            ...
            if not AUTH_STATE_PATH.exists():
                ...
                # Save sessionStorage (critical for OAuth persistence)
                try:
                    ...
                    logger.info(f"Saved sessionStorage to {SESSION_STORAGE_PATH}")
                except Exception as e:
                    print(f"⚠️  Failed to save sessionStorage: {e}")
                    logger.warning(f"Failed to save sessionStorage: {e}")
            ...
            try:
                ...
                try:
                    ...
                    # Save sessionStorage after manual login
                    try:
                        session_storage = await self.page.evaluate("() => JSON.stringify(sessionStorage)")
                        SESSION_STORAGE_PATH.write_text(session_storage)
                        _ensure_auth_state_permissions(SESSION_STORAGE_PATH)
                        print(f"💾 Session storage saved to {SESSION_STORAGE_PATH}")
                        logger.info(f"Saved sessionStorage after manual login")
                    except Exception as e:
                        print(f"⚠️  Failed to save sessionStorage: {e}")
                ...
                try:
                    ...
                    continue_selectors = [
                        'button:has-text("Continue as Jeffrey")',
                    ...
                    if continue_button:
                        ...
                        try:
                            ...
                            # Save sessionStorage after Google login
                            try:
                                session_storage = await self.page.evaluate("() => JSON.stringify(sessionStorage)")
                                SESSION_STORAGE_PATH.write_text(session_storage)
                                _ensure_auth_state_permissions(SESSION_STORAGE_PATH)
                                print(f"💾 Session storage saved to {SESSION_STORAGE_PATH}")
                                logger.info(f"Saved sessionStorage after Google login")
                            except Exception as e:
                                print(f"⚠️  Failed to save sessionStorage: {e}")
    ...
    async def find_github_mention_tasks(self) -> List[Dict[str, str]]:
        ...
        Collects all Codex tasks up to task_limit (default: 50).
        """
        ...
        try:
            ...
            # Process all Codex tasks (no filtering by "GitHub Mention:" text)
