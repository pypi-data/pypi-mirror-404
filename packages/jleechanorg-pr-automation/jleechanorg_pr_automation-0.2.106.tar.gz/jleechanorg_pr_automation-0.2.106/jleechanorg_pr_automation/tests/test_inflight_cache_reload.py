...
def test_multiple_workflows_dont_interfere(tmp_path):
    ...
    # Should be blocked because concurrent_limit=1 is enforced across workflows
    assert result is False, "Second workflow should be blocked by inflight reservation"

    # Verify both reservations persisted
    with open(inflight_file) as f:
        data = json.load(f)
        assert _get_inflight_count(data, pr_key) == 1, "Only first workflow should reserve a slot"
