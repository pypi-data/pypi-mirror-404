...
SESSION_STORAGE_PATH = Path.home() / ".chatgpt_codex_session_storage.json"
...
class CodexGitHubMentionsAutomation:
    ...
    def __init__(
        ...
        Args:
            ...
            task_limit: Maximum number of tasks to process (default: 50, None = all tasks)
    ...
    async def _save_session_storage(self, context_label: str) -> None:
        """Persist sessionStorage to disk for OAuth continuity."""
        if not self.page:
            return
        try:
            ...
            _ensure_auth_state_permissions(SESSION_STORAGE_PATH)
            print(f"💾 Session storage saved to {SESSION_STORAGE_PATH}")
            logger.info(f"Saved sessionStorage after {context_label}")
        except Exception as e:
            print(f"⚠️  Failed to save sessionStorage: {e}")
            logger.warning(f"Failed to save sessionStorage after {context_label}: {e}")
    ...
    async def setup(self) -> bool:
        ...
        if not connected:
            ...
            # Restore sessionStorage using addInitScript (CRITICAL for OAuth persistence)
            if SESSION_STORAGE_PATH.exists():
                try:
                    ...
                    await self.context.add_init_script("""(storage) => {
                    ...
                    print("✅ Session storage restored via init script")
                    logger.info("Restored sessionStorage via addInitScript")
                except Exception as e:
                    print(f"⚠️  Failed to restore sessionStorage: {e}")
    ...
    async def ensure_openai_login(self):
        ...
        try:
            ...
            if not AUTH_STATE_PATH.exists():
                ...
                await self._save_session_storage("successful login")
            ...
            try:
                ...
                try:
                    ...
                    await self._save_session_storage("manual login")
                ...
                # Check for Google sign-in "Continue as" button
                try:
                    print("🔍 Checking for Google sign-in dialog...")
                    ...
                    if continue_button:
                        print("✅ Found Google account selector - clicking 'Continue as' button")
                        ...
                        try:
                            ...
                            print("✅ Successfully logged in via Google account selector")

                            # Save authentication state
                            await self.context.storage_state(path=str(AUTH_STATE_PATH))
                            _ensure_auth_state_permissions(AUTH_STATE_PATH)
                            print(f"💾 Authentication state saved to {AUTH_STATE_PATH}")
                            logger.info(f"Saved authentication state after Google login to {AUTH_STATE_PATH}")

                            await self._save_session_storage("Google login")

                            return True
                        except PlaywrightTimeoutError:
                            print("⚠️  Login attempt via Google failed")
                            return False
                    else:
                    ...
                    print(f"⚠️  Error during Google login attempt: {google_login_error}")
                    print("   Assuming you're logged in and continuing...")
                    return True
    ...
    async def find_github_mention_tasks(self) -> List[Dict[str, str]]:
        ...
        Collects GitHub Mention tasks up to task_limit (default: 50).
        """
        ...
        try:
            ...
            # Codex uses React/dynamic rendering - need longer wait for tasks to appear
            await asyncio.sleep(5)
            ...
            await asyncio.sleep(3)
            await self.page.evaluate("window.scrollTo(0, 0)")  # Scroll back up
            await asyncio.sleep(2)

            # Click "Tasks" button/tab if present to show task list
            print("   Looking for Tasks section...")
            tasks_clicked = await self._switch_to_tab("Tasks")
            if tasks_clicked:
                print("   ✅ Switched to Tasks view")
                await asyncio.sleep(10)  # Wait for tasks to load after clicking
            else:
                print("   Tasks section not found, continuing with current view")
                await asyncio.sleep(5)

            # Collect task links, then filter to GitHub Mention tasks by text
            primary_selector = 'a[href*="/codex/tasks/"]'
            selector_candidates = [primary_selector]
            ...
            filtered = [
                ...
                if "GitHub Mention:" in (task.get("text") or "")
            ]

            if not filtered:
                print("⚠️  No GitHub Mention tasks found")
                return []
