...
"""
    ...
    def _get_fix_comment_watch_state(
        ...
        # Support pagination for PRs with >100 comments (GitHub API limit per page)
        comments_url = f"https://api.github.com/repos/{repo_full}/issues/{pr_number}/comments"
        try:
            pr_data["comments"] = self._fetch_paginated_issue_comments(comments_url, headers)
        except requests.exceptions.RequestException as exc:
            # Comments are optional, continue without them
            self.logger.warning(
                "⚠️ Failed to fetch PR comments for %s#%s: %s",
                repo_full,
                pr_number,
                exc,
            )
    ...
    def _fetch_paginated_issue_comments(self, comments_url: str, headers: dict) -> list[dict]:
        """Fetch all issue comments with pagination and safe query handling."""
    ...
    def _get_pr_comment_state(self, repo_full_name: str, pr_number: int) -> tuple[str | None, list[dict] | None]:
        ...
        try:
            ...
            try:
                comments = self._fetch_paginated_issue_comments(comments_url, headers)
            except requests.exceptions.RequestException as e:
                self.logger.warning(
    ...
    def _get_last_codex_automation_comment_time(self, comments: list[dict]) -> str | None:
        ...
        for comment in comments:
            ...
            if (
                ...
                created_at = self._get_comment_timestamp(comment)
    ...
    def _get_comment_timestamp(self, comment: dict) -> str:
        """Return best-effort ISO timestamp for a comment (REST or GraphQL)."""
        return (
    ...
    def _is_last_comment_automation(self, comments: list[dict]) -> bool:
        """Check if the most recent comment is from automation.

        This prevents infinite loops where:
        1. Automation posts comment
        2. Bot replies to automation comment
        3. Automation sees bot reply → posts another comment
        4. Bot replies again → loop continues

        WA-o3w: Anti-loop logic for comment mode
        """
        if not comments:
            return False

        # Sort by creation time (newest last) supporting REST and GraphQL payloads
        sorted_comments = sorted(
            comments,
            key=self._get_comment_timestamp,
        )
        ...
        # Check if last comment has any automation marker
        return (
            self.CODEX_COMMIT_MARKER_PREFIX in body
            or self.FIX_COMMENT_MARKER_PREFIX in body
            or self.FIX_COMMENT_RUN_MARKER_PREFIX in body
    ...
    def _has_new_bot_comments_since_codex(self, comments: list[dict]) -> bool:
        ...
        need attention.
        """
        # ANTI-LOOP: If last comment is from automation, don't reprocess
        # Prevents infinite loop where bot replies to automation → automation replies → bot replies → ...
        if self._is_last_comment_automation(comments):
            self.logger.debug(
                "🚫 Last comment is from automation - not reprocessing for bot comments (WA-o3w anti-loop)"
            )
            return False
    ...
    def run_monitoring_cycle(self, single_repo=None, max_prs=10, cutoff_hours: int = 24, fix_comment: bool = False, fixpr: bool = False, comment_validation: bool = False, agent_cli: str = "claude", model: str = None, parallel: bool = True, single_action: bool = False):
        ...
        Args:
            ...
            single_action: If True, exit after successfully processing one PR (forces sequential mode)
        """
        # Single action mode forces sequential processing
        if single_action:
            parallel = False
        ...
        for pr in open_prs:
            ...
            try:
                ...
                if success:
                    ...
                    # Single action mode: exit after first successful action
                    if single_action and comment_result == "posted":
                        self.logger.info("🎯 Single action mode: Exiting after successfully processing PR %s #%s", repo_full_name, pr_number)
...
def main():
    ...
    parser.add_argument("--single-action", action="store_true",
                        help="Exit after successfully processing one PR (forces sequential mode)")
