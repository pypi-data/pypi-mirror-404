...
class AutomationSafetyManager:
    ...
    def __init__(self, data_dir: str, limits: Optional[Dict[str, int]] = None):
        ...
        # Can be overridden via limits parameter (for testing purposes only)
        if limits:
            self.concurrent_limit = coerce_positive_int(
                limits.get("concurrent_limit", 1),
                default=1,
            )
        else:
    ...
    def _normalize_global_run_payload(
        ...
        *,
        ...
        """Normalize persisted global run data with rolling window support.

        NEW BEHAVIOR: Stores list of run timestamps instead of daily counter.
        Filters to rolling window (default 24 hours) and returns count within window.
        """
        ...
        # Ensure start_date is always present
        if "start_date" not in data or not isinstance(data.get("start_date"), str):
            data["start_date"] = current_time.isoformat()

        # Get rolling window hours from env var (default 24)
        ...
        # Handle migration from old format (total_runs counter) to new format (timestamps list)
        run_timestamps = data.get("run_timestamps", [])
        if not isinstance(run_timestamps, list):
            run_timestamps = []

        # If old format exists (total_runs but no timestamps), preserve for backwards compat
        # but don't use it for counting (rolling window is authoritative)
        if "total_runs" in data and not run_timestamps:
            # Legacy format - keep total_runs for reference but it won't be used
            data["total_runs"] = data.get("total_runs", 0)

        # Filter timestamps to rolling window
        valid_timestamps = []
        for ts in run_timestamps:
            if isinstance(ts, str):
                try:
                    ts_time = REAL_DATETIME.fromisoformat(ts)
        ...
        data["run_timestamps"] = valid_timestamps
        data["total_runs"] = len(valid_timestamps)

        # Update last_run to most recent timestamp
        if valid_timestamps:
            data["last_run"] = max(valid_timestamps)
        elif "last_run" in data:
            # Keep existing last_run if no valid timestamps
            pass

        # Keep last_reset for compatibility
        if "last_reset" not in data:
            data["last_reset"] = data["start_date"]

        # is_stale is no longer meaningful with rolling window, but keep for compatibility
        is_stale = False

        return data, len(valid_timestamps), is_stale
    ...
    def _atomic_update_with_retry(
        ...
        *,
        ...
        """Run atomic_update with retry and exponential backoff."""
        for attempt in range(max_retries):
            try:
                write_success = json_manager.atomic_update(
                    file_path, update_func, default
                )
            except Exception as exc:
                write_success = False
                self.logger.warning(
                    "⚠️ atomic_update() raised %s while %s (attempt %d/%d): %s",
                    type(exc).__name__,
                    action_label,
                    attempt + 1,
                    max_retries,
            ...
            if attempt < max_retries - 1:
                ...
                self.logger.warning(
                    "⚠️ atomic_update() failed while %s (attempt %d/%d), retrying after %.0fms...",
                    action_label,
                    attempt + 1,
                    max_retries,
                    backoff_delay * 1000,
    ...
    def try_process_pr(self, pr_number: Union[int, str], repo: str = None, branch: str = None) -> bool:
        ...
        with self.lock:
            ...
            write_success = self._atomic_update_with_retry(
                self.inflight_file,
                reserve_slot,
                {},
                action_label=f"reserving slot for PR {pr_key}",
            )

            # CRITICAL: Must enforce concurrent processing limit across all processes
            # If atomic_update() fails after all retries, we cannot allow processing because:
            # 1. reserve_slot() callback modified in-memory cache (inflight+1)
            # 2. But disk file was NOT updated (write failed)
            # 3. Other processes will read stale data from disk (inflight=0)
            # 4. This creates race condition where multiple processes bypass concurrent_limit
            #
            # Retry logic reduces false rejections, but after exhausting retries,
            # we must return False to prevent race conditions.
            if not write_success:
                effective_limit = self._get_effective_pr_limit(pr_number, repo, branch)
                self.logger.error(
                    "❌ File I/O error while reserving slot for PR %s: atomic_update() failed after %d retries. "
                    "Cannot safely reserve processing slot - rejecting to prevent race condition. "
                    "This is likely due to persistent file lock contention or disk I/O issues, NOT safety limits. "
                    "PR was under limit (%s attempts allowed) but cannot guarantee concurrent safety.",
                    pr_key,
                    3,
                    effective_limit,
                )
                # MUST return False to prevent concurrent processing race condition
                # Even though can_process_pr() passed, we cannot allow processing without
                # successful disk synchronization across processes
                return False
    ...
    def release_pr_slot(self, pr_number: Union[int, str], repo: str = None, branch: str = None):
        ...
        with self.lock:
            ...
            write_success = self._atomic_update_with_retry(
                ...
                release_slot,
                {},
                action_label=f"releasing slot for PR {pr_key}",
            )

            if not write_success:
                self.logger.error(
                    "❌ SLOT LEAK: Failed to release slot for PR %s after %d retries. "
                    "Manual intervention required - clear pr_inflight.json or this PR will be blocked. "
                    "File: %s",
                    pr_key,
                    3,
                    self.inflight_file,
    ...
    def get_global_runs(self) -> int:
        """Get total number of global runs within rolling window (default 24 hours)."""
        with self.lock:
            normalized_total = 0
    ...
    def record_global_run(self):
        """Record a global automation run atomically with rolling window."""
        self._record_global_run_at_time(datetime.now(timezone.utc))

    def _record_global_run_at_time(self, run_time: datetime):
        """Record a global run at specific time (for testing and implementation).

        Args:
            run_time: Timestamp of the run (must be timezone-aware)
        """
        with self.lock:
            new_total = 0

            def _append_timestamp(payload: Optional[dict]):
                nonlocal new_total
                # Normalize first to filter old timestamps
                normalized, total, _ = self._normalize_global_run_payload(payload, now=run_time)

                # Append new timestamp
                run_timestamps = normalized.get("run_timestamps", [])
                run_timestamps.append(run_time.isoformat())
                normalized["run_timestamps"] = run_timestamps

                # Update counts
                normalized["total_runs"] = len(run_timestamps)
                normalized["last_run"] = run_time.isoformat()

                new_total = len(run_timestamps)
                return normalized

            if not json_manager.update_json(self.global_runs_file, _append_timestamp):
                self.logger.warning(
                    "Falling back to manual append for global run timestamp"
                )
                payload = self._read_json_file(self.global_runs_file)
                normalized, total, _ = self._normalize_global_run_payload(payload, now=run_time)
    ...
    def check_and_notify_limits(self):
        ...
        with self.lock:
            ...
            for pr_key, attempts in self._pr_attempts_cache.items():
                # Count attempts within rolling window (must match can_process_pr)
                window_hours = coerce_positive_int(
                    os.environ.get("AUTOMATION_ATTEMPT_WINDOW_HOURS", "24"),
                    default=24,
                )
                ...
                total_attempts = len([
                    ...
                    and self._parse_timestamp(attempt.get("timestamp")) >= cutoff_time
                ])
