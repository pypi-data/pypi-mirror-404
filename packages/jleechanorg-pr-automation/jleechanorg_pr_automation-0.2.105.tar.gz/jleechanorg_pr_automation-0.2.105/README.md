...
│  3. SAFETY CHECKS                                           │
│  ───────────────────────────────────────────────────────────│
│  • Verify PR hasn't exceeded attempt limits (max 50)        │
│  • Check global automation limit (max 100 runs)             │
...
# - jleechanorg-pr-monitor --pr-limit 50 --global-limit 100 --approval-hours 24
...
│  • Collect task links and filter to "GitHub Mention:"       │
...
| **Task Selection** | CSS selector `a[href*="/codex/tasks/"]` + text filter | Finds GitHub PR tasks |
