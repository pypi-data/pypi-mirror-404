...
def test_no_hardcoded_google_account_name():
    """Avoid hardcoded personal account names in selectors."""
    source = inspect.getsource(CodexGitHubMentionsAutomation.ensure_openai_login)
    assert "Continue as Jeffrey" not in source, (
        "Hardcoded Google account name found in login selectors; use generic selectors only."
    )
