...
from jleechanorg_pr_automation.openai_automation import codex_github_mentions as codex_module
...
class TestLimitParameter:
    ...
    async def test_limit_applied_to_task_finding(self, limit, mock_task_count, expected_return):
        ...
        for i in range(mock_task_count):
            ...
            item.text_content = AsyncMock(return_value=f"GitHub Mention: Task {i}")
...
class TestTaskFinding:
    ...
    async def test_task_finding_matrix(self, task_count, limit, expected_found, behavior):
        ...
        for idx in range(task_count):
            ...
            item.text_content = AsyncMock(return_value=f"GitHub Mention: Task {idx}")
    ...
    @pytest.mark.asyncio
    async def test_github_mention_filter_applied_by_default(self, monkeypatch):
        """Only GitHub Mention tasks should be returned by default."""
        automation = CodexGitHubMentionsAutomation(task_limit=None)
        automation.page = AsyncMock()
        monkeypatch.setattr(codex_module.asyncio, "sleep", AsyncMock(return_value=None))
        automation._switch_to_tab = AsyncMock(return_value=False)
        automation._collect_task_links = AsyncMock(
            return_value=[
                {"href": "/codex/tasks/1", "text": "GitHub Mention: PR #1234"},
                {"href": "/codex/tasks/2", "text": "General task: Cleanup"},
            ]
        )

        tasks = await automation.find_github_mention_tasks()

        assert tasks == [{"href": "/codex/tasks/1", "text": "GitHub Mention: PR #1234"}], (
            "Expected only GitHub Mention tasks to be returned by default, "
            f"got: {tasks}"
        )
