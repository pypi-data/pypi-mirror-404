...
class TestGlobalLimits:
    ...
    def test_global_runs_auto_resets_daily(self, manager):
        """Rolling window should exclude runs outside 24 hours and clear approval requirement."""
        manager._clear_global_runs()

        # Simulate crossing the limit within the rolling window
        for _ in range(manager.global_limit):
            manager.record_global_run()
        ...
        stale_run_time = now - timedelta(hours=30)
        stale_payload = {
            "run_timestamps": [
                (stale_run_time + timedelta(minutes=i)).isoformat()
                for i in range(manager.global_limit)
            ],
            "total_runs": manager.global_limit,
            "start_date": (now - timedelta(days=4)).isoformat(),
            "last_run": stale_run_time.isoformat(),
