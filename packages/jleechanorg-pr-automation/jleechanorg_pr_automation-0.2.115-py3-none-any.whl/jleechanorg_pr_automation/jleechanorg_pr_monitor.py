...
"""
    ...
    def _build_fix_comment_prompt_body(
        ...
        return (
            ...
            "2) Fetch ALL PR feedback sources (pagination-safe) using correct GitHub API endpoints (Python):\n"
            "   ```python\n"
            "   import requests\n"
            "   from automation.jleechanorg_pr_automation.orchestrated_pr_runner import get_github_token\n"
            "   token = get_github_token()\n"
            "   headers = {'Authorization': f'Bearer {token}', 'Accept': 'application/vnd.github.v3+json'}\n"
            f"   # Issue comments:\n"
            f"   requests.get('https://api.github.com/repos/{repository}/issues/{pr_number}/comments', headers=headers, params={{'per_page': 100}})\n"
            f"   # Review summaries:\n"
            f"   requests.get('https://api.github.com/repos/{repository}/pulls/{pr_number}/reviews', headers=headers, params={{'per_page': 100}})\n"
            f"   # Inline review comments:\n"
            f"   requests.get('https://api.github.com/repos/{repository}/pulls/{pr_number}/comments', headers=headers, params={{'per_page': 100}})\n"
            "   ⚠️ WARNING: `gh pr view --json comments` ONLY fetches issue comments, NOT inline review comments. DO NOT use gh CLI.\n\n"
            ...
            "   - Use `post_pr_comment_python` as defined above.\n"
    ...
    def _get_last_codex_automation_comment_time(self, comments: list[dict]) -> str | None:
        ...
        for comment in comments:
            ...
            # CRITICAL: Include ALL workflow markers to prevent feedback loops
            if (
                ...
                or self.COMMENT_VALIDATION_MARKER_PREFIX in body  # Fix feedback loop
            ):
