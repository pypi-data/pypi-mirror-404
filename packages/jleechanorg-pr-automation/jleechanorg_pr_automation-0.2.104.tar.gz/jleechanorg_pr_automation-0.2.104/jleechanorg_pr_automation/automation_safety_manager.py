...
class AutomationSafetyManager:
    ...
    def __init__(self, data_dir: str, limits: Optional[Dict[str, int]] = None):
        ...
        self.concurrent_limit = limits.get("concurrent_limit", 1) if limits else 1
    ...
    def try_process_pr(self, pr_number: Union[int, str], repo: str = None, branch: str = None) -> bool:
        ...
        with self.lock:
            ...
            # Retry atomic_update() with exponential backoff to handle transient failures
            ...
            for attempt in range(max_retries):
                ...
                if attempt < max_retries - 1:
                    backoff_delay = 0.05 * (2 ** attempt)  # 50ms, 100ms, 200ms
                    self.logger.warning(
                        "⚠️ atomic_update() failed for PR %s (attempt %d/%d), retrying after %.0fms...",
                        pr_key,
                        attempt + 1,
                        max_retries,
                        backoff_delay * 1000,
    ...
    def release_pr_slot(self, pr_number: Union[int, str], repo: str = None, branch: str = None):
        ...
        with self.lock:
            ...
            # Retry atomic_update() with exponential backoff to prevent slot leaks
            # Slot leaks block all future processing for this PR
            max_retries = 3
            write_success = False
            for attempt in range(max_retries):
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.warning(
                        "⚠️ Failed to release slot for PR %s (attempt %d/%d), retrying after %.0fms...",
                        pr_key,
                        attempt + 1,
                        max_retries,
                        backoff_delay * 1000,
