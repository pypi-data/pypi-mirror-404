...
class AutomationSafetyManager:
    ...
    def try_process_pr(self, pr_number: Union[int, str], repo: str = None, branch: str = None) -> bool:
        ...
        with self.lock:
            ...
            # Handle file I/O failures separately from safety limit violations
            if not write_success:
                effective_limit = self._get_effective_pr_limit(pr_number, repo, branch)
                self.logger.error(
                    "❌ File I/O error while reserving slot for PR %s: atomic_update() failed. "
                    "This is likely due to file lock contention or disk I/O issues, NOT safety limits. "
                    "PR was under limit (%s attempts allowed) - allowing processing despite file write failure.",
                    pr_key,
                    effective_limit,
                )
                # DEFENSIVE FALLBACK: If can_process_pr() passed but file write failed,
                # allow processing to continue. File I/O failures shouldn't block PRs that
                # are under safety limits. The attempt will be tracked in pr_attempts.json
                # when record_attempt() is called later.
                return True
