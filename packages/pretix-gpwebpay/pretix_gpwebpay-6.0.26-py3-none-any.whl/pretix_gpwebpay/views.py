...
from .tasks import gpwebpay_sync_payment
...
def _schedule_ws_checks(provider, payment_id: int):
    if not getattr(provider, '_ws_configured')():
        return
    gpwebpay_sync_payment.apply_async((payment_id,), countdown=60)
    gpwebpay_sync_payment.apply_async((payment_id,), countdown=300)
...
class ReturnView(View):
    ...
    def _handle_response(self, request: HttpRequest, order: str, payment: int, hash: str):
        ...
        try:
            ...
            info = payment_obj.info if isinstance(payment_obj.info, dict) else {}
            info['gpwebpay_raw'] = all_params
            ...
            if _is_success(prcode, srcode):
                if payment_obj.state in (OrderPayment.PAYMENT_STATE_PENDING, OrderPayment.PAYMENT_STATE_CREATED):
                    try:
                        ...
                        _schedule_ws_checks(provider, payment_obj.id)
...
class ManualSyncView(View):
    """
    Manually sync payment status from GPWebPay WS.
    """
    ...
    def post(self, request: HttpRequest, order: str, payment: int, hash: str):
        ...
        try:
            ...
            result = provider.sync_payment_from_ws(payment_obj)
            if result == 'confirmed':
                messages.success(request, _('Payment confirmed via GPWebPay WS.'))
            elif result == 'failed':
                messages.error(request, _('Payment failed per GPWebPay WS.'))
            elif result == 'pending':
                messages.warning(request, _('Payment is still pending in GPWebPay.'))
            elif result == 'already_confirmed':
                messages.info(request, _('Payment already confirmed.'))
            elif result == 'already_failed':
                messages.info(request, _('Payment already failed.'))
            else:
                messages.warning(request, _('Payment status is unknown in GPWebPay.'))
            return redirect(_safe_order_url(order_obj))
        except Exception as e:
            logger.error('Error syncing GPWebPay payment: %s', e, exc_info=True)
            messages.error(request, _('Error syncing payment status.'))
            try:
                return redirect(_safe_order_url(order_obj))
            except Exception:
                return HttpResponseBadRequest('Error syncing payment')
