...
#
# Copyright 2002-2019, Paul McGuire, Evan Hubinger

# [CPYPARSING] use all __future__ imports
...
# [CPYPARSING] properly mock pyparsing imports and directories
import os
...
class ParseConfigFileTest(ParseTestCase):
    def runTest(self):
        ...
        test(os.path.join(TEST_DIR, "test/karthik.ini"), 23,
                [ ("users.K","8"),
                  ("users.mod_scheme","'QPSK'"),
                  ("users.Na", "K+2") ]
                  )
        test(os.path.join(TEST_DIR, "examples/Setup.ini"), 125,
...
class SetNameTest(ParseTestCase):
    def runTest(self):
        ...
        # [CPYPARSING] we always use ... for Forward reprs
...
# [CPYPARSING] disable _trim_arity testing on py < 3.10
...
def run():
    ...
    return test_runner.run(suite).wasSuccessful()
