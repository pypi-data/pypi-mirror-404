...
class DouyinVideoClient(BaseVideoClient):
    ...
    def parsefromurl(self, url: str, request_overrides: dict = None):
        ...
        # try parse
        try:
            resp = self.get(url, allow_redirects=False, **request_overrides)
            resp.raise_for_status()
            location = resp.headers.get("Location")
            if not location: resp = self.get(url, allow_redirects=True, **request_overrides); resp.raise_for_status(); location = resp.url
            vid = re.search(r"\d+", location).group(0)
            resp = self.get(f"https://www.iesdouyin.com/share/video/{vid}", **request_overrides)
            resp.raise_for_status()
            raw_data = DouyinVideoClient.ROUTER_DATA_RE.search(resp.text).group(1).strip().rstrip("; \n\r\t")
            if not raw_data.startswith("{"):
                brace_pos = raw_data.find("{")
                if brace_pos != -1: raw_data = raw_data[brace_pos:].rstrip("; \n\r\t")
            ...
            video_info.update(dict(
                title=video_title, file_path=os.path.join(self.work_dir, self.source, f'{video_title}.{ext}'), ext=ext, guess_video_ext_result=guess_video_ext_result, identifier=vid, cover_url=safeextractfromdict(video_detail, ['video', 'cover', 'url_list', 0], None),
            ))
