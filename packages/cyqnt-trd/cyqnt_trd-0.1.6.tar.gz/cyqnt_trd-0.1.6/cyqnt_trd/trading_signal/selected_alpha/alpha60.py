...
def alpha60_factor(
    ...
    **kwargs
    ...
    try:
        ...
        # 实现Alpha因子逻辑
        divisor = (high_price - low_price).replace(0, 0.0001)
        inner = ((close_price - low_price) - (high_price - close_price)) * volume / divisor
        result = - ((2 * scale(rank(inner))) - scale(rank(ts_argmax(close_price, 10))))
