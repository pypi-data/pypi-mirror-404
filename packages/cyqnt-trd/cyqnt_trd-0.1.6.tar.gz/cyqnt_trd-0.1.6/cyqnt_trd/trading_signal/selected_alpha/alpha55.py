...
def alpha55_factor(
    ...
    **kwargs
    ...
    try:
        ...
        # 实现Alpha因子逻辑
        divisor = (ts_max(high_price, 12) - ts_min(low_price, 12)).replace(0, 0.0001)
        inner = (close_price - ts_min(low_price, 12)) / (divisor)
        df = correlation(rank(inner), rank(volume), 6)
        result = -1 * df.replace([-np.inf, np.inf], 0).fillna(value=0)
