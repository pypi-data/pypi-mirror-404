...
公式: (max(Ts_Rank(decay_linear(correlation(rank(vwap), rank(volume), 3.83878),4.16783), 8.38151), Ts_Rank(decay_linear(Ts_ArgMax(correlation(Ts_Rank(close, 7.45404),Ts_Rank(adv60, 4.13242), 3.65459), 12.6556), 14.0365), 13.4143)) * -1)
...
def alpha96_factor(
    ...
    **kwargs
    ...
    try:
        ...
        p2=ts_rank(decay_linear(ts_argmax(correlation(ts_rank(close_price, 7),ts_rank(adv60, 4), 4), 13), 14), 13)
        df=pd.Series
        df.at[df['p1']>=df['p2'],'max']=df['p1']
        df.at[df['p2']>=df['p1'],'max']=df['p2']
        result = -1*df['max']
        #return (max(ts_rank(decay_linear(correlation(rank(vwap), rank(volume), 4),4), 8), ts_rank(decay_linear(ts_argmax(correlation(ts_rank(close_price, 7),ts_rank(adv60, 4), 4), 13), 14), 13)) * -1)
