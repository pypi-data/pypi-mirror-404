...
公式: (max(rank(decay_linear(delta(vwap, 4.72775), 2.91864)),Ts_Rank(decay_linear(((delta(((open * 0.147155) + (low * (1 - 0.147155))), 2.03608) / ((open *0.147155) + (low * (1 - 0.147155)))) * -1), 3.33829), 16.7411)) * -1)
...
def alpha73_factor(
    ...
    **kwargs
    ...
    try:
        ...
        p2=ts_rank(decay_linear(((delta(((open_price * 0.147155) + (low_price * (1 - 0.147155))), 2) / ((open_price *0.147155) + (low_price * (1 - 0.147155)))) * -1), 3), 17)
        df=pd.Series
        df.at[df['p1']>=df['p2'],'max']=df['p1']
        df.at[df['p2']>=df['p1'],'max']=df['p2']
        result = -1*df['max']
        #return (max(rank(decay_linear(delta(vwap, 5), 3)),ts_rank(decay_linear(((delta(((open_price * 0.147155) + (low_price * (1 - 0.147155))), 2) / ((open_price *0.147155) + (low_price * (1 - 0.147155)))) * -1), 3), 17)) * -1)
