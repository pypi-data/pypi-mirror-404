...
公式: ((rank((vwap - ts_min(vwap, 11.5783)))^Ts_Rank(correlation(Ts_Rank(vwap,19.6462), Ts_Rank(adv60, 4.02992), 18.0926), 2.70756)) * -1)
...
def alpha94_factor(
    ...
    **kwargs
    ...
    try:
        ...
        # 实现Alpha因子逻辑
        adv60 = sma(volume, 60)
        result = ((rank((vwap - ts_min(vwap, 12))).pow(ts_rank(correlation(ts_rank(vwap,20), ts_rank(adv60, 4), 18), 3)) * -1))
