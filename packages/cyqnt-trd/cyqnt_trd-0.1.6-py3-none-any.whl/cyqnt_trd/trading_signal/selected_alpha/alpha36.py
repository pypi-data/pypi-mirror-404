...
公式: (((((2.21 * rank(correlation((close - open), delay(volume, 1), 15))) + (0.7 * rank((open- close)))) + (0.73 * rank(Ts_Rank(delay((-1 * returns), 6), 5)))) + rank(abs(correlation(vwap,adv20, 6)))) + (0.6 * rank((((sum(close, 200) / 200) - open) * (close - open)))))
...
def alpha36_factor(
    ...
    **kwargs
    ...
    try:
        ...
        # 实现Alpha因子逻辑
        adv20 = sma(volume, 20)
        result = (((((2.21 * rank(correlation((close_price - open_price), delay(volume, 1), 15))) + (0.7 * rank((open_price- close_price)))) + (0.73 * rank(ts_rank(delay((-1 * returns), 6), 5)))) + rank(abs(correlation(vwap,adv20, 6)))) + (0.6 * rank((((sma(close_price, 200) / 200) - open_price) * (close_price - open_price)))))
