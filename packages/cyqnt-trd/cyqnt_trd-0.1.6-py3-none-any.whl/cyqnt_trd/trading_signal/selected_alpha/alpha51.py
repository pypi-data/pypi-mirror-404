...
公式: (((((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10)) < (-1 *0.05)) ? 1 : ((-1 * 1) * (close - delay(close, 1))))
...
def alpha51_factor(
    ...
    **kwargs
    ...
    try:
        ...
        # 实现Alpha因子逻辑
        inner = (((delay(close_price, 20) - delay(close_price, 10)) / 10) - ((delay(close_price, 10) - close_price) / 10))
        alpha = (-1 * delta(close_price))
        alpha[inner < -0.05] = 1
        result = alpha
