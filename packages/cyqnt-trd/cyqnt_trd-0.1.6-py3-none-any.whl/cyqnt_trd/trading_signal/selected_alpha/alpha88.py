...
def alpha88_factor(
    ...
    **kwargs
    ...
    try:
        ...
        # 实现Alpha因子逻辑
        adv60 = sma(volume, 60)
        p1=rank(decay_linear(((rank(open_price) + rank(low_price)) - (rank(high_price) + rank(close_price))),8))
        p2=ts_rank(decay_linear(correlation(ts_rank(close_price, 8), ts_rank(adv60,21), 8), 7), 3)
        df=pd.Series
        df.at[df['p1']>=df['p2'],'min']=df['p2']
        df.at[df['p2']>=df['p1'],'min']=df['p1']
        result = df['min']
        #return min(rank(decay_linear(((rank(open_price) + rank(low_price)) - (rank(high_price) + rank(close_price))),8)), ts_rank(decay_linear(correlation(ts_rank(close_price, 8), ts_rank(adv60,20.6966), 8), 7), 3))
