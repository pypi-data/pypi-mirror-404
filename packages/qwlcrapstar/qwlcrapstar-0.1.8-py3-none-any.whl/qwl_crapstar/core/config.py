...
class Config:
    ...
    def validate_provider(self, provider: str):
        ...
        if not key_val:
            ...
            error_msg += f"👉 Please set it in your environment: export {env_var}=your_key\n"
            
            if available:
                error_msg += f"💡 I detected keys for these other providers instead: {available}. \n"
                error_msg += f"   You can use them by setting QwlCrapstar(llm_provider='{available[0]}')"
            else:
                error_msg += "💡 No LLM API keys detected. You can also run locally by setting use_local_llm=True (requires Ollama)."
