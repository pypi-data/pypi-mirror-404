...
class Reg2Surv(nn.Module):
    """Transforms a single regression dataset (features X, targets y) into a survival analysis format
    ...
    ----------
    hp : dict
        Hyperparameters for the Reg2Surv module.
    """
    ...
    def forward(self, X: Tensor, y: Tensor) -> tuple[Tensor, Tensor]:
        ...
        tuple[Tensor, Tensor]
            ...
            - Processed targets of shape (T, 2).
        """
        if X.ndim != 2 or y.ndim != 2 or y.shape[1] != 2 or X.shape[0] != y.shape[0]:
        ...
        y_event = y[:, 0] < y[:, 1]
        if y_event.sum() < y.shape[0] // 10:
            y_event = ~y_event  # ensure that there's always at least 10% events
        y_time = torch.min(y[:, 0], y[:, 1])
        y_time = (y_time - y_time.min()) / (y_time.max() - y_time.min())

        return X.float(), torch.stack([y_event, y_time], dim=-1).float()
