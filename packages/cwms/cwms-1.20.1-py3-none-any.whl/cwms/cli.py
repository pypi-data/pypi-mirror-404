...
@main.group()
def memory() -> None:
    """Manage long-term MEMORY.md facts for a project.

    The memory system provides a tiered architecture:
    - CONTINUATION.md: Active state (generated on swap)
    - Daily logs: Session-specific context (searchable chunks)
    - MEMORY.md: Long-term facts (persistent across sessions)

    Use these commands to manage long-term facts that should persist
    across all sessions, such as project preferences, key decisions,
    and important context that shouldn't be lost.

    Examples:
        cwms memory add --project "myapp" --fact "Uses PostgreSQL for primary DB"
        cwms memory list --project "myapp"
        cwms memory search --project "myapp" --query "database"
    """
    pass


def _get_memory_file(project: str) -> Path:
    """Get the path to the MEMORY.md file for a project."""
    config = Config.load_or_default()
    return Path(config.storage_dir) / project / "MEMORY.md"


@memory.command("add")
@click.option("--project", required=True, help="Project identifier")
@click.option("--fact", required=True, help="Fact to add to long-term memory")
@click.option(
    "--category",
    default="general",
    help="Category for the fact (e.g., 'decision', 'preference', 'context')",
)
def memory_add(project: str, fact: str, category: str) -> None:
    """Add a fact to long-term memory.

    Facts are stored in MEMORY.md and persist across all sessions.
    They are not subject to context window management and should
    contain durable information that's always relevant.

    Example:
        cwms memory add --project "myapp" --fact "Auth uses JWT with 24h expiry"
        cwms memory add --project "myapp" --fact "Prefer functional components" --category "preference"
    """
    try:
        memory_file = _get_memory_file(project)
        ...
        if not memory_file.exists():
            header = f"""# MEMORY.md - Long-term Facts for {project}

> These facts persist across all sessions and are not subject to context management.
> Add durable information here that should always be available.
...
"""
        ...
        logger.exception("Error adding to memory")
        output_error(f"Error adding to memory: {e}")


@memory.command("list")
@click.option("--project", required=True, help="Project identifier")
@click.option("--category", default=None, help="Filter by category")
def memory_list(project: str, category: str | None) -> None:
    """List all facts in long-term memory.

    Example:
        cwms memory list --project "myapp"
        cwms memory list --project "myapp" --category "decision"
    """
    try:
        memory_file = _get_memory_file(project)
            ...
            output_json(
                {
                    ...
                    "project": project,
                    "facts": [],
                    "message": "No memory file found. Use 'cwms memory add' to create one.",
                }
            )
            return
                    ...
                    facts.append(
                        {
                            ...
                            "content": current_fact.strip(),
                        }
                    )
                # Parse new fact header
                try:
                    bracket_end = line.index("]")
                    current_category = line[4:bracket_end].lower()
                    current_timestamp = line[bracket_end + 2 :].strip()
                    current_fact = ""
                except (ValueError, IndexError):
                    current_category = "unknown"
                    current_timestamp = ""
                    current_fact = ""
            elif current_fact is not None and not line.startswith("#") and not line.startswith(">"):
                current_fact += line + "\n"
        ...
        if category:
            facts = [f for f in facts if f["category"] == category.lower()]
        ...
        logger.exception("Error listing memory")
        output_error(f"Error listing memory: {e}")


@memory.command("search")
@click.option("--project", required=True, help="Project identifier")
@click.option("--query", required=True, help="Search query")
def memory_search(project: str, query: str) -> None:
    """Search long-term memory for matching facts.

    Performs a case-insensitive search across all facts.
    ...
    try:
        ...
        for line in content.split("\n"):
            if line.startswith("## ["):
                # Check previous fact for match
                if current_fact and query_lower in current_fact.lower():
                    matches.append(
                        {
                            "category": current_category,
                            "timestamp": current_timestamp,
        ...
        output_error(f"Error searching memory: {e}")


@memory.command("clear")
@click.option("--project", required=True, help="Project identifier")
@click.option("--confirm", is_flag=True, help="Confirm deletion without prompt")
def memory_clear(project: str, confirm: bool) -> None:
    """Clear all facts from long-term memory.

    WARNING: This permanently deletes all stored facts.

    Example:
        cwms memory clear --project "myapp" --confirm
    """
    try:
        memory_file = _get_memory_file(project)
            ...
            output_json(
                {
                    "success": True,
                    "project": project,
                    "message": "No memory file to clear.",
                }
            )
            return
        ...
        if not confirm:
            output_error(
                "This will permanently delete all long-term memory facts.\n"
        ...
        output_error(f"Error clearing memory: {e}")
