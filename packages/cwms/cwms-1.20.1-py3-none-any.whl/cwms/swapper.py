...
class Swapper:
    ...
    def _create_combined_summary(self, chunks: list[Chunk]) -> str:
        ...
        # Combine all chunk content for status analysis
        combined_content = ""
        ...
        for chunk in chunks:
            ...
            lines_in_summary = summary.split("\n")
            for line in lines_in_summary:
        ...
        # Extract status markers from combined content
        status_info = extract_status_markers(combined_content)
        ...
        # Add status if detected with reasonable confidence
        if status_info.primary_status != "UNKNOWN" and status_info.confidence > 0.3:
        ...
        # Add pending decisions if detected
        if status_info.pending_decisions:
            lines.extend(["", "**Pending Decisions:**"])
            for decision in status_info.pending_decisions[:3]:
                lines.append(f"- {decision}")

        # Add blockers if detected
        if status_info.blockers:
            lines.extend(["", "**Blockers:**"])
            for blocker in status_info.blockers[:3]:
    ...
    def generate_continuation_document(
        self,
        chunks: list[Chunk],
        project: str,
    ) -> str:
        """Generate a CONTINUATION.md document from swapped chunks.

        Creates a structured document following the 'amnesiac future self'
        principle for optimal session recovery.
        ...
        # Gather files and keywords from all chunks
        all_files: list[str] = []
        all_keywords: list[str] = []

        for chunk in chunks:
            all_keywords.extend(chunk.keywords[:5])
            if chunk.metadata.get("files_referenced"):
                all_files.extend(chunk.metadata["files_referenced"])

        # Deduplicate
        unique_files = list(dict.fromkeys(all_files))[:10]
        unique_keywords = list(dict.fromkeys(all_keywords))[:12]

        return generate_continuation_md(
