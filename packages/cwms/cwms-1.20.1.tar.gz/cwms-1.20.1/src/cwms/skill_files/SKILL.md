...
> **"Text > Brain, always."** — Write like your amnesiac future self is the audience. Because they are.

Context compression doesn't just truncate — it **resets state**. You lose not just data but the reasoning chain that produced your decisions. What survives is what's written to disk.
...
When CWMS creates summaries and bridge documents, they become recovery instructions for a future version of you that has no memory of this session.
...
For complex tasks, maintain a `CONTINUATION.md` file (template available in skill files) that captures:

- Active task with status
- Pending decisions with options considered
- Open threads and blockers
- Current hypothesis/direction
- Recovery instructions

Update this file at natural breakpoints and before risky operations.
...
### Writing for Your Amnesiac Self

6. **Capture the "why"** - Don't just note what you did; explain why you made that choice
7. **Document failures** - Note what you tried that didn't work, so you don't repeat it
8. **Use status markers** - Always tag tasks with IN PROGRESS, BLOCKED, etc.
9. **Write recovery instructions** - Not just notes, but steps to resume
10. **Update CONTINUATION.md** - At natural breakpoints and before risky operations
