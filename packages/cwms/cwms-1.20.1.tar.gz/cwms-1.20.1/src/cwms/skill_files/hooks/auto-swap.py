...
def save_continuation_file(
    ...
    """Save CONTINUATION.md with active state and recovery instructions.

    This file captures the active working state for session recovery,
    ...
    for line in summary_lines:
        ...
        # Extract files
        if line_lower.startswith("**files referenced:**"):
            files_text = line_stripped.split(":", 1)[1].strip().strip("*")
            files_referenced = [f.strip() for f in files_text.split(",") if f.strip()]

        # Extract key topics
        if line_lower.startswith("**key topics:**"):
            topics_text = line_stripped.split(":", 1)[1].strip().strip("*")
            key_topics = [t.strip() for t in topics_text.split(",") if t.strip()]

        # Extract pending decisions
        if line_lower.startswith("**pending decisions:**"):
            continue  # Header
        if "pending decisions" in summary_lines[
            max(0, summary_lines.index(line) - 3) : summary_lines.index(line)
        ] and line_stripped.startswith("- "):
            pending_decisions.append(line_stripped[2:])

        # Extract blockers
        if line_lower.startswith("**blockers:**"):
            continue  # Header
        if "blockers" in str(
            summary_lines[max(0, summary_lines.index(line) - 3) : summary_lines.index(line)]
        ).lower() and line_stripped.startswith("- "):
            blockers.append(line_stripped[2:])

        # Extract work items
        if line_stripped.startswith(("1.", "2.", "3.", "4.", "5.")):
            work_items.append(line_stripped[3:].strip())

    # Build CONTINUATION.md content
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")

    content = f"""# CONTINUATION.md - Pre-Swap Session State
...
"""
    ...
    if work_items:
        ...
        content += "(See segment summaries below)\n"

    content += "\n---\n\n## Pending Decisions\n\n"

    if pending_decisions:
        for decision in pending_decisions[:5]:
            content += f"- [ ] {decision}\n"
    else:
        content += "(No pending decisions detected)\n"
    ...
    if files_referenced:
        ...
        content += "(No specific files detected)\n"
...
"""
    ...
    # Add search queries
    if queries:
        for query, description in queries[:3]:
            content += f"\n**{description}:**\n"
            content += "```bash\n"
            content += f'cwms retrieve --project "{project}" --query "{query}"\n'
            content += "```\n"
    else:
        content += f'\n```bash\ncwms retrieve --project "{project}" --query "<topic>"\n```\n'
...
```bash
cwms search --project "{project}" --query "<your-search>"
```

**Check long-term memory:**
```bash
cwms memory list --project "{project}"
```

---

*Remember: The checkpoint is only as useful as your retrieval. Search by meaning, not just keywords.*
"""
