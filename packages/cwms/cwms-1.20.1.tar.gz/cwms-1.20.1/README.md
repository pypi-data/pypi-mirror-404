...
- **Proactive retrieval** — automatically injects relevant swapped context when user prompts relate to past conversations
...
CWMS supports a tiered memory architecture for optimal context management:

| Tier | File | Purpose | Update Frequency |
|------|------|---------|------------------|
| **Active State** | `CONTINUATION.md` | Current task, pending decisions, recovery instructions | Before swap, at breakpoints |
| **Bridge** | Bridge summary (auto-generated) | What was discussed, key topics, suggested queries | On swap |
| **Chunks** | `~/.claude/cwms/{project}/chunks.jsonl` | Searchable conversation history | On swap |

### CONTINUATION.md (Recommended)

For complex, multi-session tasks, maintain a `CONTINUATION.md` file in your project to capture active working state. This complements CWMS's automatic bridge summaries with explicit state tracking.

The file should include:
- **Active task** with status markers (IN PROGRESS, BLOCKED ON X, DECISION NEEDED)
- **Pending decisions** with options considered
- **Open threads** and blockers
- **Current hypothesis** or direction
- **Recovery instructions** — what to do next

A template is available at `~/.claude/skills/cwms/CONTINUATION.md` after installing the skill.

### Writing for Recovery

> **Core principle**: "Text > Brain, always." Write like your amnesiac future self is the audience.

When capturing context (manually or via CWMS), include:
- **What** you're working on
- **Why** you made specific choices
- **What failed** so you don't repeat it
- **What you were reaching toward** (the "shape" of the work)

### Status Markers

Use consistent status markers:
- `IN PROGRESS` — Actively working
- `BLOCKED ON [X]` — Waiting for external input
...
## Search Modes and Embedding Providers

### Do You Need Embeddings?

**Short answer: Probably not.** For most CWMS use cases, keyword search (BM25) is sufficient.

**Why keyword search often works well:**

- **You're searching your own words** — Unlike web search where you're finding documents written by strangers, you're retrieving conversations *you* participated in. You'll naturally use similar vocabulary when searching.
- **High vocabulary overlap** — Queries like "the chunker bug" or "config issue" match well with exact terms.
- **Rich metadata already extracted** — Keywords, file paths, timestamps, and summaries provide strong retrieval signals.

**When embeddings actually help:**

- **Large archives** — Hundreds of chunks across many projects benefit from semantic matching
- **Long time gaps** — Searching for something from weeks ago where you've forgotten exact terms
- **Exploratory queries** — Broad conceptual searches like "What have I worked on related to databases?"
- **Terminology drift** — When words you used evolved during the conversation

**Recommendation:** Start with `embeddings.provider: none` (the default). Enable local embeddings later if you find keyword search insufficient for your retrieval patterns.

### Provider Comparison

| Provider | Setup | Pros | Cons |
|----------|-------|------|------|
| **none** (recommended) | No setup | Works immediately, no dependencies, fast | Keyword search only |
| **local** | `pip install 'cwms[local]'` | Offline, no API costs, privacy, ChromaDB | ~90MB model download, slower first load |

When `embeddings.provider: local` is set, CWMS automatically uses **ChromaDB** as the vector store backend. ChromaDB provides:
...
**Embeddings capture semantic similarity but not conversation flow.** When you retrieve chunk N based on topic relevance, you lose the context of what came before (N-1) and after (N+1). This means:

- Implicit references ("that bug we discussed") may not resolve correctly
- Temporal relationships ("what happened before X") aren't captured
- Follow-up discussions get separated from their original context

**Planned improvement:** Sliding window retrieval (automatically include adjacent chunks) and enhanced metadata tracking for conversation flow. See the Roadmap section.
...
- **Try different search terms** — Keyword search works best with exact terms you used in the conversation
- **Increase retrieval count**: `retrieval.top_k: 10` — Get more candidates to review
- **Lower similarity threshold**: `retrieval.min_similarity: 0.6` — Accept less exact matches
- **Enable embeddings** (if keyword search consistently fails): `embeddings.provider: local` — Helps with fuzzy/conceptual queries, but adds ~90MB dependency
...
#### v1.19.0 - Project Rename & API Summarization
- **Project Rename**: Renamed from `context-window-management` to `cwms` for brevity
...
#### v1.14.0-1.18.0 - Architecture Improvements
- **Exception Hierarchy**: Comprehensive exception classes for better error handling
- **TTL Cache**: Generic time-to-live cache with LRU eviction for performance
- **Constants Module**: Centralized configuration constants
- **TypedDict for Metadata**: Improved type safety for chunk metadata
- **BM25 Improvements**: Better edge case handling with debug logging
- **Testing Improvements**: ChunkIndex deprecation, expanded test coverage (895 tests)

#### v1.13.0 - Proactive Retrieval
- **Proactive Context Retrieval**: Automatically queries swapped context on user prompts
- **Smart Filtering**: Skips short messages, slash commands, and simple acknowledgments
- **Relevance Threshold**: Only injects meaningful context (0.6 similarity threshold)

#### v1.12.0 - ChromaDB Integration
- **ChromaDB Vector Store**: Persistent vector storage with efficient ANN search when `embeddings.provider: local`

### Roadmap

Planned features to achieve truly transparent extended memory:

- ~~**LLM-Powered Summarization**: Use Claude to generate abstractive summaries instead of regex extraction~~ ✅ **Implemented in v1.19.0** (opt-in via API)
- ~~**Proactive Retrieval**: Automatically inject relevant swapped context when user queries relate to past conversations~~ ✅ **Implemented in v1.13.0**
- **Automatic /clear**: If Claude Code exposes session management APIs in the future
- **Sliding Window Retrieval**: When chunk N is retrieved, automatically include N-1 and N+1 to preserve conversation flow across chunk boundaries
- **Enhanced Metadata Tracking**: Store temporal relationships (sequence numbers, thread IDs) to enable "what happened before/after X" queries and maintain conversational context
