...
class IlulabClient:
    ...
    def create_run(
        ...
        Note:
            In worker processes (e.g., DataLoader workers on Windows), this
            returns a no-op Run without making an API call.
        """
        # Skip API call in worker processes - return a no-op Run
        if _is_worker_process():
            return Run(UUID(int=0), self)
