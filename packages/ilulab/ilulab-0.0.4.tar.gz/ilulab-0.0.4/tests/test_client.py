...
def test_create_run_noop_in_worker_process():
    """Test that create_run skips API call in worker processes."""
    ...
    with patch("ilulab.client._is_worker_process", return_value=True):
        with patch.object(client, "_post") as mock_post:
            run = client.create_run(project_id="test-project-id", name="test")

            # API should NOT be called
            mock_post.assert_not_called()

            # Run should be in worker mode
            assert run._is_worker is True
            assert run.run_id == UUID(int=0)
