...
class APIClient:
    ...
    def delete_experiments(
        ...
        """
        # Normalize to list
        ids = [experiment_ids] if isinstance(experiment_ids, str) else experiment_ids

        deleted_count = 0
        for exp_id in ids:
            url = f"{self.base_url}/api/experiments/{exp_id}"
    ...
    def delete_api_keys(
        ...
        """
        # Normalize to list
        ids = [api_key_ids] if isinstance(api_key_ids, str) else api_key_ids

        deleted_count = 0
        for key_id in ids:
            url = f"{self.base_url}/api/api-keys/{key_id}"
            response = self._request("DELETE", url)
            data = response.json()
            if data.get("success"):
                deleted_count += 1
