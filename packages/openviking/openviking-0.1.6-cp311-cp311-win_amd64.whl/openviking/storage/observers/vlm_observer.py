...
class VLMObserver(BaseObserver):
    ...
    def _format_status_as_table(self) -> str:
        ...
        for model_name, model_data in usage_data["usage_by_model"].items():
            for backend_name, backend_data in model_data["usage_by_backend"].items():
                data.append(
                    {
                        "Model": model_name,
                        "Backend": backend_name,
                        "Prompt": backend_data["prompt_tokens"],
                        "Completion": backend_data["completion_tokens"],
                        "Total": backend_data["total_tokens"],
                        "Last Updated": backend_data["last_updated"],
                    }
                )
                total_prompt += backend_data["prompt_tokens"]
                total_completion += backend_data["completion_tokens"]
                total_all += backend_data["total_tokens"]
