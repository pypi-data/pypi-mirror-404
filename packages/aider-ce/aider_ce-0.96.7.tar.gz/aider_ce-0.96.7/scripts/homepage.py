...
# Cache settings
CACHE_DIR = os.path.expanduser("~/.cache/cecli-badges")
...
def get_latest_release_cecli_percentage():
    """
    Get the percentage of code written by cecli in the LATEST release
    ...
    try:
        ...
        if latest_release:
            percentage = latest_release.get("cecli_percentage", 0)
...
def generate_badges_md(downloads, stars, cecli_percentage):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Get total downloads and GitHub stars for cecli")
    parser.add_argument(
        "--api-key",
        help=(
    ...
    # Get cecli contribution percentage in latest release
    percentage, version = get_latest_release_cecli_percentage()
    print(f"cecli wrote {percentage:.2f}% of code in the LATEST release ({version})")
