...
def get_type_for_field(field: CompatModelField, is_input: bool, compat: PydanticCompat):  # noqa: ANN201
    ...
    # Make all fields Optional regardless of Pydantic's allow_none
    if is_input:
        return Optional[replaced_type]

    # Original logic for non-input types
    if field.is_v1:
...
def _build_dataclass_creation_fields(
    ...
    if (
        ...
        # For input types: use field's default if it exists, otherwise None
        if is_input:
            default_value = field.default if field.has_default else None
            default_factory = field.default_factory if field.has_default_factory else None
        else:
