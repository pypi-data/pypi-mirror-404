...
def _ensure_ort_symlink() -> None:
    """Ensure ``_bithuman_py`` finds the correct ``libonnxruntime.so.1``.

    The C++ extension has ``RUNPATH=$ORIGIN`` and ``NEEDED: libonnxruntime.so.1``,
    so the dynamic linker checks ``bithuman/lib/`` first.  When the Python
    ``onnxruntime`` package is also installed (e.g. via silero VAD), its pybind11
    extension has ORT **statically linked** while a separate
    ``libonnxruntime.so.1.*`` ships for C API consumers like us.

    To prevent loading a second, independent ORT instance we create a symlink::

        bithuman/lib/libonnxruntime.so.1  →  onnxruntime/capi/libonnxruntime.so.1.*

    This makes ``_bithuman_py`` resolve to the **same file** that an explicit
    ``onnxruntime`` install provides.  Crucially the symlink is found via the
    normal ``RUNPATH`` mechanism so the library is loaded with ``RTLD_LOCAL``
    (Python's default for extension imports) — no global symbol pollution that
    could interfere with onnxruntime's statically-linked copy.

    If ``onnxruntime`` is not installed, this is a no-op and ``_bithuman_py``
    will use whatever ``libonnxruntime.so.1`` is available at ``$ORIGIN``.

    On macOS the equivalent is a symlink for ``libonnxruntime.*.dylib``.
    On Windows this is a no-op (DLL loading semantics differ).
    """
    ...
    lib_dir = os.path.dirname(__file__)  # bithuman/lib/
    ort_version = getattr(onnxruntime, "__version__", "")

    if sys.platform == "linux":
        link_name = "libonnxruntime.so.1"
        candidates = sorted(
            (f for f in os.listdir(ort_dir) if f.startswith("libonnxruntime.so.1")),
            reverse=True,
        )
        # Prefer the file matching the installed onnxruntime version to avoid
        # ABI mismatches from stale .so files left by previous installs.
        version_match = f"libonnxruntime.so.{ort_version}"
        if ort_version and version_match in candidates:
            candidates = [version_match] + [c for c in candidates if c != version_match]
    else:  # darwin
        link_name = None
        candidates = sorted(
    ...
    if not candidates or link_name is None:
        return

    target = os.path.join(ort_dir, candidates[0])
    link_path = os.path.join(lib_dir, link_name)

    # Check if a valid symlink already points to the right target.
    try:
        if os.path.islink(link_path) and os.path.realpath(link_path) == os.path.realpath(target):
            logger.debug(f"ORT symlink already valid: {link_path} -> {target}")
            return
    except OSError:
        pass
    ...
    try:
        # Remove existing file/symlink (could be a bundled copy or stale symlink).
        if os.path.islink(link_path) or os.path.exists(link_path):
            os.unlink(link_path)
        os.symlink(target, link_path)
        logger.debug(f"ORT symlink created: {link_path} -> {target}")
    except OSError as exc:
        # Read-only filesystem or no permission — fall back to RTLD_GLOBAL
        # preload so the dynamic linker finds the already-resident library.
        logger.debug(f"Cannot create ORT symlink ({exc}), falling back to RTLD_GLOBAL preload")
        try:
            ctypes.CDLL(target, mode=ctypes.RTLD_GLOBAL)
            logger.debug(f"Preloaded onnxruntime library: {target}")
        except OSError:
            pass
