...
# 4. Regenerate — only generated/ changes, your code is safe
prisme generate
...
Core loop: **create once**, then iterate between editing specs and running **generate**. Your code in `app/` and `packages/` is never touched by `generate`.
...
| `prisme generate` | Regenerate `generated/` from specs + config | Core |
...
- Scaffolds a fully bootable project
- Writes non-generated files using **GENERATE_ONCE**
- Runs an initial `generate` to populate `generated/`

**Phase B: `prisme generate`**

- Regenerates **only** `generated/`
- Safe to run anytime
...
- User customization happens in `app/` and `packages/`, which are invisible to `generate`
...
├── generated/                     # FULLY REPLACEABLE — only prisme generate touches this
│   ├── .prisme-manifest.json      # Provenance metadata
│   ├── backend/
│   │   ├── models/
│   │   ├── schemas/
│   │   ├── services/
...
│       ├── hooks/
│       └── pages/
├── app/                           # USER-OWNED — never touched after create
│   ├── backend/
│   │   ├── extensions/            # Extension hooks (stubs from create)
│   │   ├── overrides/             # Override files
│   │   └── custom/                # Custom endpoints, logic
│   └── frontend/
│       ├── components/            # Custom components
│       ├── overrides/             # Component overrides
│       └── pages/                 # Custom pages
├── packages/                      # Monorepo structure (scaffolded by create)
│   ├── backend/
│   │   └── src/{name}/
│   │       ├── main.py            # Entry point (from create, user-owned)
│   │       ├── config.py          # App config (from create, user-owned)
│   │       ├── database.py        # DB setup (from create, user-owned)
│   │       └── ...                # Imports from generated/ + app/
│   └── frontend/
│       └── src/
│           ├── App.tsx            # Entry point (from create, user-owned)
│           ├── router.tsx         # Client routing (from create, user-owned)
│           └── ...
├── docker/                        # Infrastructure (from create, user-owned)
│   ├── docker-compose.yml
│   ├── docker-compose.prod.yml
│   └── Dockerfile
├── .github/                       # CI workflows (from create, user-owned)
└── .devcontainer/                 # Dev container (from create, user-owned)
```
...
|---|---|---|---|
| `ALWAYS_OVERWRITE` | Replaced every regeneration | `prisme generate` | Everything in `generated/` |
| `GENERATE_ONCE` | Created if missing, never overwritten | `prisme create` | Entry points, config, docker, CI, hook scaffolds |

**Hard rule:** `prisme generate` may only write inside `generated/`. No exceptions.

### 6.4 Rules

> **`prisme generate`** may only write inside `generated/`. No exceptions.
>
> **`prisme create`** writes the full project scaffold using `GENERATE_ONCE`. It also runs an initial `generate` to populate `generated/`. It writes three key artifacts: `prisme.toml`, `specs/models.py`, `specs/project.py`.
>
> There is no `USER_OWNED` strategy — `app/` contents are scaffolded by `create` and then belong entirely to the user. The generator does not know `app/` exists.
>
> Editing files inside `generated/` is unsupported and detectable via manifest hashes.

### 6.5 Ejection Story

Prisme is designed so ejection is trivial even without dedicated tooling. To eject:

1. Copy `generated/` contents into `packages/` (or wherever you prefer)
2. Delete `generated/`, `prisme.toml`, and `specs/`
3. You now own all the code — it's standard Python/TypeScript with no prisme runtime dependency
...
```
packages/{name}/main.py
  ├── imports from generated/backend/...  (models, schemas, services, API routes)
  ├── imports from app/backend/...        (extensions, overrides, custom logic)
  └── wires them together
```

Generated code is **pure library code** — it exports symbols but does not import from `app/` or `packages/`. The entry points in `packages/` are the composition root that wires generated code with user customizations.
...
|---|---|---|
| `dependencies.py` | FastAPI dependency injection overrides | `app/backend/extensions/` |
| `routers.py` | Additional API routes | `app/backend/extensions/` |
| `events.py` | Startup/shutdown events | `app/backend/extensions/` |
| `policies.py` | Auth/authorization rules | `app/backend/extensions/` |
| `services_override.py` | Override service methods per model | `app/backend/overrides/` |

### Frontend

| Hook | Purpose | Location |
|---|---|---|
| Component overrides | Replace generated components | `app/frontend/overrides/{ModelName}/` |
| Custom pages | Add non-generated pages | `app/frontend/pages/` |
| Cell renderers | Custom table cell rendering | `app/frontend/overrides/renderers/` |
| Form widgets | Custom form field widgets | `app/frontend/overrides/widgets/` |

### MCP

| Hook | Purpose | Location |
|---|---|---|
| Tool hooks | Pre/post processing for MCP tools | `app/backend/extensions/mcp_hooks.py` |
| Tool policies | Access control per tool | `app/backend/extensions/mcp_policies.py` |

All hooks are preserved across regeneration — they live in `app/`, which the generator never touches.

### Discovery Mechanism (Composition Root)

Extension discovery is **not magic** — it happens in the user-owned entry point (`packages/backend/src/{name}/main.py`), which is scaffolded by `prisme create`. The entry point contains explicit import-and-register blocks grouped by concern:

```python
# packages/backend/src/wind_pipeline/main.py (scaffolded by create, user-owned)

from fastapi import FastAPI
from generated.backend.api.rest import router as generated_router

app = FastAPI()
app.include_router(generated_router)

# Extension discovery — import if present, skip if missing
try:
    from app.backend.extensions.routers import router as ext_router
    app.include_router(ext_router)
except ImportError:
    pass  # No custom routes — that's fine

try:
    from app.backend.extensions.events import on_startup, on_shutdown
...
This means:
- **No runtime magic.** Discovery is plain Python imports with `try/except ImportError`.
- **Deleting a hook file is safe.** The app skips it silently — no crash, no error.
- **Adding a hook is explicit.** Create the file, restart the server, it's picked up.
- **The user owns the wiring.** Since `main.py` is `GENERATE_ONCE`, users can change the discovery logic however they want.

### Contract Between `app/` and `packages/`

The `app/` directory is invisible to the generator but **not invisible to the runtime**. The contract:

| Scenario | Behavior |
|---|---|
| Hook file exists and exports expected symbols | Imported and used |
| Hook file exists but has an import error | **App crashes at startup** (intentional — this is a bug in user code, not a silent failure) |
| Hook file is missing | Silently skipped via `ImportError` |
| Hook file exists but exports wrong names | Silently ignored (no matching symbols to import) |
...
✗ generated/backend/models/turbine.py has been manually edited
  → Run `prisme generate` to restore, or move your changes to app/backend/overrides/
...
Each generation writes `generated/.prisme-manifest.json`:
...
```json
{
  ...
  "files": {
    "generated/backend/models/turbine.py": "sha256:...",
    "generated/backend/models/reading.py": "sha256:..."
  }
}
```

Used for: debugging, CI validation, support triage, detecting edited generated files.

---

## 10. Plan / Apply Workflow (P2)

**Default workflow:** `prisme generate` — this is what users run. It regenerates `generated/` in one step. There is no decision to make.
...
2. **Change domain spec → regenerate → only `generated/` changes**
3. **Change project spec → regenerate → only `generated/` changes**
...
- [ ] `prisme create` scaffolds full bootable project (GENERATE_ONCE for everything outside `generated/`)
- [ ] `prisme create` writes 3 artifacts: `prisme.toml`, `specs/models.py`, `specs/project.py`
- [ ] `prisme create` runs initial `prisme generate` at end to populate `generated/`
- [ ] `prisme generate` reads from `prisme.toml` + `specs/` (fail if missing)
- [ ] `prisme generate` writes strictly into `generated/` — nothing else
- [x] Add `PRISME_DOMAIN_VERSION` to domain spec files
- [x] Add `PRISME_PROJECT_VERSION` to project spec files
- [ ] Separate `generated/` directory from user-owned code
...
- [ ] Ensure generated code is pure library code (no imports from app/ or packages/)
- [ ] Entry points in packages/ wire generated/ + app/ together
...
- [ ] `generated/.prisme-manifest.json` written on every generation
...
- [ ] Detection of edited generated files (warn/block)
...
| 6 | Create vs Generate separation | **Two-phase model** — `create` scaffolds a fully bootable project (GENERATE_ONCE for everything outside `generated/`). `generate` only writes into `generated/` (ALWAYS_OVERWRITE). No USER_OWNED strategy needed — `app/` is scaffolded by `create` and then invisible to the generator. |
...
| 8 | Lifecycle hooks | **Keep in domain spec** — `before_create`, `after_create`, etc. on Model. Implementation lives in `app/backend/extensions/`. Spec declares intent, user code provides the function body. |
