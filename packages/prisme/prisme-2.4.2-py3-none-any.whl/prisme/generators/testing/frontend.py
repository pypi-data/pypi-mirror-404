...
"""
  ...
  updatedAt: '2024-01-01T00:00:00Z',"""
        ...
        # Build vitest imports based on usage
        vitest_imports = ["describe", "it", "expect"]
        if has_form and (has_create or has_update):
            vitest_imports.append("vi")
        vitest_imports_str = ", ".join(vitest_imports)

        # Build test util imports based on usage
        util_imports = ["render", "screen"]
        if has_delete or has_update:
            util_imports.append("fireEvent")
        if has_form and (has_create or has_update):
            util_imports.append("waitFor")
        util_imports_str = ", ".join(util_imports)
...
import {{ {vitest_imports_str} }} from 'vitest';
import {{ {util_imports_str} }} from '../utils';
{import_lines}
