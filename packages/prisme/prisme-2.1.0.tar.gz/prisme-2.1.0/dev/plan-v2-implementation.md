...
- **P1.3** — `prisme doctor` command (config valid, specs valid, versions compat, no edited `GENERATE_ONCE` files, hooks valid, deps match)
- **P1.4** — Golden regeneration tests: generate→edit user→regenerate→`GENERATE_ONCE` files unchanged; spec change→only `ALWAYS_OVERWRITE` files change; deterministic output
