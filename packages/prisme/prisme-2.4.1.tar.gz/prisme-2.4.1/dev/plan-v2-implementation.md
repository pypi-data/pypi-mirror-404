...
**Source:** `dev/PRD-v2-refactor.md` | **Branch:** `main`
...
|-------|--------|-------|-------|
| P0 Foundation | ✅ Done | 802 passed | Package rename, spec models, loaders, generators, file-strategy, templates, manifest, CLI, exports, frontend pages |
| P1 Safety & Trust | ✅ Done | 817 passed | Manifest integration, extension hooks, `prisme doctor`, golden regen tests, deterministic generation |
| P2 Evolution & Teams | ✅ Done | 844 passed | `prisme migrate`, `prisme plan`/`apply`, config includes, strict mode, fix-it messages |
...
| Step | Status | Notes |
|------|--------|-------|
| P2.1 `prisme migrate` | ✅ Done | Version detection (`migration/detector.py`), domain v1→v2 migration (`domain_v1_to_v2.py`), project spec extraction (`project_extractor.py`). CLI: `--dry-run`, `--write` flags |
| P2.2 `prisme plan`/`apply` | ✅ Done | `prisme plan` writes `.prisme/plan.json` with create/modify/skip actions. `prisme apply` executes the saved plan. Reuses dry-run infrastructure |
| P2.3 Config includes | ✅ Done | `prisme.d/*.toml` files deep-merged into main `prisme.toml` in sorted order |
| P2.4 Strict mode | ✅ Done | `GeneratorBase` collects warnings; strict mode promotes them to errors. Configured via `PrismeConfig.generation.mode` |
| P2.5 Fix-it messages | ✅ Done | Validation errors include `fix` field with actionable suggestions. `prisme validate` displays them |
...
**Modify:** `src/prisme/cli.py`, `src/prisme/generators/base.py`

- **P1.1 — Manifest integration:** Hash verification wired into `GeneratorBase._write_file()`. Pre-generation edited-file detection via `ManifestManager.is_modified()`. `GENERATE_ONCE` files with user edits are never overwritten. `ALWAYS_OVERWRITE` files warn on overwrite of edited files.
- **P1.2 — Extension hooks:** Backend + frontend extension stubs generated with `GENERATE_ONCE` strategy during `prisme create`.
- **P1.3 — `prisme doctor`:** Health check command validating config, specs, version compatibility, edited `GENERATE_ONCE` files, dependency availability.
- **P1.4 — Golden regeneration tests:** `tests/tracking/test_golden_regeneration.py` — generate→edit→regenerate→verify `GENERATE_ONCE` unchanged; spec change→only `ALWAYS_OVERWRITE` files change; deterministic output across runs.
- **P1.5 — Deterministic generation:** Stable model/field sorting in all generators, consistent formatting.

**Tests:** `tests/cli/test_doctor.py`, `tests/tracking/test_generate_manifest.py`, `tests/tracking/test_golden_regeneration.py`
...
**New module:** `src/prisme/migration/`

- `detector.py` — `detect_versions(project_dir)` returns `VersionInfo` with detected config/domain/project versions, python manager, and migration-needed flags.
- `domain_v1_to_v2.py` — `migrate_domain_v1_to_v2(spec_path, write=False)` removes infrastructure fields from v1 StackSpec, extracts them for ProjectSpec, updates `prism` → `prisme` imports.
- `project_extractor.py` — `extract_project_spec(extracted_fields, project_name)` generates a `specs/project.py` from extracted v1 fields.

**CLI:** `prisme migrate [--dry-run] [--write]` — detects current state, applies safe migrations, reports legacy config that needs manual migration.

**Tests:** `tests/migration/test_detector.py`, `tests/migration/test_domain_migration.py`, `tests/migration/test_project_extractor.py`

### ✅ P2.2 — `prisme plan` / `prisme apply` workflow

**New module:** `src/prisme/planning/`

- `planner.py` — `GenerationPlan` dataclass with `PlannedFile` entries (path, action, strategy, content_hash). `create_plan()` from dry-run results, `save_plan()`/`load_plan()` for `.prisme/plan.json`.
- `executor.py` — `apply_plan()` loads saved plan, re-runs generation filtered to planned files.

**CLI:**
- `prisme plan [--spec PATH]` — runs generators in dry-run, saves plan with create/modify/skip summary.
- `prisme apply` — executes saved plan, removes plan file on completion.

**Tests:** `tests/cli/test_plan_apply.py`

### ✅ P2.3 — Config includes (`prisme.d/*.toml`)

**Modify:** `src/prisme/config/loader.py`

- `_deep_merge(base, override)` — recursive dict merge.
- `_load_includes(config_dir)` — loads all `prisme.d/*.toml` files in sorted order.
- `load_prisme_config()` now merges includes on top of main config.

**Tests:** `tests/config/test_includes.py`

### ✅ P2.4 — Strict vs lenient generation mode

**Modify:** `src/prisme/generators/base.py`

- `GeneratorResult.warnings` field added.
- `GeneratorBase._generation_mode` property reads `config.generation.mode`.
- `GeneratorBase.generate()` collects warnings during file writes; in strict mode, promotes all warnings to errors.
- `GeneratorBase._write_file()` records a warning when overwriting manually edited `ALWAYS_OVERWRITE` files.

**Tests:** `tests/generators/test_strict_mode.py`

### ✅ P2.5 — Error taxonomy with fix-it messages

**Modify:** `src/prisme/utils/spec_loader.py`

All validation error dicts now include a `fix` field with actionable suggestions:
- `duplicate_model` → "Rename one of the 'X' models to a unique name."
- `duplicate_field` → "Rename the duplicate 'X' field in Model."
- `invalid_reference` → "Change references='X' to one of: AvailableModel, ..."
- `invalid_relationship` → "Change target_model='X' to one of: AvailableModel, ..."

**Modify:** `src/prisme/cli.py` (validate command)

`prisme validate` now displays fix-it messages below each error: `fix: <suggestion>`.
...
8. `prisme doctor` — all checks pass
9. `prisme migrate --dry-run` — no migrations needed (already v2)
10. `prisme plan` → `.prisme/plan.json` → `prisme apply` — round-trip works
11. Project reaches current wind-pipeline stability level
