    ...
    async def save_file(
        ...
        async with self.save_file_semaphore:
            ...
            async def worker(session):
                while True:
                    ...
                    try:
                        ...
                        log.warning(f"Worker retrying due to: {e}")
            ...
            if isinstance(path, (str, PurePath)):
                fp = open(path, "rb", buffering=4096)
            elif isinstance(path, io.IOBase):
                fp = path
            else:
                raise ValueError("Invalid file type: must be str path or binary file pointer")
            ...
            try:
                ...
                if self.upload_boost and is_big:
                    workers_count = 6
                else:
                ...
                log.error(f"Error uploading part {file_part}: {e}", exc_info=True)
            else:
                if is_big:
