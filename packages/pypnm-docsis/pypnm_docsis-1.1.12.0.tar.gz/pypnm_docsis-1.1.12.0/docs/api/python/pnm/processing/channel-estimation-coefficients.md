...
[`CmDsOfdmChanEstimateCoef`](https://github.com/PyPNMApps/PyPNM/blob/main/src/pypnm/pnm/parser/CmDsOfdmChanEstimateCoef.py) validates that the input PNM payload is an OFDM *Channel Estimation Coefficients* file, unpacks header fields, decodes complex per-subcarrier coefficients from a signed-magnitude fixed-point format, and returns a typed model (`CmDsOfdmChanEstimateCoefModel`) with frequency metadata and a rounded view for downstream consumers.
...
## Link To Code: [CmDsOfdmChanEstimateCoef](https://github.com/PyPNMApps/PyPNM/blob/main/src/pypnm/pnm/parser/CmDsOfdmChanEstimateCoef.py)
