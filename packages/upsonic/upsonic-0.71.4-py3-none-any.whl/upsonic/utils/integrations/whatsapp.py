...
async def send_image_message_async(
    ...
    """
    debug_log(f"Sending WhatsApp image to {recipient}: {text}")
    phone_number_id = get_phone_number_id()
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            debug_log(f"Request data: {json.dumps(data, indent=2)}")
            response = await client.post(url, headers=headers, json=data)
            response.raise_for_status()
            debug_log(f"Response: {response.text}")

    except httpx.HTTPStatusError as e:
        error_log(f"Failed to send WhatsApp image message: {e}")
        error_log(f"Error response: {e.response.text if hasattr(e, 'response') else 'No response text'}")
        raise
    except Exception as e:
        error_log(f"Unexpected error sending WhatsApp image message: {str(e)}")
        raise
...
def send_image_message(
    ...
    try:
        ...
        error_log(f"Failed to send WhatsApp image message: {e}")
        error_log(f"Error response: {e.response.text if hasattr(e, 'response') else 'No response text'}")  # type: ignore
        raise
    except Exception as e:
        error_log(f"Unexpected error sending WhatsApp image message: {str(e)}")
        raise
...
async def typing_indicator_async(message_id: Optional[str] = None) -> Optional[dict]:
    ...
    try:
        ...
        error_log(f"Unexpected error sending WhatsApp typing indicator: {str(e)}")
        raise