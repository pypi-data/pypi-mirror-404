...
class GmailTools:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            include_tools (Optional[List[str]]): List of tool names to include. If provided, only these tools will be enabled.
                Available tools: 'get_latest_emails', 'get_emails_from_user', 'get_unread_emails', 'get_starred_emails',
                'get_emails_by_context', 'get_emails_by_date', 'get_emails_by_thread', 'search_emails',
                'mark_email_as_read', 'mark_email_as_unread', 'create_draft_email', 'send_email', 'send_email_reply',
                'list_custom_labels', 'apply_label', 'remove_label', 'delete_custom_label'. Defaults to None (all tools enabled).
            exclude_tools (Optional[List[str]]): List of tool names to exclude. If provided, these tools will be disabled.
                Only applied if include_tools is not provided. Defaults to None.
        """
        ...
        all_tools_map = {
            # Reading emails
            "get_latest_emails": self.get_latest_emails,
            "get_emails_from_user": self.get_emails_from_user,
            "get_unread_emails": self.get_unread_emails,
            "get_starred_emails": self.get_starred_emails,
            "get_emails_by_context": self.get_emails_by_context,
            "get_emails_by_date": self.get_emails_by_date,
            "get_emails_by_thread": self.get_emails_by_thread,
            "search_emails": self.search_emails,
            # Email management
            "mark_email_as_read": self.mark_email_as_read,
            "mark_email_as_unread": self.mark_email_as_unread,
            # Composing emails
            "create_draft_email": self.create_draft_email,
            "send_email": self.send_email,
            "send_email_reply": self.send_email_reply,
            # Label management
            "list_custom_labels": self.list_custom_labels,
            "apply_label": self.apply_label,
            "remove_label": self.remove_label,
            "delete_custom_label": self.delete_custom_label,
        }

        # Filter tools based on include_tools and exclude_tools
        if include_tools:
            # If include_tools is provided, only include those tools
            self._tools = [all_tools_map[tool] for tool in include_tools if tool in all_tools_map]
            # Validate that all requested tools exist
            invalid_tools = [tool for tool in include_tools if tool not in all_tools_map]
            if invalid_tools:
                raise ValueError(
                    f"Invalid tool names in include_tools: {invalid_tools}. "
                    f"Available tools: {list(all_tools_map.keys())}"
                )
        elif exclude_tools:
            # If exclude_tools is provided, exclude those from all tools
            self._tools = [
                tool_func for tool_name, tool_func in all_tools_map.items() if tool_name not in exclude_tools
            ]
            # Validate that all excluded tools exist
            invalid_tools = [tool for tool in exclude_tools if tool not in all_tools_map]
            if invalid_tools:
                raise ValueError(
                    f"Invalid tool names in exclude_tools: {invalid_tools}. "
                    f"Available tools: {list(all_tools_map.keys())}"
                )
        else:
            # Default: include all tools
            self._tools = list(all_tools_map.values())

        # Validate that required scopes are present for requested operations
        function_names = [func.__name__ for func in self._tools]
        self._validate_scopes(function_names)
    ...
    def get_unread_messages_raw(self, count: int) -> List[dict]:
        """
        Get the X number of latest unread emails as raw dictionaries.
        ...
        Returns:
            List[dict]: List of email details dictionaries
        """
        try:
            results = self.service.users().messages().list(userId="me", q="is:unread", maxResults=count).execute()  # type: ignore
    ...
    def enable_all_tools(self) -> None:
        ...
        # Re-validate scopes after enabling all tools
        function_names = [func.__name__ for func in self._tools]
        self._validate_scopes(function_names)
    ...
    def _validate_scopes(self, function_names: List[str]) -> None:
        ...
        if any(read_operation in function_names for read_operation in read_operations):
        ...
        if any(modify_operation in function_names for modify_operation in modify_operations):
