...
use rshogi::board;
use rshogi::board::movegen::{generate_moves, Evasions};
use rshogi::mate;
use rshogi::types::{Bitboard, Move, Move16, Piece, PieceType, Square};
...
fn bitboard_squares(mut bb: Bitboard) -> Vec<Square> {
    let mut out = Vec::new();
...
fn analyze_evasions(pos: &board::Position, mv: Move) {
    ...
    let mut evasions = board::MoveList::new();
